...
///|/
#include "Plater.hpp"
#include "GUI/Jobs/UIThreadWorker.hpp"
...
#include "libslic3r/src/libslic3r.h"
#include "libslic3r/src/Format/STL.hpp"
#include "libslic3r/src/Format/AMF.hpp"
#include "libslic3r/src/Format/3mf.hpp"
#include "libslic3r/src/Format/OBJ.hpp"
#include "libslic3r/src/GCode/ThumbnailData.hpp"
#include "libslic3r/src/Model.hpp"
#include "libslic3r/src/SLA/SupportPoint.hpp"
#include "libslic3r/src/SLA/ReprojectPointsOnMesh.hpp"
#include "libslic3r/src/Print.hpp"
#include "libslic3r/src/PrintConfig.hpp"
#include "libslic3r/src/SLAPrint.hpp"
#include "libslic3r/src/Utils.hpp"
#include "libslic3r/src/PresetBundle.hpp"
#include "libslic3r/src/miniz_extension.hpp"

// For stl export
#include "libslic3r/src/CSGMesh/ModelToCSGMesh.hpp"
#include "libslic3r/src/CSGMesh/PerformCSGMeshBooleans.hpp"
...
#include "libslic3r/src/CustomGCode.hpp"
#include "libslic3r/src/Platform.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		enum ExportingStatus {
			NOT_EXPORTING, EXPORTING_TO_REMOVABLE, EXPORTING_TO_LOCAL
		};
		...
		struct Plater::priv {
							...
							if (act.empty()) {
								...
								if (res != wxID_CANCEL)
									if (dialog.IsCheckBoxChecked()) {
										...
										wxString msg = _L("Printing3D will remember your choice.") + "\n\n" + _L("You will not be asked about it again, when: \n"
										                                                                          "- Closing Printing3D,\n"
										                                                                          "- Loading or creating a new project") + "\n\n" + format_wxstr(_L("Visit \"Preferences\" and check \"%1%\"\nto changes your choice."), preferences_item);

										MessageDialog msg_dlg(mainframe, msg, _L("Printing3D: Don't ask me again"), wxOK | wxCANCEL | wxICON_INFORMATION);
				...
				void take_snapshot(const wxString &snapshot_name, UndoRedo::SnapshotType snapshot_type = UndoRedo::SnapshotType::Action) { this->take_snapshot(std::string(snapshot_name.ToUTF8().data()), snapshot_type); }
				...
				enum UpdateBackgroundProcessReturnState {
					...
					UPDATE_BACKGROUND_PROCESS_RESTART = 1, // update_background_process() reports, that the Print / SLAPrint was updated in a way,
					// that a scene needs to be refreshed (you should call _3DScene::reload_scene(canvas3Dwidget, false))
					UPDATE_BACKGROUND_PROCESS_REFRESH_SCENE = 2, // update_background_process() reports, that the Print / SLAPrint is invalid, and the error message
					// was sent to the status line.
					UPDATE_BACKGROUND_PROCESS_INVALID = 4, // Restart even if the background processing is disabled.
					UPDATE_BACKGROUND_PROCESS_FORCE_RESTART = 8, // Restart for G-code (or SLA zip) export or upload.
		...
		Plater::priv::priv(Plater *q, MainFrame *main_frame) : q(q), main_frame(main_frame), config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
				{"bed_shape", "bed_custom_texture", "bed_custom_model", "complete_objects", "duplicate_distance", "extruder_clearance_radius", "skirts", "skirt_distance", "brim_width", "brim_separation", "brim_type", "variable_layer_height", "nozzle_diameter", "single_extruder_multi_material", "wipe_tower", "wipe_tower_x", "wipe_tower_y", "wipe_tower_width", "wipe_tower_rotation_angle", "wipe_tower_brim_width", "wipe_tower_cone_angle", "wipe_tower_extra_spacing", "wipe_tower_extra_flow",
				 "wipe_tower_extruder", "extruder_colour", "filament_colour", "material_colour", "max_print_height", "printer_model", "printer_notes", "printer_technology",
						// These values are necessary to construct SlicingParameters by the Canvas3D variable layer height editor.
                 "layer_height", "first_layer_height", "min_layer_height", "max_layer_height", "brim_width", "perimeters", "perimeter_extruder", "fill_density", "infill_extruder", "top_solid_layers", "support_material", "support_material_extruder", "support_material_interface_extruder", "support_material_contact_distance", "support_material_bottom_contact_distance", "raft_layers"})), sidebar(new Sidebar(q)), notification_manager(std::make_unique<NotificationManager>(q))
				...
				, m_worker{q, std::make_unique<NotificationProgressIndicator>(notification_manager.get()), "ui_worker"}, m_sla_import_dlg{new SLAImportDialog{q}}, delayed_scene_refresh(false), view_toolbar(GLToolbar::Radio, "View"), collapse_toolbar(GLToolbar::Normal, "Collapse"), m_project_filename(wxEmptyString) {
			...
			if (wxGetApp().is_editor()) {
				...
				view3D_canvas->Bind(EVT_GLCANVAS_INCREASE_INSTANCES, [this](Event<int> &evt) { if (evt.data == 1) this->q->increase_instances(); else if (this->can_decrease_instances()) this->q->decrease_instances(); });
			...
			if (wxGetApp().is_editor()) {
				...
				this->q->Bind(EVT_REMOVABLE_DRIVE_EJECTED, [this, q](RemovableDriveEjectEvent &evt) {
					if (evt.data.second) {
						...
						notification_manager->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::RegularNotificationLevel, format(_L("Successfully unmounted. The device %s(%s) can now be safely removed from the computer."), evt.data.first.name, evt.data.first.path));
					} else {
						notification_manager->close_notification_of_type(NotificationType::ExportFinished);
						notification_manager->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::ErrorNotificationLevel, format(_L("Ejecting of device %s(%s) has failed."), evt.data.first.name, evt.data.first.path));
					}
				});
				...
				this->q->Bind(EVT_REMOVABLE_DRIVE_ADDED, [this](wxCommandEvent &evt) {
					...
					if (evt.GetInt() == 0) { // not at startup, show dialog
						...
						notification_manager->push_notification(NotificationType::WifiConfigFileDetected, NotificationManager::NotificationLevel::ImportantNotificationLevel
		...
		void Plater::notify_about_installed_presets() {
			...
			if (!names.empty()) {
				std::string notif_text = into_u8(_L_PLURAL("The preset below was temporarily installed on the active instance of Printing3D", "The presets below were temporarily installed on the active instance of Printing3D", names.size())) + ":";
				for (const std::string &name: names)
					notif_text += "\n - " + name;
				get_notification_manager()->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::PrintInfoNotificationLevel, notif_text);
			}
		}
    ...
    Slic3r::ScopeGuard([&progress_dlg](){ if (progress_dlg) progress_dlg->Destroy(); progress_dlg = nullptr; });
        ...
        const auto &path = input_files[i];
				...
				if (type_printRequest && printer_technology != ptSLA) {
					Slic3r::GUI::show_info(nullptr, _L("PrintRequest can only be loaded if an SLA printer is selected."), _L("Error!"));
					continue;
				}
								...
								if (loaded_printer_technology == ptSLA) {
									...
									for (auto object: objects)
										if (object->volumes.size() > 1) {
											Slic3r::GUI::show_info(nullptr, _L("You cannot load SLA project with a multi-part object on the bed") + "\n\n" + _L("Please check your object list before preset changing."), _L("Attention!"));
											return obj_idxs;
										}
								}

								config.apply(loaded_printer_technology == ptFFF ? static_cast<const ConfigBase &>(FullPrintConfig::defaults()) : static_cast<const ConfigBase &>(SLAFullPrintConfig::defaults()));
				...
				if (load_model) {
					...
					if (!is_project_file) {
						if (int deleted_objects = model.removed_objects_with_zero_volume(); deleted_objects > 0) {
							MessageDialog(q, format_wxstr(_L_PLURAL("Object size from file %s appears to be zero.\n"
						...
						if (imperial_units)
							...
							if (answer_convert_from_meters == wxOK_DEFAULT) {
								RichMessageDialog dlg(q, format_wxstr(_L_PLURAL("The dimensions of the object from file %s seem to be defined in meters.\n"
								                                                "The internal unit of Printing3D is a millimeter. Do you want to recalculate the dimensions of the object?", "The dimensions of some objects from file %s seem to be defined in meters.\n"
								                                                                                                                                                              "The internal unit of Printing3D is a millimeter. Do you want to recalculate the dimensions of these objects?", model.objects.size()), from_path(filename)) + "\n", _L("The object is too small"), wxICON_QUESTION | wxYES_NO);
							...
							if (answer_convert_from_imperial_units == wxOK_DEFAULT) {
								RichMessageDialog dlg(q, format_wxstr(_L_PLURAL("The dimensions of the object from file %s seem to be defined in inches.\n"
								                                                "The internal unit of Printing3D is a millimeter. Do you want to recalculate the dimensions of the object?", "The dimensions of some objects from file %s seem to be defined in inches.\n"
								                                                                                                                                                              "The internal unit of Printing3D is a millimeter. Do you want to recalculate the dimensions of these objects?", model.objects.size()), from_path(filename)) + "\n", _L("The object is too small"), wxICON_QUESTION | wxYES_NO);
								dlg.ShowCheckBox(_L("Apply to all the remaining small objects being loaded."));
								int answer = dlg.ShowModal();
								if (dlg.IsCheckBoxChecked())
					...
					if (type_printRequest) {
						...
						for (const auto &material: model.materials) {
							std::string preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias_invisible(Preset::Type::TYPE_SLA_MATERIAL, Preset::remove_suffix_modified(material.first));
							...
							if (!prst) { //did not find compatible profile
								...
								for (auto it = prints.begin(); it != prints.end(); ++it) {
									if (it->name != edited_print_name) {
										...
										preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias_invisible(Preset::Type::TYPE_SLA_MATERIAL, Preset::remove_suffix_modified(material.first));
								...
								if (!found) {
									...
									q->get_notification_manager()->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::PrintInfoNotificationLevel, notif_text);
									break;
								}
							}
			...
			if (new_model != nullptr && new_model->objects.size() > 1) {
				...
				MessageDialog msg_dlg(q, _L("Multiple objects were loaded for a multi-material printer.\n"
		...
		std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs &model_objects, bool allow_negative_z, bool call_selection_changed /*= true*/) {
            ...
            new_instances.emplace_back(object->add_instance());
				...
				for (size_t i = 0; i < object->instances.size() && !object->is_cut()       // don't apply scaled_down functionality to cut objects
						; ++i) {
    ...
    view3D->get_canvas3d()->arrange_wipe_tower(wti);
			...
			if (scaled_down) {
				GUI::show_info(q, _L("Your object appears to be too large, so it was automatically scaled down to fit your print bed."), _L("Object too large?"));
			}
		...
		wxString Plater::priv::get_export_file(GUI::FileType file_type) {
			...
			wxFileDialog dlg(q, dlg_title, out_dir == temp_dir ? from_u8(wxGetApp().app_config->get("last_output_path")) : (is_shapes_dir(out_dir) ? from_u8(wxGetApp().app_config->get_last_dir()) : from_path(output_file.parent_path())), from_path(output_file.filename()), wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
		...
		bool Plater::priv::delete_object_from_model(size_t obj_idx) {
			...
			if (obj->is_cut()) {
				InfoDialog dialog(q, _L("Delete object which is a part of cut object"), _L("You try to delete an object which is a part of a cut object.") + "\n" + _L("This action will break a cut information.\n"
				                                                                                                                                                       "After that Printing3D can't guarantee model consistency"), false, wxYES | wxCANCEL | wxCANCEL_DEFAULT | wxICON_WARNING);
		...
		void Plater::priv::split_object() {
			...
			if (new_objects.size() == 1)
				...
				if (current_model_object->volumes.size() > 1 && current_model_object->volumes.size() != new_objects.size())
					notification_manager->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::PrintInfoNotificationLevel, _u8L("All non-solid parts (modifiers) were deleted"));
		...
		void Plater::priv::process_validation_warning(const std::vector<std::string> &warnings) const {
			...
			for (std::string text: warnings) {
				...
				notification_manager->push_notification(notification_type, NotificationManager::NotificationLevel::WarningNotificationLevel, _u8L("WARNING:") + "\n" + text, hypertext, action_fn);
			}
		}
		...
		unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages) {
			...
			if (invalidated != Print::APPLY_STATUS_UNCHANGED && was_running && !this->background_process.running() && (return_state & UPDATE_BACKGROUND_PROCESS_RESTART) == 0) {
			...
			if ((return_state & UPDATE_BACKGROUND_PROCESS_INVALID) != 0) {
				...
				const wxString slice_string = background_process.running() && wxGetApp().get_mode() == comSimple ? _L("Slicing") + dots : _L("Slice now");
				sidebar->set_btn_label(ActionButtonType::Reslice, slice_string);

				if (background_process.finished())
					show_action_buttons(false);
				else if (!background_process.empty() && !background_process.running()) /* Do not update buttons if background process is running
		...
		bool Plater::priv::restart_background_process(unsigned int state) {
			...
			if (!this->background_process.empty() && (state & priv::UPDATE_BACKGROUND_PROCESS_INVALID) == 0 && (((state & UPDATE_BACKGROUND_PROCESS_FORCE_RESTART) != 0 && !this->background_process.finished()) || (state & UPDATE_BACKGROUND_PROCESS_FORCE_EXPORT) != 0 || (state & UPDATE_BACKGROUND_PROCESS_RESTART) != 0)) {
		...
		void Plater::priv::reload_from_disk() {
			...
			while (!missing_input_paths.empty()) {
				...
				if (boost::algorithm::iequals(search.filename().string(), sel_filename)) {
					...
					wxString message = _L("The selected file") + " (" + from_u8(sel_filename) + ") " + _L("differs from the original file") + " (" + from_u8(search.filename().string()) + ").\n" + _L("Do you want to replace it") + " ?";
			...
			for (size_t i = 0; i < input_paths.size(); ++i) {
				...
				for (auto [obj_idx, vol_idx]: selected_volumes) {
					...
					if (has_source || has_name) {
						...
						new_volume->set_type(old_volume->type());
						new_volume->set_material_id(old_volume->material_id());
						new_volume->set_transformation(old_volume->get_transformation().get_matrix() * old_volume->source.transform.get_matrix_no_offset() * Geometry::translation_transform(new_volume->source.mesh_offset - old_volume->source.mesh_offset) * new_volume->source.transform.get_matrix_no_offset().inverse());
		...
		void Plater::priv::on_slicing_update(SlicingStatusEvent &evt) {
			...
			if (evt.status.percent >= 100 && this->printer_technology == ptFFF) {
				...
				if (templ_cnt > 0) {
					const std::string message_notif = GUI::format("%1%\n%2%\n\n%3%\n\n%4% ", _L_PLURAL("You are using template filament preset.", "You are using template filament presets.", templ_cnt), names, _u8L("Please note that template presets are not customized for specific printer and should only be used as a starting point for creating your own user presets."), _u8L("More info at"));
					// warning dialog proccessing cuts text at first '/n' - pass the text without new lines (and without filament names).
					const std::string message_dial = GUI::format("%1% %2% %3%", _L_PLURAL("You are using template filament preset.", "You are using template filament presets.", templ_cnt), _u8L("Please note that template presets are not customized for specific printer and should only be used as a starting point for creating your own user presets."), "<a href=https://help.prusa3d.com/article/template-filaments_467599>https://help.prusa3d.com/</a>");
					BOOST_LOG_TRIVIAL(warning) << message_notif;
					notification_manager->push_slicing_warning_notification(message_notif, false, 0, 0, "https://help.prusa3d.com/", [](wxEvtHandler *evnthndlr) {
						wxGetApp().open_browser_with_warning_dialog("https://help.prusa3d.com/article/template-filaments_467599");
						return false;
					});
			...
			if ((evt.status.flags & PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS) && static_cast<PrintStep>(evt.status.warning_step) == psAlertWhenSupportsNeeded && !get_app_config()->get_bool("alert_when_supports_needed")) {
				// This alerts are from psAlertWhenSupportsNeeded and the respective app settings is not Enabled, so discard the alerts.
			} else if (evt.status.flags & (PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS | PrintBase::SlicingStatus::UPDATE_PRINT_OBJECT_STEP_WARNINGS)) {
				...
				if (evt.status.flags & PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS) {
					state = this->printer_technology == ptFFF ? this->fff_print.step_state_with_warnings(static_cast<PrintStep>(warning_step)) : this->sla_print.step_state_with_warnings(static_cast<SLAPrintStep>(warning_step));
		...
		void Plater::priv::on_right_click(RBtnEvent &evt) {
			...
			if (obj_idx == -1) { // no one or several object are selected
				...
				const bool is_some_full_instances = selection.is_single_full_instance() || selection.is_single_full_object() || selection.is_multiple_full_instance();
				...
				if (is_some_full_instances)
					...
					menu = (model_volume != nullptr && model_volume->is_text()) ? menus.text_part_menu() : (model_volume != nullptr && model_volume->is_svg()) ? menus.svg_part_menu() : menus.part_menu();
				} else
					menu = menus.multi_selection_menu();
				//        }
			}
		...
		bool Plater::priv::can_scale_to_print_volume() const {
			...
			return !sidebar->obj_list()->has_selected_cut_object() && !view3D->get_canvas3d()->get_selection().is_empty() && (type == BuildVolume::Type::Rectangle || type == BuildVolume::Type::Circle);
		}
		...
		bool Plater::priv::layers_height_allowed() const {
			...
			int obj_idx = get_selected_object_idx();
			return 0 <= obj_idx && obj_idx < (int) model.objects.size() && model.objects[obj_idx]->max_z() > SINKING_Z_THRESHOLD && config->opt_bool("variable_layer_height") && view3D->is_layers_editing_allowed();
		}
    ...
    return false;
		...
		bool Plater::priv::can_simplify() const {
			...
			// is already opened?
			if (q->canvas3D()->get_gizmos_manager().get_current_type() == GLGizmosManager::EType::Simplify)
				return false;
			return true;
		}
		...
		bool Plater::priv::can_decrease_instances(int obj_idx /*= -1*/) const {
			...
			return obj_idx < (int) model.objects.size() && (model.objects[obj_idx]->instances.size() > 1) && !sidebar->obj_list()->has_selected_cut_object();
		}
		...
		void Plater::priv::show_action_buttons(const bool ready_to_slice_) const {
			...
			if (get_config_bool("background_processing")) {
				...
				if (sidebar->show_reslice(false) | sidebar->show_export(true) | sidebar->show_send(send_gcode_shown) |
				...
				if (sidebar->show_reslice(ready_to_slice) | sidebar->show_export(!ready_to_slice) | sidebar->show_send(send_gcode_shown && !ready_to_slice) |
		...
		void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot) {
			...
			// Do the jump in time.
			if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ? this->undo_redo_stack().undo(model, this->view3D->get_canvas3d()->get_selection(), this->view3D->get_canvas3d()->get_gizmos_manager(), top_snapshot_data, it_snapshot->timestamp) : this->undo_redo_stack().redo(model, this->view3D->get_canvas3d()->get_gizmos_manager(), it_snapshot->timestamp)) {
				if (printer_technology_changed) {
				...
				this->sidebar->obj_list()->set_selection_mode(new_selected_settings_on_sidebar ? ObjectList::SELECTION_MODE::smSettings : new_selected_layer_on_sidebar ? ObjectList::SELECTION_MODE::smLayer : new_selected_layerroot_on_sidebar ? ObjectList::SELECTION_MODE::smLayerRoot : ObjectList::SELECTION_MODE::smUndef);
    ...
    return;
		...
		Plater::Plater(wxWindow *parent, MainFrame *main_frame) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxGetApp().get_min_size(parent)), p(new priv(this, main_frame)) {
		...
		void Plater::load_gcode(const wxString &filename) {
			...
			try {
				...
				p->preview->reload_print();
				p->get_current_canvas3D()->render();
				MessageDialog(this, _L("The selected file") + ":\n" + filename + "\n" + _L("does not contain valid gcode."), wxString(GCODEVIEWER_APP_NAME) + " - " + _L("Error while loading .gcode file"), wxOK | wxICON_WARNING | wxCENTRE).ShowModal();
				set_project_filename(wxEmptyString);
				return;
			}
		...
		void Plater::convert_gcode_to_ascii() {
			...
			MessageDialog msg_dlg(this, Slic3r::GUI::format_wxstr("%1%\n%2%", _L("Successfully created G-code ASCII file"), output_file), _L("Convert G-code file to ASCII format"), wxICON_ERROR | wxOK);
			msg_dlg.ShowModal();
		}
		...
		void Plater::convert_gcode_to_binary() {
			...
			MessageDialog msg_dlg(this, Slic3r::GUI::format_wxstr("%1%\n%2%", _L("Successfully created G-code binary file"), output_file), _L("Convert G-code file to binary format"), wxICON_ERROR | wxOK);
			msg_dlg.ShowModal();
		}
		...
		class LoadProjectsDialog : public DPIDialog {
			...
			public:
				enum class LoadProjectOption : unsigned char {
					Unknown, AllGeometry, AllNewWindow, OneProject, OneConfig
				};
		...
		LoadProjectsDialog::LoadProjectsDialog(const std::vector<fs::path> &paths) : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, format_wxstr(_L("%1% - Multiple projects file"), SLIC3R_APP_NAME), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE) {
			...
			if (contains_projects)
				main_sizer->Add(new wxStaticText(this, wxID_ANY, get_wraped_wxString(_L("There are several files being loaded, including Project files.") + "\n" + _L("Select an action to apply to all files."))), 0, wxEXPAND | wxALL, 10);
			else
				main_sizer->Add(new wxStaticText(this, wxID_ANY, get_wraped_wxString(_L("There are several files being loaded.") + "\n" + _L("Select an action to apply to all files."))), 0, wxEXPAND | wxALL, 10);
			...
			if (instances_allowed) {
				btn = new wxRadioButton(this, wxID_ANY, _L("Start a new instance of Printing3D"), wxDefaultPosition, wxDefaultSize, id == 0 ? wxRB_GROUP : 0);
				btn->SetValue(id == m_action);
				btn->Bind(wxEVT_RADIOBUTTON, [this, id, contains_projects](wxCommandEvent &) {
					m_action = id;
					if (contains_projects) {
			...
			try {
				...
				if (dlg.ShowModal() == wxID_OK) {
						...
						for (mz_uint i = 0; i < num_entries; ++i) {
							if (mz_zip_reader_file_stat(&archive, i, &stat)) {
								...
								try {
									...
									if ((boost::algorithm::iends_with(filename, ".3mf") && !is_project_3mf(final_path.string())) || (boost::algorithm::iends_with(filename, ".amf") && !boost::algorithm::iends_with(filename, ".zip.amf"))) {
										non_project_paths.emplace_back(final_path);
										break;
									}
					...
					if (non_project_paths.size() + project_paths.size() != selected_paths.size())
						BOOST_LOG_TRIVIAL(error) << "Decompresing of archive did not retrieve all files. Expected files: " << selected_paths.size() << " Decopressed files: " << non_project_paths.size() + project_paths.size();
				} else {
					close_zip_reader(&archive);
					return false;
				}
    ...
    if (!delete_after)
		...
		class ProjectDropDialog : public DPIDialog {
			...
			public:
				enum class LoadType : unsigned char {
					Unknown, OpenProject, LoadGeometry, LoadConfig, OpenWindow
				};
		...
		ProjectDropDialog::ProjectDropDialog(const std::string &filename) : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, format_wxstr("%1% - %2%", SLIC3R_APP_NAME, _L("Load project file")), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE) {
			SetFont(wxGetApp().normal_font());
			...
			if (!single_instance_only)
				choices.Add(_L("Start new Printing3D instance"));

			main_sizer->Add(new wxStaticText(this, wxID_ANY, get_wraped_wxString(_L("Select an action to apply to the file") + ": " + from_u8(filename))), 0, wxEXPAND | wxALL, 10);

			m_action = std::clamp(std::stoi(wxGetApp().app_config->get("drop_project_action")), static_cast<int>(LoadType::OpenProject), single_instance_only ? static_cast<int>(LoadType::LoadConfig) : static_cast<int>(LoadType::OpenWindow)) - 1;
		...
		bool Plater::load_files(const wxArrayString &filenames, bool delete_after_load/*=false*/) {
			...
			if (wxGetApp().is_gcode_viewer()) {
				...
				if (paths.size() > 1) {
					...
					MessageDialog(static_cast<wxWindow *>(this), _L("You can open only one .gcode file at a time."), wxString(SLIC3R_APP_NAME) + " - " + _L("Drag and drop G-code file"), wxCLOSE | wxICON_WARNING | wxCENTRE).ShowModal();
					return false;
					...
					ProjectDropDialog::LoadType load_type = ProjectDropDialog::LoadType::Unknown;
					{
						if ((boost::algorithm::iends_with(filename, ".3mf") && !is_project_3mf(it->string())) || (boost::algorithm::iends_with(filename, ".amf") && !boost::algorithm::iends_with(filename, ".zip.amf")))
							...
							if (wxGetApp().app_config->get_bool("show_drop_project_dialog")) {
								...
								load_type = static_cast<ProjectDropDialog::LoadType>(std::clamp(std::stoi(wxGetApp().app_config->get("drop_project_action")), static_cast<int>(ProjectDropDialog::LoadType::OpenProject), static_cast<int>(ProjectDropDialog::LoadType::LoadConfig)));
						}
					}
		...
		void Plater::set_number_of_copies() {
			...
			const int num = GetNumberFromUser(" ", _L("Enter the number of copies:"), _L("Copies of the selected object"), init_cnt, 0, 1000, this);
			if (num < 0)
				return;
		...
		void Plater::convert_unit(ConversionType conv_type) {
			...
			TakeSnapshot snapshot(this, conv_type == ConversionType::CONV_FROM_INCH ? _L("Convert from imperial units") : conv_type == ConversionType::CONV_TO_INCH ? _L("Revert conversion from imperial units") : conv_type == ConversionType::CONV_FROM_METER ? _L("Convert from meters") : _L("Revert conversion from meters"));
			wxBusyCursor wait;
		...
		static void alert_when_exporting_binary_gcode(bool binary_output, const std::string &printer_notes) {
			if (binary_output && (boost::algorithm::contains(printer_notes, "PRINTER_MODEL_XL") || boost::algorithm::contains(printer_notes, "PRINTER_MODEL_MINI") || boost::algorithm::contains(printer_notes, "PRINTER_MODEL_MK4") || boost::algorithm::contains(printer_notes, "PRINTER_MODEL_MK3.9"))) {
				AppConfig *app_config = wxGetApp().app_config;
				...
				if (app_config->get(option_key) != "1") {
					...
					HtmlCapableRichMessageDialog dialog(parent, format_wxstr(_L("You are exporting binary G-code for a Prusa printer. "
					                                                            ...
					                                                            "To learn more about binary G-code, visit <a href=%1%>%1%</a>."), url), _L("Warning"), wxOK, [&url](const std::string &) { wxGetApp().open_browser_with_warning_dialog(url); });
			...
			fs::path output_path;
			{
				...
				if (dlg.ShowModal() == wxID_OK) {
					...
					if (check_for_error(output_path, error_str)) {
						...
						bool supports_binary = wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_bool("binary_gcode");
						bool uses_binary = wxGetApp().app_config->get_bool("use_binary_gcode_when_supported");
						alert_when_exporting_binary_gcode(supports_binary && uses_binary, wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_string("printer_notes"));
					}
				}
			}
		...
		void Plater::export_stl_obj(bool extended, bool selection_only) {
			...
			auto mesh_to_export_fff = [this](const ModelObject &mo, int instance_id) {
				...
				csg::model_to_csgmesh(mo, Transform3d::Identity(), std::back_inserter(csgmesh), csg::mpartsPositive | csg::mpartsNegative | csg::mpartsDoSplits);
			...
			auto mesh_to_export_sla = [&, this](const ModelObject &mo, int instance_id) {
				...
				if (!object || !object->get_mesh_to_print() || object->get_mesh_to_print()->empty()) {
					...
					for (const SLAPrintObject::Instance &obj_instance: obj_instances) {
						auto it = std::find_if(object->model_object()->instances.begin(), object->model_object()->instances.end(), [&obj_instance](const ModelInstance *mi) { return mi->id() == obj_instance.instance_id; });
						...
						if (it != object->model_object()->instances.end()) {
							...
							const Transform3d &inst_transform = one_inst_only ? Transform3d::Identity() : object->model_object()->instances[instance_idx]->get_transformation().get_matrix();
		...
		namespace {
			...
			void publish(Model &model) {
				...
				if (exist_new) {
					MessageDialog dialog(nullptr, _L("Are you sure you want to store original SVGs with their local paths into the 3MF file?\n"
					                                 "If you hit 'NO', all SVGs in the project will not be editable any more."), _L("Private protection"), wxYES_NO | wxICON_QUESTION);
    ...
    */
		...
		void Plater::send_gcode_inner(DynamicPrintConfig *physical_printer_config) {
			...
			if (dlg.ShowModal() == wxID_OK) {
				...
				if (printer_technology() == ptFFF) {
					...
					const bool binary_output = wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_bool("binary_gcode") && wxGetApp().app_config->get_bool("use_binary_gcode_when_supported");
		...
		void Plater::set_bed_shape() const {
			set_bed_shape(p->config->option<ConfigOptionPoints>("bed_shape")->values, p->config->option<ConfigOptionFloat>("max_print_height")->value, p->config->option<ConfigOptionString>("bed_custom_texture")->value, p->config->option<ConfigOptionString>("bed_custom_model")->value);
		}
		...
		void Plater::force_filament_colors_update() {
			...
			const auto &extruders_filaments = wxGetApp().preset_bundle->extruders_filaments;
			if (extruders_filaments.size() > 1 && p->config->option<ConfigOptionStrings>("filament_colour")->values.size() == extruders_filaments.size()) {
				std::vector<std::string> filament_colors;
				filament_colors.reserve(extruders_filaments.size());
 ...
 */
		...
		static std::string concat_strings(const std::set<std::string> &strings, const std::string &delim = "\n") {
			return std::accumulate(strings.begin(), strings.end(), std::string(""), [delim](const std::string &s, const std::string &name) {
				return s + name + delim;
			});
		}
		...
		void Plater::arrange(Worker &w, bool selected) {
			ArrangeSelectionMode mode = selected ? ArrangeSelectionMode::SelectionOnly : ArrangeSelectionMode::Full;
		...
		void Plater::clear_before_change_volume(ModelVolume &mv, const std::string &notification_msg) {
			...
			if (const bool paint_removed = !mv.supported_facets.empty() || !mv.seam_facets.empty() || !mv.mm_segmentation_facets.empty(); paint_removed) {
				...
				get_notification_manager()->push_notification(NotificationType::CustomSupportsAndSeamRemovedAfterRepair, NotificationManager::NotificationLevel::PrintInfoNotificationLevel, notification_msg);
			}
		}
...
unsigned int Plater::get_environment_texture_id() const
{
    return p->environment_texture.get_id();
		...
		Plater::TakeSnapshot::TakeSnapshot(Plater *plater, const std::string &snapshot_name) : TakeSnapshot(plater, from_u8(snapshot_name)) {}
		Plater::TakeSnapshot::TakeSnapshot(Plater *plater, const std::string &snapshot_name, UndoRedo::SnapshotType snapshot_type) : TakeSnapshot(plater, from_u8(snapshot_name), snapshot_type) {}
		...
		SuppressBackgroundProcessingUpdate::SuppressBackgroundProcessingUpdate() : m_was_scheduled(wxGetApp().plater()->is_background_process_update_scheduled()) {
		...
		PlaterAfterLoadAutoArrange::PlaterAfterLoadAutoArrange() {
			...
			m_enabled = plater->model().objects.empty() && plater->printer_technology() == ptFFF && is_XL_printer(plater->fff_print().config());
		}

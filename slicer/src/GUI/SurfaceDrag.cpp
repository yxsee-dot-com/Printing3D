...
#include <libslic3r/src/Model.hpp> // ModelVolume
#include <libslic3r/src/Emboss.hpp>

#include "Utils/RaycastManager.hpp"
...
namespace {
	...
	bool start_dragging(const Vec2d &mouse_pos, const Camera &camera, std::optional<SurfaceDrag> &surface_drag, GLCanvas3D &canvas, RaycastManager &raycast_manager, const std::optional<double> &up_limit);

	bool dragging(const Vec2d &mouse_pos, const Camera &camera, SurfaceDrag &surface_drag, // need to write whether exist hit
	              GLCanvas3D &canvas, const RaycastManager &raycast_manager, const std::optional<double> &up_limit);
	...
	Transform3d get_volume_transformation(Transform3d world, // from volume
			...
			// initial rotation in Z axis
			                              std::optional<float> current_angle = {}, const std::optional<double> &up_limit = {});
...
namespace Slic3r::GUI {
	...
	bool on_mouse_surface_drag(const wxMouseEvent &mouse_event, const Camera &camera, std::optional<SurfaceDrag> &surface_drag, GLCanvas3D &canvas, RaycastManager &raycast_manager, const std::optional<double> &up_limit) {
	...
	std::optional<float> calc_distance(const GLVolume &gl_volume, const RaycastManager &raycaster, const RaycastManager::ISkip *condition, const std::optional<Slic3r::Transform3d> &fix) {
	...
	bool face_selected_volume_to_camera(const Camera &camera, GLCanvas3D &canvas, const std::optional<double> &wanted_up_limit) {
		...
		Vec3d wanted_direction = (camera.get_type() == Camera::EType::Perspective) ? Vec3d(camera.get_position() - world_position) : (-camera.get_dir_forward());

		Transform3d new_volume_tr = get_volume_transformation(world_tr, wanted_direction, world_position, fix, instance_tr_inv, current_angle, wanted_up_limit);
...
namespace {
	...
	bool dragging(const Vec2d &mouse_pos, const Camera &camera, SurfaceDrag &surface_drag, GLCanvas3D &canvas, const RaycastManager &raycast_manager, const std::optional<double> &up_limit) {
		...
		if (volume != nullptr && volume->emboss_shape.has_value() && volume->emboss_shape->fix_3mf_tr.has_value())
			fix = volume->emboss_shape->fix_3mf_tr;
		Transform3d volume_new = get_volume_transformation(surface_drag.world, hit->normal, hit->position, fix, surface_drag.instance_inv, surface_drag.start_angle, up_limit);

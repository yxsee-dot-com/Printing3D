...
#include "libslic3r/src/BuildVolume.hpp"
#include "libslic3r/src/ClipperUtils.hpp"
#include "libslic3r/src/PrintConfig.hpp"
#include "libslic3r/src/GCode/ThumbnailData.hpp"
#include "libslic3r/src/Geometry/ConvexHull.hpp"
#include "libslic3r/src/ExtrusionEntity.hpp"
#include "libslic3r/src/Layer.hpp"
#include "libslic3r/src/Utils.hpp"
#include "libslic3r/src/Technologies.hpp"
#include "libslic3r/src/Tesselate.hpp"
...
#include "GUI/Gizmos/GLGizmoPainterBase.hpp"
...
#undef Convex
		...
		void GLCanvas3D::LayersEditing::select_object(const Model &model, int object_id) {
			...
			if (m_model_object != model_object_new || this->last_object_id != object_id || m_object_max_z != new_max_z || (model_object_new != nullptr && m_model_object->id() != model_object_new->id())) {
				m_layer_height_profile.clear();
				m_layer_height_profile_modified = false;
				delete m_slicing_parameters;
				m_slicing_parameters = nullptr;
		...
		void GLCanvas3D::LayersEditing::render_overlay(const GLCanvas3D &canvas) {
			...
			ImGuiPureWrap::set_next_window_pos(static_cast<float>(cnv_size.get_width()) - wxGetApp().imgui()->get_style_scaling() * THICKNESS_BAR_WIDTH, static_cast<float>(cnv_size.get_height()), ImGuiCond_Always, 1.0f, 1.0f);
			...
			if (m_enabled && m_layer_height_profile.size() >= 4) {
				...
				if (z != -1000.0f) {
					...
					for (size_t i = m_layer_height_profile.size() - 2; i >= 2; i -= 2) {
						...
						if (zi_1 <= z && z <= zi) {
							float dz = zi - zi_1;
							h = (dz != 0.0f) ? static_cast<float>(lerp(m_layer_height_profile[i - 1], m_layer_height_profile[i + 1], (z - zi_1) / dz)) : static_cast<float>(m_layer_height_profile[i + 1]);
							break;
						}
					}
					if (h > 0.0f)
		...
		void GLCanvas3D::LayersEditing::render_profile(const GLCanvas3D &canvas) {
			...
			if (!m_profile.profile.is_initialized() || m_profile.old_layer_height_profile != m_layer_height_profile || m_profile.old_canvas_width.profile != cnv_width) {
				...
				for (unsigned int i = 0; i < (unsigned int) m_layer_height_profile.size(); i += 2) {
					init_data.add_vertex(Vec2f(left + 2.0f * float(m_layer_height_profile[i + 1]) * scale_x * cnv_inv_width, 2.0f * (float(m_layer_height_profile[i]) * scale_y * cnv_inv_height - 0.5)));
		...
		void GLCanvas3D::LayersEditing::generate_layer_height_texture() {
			...
			m_layers_texture.cells = Slic3r::generate_layer_height_texture(*m_slicing_parameters, Slic3r::generate_object_layers(*m_slicing_parameters, m_layer_height_profile), m_layers_texture.data.data(), m_layers_texture.height, m_layers_texture.width, level_of_detail_2nd_level);
		...
		void GLCanvas3D::SLAView::detect_type_from_volumes(const GLVolumePtrs &volumes) {
			...
			for (const GLVolume *v: volumes) {
				if (v->volume_idx() == -(int) slaposDrillHoles) {
					if (object_contains_negative_volumes(*m_parent.get_model(), v->composite_id.object_id) || object_has_sla_drain_holes(*m_parent.get_model(), v->composite_id.object_id)) {
		...
		void GLCanvas3D::SLAView::update_instances_cache(const std::vector<std::pair<GLVolume::CompositeID, GLVolume::CompositeID>> &new_to_old_ids_map) {
			...
			std::sort(new_instances_cache.begin(), new_instances_cache.end(), [](const InstancesCacheItem &i1, const InstancesCacheItem &i2) {
			...
			new_instances_cache.erase(std::unique(new_instances_cache.begin(), new_instances_cache.end(), [](const InstancesCacheItem &i1, const InstancesCacheItem &i2) {
				return composite_id_match(i1.first, i2.first);
			}), new_instances_cache.end());
		...
		void GLCanvas3D::SLAView::render_switch_button() {
			...
			if (ImGuiPureWrap::draw_radio_button(_u8L("SLA view"), 1.5f * icon_size, true, [sel_instance](ImGuiWindow &window, const ImVec2 &pos, float size) {
				...
				switch (sel_instance->second) {
					case ESLAViewType::Original: {
						m_parent.set_sla_view_type(sel_instance->first, ESLAViewType::Processed);
						break;
					}
					case ESLAViewType::Processed: {
						m_parent.set_sla_view_type(sel_instance->first, ESLAViewType::Original);
						break;
					}
					default: {
						assert(false);
			...
			if (ImGui::IsItemHovered()) {
				...
				switch (type) {
					case ESLAViewType::Original: {
						tooltip = _u8L("Show as processed");
						break;
					}
					case ESLAViewType::Processed: {
						tooltip = _u8L("Show as original");
						break;
					}
					default: {
						assert(false);
																																...
																																void GLCanvas3D::SLAView::render_debug_window()
{
    ...
    ImGuiPureWrap::end();
		...
		GLCanvas3D::SLAView::InstancesCacheItem *GLCanvas3D::SLAView::find_instance_item(const GLVolume::CompositeID &id) {
			auto it = std::find_if(m_instances_cache.begin(), m_instances_cache.end(), [&id](const InstancesCacheItem &item) { return composite_id_match(item.first, id); });
		...
		bool GLCanvas3D::check_volumes_outside_state(GLVolumeCollection &volumes, ModelInstanceEPrintVolumeState *out_state, bool selection_only) const {
			auto volume_below = [](GLVolume &volume) -> bool { return volume.object_idx() != -1 && volume.volume_idx() != -1 && volume.is_below_printbed(); };
			// Volume is partially below the print bed, thus a pre-calculated convex hull cannot be used.
			auto volume_sinking = [](GLVolume &volume) -> bool { return volume.object_idx() != -1 && volume.volume_idx() != -1 && volume.is_sinking(); };
			// Cached bounding box of a volume above the print bed.
			auto volume_bbox = [volume_sinking](GLVolume &volume) -> BoundingBoxf3 { return volume_sinking(volume) ? volume.transformed_non_sinking_bounding_box() : volume.transformed_convex_hull_bounding_box(); };
			// Cached 3D convex hull of a volume above the print bed.
			auto volume_convex_mesh = [this, volume_sinking](GLVolume &volume) -> const TriangleMesh & { return volume_sinking(volume) ? m_model->objects[volume.object_idx()]->volumes[volume.volume_idx()]->mesh() : *volume.convex_hull(); };
		...
		void GLCanvas3D::toggle_sla_auxiliaries_visibility(bool visible, const ModelObject *mo, int instance_idx) {
			...
			for (GLVolume *vol: m_volumes.volumes) {
				if ((mo == nullptr || m_model->objects[vol->composite_id.object_id] == mo) && (instance_idx == -1 || vol->composite_id.instance_id == instance_idx) && vol->composite_id.volume_id < 0) {
		...
		void GLCanvas3D::toggle_model_objects_visibility(bool visible, const ModelObject *mo, int instance_idx, const ModelVolume *mv) {
			...
			for (GLVolume *vol: m_volumes.volumes) {
				if (vol->is_wipe_tower)
					...
					if ((mo == nullptr || m_model->objects[vol->composite_id.object_id] == mo) && (instance_idx == -1 || vol->composite_id.instance_id == instance_idx) && (mv == nullptr || m_model->objects[vol->composite_id.object_id]->volumes[vol->composite_id.volume_id] == mv)) {
						vol->is_active = visible;
						if (!vol->is_modifier)
							vol->color.a(1.f);
						...
						if (instance_idx == -1) {
							...
							if ((gizmo_type == GLGizmosManager::FdmSupports || gizmo_type == GLGizmosManager::Seam || gizmo_type == GLGizmosManager::Cut) && !vol->is_modifier) {
		...
		void GLCanvas3D::render_thumbnail(ThumbnailData &thumbnail_data, unsigned int w, unsigned int h, const ThumbnailsParams &thumbnail_params, const GLVolumeCollection &volumes, Camera::EType camera_type) {
			switch (OpenGLManager::get_framebuffers_type()) {
				case OpenGLManager::EFramebufferType::Arb: {
					_render_thumbnail_framebuffer(thumbnail_data, w, h, thumbnail_params, volumes, camera_type);
					break;
				}
				case OpenGLManager::EFramebufferType::Ext: {
					_render_thumbnail_framebuffer_ext(thumbnail_data, w, h, thumbnail_params, volumes, camera_type);
					break;
				}
				default: {
					_render_thumbnail_legacy(thumbnail_data, w, h, thumbnail_params, volumes, camera_type);
					break;
				}
			}
		}
		...
		void GLCanvas3D::reload_scene(bool refresh_immediately, bool force_full_scene_refresh) {
			...
			struct ModelVolumeState {
				...
				ModelVolumeState(const ModelVolume *model_volume, const ObjectID &instance_id, const GLVolume::CompositeID &composite_id) : model_volume(model_volume), geometry_id(std::make_pair(model_volume->id().id, instance_id.id)), composite_id(composite_id), volume_idx(-1) {}
				ModelVolumeState(const ObjectID &volume_id, const ObjectID &instance_id) : model_volume(nullptr), geometry_id(std::make_pair(volume_id.id, instance_id.id)), volume_idx(-1) {}
        ...
        check_model_ids_equal(*m_model, sla_print->model());
			...
			if (printer_technology == ptSLA) {
				...
				for (const SLAPrintObject *print_object: sla_print->objects()) {
					...
					for (size_t print_instance_idx = 0; print_instance_idx < print_object->instances().size(); ++print_instance_idx) {
						...
						auto it = std::find_if(model_object->instances.begin(), model_object->instances.end(), [&instance](const ModelInstance *mi) { return mi->id() == instance.instance_id; });
...
#else
						int volume_idx_wipe_tower_new = m_volumes.load_wipe_tower_preview(x, y, w, depth, z_and_depth_pairs, (float) height, ca, a, !print->is_step_done(psWipeTower), bw);
#endif // SLIC3R_OPENGL_ES
			...
			if (printer_technology == ptSLA) {
				std::sort(new_to_old_ids_map.begin(), new_to_old_ids_map.end(), [](const std::pair<GLVolume::CompositeID, GLVolume::CompositeID> &i1, const std::pair<GLVolume::CompositeID, GLVolume::CompositeID> &i2) {
					return i1.first.object_id < i2.first.object_id || (i1.first.object_id == i2.first.object_id && i1.first.instance_id < i2.first.instance_id);
				});

				new_to_old_ids_map.erase(std::unique(new_to_old_ids_map.begin(), new_to_old_ids_map.end(), [](const std::pair<GLVolume::CompositeID, GLVolume::CompositeID> &i1, const std::pair<GLVolume::CompositeID, GLVolume::CompositeID> &i2) {
					return composite_id_match(i1.first, i2.first);
				}), new_to_old_ids_map.end());
		...
		void GLCanvas3D::load_gcode_preview(const GCodeProcessorResult &gcode_result, const std::vector<std::string> &str_tool_colors, const std::vector<std::string> &str_color_print_colors) {
		...
		void GLCanvas3D::load_preview(const std::vector<std::string> &str_tool_colors, const std::vector<std::string> &str_color_print_colors, const std::vector<CustomGCode::Item> &color_print_values) {
			...
			libvgcode::GCodeInputData data = libvgcode::convert(*print, str_tool_colors, str_color_print_colors, color_print_values, static_cast<size_t>(wxGetApp().extruders_edited_cnt()));
		...
		void GLCanvas3D::on_char(wxKeyEvent &evt) {
			...
			if (m_gizmos.on_char(evt)) {
				if (m_gizmos.get_current_type() == GLGizmosManager::EType::Scale && m_gizmos.get_current()->get_state() == GLGizmoBase::EState::On) {
        ...
        case 'Z':
				...
				switch (keyCode) {
					...
					case WXK_F5: {
						if ((wxGetApp().is_editor() && !wxGetApp().plater()->model().objects.empty()) || (wxGetApp().is_gcode_viewer() && !wxGetApp().plater()->get_last_loaded_gcode().empty()))
					...
					case 'C':
					case 'c': {
						m_gcode_viewer.toggle_gcode_window_visibility();
						m_dirty = true;
						request_extra_frame();
						break;
					}
					...
					case 'K':
					case 'k': {
						wxGetApp().plater()->get_camera().select_next_type();
						m_dirty = true;
						break;
					}
					case 'L':
		...
		void GLCanvas3D::on_key(wxKeyEvent &evt) {
			static TranslationProcessor translationProcessor([this]() {
					...
					if (evt.GetEventType() == wxEVT_KEY_UP) {
						...
						if (m_tab_down && keyCode == WXK_TAB && !evt.HasAnyModifiers()) {
							...
							post_event(SimpleEvent(EVT_GLCANVAS_TAB));
						           ...
						           // (see https://github.com/prusa3d/Printing3D/issues/7799)
						...
						if (keyCode == WXK_SHIFT) {
							...
							if (keyCode == WXK_LEFT || keyCode == WXK_RIGHT || keyCode == WXK_UP || keyCode == WXK_DOWN) {
								if (dynamic_cast<Preview *>(m_canvas->GetParent()) != nullptr)
									post_event(wxKeyEvent(EVT_GLCANVAS_SLIDERS_MANIPULATION, evt));
							}
						}
    ...
    evt.SetY(evt.GetY() * scale);
			...
			// https://github.com/prusa3d/Printing3D/issues/2750
			...
			if (m_undoredo_toolbar.is_item_pressed("undo") || m_undoredo_toolbar.is_item_pressed("redo")) {
																																...
																																std::string format_mouse_event_debug_message(const wxMouseEvent &evt)
{
    ...
    evt.SetY(evt.GetY() * scale);
			...
			if (m_gizmos.on_mouse(evt)) {
				...
				} else if (evt.LeftUp() && m_gizmos.get_current_type() == GLGizmosManager::EType::Scale && m_gizmos.get_current()->get_state() == GLGizmoBase::EState::On) {
					// Update selection from object list to check selection of the cut objects
					// It's not allowed to scale separate ct parts
					wxGetApp().obj_list()->selection_changed();
				}
			...
			if (evt.Entering()) {
				...
				if (layer_editing_object_idx != -1 && m_layers_editing.bar_rect_contains(*this, pos(0), pos(1))) {
					...
					if (evt.LeftDown() && (evt.ShiftDown() || evt.AltDown()) && m_picking_enabled) {
						if (m_gizmos.get_current_type() != GLGizmosManager::SlaSupports && m_gizmos.get_current_type() != GLGizmosManager::FdmSupports && m_gizmos.get_current_type() != GLGizmosManager::Seam && m_gizmos.get_current_type() != GLGizmosManager::Cut && m_gizmos.get_current_type() != GLGizmosManager::Measure && m_gizmos.get_current_type() != GLGizmosManager::MmuSegmentation) {
			...
			} else if (evt.Dragging() && evt.LeftIsDown() && !evt.CmdDown() && m_layers_editing.state == LayersEditing::Unknown && m_mouse.drag.move_volume_idx != -1 && m_mouse.is_start_position_3D_defined()) {
            ...
            logical_pos = logical_pos.cwiseQuotient(Vec2d(factor, factor));
					...
					if (!m_mouse.dragging) {
						...
						const bool post_right_click_event = (m_hover_volume_idxs.empty() || !m_volumes.volumes[get_first_hover_volume_idx()]->is_wipe_tower) && m_gizmos.get_current_type() != GLGizmosManager::Measure;
			...
			if (evt.LeftDClick() && !m_hover_volume_idxs.empty() && (type == GLGizmosManager::EType::Undefined || type == GLGizmosManager::EType::Move || type == GLGizmosManager::EType::Rotate || type == GLGizmosManager::EType::Scale || type == GLGizmosManager::EType::Emboss || type == GLGizmosManager::EType::Svg)) {
    ...
    if (new_scaling != orig_scaling) {
		...
		GLCanvas3D::WipeTowerInfo GLCanvas3D::get_wipe_tower_info() const {
			...
			for (const GLVolume *vol: m_volumes.volumes) {
				if (vol->is_wipe_tower) {
					wti.m_pos = Vec2d(m_config->opt_float("wipe_tower_x"), m_config->opt_float("wipe_tower_y"));
		...
		void GLCanvas3D::update_sequential_clearance(bool force_contours_generation) {
			...
			auto instance_trafo = [](const Transform3d &hull_trafo, const Geometry::Transformation &inst_trafo) {
				...
				offset.z() = 0.0;
				return Geometry::translation_transform(offset) * Geometry::rotation_transform(Geometry::rotation_diff_z(hull_trafo, inst_trafo.get_matrix()) * Vec3d::UnitZ());
			};
        ...
        */
		...
		BoundingBoxf3 GLCanvas3D::_max_bounding_box(bool include_gizmos, bool include_bed_model) const {
			...
			if (!m_picking_enabled) {
				...
				if ((bed_bb_size.x() > 0.0 && bb_size.x() > max_scale_factor * bed_bb_size.x()) || (bed_bb_size.y() > 0.0 && bb_size.y() > max_scale_factor * bed_bb_size.y()) || (bed_bb_size.z() > 0.0 && bb_size.z() > max_scale_factor * bed_bb_size.z())) {
			...
			if (hit.is_valid()) {
				switch (hit.type) {
					...
					case SceneRaycaster::EType::FallbackGizmo: {
						const Size &cnv_size = get_canvas_size();
						const bool inside = 0 <= m_mouse.position.x() && m_mouse.position.x() < cnv_size.get_width() && 0 <= m_mouse.position.y() && m_mouse.position.y() < cnv_size.get_height();
...
#define USE_PARALLEL 1
				...
				tbb::parallel_for(tbb::blocked_range<size_t>(0, frame.size(), (size_t) width), [this, &frame, &idxs, &mutex](const tbb::blocked_range<size_t> &range) {
		...
		void GLCanvas3D::_render_bed(const Transform3d &view_matrix, const Transform3d &projection_matrix, bool bottom) {
			...
			bool show_texture = !bottom || (m_gizmos.get_current_type() != GLGizmosManager::FdmSupports && m_gizmos.get_current_type() != GLGizmosManager::SlaSupports && m_gizmos.get_current_type() != GLGizmosManager::Hollow && m_gizmos.get_current_type() != GLGizmosManager::Seam && m_gizmos.get_current_type() != GLGizmosManager::MmuSegmentation);
				...
				switch (build_volume.type()) {
					case BuildVolume::Type::Rectangle: {
						...
						m_volumes.set_print_volume({0, // circle
						                            {float(bed_bb.min.x()), float(bed_bb.min.y()), float(bed_bb.max.x()), float(bed_bb.max.y())}, {0.0f, float(build_volume.max_print_height())}});
						break;
					}
					case BuildVolume::Type::Circle: {
						m_volumes.set_print_volume({1, // rectangle
						                            {unscaled<float>(build_volume.circle().center.x()), unscaled<float>(build_volume.circle().center.y()), unscaled<float>(build_volume.circle().radius + BuildVolume::SceneEpsilon), 0.0f}, {0.0f, float(build_volume.max_print_height() + BuildVolume::SceneEpsilon)}});
						break;
					}
					default:
					case BuildVolume::Type::Convex:
		...
		bool GLCanvas3D::check_toolbar_icon_size(float init_scale, float &new_scale_to_save, bool is_custom, int counter/* = 3*/) {
			...
			// See: https://github.com/prusa3d/Printing3D/issues/8563
		...
		void GLCanvas3D::_check_and_update_toolbar_icon_scale() {
			...
			if (check_toolbar_icon_size(init_scale, new_scale_to_save, is_custom) && !is_approx(init_scale, new_scale_to_save, 0.015f))
		...
		void GLCanvas3D::_render_collapse_toolbar() const {
			...
			const float left = wxGetApp().is_gcode_viewer() ? 0.5f * (float) cnv_size.get_width() : 0.5f * (float) cnv_size.get_width() - collapse_toolbar.get_width() - band;
#else
    ...
    if (shader != nullptr) {
		...
		void GLCanvas3D::_render_sla_slices() {
			...
			for (unsigned int i = 0; i < (unsigned int) print_objects.size(); ++i) {
				...
				if ((!bottom_obj_triangles.is_initialized() || !bottom_sup_triangles.is_initialized() || !top_obj_triangles.is_initialized() || !top_sup_triangles.is_initialized()) && !obj->get_slice_index().empty()) {
				...
				if (shader != nullptr) {
					...
					for (const SLAPrintObject::Instance &inst: obj->instances()) {
						const Camera &camera = wxGetApp().plater()->get_camera();
						Transform3d view_model_matrix = camera.get_view_matrix() * Geometry::translation_transform({unscale<double>(inst.shift.x()), unscale<double>(inst.shift.y()), 0.0}) * Geometry::rotation_transform(inst.rotation * Vec3d::UnitZ());
		...
		void GLCanvas3D::_update_volumes_hover_state() {
			...
			for (int i: m_hover_volume_idxs) {
				...
				if (select || deselect) {
					const bool as_volume = volume.is_modifier && hover_from_single_instance && !ctrl_pressed && (!deselect || (deselect && !m_selection.is_single_full_instance() && volume.object_idx() == m_selection.get_object_idx() && volume.instance_idx() == m_selection.get_instance_idx()));
		...
		void GLCanvas3D::_perform_layer_editing_action(wxMouseEvent *evt) {
			...
			if (evt != nullptr) {
				...
				m_layers_editing.last_action = evt->ShiftDown() ? (evt->RightIsDown() ? LAYER_HEIGHT_EDIT_ACTION_SMOOTH : LAYER_HEIGHT_EDIT_ACTION_REDUCE) : (evt->RightIsDown() ? LAYER_HEIGHT_EDIT_ACTION_INCREASE : LAYER_HEIGHT_EDIT_ACTION_DECREASE);
			}
		...
		void GLCanvas3D::_load_sla_shells() {
			...
			auto add_volume = [this](const SLAPrintObject &object, int volume_id, const SLAPrintObject::Instance &instance, const indexed_triangle_set &mesh, const ColorRGBA &color, bool outside_printer_detection_enabled) {
		...
		void GLCanvas3D::_set_warning_notification(EWarning warning, bool state) {
			enum ErrorType {
				PLATER_WARNING, PLATER_ERROR, SLICING_ERROR
			};
			std::string text;
			ErrorType error = ErrorType::PLATER_WARNING;
			switch (warning) {
				case EWarning::ObjectOutside:
					text = _u8L("An object outside the print area was detected.");
					break;
				case EWarning::ToolpathOutside:
					text = _u8L("A toolpath outside the print area was detected.");
				...
				case EWarning::SlaSupportsOutside:
					...
					break;
				case EWarning::SomethingNotShown:
					text = _u8L("Some objects are not visible during editing.");
					break;
				...
				case EWarning::GCodeConflict: {
					...
					text = format(_u8L("Conflicts in G-code paths have been detected at layer %1%, z=%2$.2f mm. Please reposition the conflicting objects (%3% <-> %4%) further apart."), layer, height, objName1, objName2);
					error = ErrorType::SLICING_ERROR;
					break;
				}
			}
			...
			if (warning == EWarning::GCodeConflict) {
				if (conflict_result.has_value()) {
					...
					notification_manager.push_notification(NotificationType::SlicingError, NotificationManager::NotificationLevel::ErrorNotificationLevel, _u8L("ERROR:") + "\n" + text, hypertext, action_fn);
				} else

...
#include "GUI/Sidebar.hpp" // IWYU pragma: keep
#include "format.hpp"
#include "GUI_App.hpp"
#include "Plater.hpp"
#include "Tab.hpp"
...
namespace Slic3r {
	...
	namespace GUI {
		...
		static const std::map<Preset::Type, std::string> type_icon_names = {{Preset::TYPE_PRINT,        "cog"},
		...
		// preset(root) node
		ModelNode::ModelNode(Preset::Type preset_type, wxWindow *parent_win, const wxString &text, const std::string &icon_name, const wxString &new_val_column_text) : m_parent_win(parent_win), m_parent(nullptr), m_preset_type(preset_type), m_icon_name(icon_name), m_text(text), m_new_value(new_val_column_text) {
			UpdateIcons();
		}

		// category node
		ModelNode::ModelNode(ModelNode *parent, const wxString &text, const std::string &icon_name) : m_parent_win(parent->m_parent_win), m_parent(parent), m_icon_name(icon_name), m_text(text) {
			UpdateIcons();
		}

		// group node
		ModelNode::ModelNode(ModelNode *parent, const wxString &text) : m_parent_win(parent->m_parent_win), m_parent(parent), m_icon_name("dot_small"), m_text(text) {
			UpdateIcons();
		}
		...
		// option node
		ModelNode::ModelNode(ModelNode *parent, const wxString &text, const wxString &old_value, const wxString &mod_value, const wxString &new_value) : m_parent_win(parent->m_parent_win), m_parent(parent), m_old_color(old_value.StartsWith("#") ? old_value : ""), m_mod_color(mod_value.StartsWith("#") ? mod_value : ""), m_new_color(new_value.StartsWith("#") ? new_value : ""), m_icon_name("empty"), m_text(text), m_old_value(old_value), m_mod_value(mod_value), m_new_value(new_value),
		                                                                                                                                                 m_container(false) {
		...
		DiffModel::DiffModel(wxWindow *parent) : m_parent_win(parent) {
		}
		...
		ModelNode *DiffModel::AddOptionWithGroupAndCategory(ModelNode *preset_node, wxString category_name, wxString group_name, wxString option_name, wxString old_value, wxString mod_value, wxString new_value, const std::string category_icon_name) {
		...
		wxDataViewItem DiffModel::AddOption(Preset::Type type, wxString category_name, wxString group_name, wxString option_name, wxString old_value, wxString mod_value, wxString new_value, const std::string category_icon_name) {
			// "color" strings
		...
		wxDataViewItem DiffModel::Delete(const wxDataViewItem &item) {
			...
			auto it = find_if(parents_children.begin(), parents_children.end(), [node](std::unique_ptr<ModelNode> &child) { return child.get() == node; });
		...
		DiffViewCtrl::DiffViewCtrl(wxWindow *parent, wxSize size) : wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, size, wxDV_VARIABLE_LINE_HEIGHT | wxDV_ROW_LINES
		                                                                                                                      #ifdef _WIN32
		                                                                                                                      | wxBORDER_SIMPLE
#endif
		...
		void DiffViewCtrl::Append(const std::string &opt_key, Preset::Type type, wxString category_name, wxString group_name, wxString option_name, wxString old_value, wxString mod_value, wxString new_value, const std::string category_icon_name) {
		...
		wxString DiffViewCtrl::get_short_string(wxString full_string) {
			size_t max_len = 30;
			if (full_string.IsEmpty() || full_string.StartsWith("#") || (full_string.Find("\n") == wxNOT_FOUND && full_string.Length() < max_len))
				return full_string;
		...
		void DiffViewCtrl::context_menu(wxDataViewEvent &event) {
			...
			FullCompareDialog(it->second.opt_name, it->second.old_val, it->second.mod_val, it->second.new_val, old_value_header, mod_value_header, new_value_header).ShowModal();
		...
		UnsavedChangesDialog::UnsavedChangesDialog(const wxString &caption, const wxString &header, const std::string &app_config_key, int act_buttons) : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, caption + ": " + _L("Unsaved Changes"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER), m_app_config_key(app_config_key), m_buttons(act_buttons) {
			...
			if (def_action == none)
				...
				m_exit_action = def_action == ActTransfer ? Action::Transfer : def_action == ActSave ? Action::Save : Action::Discard;
				if (m_exit_action != Action::Discard)
					save(nullptr, m_exit_action == Action::Save);
			}
		}

		UnsavedChangesDialog::UnsavedChangesDialog(Preset::Type type, PresetCollection *dependent_presets, const std::string &new_selected_preset) : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _L("Switching Presets: Unsaved Changes"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {
		...
		void UnsavedChangesDialog::build(Preset::Type type, PresetCollection *dependent_presets, const std::string &new_selected_preset, const wxString &header) {
			...
			bool add_new_value_column = !new_selected_preset.empty() && dependent_presets && dependent_presets->get_edited_preset().type == type && new_selected_preset != dependent_presets->get_edited_preset().name;
			if (add_new_value_column && dependent_presets->get_edited_preset().type == Preset::TYPE_PRINTER && dependent_presets->get_edited_preset().printer_technology() != dependent_presets->find_preset(new_selected_preset)->printer_technology())
			...
			if (ActionButtons::TRANSFER & m_buttons) {
				...
				if (dependent_presets && switched_presets && (type == dependent_presets->type() ? dependent_presets->get_edited_preset().printer_technology() == dependent_presets->find_preset(new_selected_preset)->printer_technology() : switched_presets->get_edited_preset().printer_technology() == switched_presets->find_preset(new_selected_preset)->printer_technology()))
			...
			if (action == Action::Undef && !m_tree->has_long_strings())
				...
				if (action == Action::Undef)
					...
					if (preset_name.empty())
						text = action == Action::Save ? _L("Save the selected options.") : ActionButtons::KEEP & m_buttons ? _L("Keep the selected settings.") : _L("Transfer the selected settings to the newly selected preset.");
					else
						text = format_wxstr(action == Action::Save ? _L("Save the selected options to preset \"%1%\".") : _L("Transfer the selected options to the newly selected preset \"%1%\"."), preset_name);
		...
		void UnsavedChangesDialog::update_config(Action action) {
			...
			std::string act = action == Action::Transfer ? ActTransfer : action == Action::Discard ? ActDiscard : ActSave;
						...
						if (opt_key == "gcode_substitutions") {
							if (!strings->empty())
								for (size_t id = 0; id < strings->size(); id += 4)
									out += from_u8(strings->get_at(id)) + ";\t" + from_u8(strings->get_at(id + 1)) + ";\t" + from_u8(strings->get_at(id + 2)) + ";\t" + from_u8(strings->get_at(id + 3)) + ";\n";
							return out;
						}
		...
		void UnsavedChangesDialog::update(Preset::Type type, PresetCollection *dependent_presets, const std::string &new_selected_preset, const wxString &header) {
			...
			if (m_transfer_btn) {
				...
				m_transfer_btn->Bind(wxEVT_ENTER_WINDOW, [this, new_selected_preset, is_empty_name](wxMouseEvent &e) {
					show_info_line(Action::Transfer, is_empty_name ? "" : new_selected_preset);
					e.Skip();
				});
			}
			if (m_discard_btn)
				m_discard_btn->Bind(wxEVT_ENTER_WINDOW, [this](wxMouseEvent &e) {
					show_info_line(Action::Discard);
					e.Skip();
				});
			...
			if (type == Preset::TYPE_INVALID) {
				...
				for (Tab *tab: wxGetApp().tabs_list)
					if (tab->supports_printer_technology(printer_technology) && tab->current_preset_is_dirty())
						presets_cnt++;
				m_action_line->SetLabel((header.IsEmpty() ? "" : header + "\n\n") + _L_PLURAL("The following preset was modified", "The following presets were modified", presets_cnt));
			} else {
				wxString action_msg;
				if (dependent_presets && type == dependent_presets->type()) {
					...
					action_msg = format_wxstr(type == Preset::TYPE_PRINTER ? _L("Preset \"%1%\" is not compatible with the new printer profile and it has the following unsaved changes:") : _L("Preset \"%1%\" is not compatible with the new print profile and it has the following unsaved changes:"), presets->get_edited_preset().name);
				}
		...
		void UnsavedChangesDialog::update_tree(Preset::Type type, PresetCollection *presets_, const std::string &new_selected_preset) {
			...
			for (PresetCollection *presets: presets_list) {
				...
				if (type == Preset::TYPE_PRINTER && old_pt == ptFFF && old_config.opt<ConfigOptionStrings>("extruder_colour")->values.size() != mod_config.opt<ConfigOptionStrings>("extruder_colour")->values.size()) {
				...
				for (const std::string &opt_key: dirty_options) {
					...
					m_tree->Append(opt_key, type, option.category_local, option.group_local, option.label_local, get_string_value(opt_key, old_config), get_string_value(opt_key, mod_config), m_tree->has_new_value_column() ? get_string_value(opt_key, new_config) : "", category_icon_map.at(option.category));
				}
		...
		wxString UnsavedChangesDialog::msg_success_saved_modifications(size_t saved_presets_cnt) {
			return _L_PLURAL("The preset modifications are successfully saved", "The presets modifications are successfully saved", static_cast<unsigned int>(saved_presets_cnt));
		}
		...
		FullCompareDialog::FullCompareDialog(const wxString &option_name, const wxString &old_value, const wxString &mod_value, const wxString &new_value, const wxString &old_value_header, const wxString &mod_value_header, const wxString &new_value_header) : wxDialog(nullptr, wxID_ANY, option_name, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {
		...
		static PresetCollection *get_preset_collection(Preset::Type type, PresetBundle *preset_bundle = nullptr) {
			...
			return type == Preset::Type::TYPE_PRINT ? &preset_bundle->prints : type == Preset::Type::TYPE_SLA_PRINT ? &preset_bundle->sla_prints : type == Preset::Type::TYPE_FILAMENT ? &preset_bundle->filaments : type == Preset::Type::TYPE_SLA_MATERIAL ? &preset_bundle->sla_materials : type == Preset::Type::TYPE_PRINTER ? &preset_bundle->printers : nullptr;
		}
		...
		DiffPresetDialog::DiffPresetDialog(MainFrame *mainframe) : DPIDialog(mainframe, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER, "diff_presets_dialog", mainframe->normal_font().GetPointSize()), m_pr_technology(wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology()) {
			// Init bundles
		...
		void DiffPresetDialog::update_controls_visibility(Preset::Type type /* = Preset::TYPE_INVALID*/) {
			for (auto preset_combos: m_preset_combos) {
				...
				bool show = type != Preset::TYPE_INVALID ? type == cb_type : cb_type == Preset::TYPE_PRINTER ? true : m_pr_technology == ptFFF ? cb_type == Preset::TYPE_PRINT || cb_type == Preset::TYPE_FILAMENT : cb_type == Preset::TYPE_SLA_PRINT || cb_type == Preset::TYPE_SLA_MATERIAL;
		...
		void DiffPresetDialog::update_tree() {
			...
			for (auto preset_combos: m_preset_combos) {
				...
				// Collect dirty options.
				const bool deep_compare = type != Preset::TYPE_FILAMENT;
				auto dirty_options = type == Preset::TYPE_PRINTER && left_pt == ptFFF && left_config.opt<ConfigOptionStrings>("extruder_colour")->values.size() < right_congig.opt<ConfigOptionStrings>("extruder_colour")->values.size() ? presets->dirty_options(right_preset, left_preset, deep_compare) : presets->dirty_options(left_preset, right_preset, deep_compare);
				...
				// process changes of extruders count
				if (type == Preset::TYPE_PRINTER && left_pt == ptFFF && left_config.opt<ConfigOptionStrings>("extruder_colour")->values.size() != right_congig.opt<ConfigOptionStrings>("extruder_colour")->values.size()) {
					wxString local_label = _L("Extruders count");
				...
				for (const std::string &opt_key: dirty_options) {
					...
					m_tree->Append(opt_key, type, option.category_local, option.group_local, option.label_local, left_val, right_val, "", category_icon_map.at(option.category));
				}
			}
		...
		void DiffPresetDialog::update_compatibility(const std::string &preset_name, Preset::Type type, PresetBundle *preset_bundle) {
			...
			auto update_compatible_type = [](bool technology_changed, bool on_page, bool show_incompatible_presets) {
				return technology_changed ? PresetSelectCompatibleType::Always : on_page ? PresetSelectCompatibleType::Never : show_incompatible_presets ? PresetSelectCompatibleType::OnlyIfWasCompatible : PresetSelectCompatibleType::Always;
			};
			...
			for (auto preset_combos: m_preset_combos) {
				...
				if ((print_tab && ((pr_tech == ptFFF && presets_type == Preset::TYPE_FILAMENT) || (pr_tech == ptSLA && presets_type == Preset::TYPE_SLA_MATERIAL))) || (printer_tab && ((pr_tech == ptFFF && (presets_type == Preset::TYPE_PRINT || presets_type == Preset::TYPE_FILAMENT)) || (pr_tech == ptSLA && (presets_type == Preset::TYPE_SLA_PRINT || presets_type == Preset::TYPE_SLA_MATERIAL)))))
					cb->update();
			}

			if (technology_changed && m_preset_bundle_left.printers.get_selected_preset().printer_technology() == m_preset_bundle_right.printers.get_selected_preset().printer_technology()) {
				m_pr_technology = m_preset_bundle_left.printers.get_edited_preset().printer_technology();
				...
				if (act == Action::Transfer)
					process_options([this](Preset::Type type) {
						if (Tab *tab = wxGetApp().get_tab(type))
							tab->transfer_options(get_left_preset_name(type), get_right_preset_name(type), get_selected_options(type));
					});

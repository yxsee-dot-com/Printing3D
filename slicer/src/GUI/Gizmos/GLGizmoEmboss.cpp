...
#include "GUI/GUI_ObjectManipulation.hpp"
#include "GUI/MainFrame.hpp" // to update title when add text
#include "GUI/NotificationManager.hpp"
#include "GUI/Plater.hpp"
#include "GUI/MsgDialog.hpp"
#include "GUI/format.hpp"
#include "GUI/I18N.hpp"
#include "GUI/CameraUtils.hpp"
#include "GUI/Jobs/EmbossJob.hpp"
#include "GUI/Jobs/CreateFontNameImageJob.hpp"
#include "GUI/Jobs/NotificationProgressIndicator.hpp"
#include "Utils/WxFontUtils.hpp"
#include "Utils/UndoRedo.hpp"

#include "libslic3r/src/Geometry.hpp" // to range pi pi
#include "libslic3r/src/Timer.hpp"

#include "libslic3r/src/Model.hpp"
#include "libslic3r/src/Preset.hpp"
#include "libslic3r/src/ClipperUtils.hpp" // union_ex
#include "libslic3r/src/AppConfig.hpp"    // store/load font list
#include "libslic3r/src/Format/OBJ.hpp" // load obj file for default object
...
namespace {
	...
	static const struct Limits {
		...
		Limit<float> boldness{{-.1f,  .1f},
		                      {-5e5f, 5e5f}}; // in font points
		Limit<float> skew{{-1.f,   1.f},
		                  {-100.f, 100.f}}; // ration without unit
	...
	std::unique_ptr<DataBase> create_emboss_data_base(const std::string &text, StyleManager &style_manager, TextLinesModel &text_lines, const Selection &selection, ModelVolumeType type, std::shared_ptr<std::atomic<bool>> &cancel);
	...
	enum class IconType : unsigned {
		rename = 0, erase, add, save, undo, italic, unitalic, bold, unbold, system_selector, open_file, exclamation, lock, lock_bold, unlock, unlock_bold, align_horizontal_left, align_horizontal_center, align_horizontal_right, align_vertical_top, align_vertical_center, align_vertical_bottom, // automatic calc of icon's count
		_count
	};
	// Define rendered version of icon
	enum class IconState : unsigned {
		activable = 0, hovered /*1*/, disabled /*2*/ };
...
GLGizmoEmboss::GLGizmoEmboss(GLCanvas3D &parent) : GLGizmoBase(parent, M_ICON_FILENAME, -2), m_gui_cfg(nullptr), m_style_manager(m_imgui->get_glyph_ranges(), create_default_styles), m_face_names(std::make_unique<Facenames>()), m_rotate_gizmo(parent, GLGizmoRotate::Axis::Z) // grab id = 2 (Z axis)
{
...
CreateVolumeParams GLGizmoEmboss::create_input(ModelVolumeType volume_type) {
	...
	return CreateVolumeParams{std::move(base), m_parent, plater->get_camera(), plater->build_volume(), plater->get_ui_job_worker(), volume_type, m_raycast_manager, gizmo, gl_volume, style.distance, style.angle};
}
...
bool GLGizmoEmboss::on_mouse(const wxMouseEvent &mouse_event) {
	// not selected volume
	if (m_volume == nullptr || get_model_volume(m_volume_id, m_parent.get_selection().get_model()->objects) == nullptr || !m_volume->text_configuration.has_value()) return false;
...
void GLGizmoEmboss::volume_transformation_changed() {
	if (m_volume == nullptr || !m_volume->text_configuration.has_value() || !m_volume->emboss_shape.has_value() || !m_style_manager.is_active_font()) {
		assert(false);
		return;
	}
...
void GLGizmoEmboss::on_render() {
	...
	if (!selection.is_single_volume_or_modifier() && !selection.is_single_volume_instance()) return;
...
void GLGizmoEmboss::on_render_input_window(float x, float y, float bottom_limit) {
	...
	if (m_volume == nullptr || get_model_volume(m_volume_id, m_parent.get_selection().get_model()->objects) == nullptr || !m_volume->text_configuration.has_value()) {
...
void GLGizmoEmboss::on_set_state() {
	...
	if (m_state == GLGizmoBase::Off) {
		...
		if (m_allow_open_near_volume) {
			...
			m_set_window_offset = (m_gui_cfg != nullptr) ? ImGuiPureWrap::change_window_position(on_get_name().c_str(), false) : ImVec2(-1, -1);
		}
	}
}
...
EmbossStyles GLGizmoEmboss::create_default_styles() {
	...
	EmbossStyles styles = {WxFontUtils::create_emboss_style(wx_font_normal, _u8L("NORMAL")), // wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT)
...
void GLGizmoEmboss::set_volume_by_selection() {
	...
	if (m_volume == nullptr || get_model_volume(m_volume_id, objects) == nullptr || (m_volume->get_object()->volumes.size() == 1) != (volume->get_object()->volumes.size() == 1)) {
...
void GLGizmoEmboss::draw_text_input() {
	...
	if (!warning_tool_tip.empty()) {
		...
		ImVec2 icon_pos(width - m_gui_cfg->icon_width - scrollbar_width + padding.x, cursor.y - m_gui_cfg->icon_width - scrollbar_height - 2 * padding.y);
	...
	// Extend font ranges
	if (!range_text.empty() && !ImGuiPureWrap::contain_all_glyphs(imgui_font, range_text))
		m_style_manager.clear_imgui_font();
}
...
void GLGizmoEmboss::draw_font_list() {
	...
	const char *selected = (!actual_face_name.empty()) ? actual_face_name.ToUTF8().data() : " --- ";
	...
	ImGuiWindowFlags popup_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_ChildWindow;
	if (ImGui::BeginPopup(popup_id, popup_flags)) {
		...
		if (!ImGui::IsWindowFocused() || (!is_input_text_active && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Escape)))) {
			// closing of popup
...
void GLGizmoEmboss::draw_model_type() {
	...
	if (m_volume != nullptr && new_type.has_value() && !is_last_solid_part) {
		...
		wxDataViewItemArray sel = obj_list->reorder_volumes_and_get_selection(obj_list->get_selected_obj_idx(), [volume = m_volume](const ModelVolume *vol) { return vol == volume; });
...
namespace {
	...
	std::optional<Transform3d> fix_transformation(const StyleManager::Style &from, const StyleManager::Style &to, GLCanvas3D &canvas) {
...
void GLGizmoEmboss::draw_style_list() {
	...
	if (ImGui::BeginCombo("##style_selector", add_text_modify(trunc_name).c_str())) {
		...
		if (ImGui::IsItemHovered()) {
			...
			std::string tooltip = is_modified ? GUI::format(_L("Modified style \"%1%\""), current_style.name) : GUI::format(_L("Current style is \"%1%\""), current_style.name);
			ImGui::SetTooltip(" %s", tooltip.c_str());
		}
	}
...
template<typename T>
bool exist_change(const T &value, const T *default_value) {
...
bool GLGizmoEmboss::revertible(const std::string &name, T &value, const T *default_value, const std::string &undo_tooltip, float undo_offset, Draw draw) const {
...
template<typename T>
bool imgui_input(const char *label, T *v, T step, T step_fast, const char *format, ImGuiInputTextFlags flags);
template<>
bool imgui_input(const char *label, float *v, float step, float step_fast, const char *format, ImGuiInputTextFlags flags) { return ImGui::InputFloat(label, v, step, step_fast, format, flags); }
template<>
bool imgui_input(const char *label, double *v, double step, double step_fast, const char *format, ImGuiInputTextFlags flags) { return ImGui::InputDouble(label, v, step, step_fast, format, flags); }

template<typename T>
bool GLGizmoEmboss::rev_input(const std::string &name, T &value, const T *default_value, const std::string &undo_tooltip, T step, T step_fast, const char *format, ImGuiInputTextFlags flags) const {
	// draw offseted input
...
template<typename T>
bool GLGizmoEmboss::rev_input_mm(const std::string &name, T &value, const T *default_value_ptr, const std::string &undo_tooltip, T step, T step_fast, const char *format, bool use_inch, const std::optional<float> &scale) const {
...
bool GLGizmoEmboss::rev_slider(const std::string &name, std::optional<float> &value, const std::optional<float> *default_value, const std::string &undo_tooltip, const MinMax<float> &min_max, const std::string &format, const wxString &tooltip) const {
	auto draw_slider_optional_float = [&]() -> bool {
		...
		return m_imgui->slider_optional_float(("##" + name).c_str(), value, min_max.min, min_max.max, format.c_str(), 1.f, false, tooltip);
	};
	float undo_offset = ImGui::GetStyle().FramePadding.x;
	return revertible(name, value, default_value, undo_tooltip, undo_offset, draw_slider_optional_float);
}

bool GLGizmoEmboss::rev_slider(const std::string &name, float &value, const float *default_value, const std::string &undo_tooltip, const MinMax<float> &min_max, const std::string &format, const wxString &tooltip) const {
	auto draw_slider_float = [&]() -> bool {
		float slider_offset = m_gui_cfg->advanced_input_offset;
		float slider_width = m_gui_cfg->input_width;
		ImGui::SameLine(slider_offset);
		ImGui::SetNextItemWidth(slider_width);
		return m_imgui->slider_float("##" + name, &value, min_max.min, min_max.max, format.c_str(), 1.f, false, tooltip);
	};
	float undo_offset = ImGui::GetStyle().FramePadding.x;
...
void GLGizmoEmboss::draw_use_surface() {
	...
	const bool *def_use_surface = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->projection.use_surface : nullptr;
...
void GLGizmoEmboss::draw_per_glyph() {
	bool can_use_per_glyph = (m_volume->text_configuration->style.prop.per_glyph) ? true : // already used surface must have option to uncheck
	                         !m_volume->is_the_only_one_part();
	...
	const bool *def_per_glyph = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->prop.per_glyph : nullptr;
	// TRN - Tooltip after mouse hover above reverting button (undo icon)
...
void GLGizmoEmboss::draw_align() {
	...
	auto draw_fnc = [&align, input_offset = m_gui_cfg->advanced_input_offset, &icons = m_icons]() {
		struct HAlignItem {
			FontProp::HorizontalAlign align;
			IconType icon;
			std::string tooltip;
		};
		static const std::array<HAlignItem, 3> h_aligns{{{FontProp::HorizontalAlign::left, IconType::align_horizontal_left, _CTX_utf8(L_CONTEXT("Left", "Alignment"), "Alignment")}, {FontProp::HorizontalAlign::center, IconType::align_horizontal_center, _CTX_utf8(L_CONTEXT("Center", "Alignment"), "Alignment")}, {FontProp::HorizontalAlign::right, IconType::align_horizontal_right, _CTX_utf8(L_CONTEXT("Right", "Alignment"), "Alignment")}}};
		struct VAlignItem {
			FontProp::VerticalAlign align;
			IconType icon;
			std::string tooltip;
		};
		static const std::array<VAlignItem, 3> v_aligns{{{FontProp::VerticalAlign::top, IconType::align_vertical_top, _CTX_utf8(L_CONTEXT("Top", "Alignment"), "Alignment")}, {FontProp::VerticalAlign::center, IconType::align_vertical_center, _CTX_utf8(L_CONTEXT("Middle", "Alignment"), "Alignment")}, {FontProp::VerticalAlign::bottom, IconType::align_vertical_bottom, _CTX_utf8(L_CONTEXT("Bottom", "Alignment"), "Alignment")}}};
		...
		for (const HAlignItem &h_align: h_aligns) {
			...
			else if (draw_button(icons, h_align.icon)) {
				align.first = h_align.align;
				is_change = true;
			}
		...
		for (const VAlignItem &v_align: v_aligns) {
			...
			else if (draw_button(icons, v_align.icon)) {
				align.second = v_align.align;
				is_change = true;
			}
	...
	const FontProp::Align *def_align = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->prop.align : nullptr;
	float undo_offset = ImGui::GetStyle().FramePadding.x;
...
void GLGizmoEmboss::draw_char_gap() {
	...
	auto def_char_gap_font_point = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->prop.char_gap : nullptr;
	...
	float def_char_gap_value = def_char_gap_font_point ? static_cast<float>(def_char_gap_font_point->value_or(0) * scale) : 0.f;
	...
	bool exist_change = false;
	if (rev_slider(tr_char_gap, char_gap, def_char_gap, undo_tooltip, min_max_char_gap, units_fmt, tooltip)) {
...
void GLGizmoEmboss::draw_line_gap() {
	...
	auto def_line_gap_font_point = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->prop.line_gap : nullptr;
	float def_line_gap_value = def_line_gap_font_point ? static_cast<float>(def_line_gap_font_point->value_or(0) * scale) : 0.f;
	...
	bool exist_change = false;
	if (rev_slider(tr_line_gap, line_gap, def_line_gap, undo_tooltip, min_max_line_gap, units_fmt, tooltip)) {
...
void GLGizmoEmboss::draw_boldness() {
	...
	auto def_boldness_font_point = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->prop.boldness : nullptr;
	float def_boldness_value = def_boldness_font_point ? static_cast<float>(def_boldness_font_point->value_or(0) * scale) : 0.f;
	...
	bool exist_change = false;
	if (rev_slider(tr_boldness, boldness, def_boldness, undo_tooltip, min_max_boldness, units_fmt, tooltip)) {
...
void GLGizmoEmboss::draw_skew() {
	...
	std::optional<float> &skew = m_style_manager.get_font_prop().skew;
	auto def_skew = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->prop.skew : nullptr;
	...
	bool exist_change = false;
	if (rev_slider(tr_skew, skew, def_skew, undo_tooltip, limits.skew.gui, "%.2f", tooltip)) {
...
void GLGizmoEmboss::draw_surface_distance() {
	...
	bool allowe_surface_distance = !projection.use_surface && !m_volume->is_the_only_one_part();
	...
	auto def_distance = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->distance : nullptr;
	...
	if (use_inch) {
		...
		if (rev_slider(tr_from_surface, distance, def_distance, undo_move_tooltip, min_max_distance, "%.2f mm", move_tooltip)) is_moved = true;
	}
...
void GLGizmoEmboss::draw_rotation() {
	...
	const auto def_angle_rad = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->angle : nullptr;
	float def_angle_deg_val = def_angle_rad ? static_cast<float>(def_angle_rad->value_or(0.f) * -180 / M_PI) : 0.f;
	...
	bool exist_change = false;
	if (rev_slider(tr_rotation, angle_deg, def_angle_deg, undo_tooltip, limits.angle, u8"%.2f °", tooltip)) {
	...
	if (!m_volume->is_the_only_one_part()) {
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("%s", (m_keep_up ? _u8L("Unlock the text's rotation when moving text along the object's surface.") : _u8L("Lock the text's rotation when moving text along the object's surface.")).c_str());
	}
}
...
void GLGizmoEmboss::create_notification_not_valid_font(const TextConfiguration &tc) {
	...
	std::string text = GUI::format(_L("Can't load exactly same font(\"%1%\"). "
	                                  ...
	                                  "You have to specify font for enable edit text."), face_name_3mf, face_name);
...
void GLGizmoEmboss::init_icons() {
	...
	std::vector<std::string> filenames{"edit_button.svg", "delete.svg", "add_copies.svg", "save.svg", "undo.svg", "make_italic.svg", "make_unitalic.svg", "make_bold.svg", "make_unbold.svg", "search.svg", "open.svg", "exclamation.svg", "lock_closed.svg",  // lock,
	                                   "lock_closed_f.svg",// lock_bold,
	                                   "lock_open.svg",    // unlock,
	                                   "lock_open_f.svg",  // unlock_bold,
	                                   "align_horizontal_left.svg", "align_horizontal_center.svg", "align_horizontal_right.svg", "align_vertical_top.svg", "align_vertical_center.svg", "align_vertical_bottom.svg"};
...
namespace {
	...
	TextDataBase::TextDataBase(DataBase &&parent, const FontFileWithCache &font_file, TextConfiguration &&text_configuration, const EmbossProjection &projection) : DataBase(std::move(parent)), m_font_file(font_file) /* copy */, m_text_configuration(std::move(text_configuration)) {
	...
	bool TextDataBase::create_text_lines(const Transform3d &tr, const ModelVolumePtrs &vols) {
		...
		text_lines = Slic3r::Emboss::create_text_lines(tr, vols, *m_font_file.font_file, m_text_configuration.style.prop, count_lines);
		if (text_lines.empty())
	...
	void init_face_names(Facenames &face_names) {
		...
		ScopeGuard sg([&enumerate_start, &face_names = face_names]() {
			...
			BOOST_LOG_TRIVIAL(info) << "OS enumerate " << face_names.faces.size() << " fonts " << "(+ " << face_names.bad.size() << " can't load " << "= " << face_names.faces.size() + face_names.bad.size() << " fonts) " << "in " << enumerate_duration << " ms\n" << concat(face_names.bad);
		});
		...
		if (face_names.hash == hash) {
			...
			BOOST_LOG_TRIVIAL(info) << "Same FontNames hash, cache is used. " << "For clear cache delete file: " << get_fontlist_cache_path().string();
			return;
		}
	...
	void draw_font_preview(FaceName &face, const std::string &text, Facenames &faces, const GuiCfg &cfg, bool is_visible) {
		...
		if (face.is_created != nullptr) {
			...
			FontImageData data{text, face.wx_name, faces.encoding, faces.texture_id, faces.texture_index, cfg.face_name_size, gray_level, format, type, level, &count_opened_fonts, face.cancel,    // copy
	...
	GuiCfg create_gui_configuration() {
		...
		float max_advanced_text_width = std::max(
				{ImGui::CalcTextSize(tr.use_surface.c_str()).x, ImGui::CalcTextSize(tr.per_glyph.c_str()).x, ImGui::CalcTextSize(tr.alignment.c_str()).x, ImGui::CalcTextSize(tr.char_gap.c_str()).x, ImGui::CalcTextSize(tr.line_gap.c_str()).x, ImGui::CalcTextSize(tr.boldness.c_str()).x, ImGui::CalcTextSize(tr.skew_ration.c_str()).x, ImGui::CalcTextSize(tr.from_surface.c_str()).x, ImGui::CalcTextSize(tr.rotation.c_str()).x + cfg.icon_width + 2 * space, ImGui::CalcTextSize(tr.keep_up.c_str()).x,
		...
		float window_width = cfg.input_offset + cfg.input_width + 2 * style.WindowPadding.x + 2 * (cfg.icon_width + space);
		...
		cfg.minimal_window_size_with_advance = ImVec2(cfg.minimal_window_size.x, cfg.minimal_window_size.y + advance_height);

		cfg.minimal_window_size_with_collections = ImVec2(cfg.minimal_window_size_with_advance.x, cfg.minimal_window_size_with_advance.y + input_height);

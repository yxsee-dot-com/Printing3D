...
#include "libslic3r/src/QuadricEdgeCollapse.hpp"
...
namespace Slic3r::GUI {
	...
	GLGizmoSimplify::GLGizmoSimplify(GLCanvas3D &parent) : GLGizmoBase(parent, M_ICON_FILENAME, -1), m_show_wireframe(false), m_move_to_center(false), m_original_triangle_count(0), m_triangle_count(0)
			// translation for GUI size
			, tr_mesh_name(_u8L("Mesh name")), tr_triangles(_u8L("Triangles")), tr_detail_level(_u8L("Level of detail")), tr_decimate_ratio(_u8L("Decimate ratio")) {}
	...
	void GLGizmoSimplify::add_simplify_suggestion_notification(const std::vector<size_t> &object_ids, const std::vector<ModelObject *> &objects, NotificationManager &manager) {
	...
	void GLGizmoSimplify::on_render_input_window(float x, float y, float bottom_limit) {
		...
		if (act_volume_ids.empty()) {
			...
			if (m_parent.get_selection().volumes_count() != 1) {
				MessageDialog msg((wxWindow *) wxGetApp().mainframe, _L("Simplification is currently only allowed when a single part is selected"), _L("Error"));
				msg.ShowModal();
			}
			return;
		}
		...
		if (m_volume_ids != act_volume_ids) {
			...
			if (change_window_position) {
				...
				if (m_move_to_center) {
					...
					pos = ImVec2(parent_size.get_width() / 2 - m_gui_cfg->window_offset_x, parent_size.get_height() / 2 - m_gui_cfg->window_offset_y);
				} else {
					...
					ImVec2 br(parent_size.get_width() - (2 * m_gui_cfg->window_offset_x + m_gui_cfg->window_padding), parent_size.get_height() - (2 * m_gui_cfg->window_offset_y + m_gui_cfg->window_padding));
		...
		std::vector<std::string> reduce_captions = {static_cast<std::string>(_u8L("Extra high")), static_cast<std::string>(_u8L("High")), static_cast<std::string>(_u8L("Medium")), static_cast<std::string>(_u8L("Low")), static_cast<std::string>(_u8L("Extra low"))};
		...
		if (!m_configuration.use_count) {
			...
			m_configuration.decimate_ratio = (1.0f - (m_configuration.wanted_count / (float) m_original_triangle_count)) * 100.f;
		}
	...
	void GLGizmoSimplify::create_gui_cfg() {
		...
		GuiCfg cfg;
		cfg.top_left_width = std::max(ImGuiPureWrap::calc_text_size(tr_mesh_name).x, ImGuiPureWrap::calc_text_size(tr_triangles).x) + space_size;

		const float radio_size = ImGui::GetFrameHeight();
		cfg.bottom_left_width = std::max(ImGuiPureWrap::calc_text_size(tr_detail_level).x, ImGuiPureWrap::calc_text_size(tr_decimate_ratio).x) + space_size + radio_size;

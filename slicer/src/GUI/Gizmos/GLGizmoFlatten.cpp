...
namespace Slic3r {
	namespace GUI {
		...
		GLGizmoFlatten::GLGizmoFlatten(GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id) : GLGizmoBase(parent, icon_filename, sprite_id) {}
		...
		void GLGizmoFlatten::on_register_raycasters_for_picking() {
			...
			if (!m_planes.empty()) {
				const Selection &selection = m_parent.get_selection();
				const Transform3d matrix = Geometry::translation_transform(selection.get_first_volume()->get_sla_shift_z() * Vec3d::UnitZ()) * selection.get_first_volume()->get_instance_transformation().get_matrix();
		...
		void GLGizmoFlatten::update_planes() {
			...
			while (1) {
				...
				if (verts.size() == 3 && ((verts[0] - verts[1]).norm() < minimal_side || (verts[0] - verts[2]).norm() < minimal_side || (verts[1] - verts[2]).norm() < minimal_side))
					m_planes.pop_back();
			}
			...
			for (unsigned int polygon_id = 0; polygon_id < m_planes.size(); ++polygon_id) {
				...
				if (k != 0) {
					...
					for (unsigned int i = 0; i < k; ++i) {
						...
						for (unsigned int j = 0; j < N; ++j)
							points_out[2 * k * j] = (1 - aggressivity) * points_out[2 * k * j] + aggressivity * 0.5f * (points_out[neighbours[j].first] + points_out[neighbours[j].second]);
		...
		bool GLGizmoFlatten::is_plane_update_necessary() const {
			...
			if (!mo->instances.front()->get_scaling_factor().isApprox(m_first_instance_scale) || !mo->instances.front()->get_mirror().isApprox(m_first_instance_mirror))
				return true;

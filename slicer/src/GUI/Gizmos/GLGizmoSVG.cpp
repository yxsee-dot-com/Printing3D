...
#include "libslic3r/src/SVG.hpp"      // debug store
#include "libslic3r/src/Geometry.hpp" // covex hull 2d
#include "libslic3r/src/Timer.hpp" // covex hull 2d
#include "libslic3r/src/Emboss.hpp" // heal_shape

#include "libslic3r/src/NSVGUtils.hpp"
...
GLGizmoSVG::GLGizmoSVG(GLCanvas3D &parent) : GLGizmoBase(parent, M_ICON_FILENAME, -3), m_gui_cfg(nullptr), m_rotate_gizmo(parent, GLGizmoRotate::Axis::Z) // grab id = 2 (Z axis)
{
...
namespace {
	...
	enum class IconType : unsigned {
		reset_value, reset_value_hover, refresh, refresh_hover, change_file, bake, bake_inactive, save, exclamation, lock, lock_hover, unlock, unlock_hover, reflection_x, reflection_x_hover, reflection_y, reflection_y_hover, // automatic calc of icon's count
		_count
	};
...
CreateVolumeParams GLGizmoSVG::create_input(ModelVolumeType volume_type, std::string_view svg_filepath) {
	...
	return CreateVolumeParams{std::move(base), m_parent, plater->get_camera(), plater->build_volume(), plater->get_ui_job_worker(), volume_type, m_raycast_manager, gizmo, gl_volume};
}
...
void GLGizmoSVG::on_mouse_confirm_edit(const wxMouseEvent &mouse_event) {
	...
	if ((mouse_event.LeftUp() || mouse_event.RightUp()) && m_parent.get_first_hover_volume_idx() < 0 && !was_dragging && m_volume != nullptr && m_volume->is_svg()) {
		// current volume
...
bool GLGizmoSVG::on_mouse(const wxMouseEvent &mouse_event) {
	// not selected volume
	if (m_volume == nullptr || get_model_volume(m_volume_id, m_parent.get_selection().get_model()->objects) == nullptr || !m_volume->emboss_shape.has_value()) return false;
...
namespace {
	IconManager::Icons init_icons(IconManager &mng, const GuiCfg &cfg) {
		...
		IconManager::InitTypes init_types{{"undo.svg",          size, IconManager::RasterType::white_only_data}, // reset_value
...
void GLGizmoSVG::on_render_input_window(float x, float y, float bottom_limit) {
	...
	const ImVec2 &window_size = m_volume->is_the_only_one_part() ? m_gui_cfg->window_size_for_object : m_gui_cfg->window_size;
...
#include "GUI/BitmapCache.hpp"
#include "nanosvg/nanosvgrast.h"
#include "libslic3r/src/AABBTreeLines.hpp" // aabb lines for draw filled expolygon
...
namespace {
	...
	GuiCfg create_gui_configuration() {
		...
		float max_tr_width = std::max({ImGui::CalcTextSize(tr.depth.c_str()).x, ImGui::CalcTextSize(tr.size.c_str()).x + lock_width, ImGui::CalcTextSize(tr.use_surface.c_str()).x, ImGui::CalcTextSize(tr.distance.c_str()).x, ImGui::CalcTextSize(tr.rotation.c_str()).x + lock_width, ImGui::CalcTextSize(tr.mirror.c_str()).x,});

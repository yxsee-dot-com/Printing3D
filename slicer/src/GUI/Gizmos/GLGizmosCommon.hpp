...
namespace Slic3r {
	...
	namespace GUI {
		...
		enum class SLAGizmoEventType : unsigned char {
			LeftDown = 1, LeftUp, RightDown, RightUp, Dragging, Delete, SelectAll, CtrlDown, CtrlUp, ShiftDown, ShiftUp, AltUp, Escape, ApplyChanges, DiscardChanges, AutomaticGeneration, ManualEditing, MouseWheelUp, MouseWheelDown, ResetClippingPlane, Moving
		};
		...
		enum class CommonGizmosDataID {
			None = 0, SelectionInfo = 1 << 0, InstancesHider = 1 << 1, Raycaster = 1 << 3, ObjectClipper = 1 << 4, SupportsClipper = 1 << 5,
		...
		namespace CommonGizmosDataObjects {
			...
			class SupportsClipper : public CommonGizmosDataBase {
				public:
					explicit SupportsClipper(CommonGizmosDataPool *cgdp) : CommonGizmosDataBase(cgdp) {}
#ifndef NDEBUG

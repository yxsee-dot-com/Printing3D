...
#include "GUI/GUI.hpp"
#include "GUI/GUI_ObjectSettings.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		GLGizmoSlaSupports::GLGizmoSlaSupports(GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id) : GLGizmoSlaBase(parent, icon_filename, sprite_id, slaposDrillHoles) {
		...
		void GLGizmoSlaSupports::render_points(const Selection &selection) {
			...
			for (size_t i = 0; i < cache_size; ++i) {
				...
				if (m_editing_mode) {
					...
					const Transform3d model_matrix = transformation.get_matrix() * support_matrix * Transform3d(aa.toRotationMatrix()) * Geometry::translation_transform((CONE_HEIGHT + support_point.head_front_radius * RenderPointScale) * Vec3d::UnitZ()) * Geometry::rotation_transform({double(PI), 0.0, 0.0}) * Geometry::scale_transform({CONE_RADIUS, CONE_RADIUS, CONE_HEIGHT});
		...
		bool GLGizmoSlaSupports::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down, bool control_down) {
			...
			if (m_editing_mode) {
				...
				if ((action == SLAGizmoEventType::LeftUp || action == SLAGizmoEventType::ShiftUp || action == SLAGizmoEventType::AltUp) && m_selection_rectangle.is_dragging()) {
					...
					for (size_t idx: m_c->raycaster()->raycaster()->get_unobscured_idxs(trafo, wxGetApp().plater()->get_camera(), points_inside, m_c->object_clipper()->get_clipping_plane())) {
				...
				if (action == SLAGizmoEventType::DiscardChanges) {
					ask_about_changes([this]() { editing_mode_apply_changes(); }, [this]() { editing_mode_discard_changes(); });
					return true;
				}
...
RENDER_AGAIN:
		...
		void GLGizmoSlaSupports::on_set_state() {
			if (m_state == On) { // the gizmo was just turned on
				...
				if (m_editing_mode && unsaved_changes() && on_is_activable()) {
					if (!ask_about_changes([this]() { editing_mode_apply_changes(); }, [this]() { editing_mode_discard_changes(); })) {
						m_state = On;
						return;
					}
				} else {
		...
		bool GLGizmoSlaSupports::on_mouse(const wxMouseEvent &mouse_event) {
			...
			if (!mouse_event.ShiftDown() && !mouse_event.AltDown() && use_grabbers(mouse_event)) return true;
			...
			if (mouse_event.LeftDown()) {
				...
				if (m_parent.get_move_volume_id() != -1) {
					...
					return true;
				} else if (!control_down && gizmo_event(SLAGizmoEventType::Dragging, mouse_pos, mouse_event.ShiftDown(), mouse_event.AltDown(), false)) {
		...
		void GLGizmoSlaSupports::auto_generate() {
			...
			MessageDialog dlg(GUI::wxGetApp().plater(), _L("Autogeneration will erase all manually edited points.") + "\n\n" + _L("Are you sure you want to do it?") + "\n", _L("Warning"), wxICON_WARNING | wxYES | wxNO);
		...
		void GLGizmoSlaSupports::register_point_raycasters_for_picking() {
			...
			if (m_editing_mode && !m_editing_cache.empty()) {
				for (size_t i = 0; i < m_editing_cache.size(); ++i) {
					m_point_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, i, *m_sphere.mesh_raycaster, Transform3d::Identity()), m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, i, *m_cone.mesh_raycaster, Transform3d::Identity()));
				}
				update_point_raycasters_for_picking_transform();
			}
		}
		...
		void GLGizmoSlaSupports::update_point_raycasters_for_picking_transform() {
			...
			for (size_t i = 0; i < m_editing_cache.size(); ++i) {
				...
				q.setFromTwoVectors(Vec3d::UnitZ(), instance_scaling_matrix_inverse * m_editing_cache[i].normal.cast<double>());
				const Eigen::AngleAxisd aa(q);
				const Transform3d cone_matrix = transformation.get_matrix() * support_matrix * Transform3d(aa.toRotationMatrix()) * Geometry::assemble_transform((CONE_HEIGHT + m_editing_cache[i].support_point.head_front_radius * RenderPointScale) * Vec3d::UnitZ(), Vec3d(PI, 0.0, 0.0), Vec3d(CONE_RADIUS, CONE_RADIUS, CONE_HEIGHT));

...
#include "GUI/GUI_Factories.hpp"
#include "GUI/format.hpp"
#include "GUI/Field.hpp"
#include "Utils/UndoRedo.hpp"
#include "Utils/FixModelByWin10.hpp"
#include "libslic3r/src/AppConfig.hpp"
#include "libslic3r/src/TriangleMeshSlicer.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		GLGizmoCut3D::GLGizmoCut3D(GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id) : GLGizmoBase(parent, icon_filename, sprite_id), m_connectors_group_id(GrabberID::Count), m_connector_type(CutConnectorType::Plug), m_connector_style(int(CutConnectorStyle::Prism)), m_connector_shape_id(int(CutConnectorShape::Circle)) {
			...
			std::map<const wchar_t, std::string> connetor_types = {{ImGui::PlugMarker,  _u8L("Plug")},
			...
			m_part_orientation_names = {{"none",   _u8L("Keep orientation")},
			                            {"on_cut", _u8L("Place on cut")},
			                            {"flip",   _u8L("Flip upside down")},};

			m_labels_map = {{"Connectors",   _u8L("Connectors")},
			                {"Type",         _u8L("Type")},
			                {"Style",        _u8L("Style")},
			                {"Shape",        _u8L("Shape")},
			                {"Depth",        _u8L("Depth")},
		...
		bool GLGizmoCut3D::on_mouse(const wxMouseEvent &mouse_event) {
			...
			if (mouse_event.LeftDown()) {
				...
				if ((!shift_down || grabber_contains_mouse) && gizmo_event(SLAGizmoEventType::LeftDown, mouse_pos, mouse_event.ShiftDown(), mouse_event.AltDown(), false))
					return true;
		...
		bool GLGizmoCut3D::render_slider_double_input(const std::string &label, float &value_in, float &tolerance_in, float min_val/* = -0.1f*/, float max_tolerance/* = -0.1f*/) {
			...
			auto render_slider = [this, f_mm_to_in](const std::string &label, float &val, float def_val, float max_val, const wxString &tooltip) {
		...
		void GLGizmoCut3D::render_cut_plane_grabbers() {
			...
			if (!no_xy_dragging && m_hover_id != CutPlaneZRotation && m_hover_id != CutPlaneXMove && m_hover_id != CutPlaneYMove) {
				...
				color = m_hover_id == Y ? complementary(ColorRGBA::GREEN()) : m_hover_id == X ? complementary(ColorRGBA::RED()) : m_hover_id == Z ? GRABBER_COLOR : ColorRGBA::GRAY();
		...
		void GLGizmoCut3D::on_load(cereal::BinaryInputArchive &ar) {
			...
			ar(m_keep_upper, m_keep_lower, m_rotate_lower, m_rotate_upper, m_hide_cut_plane, mode, m_connectors_editing, m_ar_plane_center, m_rotation_m, groove_depth, groove_width, groove_flaps_angle, groove_angle, groove_depth_tolerance, groove_width_tolerance);
			...
			if (m_mode != mode)
				...
				if (!is_approx(m_groove.depth, groove_depth) || !is_approx(m_groove.width, groove_width) || !is_approx(m_groove.flaps_angle, groove_flaps_angle) || !is_approx(m_groove.angle, groove_angle) || !is_approx(m_groove.depth_tolerance, groove_depth_tolerance) || !is_approx(m_groove.width_tolerance, groove_width_tolerance)) {
		...
		void GLGizmoCut3D::on_save(cereal::BinaryOutputArchive &ar) const {
			ar(m_keep_upper, m_keep_lower, m_rotate_lower, m_rotate_upper, m_hide_cut_plane, m_mode, m_connectors_editing, m_ar_plane_center, m_start_dragging_m, m_groove.depth, m_groove.width, m_groove.flaps_angle, m_groove.angle, m_groove.depth_tolerance, m_groove.width_tolerance);
		}
		...
		void GLGizmoCut3D::init_picking_models() {
			...
			if (!m_plane.model.is_initialized() && !m_hide_cut_plane && !m_connectors_editing) {
				...
				indexed_triangle_set its = m_mode == size_t(CutMode::cutTongueAndGroove) ? its_make_groove_plane() : its_make_frustum_dowel((double) m_cut_plane_radius_koef * m_radius, cp_width, m_cut_plane_as_circle ? 180 : 4);
		...
		GLGizmoCut3D::PartSelection::PartSelection(const ModelObject *mo, const Transform3d &cut_matrix, int instance_idx_in, const Vec3d &center, const Vec3d &normal, const CommonGizmosDataObjects::ObjectClipper &oc) : m_instance_idx(instance_idx_in) {
		...
		void GLGizmoCut3D::PartSelection::render(const Vec3d *normal, GLModel &sphere_model) {
			...
			if (GLShaderProgram *shader = wxGetApp().get_shader("gouraud_light")) {
				...
				for (size_t id = 0; id < m_parts.size(); ++id) {
					if (!m_parts[id].is_modifier && normal && ((is_looking_forward && m_parts[id].selected) || (!is_looking_forward && !m_parts[id].selected)))
						continue;
		...
		void GLGizmoCut3D::PartSelection::toggle_selection(const Vec2d &mouse_pos) {
			...
			if (!hits_id_and_sqdist.empty()) {
				size_t id = std::min_element(hits_id_and_sqdist.begin(), hits_id_and_sqdist.end(), [](const std::pair<size_t, double> &a, const std::pair<size_t, double> &b) { return a.second < b.second; })->first;
...
*/
		...
		void GLGizmoCut3D::render_build_size() {
			...
			std::string size = "X: " + into_u8(double_to_string(tbb_sz.x() * koef, 2)) + unit_str + ",  Y: " + into_u8(double_to_string(tbb_sz.y() * koef, 2)) + unit_str + ",  Z: " + into_u8(double_to_string(tbb_sz.z() * koef, 2)) + unit_str;
		...
		void GLGizmoCut3D::render_cut_plane_input_window(CutConnectors &connectors) {
			...
			if (mode == CutMode::cutPlanar || mode == CutMode::cutTongueAndGroove) {
				...
				ImGui::SameLine();
				ImGuiPureWrap::text_colored(ImGuiPureWrap::COL_ORANGE_LIGHT, into_u8(get_wraped_wxString(_L("Hold SHIFT key to draw a cut line"), 40)));
				ImGui::Separator();
		...
		void GLGizmoCut3D::render_input_window_warning() const {
			...
			if (!m_invalid_connectors_idxs.empty()) {
				...
				if (m_info_stats.outside_cut_contour > size_t(0))
					out += "\n - " + GUI::format(_L_PLURAL("%1$d connector is out of cut contour", "%1$d connectors are out of cut contour", m_info_stats.outside_cut_contour), m_info_stats.outside_cut_contour);
				if (m_info_stats.outside_bb > size_t(0))
					out += "\n - " + GUI::format(_L_PLURAL("%1$d connector is out of object", "%1$d connectors are out of object", m_info_stats.outside_bb), m_info_stats.outside_bb);
		...
		bool GLGizmoCut3D::is_outside_of_cut_contour(size_t idx, const CutConnectors &connectors, const Vec3d cur_pos) {
			...
			const int sectorCount = shape == CutConnectorShape::Triangle ? 3 : shape == CutConnectorShape::Square ? 4 : shape == CutConnectorShape::Circle ? 60 : // supposably, 60 points are enough for conflict detection
		...
		bool GLGizmoCut3D::is_conflict_for_connector(size_t idx, const CutConnectors &connectors, const Vec3d cur_pos) {
			...
			const Transform3d matrix = translation_transform(cur_pos) * m_rotation_m * scale_transform(Vec3f(cur_connector.radius, cur_connector.radius, cur_connector.height).cast<double>());
		...
		void GLGizmoCut3D::render_connectors() {
			...
			if (cut_line_processing() || CutMode(m_mode) != CutMode::cutPlanar || m_connector_mode == CutConnectorMode::Auto || !m_c->selection_info())
				return;
			...
			for (size_t i = 0; i < connectors.size(); ++i) {
				...
				const Transform3d view_model_matrix = camera.get_view_matrix() * translation_transform(pos) * m_rotation_m * rotation_transform(-connector.z_angle * Vec3d::UnitZ()) * scale_transform(Vec3f(connector.radius, connector.radius, height).cast<double>());
		...
		static void check_objects_after_cut(const ModelObjectPtrs &objects) {
			...
			if (!err_objects_names.empty()) {
				...
				WarningDialog(plater, format_wxstr("Objects(%1%) have duplicated connectors. "
				                                   ...
				                                   "Please report to Printing3D team in which scenario this issue happened.\n"
			...
			if (is_windows10() && !err_objects_idxs.empty()) {
				auto dlg = WarningDialog(plater, _L("Open edges or errors were detected after the cut.\n"
				                                    "Do you want to fix them by Windows repair algorithm?"), _L("Errors detected after cut operation"), wxYES_NO);
				if (dlg.ShowModal() == wxID_YES) {
					...
					auto fix_and_update_progress = [model_names, &objects](const int obj_idx, int model_idx, wxProgressDialog &progress_dlg, std::vector<std::string> &succes_models, std::vector<std::pair<std::string, std::string>> &failed_models) -> bool {
		...
		void GLGizmoCut3D::perform_cut(const Selection &selection) {
			...
			// perform cut
			{
				...
				ModelObjectCutAttributes attributes =
						only_if(has_connectors ? true : m_keep_upper, ModelObjectCutAttribute::KeepUpper) | only_if(has_connectors ? true : m_keep_lower, ModelObjectCutAttribute::KeepLower) | only_if(has_connectors ? false : m_keep_as_parts, ModelObjectCutAttribute::KeepAsParts) | only_if(m_place_on_cut_upper, ModelObjectCutAttribute::PlaceOnCutUpper) | only_if(m_place_on_cut_lower, ModelObjectCutAttribute::PlaceOnCutLower) | only_if(m_rotate_upper, ModelObjectCutAttribute::FlipUpper) |
						only_if(m_rotate_lower, ModelObjectCutAttribute::FlipLower) | only_if(dowels_count > 0, ModelObjectCutAttribute::CreateDowels) | only_if(!has_connectors && !cut_with_groove && cut_mo->cut_id.id().invalid(), ModelObjectCutAttribute::InvalidateCutInfo);
				...
				const ModelObjectPtrs &new_objects = cut_by_contour ? cut.perform_by_contour(m_part_selection.get_cut_parts(), dowels_count) : cut_with_groove ? cut.perform_with_groove(m_groove, m_rotation_m) : cut.perform_with_plane();
		...
		bool GLGizmoCut3D::add_connector(CutConnectors &connectors, const Vec2d &mouse_position) {
			...
			if (unproject_on_cut_plane(mouse_position.cast<double>(), pos, pos_world)) {
				...
				connectors.emplace_back(pos, m_rotation_m, m_connector_size * 0.5f, m_connector_depth_ratio, m_connector_size_tolerance * 0.5f, m_connector_depth_ratio_tolerance, m_connector_angle, CutConnectorAttributes(CutConnectorType(m_connector_type), CutConnectorStyle(m_connector_style), CutConnectorShape(m_connector_shape_id)));
		...
		bool GLGizmoCut3D::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down, bool control_down) {
			...
			if ((m_hover_id < 0 || m_hover_id == CutPlane) && shift_down && !m_connectors_editing && (action == SLAGizmoEventType::LeftDown || action == SLAGizmoEventType::LeftUp || action == SLAGizmoEventType::Moving))
		...
		CommonGizmosDataID GLGizmoCut3D::on_get_requirements() const {
			return CommonGizmosDataID(int(CommonGizmosDataID::SelectionInfo) | int(CommonGizmosDataID::InstancesHider) | int(CommonGizmosDataID::ObjectClipper));
		}
		...
		void GLGizmoCut3D::apply_cut_connectors(ModelObject *mo, const std::string &connector_name) {
			...
			for (const CutConnector &connector: mo->cut_connectors) {
				...
				new_volume->set_transformation(translation_transform(connector.pos) * connector.rotation_m * rotation_transform(-connector.z_angle * Vec3d::UnitZ()) * scale_transform(Vec3f(connector.radius, connector.radius, connector.height).cast<double>()));

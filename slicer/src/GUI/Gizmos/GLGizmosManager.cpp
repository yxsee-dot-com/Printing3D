...
#include "GUI/Gizmos/GLGizmoSeam.hpp"
#include "GUI/Gizmos/GLGizmoMmuSegmentation.hpp"
#include "GUI/Gizmos/GLGizmoSimplify.hpp"
#include "GUI/Gizmos/GLGizmoEmboss.hpp"
#include "GUI/Gizmos/GLGizmoSVG.hpp"
#include "GUI/Gizmos/GLGizmoMeasure.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		GLGizmosManager::GLGizmosManager(GLCanvas3D &parent) : m_parent(parent), m_enabled(false), m_icons_texture_dirty(true), m_current(Undefined), m_hover(Undefined), m_tooltip(""), m_serializing(false) {
		}
		...
		GLGizmosManager::EType GLGizmosManager::get_gizmo_from_mouse(const Vec2d &mouse_pos) const {
			...
			if (border <= (float) mouse_pos(0) && (float) mouse_pos(0) <= (border + gap_x + icons_size)) {
				// which icon is it on?
		...
		bool GLGizmosManager::check_gizmos_closed_except(EType type) const {
			if (get_current_type() != type && get_current_type() != Undefined) {
				wxGetApp().plater()->get_notification_manager()->push_notification(NotificationType::CustomSupportsAndSeamRemovedAfterRepair, NotificationManager::NotificationLevel::PrintInfoNotificationLevel, _u8L("ERROR: Please close all manipulators available from "
		...
		ClippingPlane GLGizmosManager::get_clipping_plane() const {
			if (!m_common_gizmos_data || !m_common_gizmos_data->object_clipper() || m_common_gizmos_data->object_clipper()->get_position() == 0.)
		...
		bool GLGizmosManager::wants_reslice_supports_on_undo() const {
			return (m_current == SlaSupports && dynamic_cast<const GLGizmoSlaSupports *>(m_gizmos.at(SlaSupports).get())->has_backend_supports());
		}
		...
		void GLGizmosManager::update_after_undo_redo(const UndoRedo::Snapshot &snapshot) {
			update_data();
			m_serializing = false;
			if (m_current == SlaSupports && snapshot.snapshot_data.flags & UndoRedo::SnapshotData::RECALCULATE_SLA_SUPPORTS)
		...
		void GLGizmosManager::render_arrow(const GLCanvas3D &parent, EType highlighted_type) const {
			...
			for (size_t idx: selectable_idxs) {
				if (idx == highlighted_type) {
					...
					GLTexture::render_sub_texture(tex_id, left, right, top_y, top_y + icons_size_y, {{left_uv,  bottom_uv},
					                                                                                 {left_uv,  top_uv},
					                                                                                 {right_uv, top_uv},
					                                                                                 {right_uv, bottom_uv}});
					break;
		...
		void GLGizmosManager::do_render_overlay() const {
			...
			for (size_t idx: selectable_idxs) {
				...
				GLTexture::render_sub_texture(icons_texture_id, top_x, top_x + icons_size_x, top_y - icons_size_y, top_y, {{u_left,  v_bottom},
				                                                                                                           {u_right, v_bottom},
				                                                                                                           {u_right, v_top},
				                                                                                                           {u_left,  v_top}});
		...
		bool GLGizmosManager::activate_gizmo(EType type) {
			...
			if (m_current != Undefined) {
				...
				if (!m_serializing && old_gizmo.wants_enter_leave_snapshots())
					Plater::TakeSnapshot snapshot(wxGetApp().plater(), old_gizmo.get_gizmo_leaving_text(), UndoRedo::SnapshotType::LeavingGizmoWithAction);
			}
			...
			if (!m_serializing && new_gizmo.wants_enter_leave_snapshots())
				Plater::TakeSnapshot snapshot(wxGetApp().plater(), new_gizmo.get_gizmo_entering_text(), UndoRedo::SnapshotType::EnteringGizmo);
		...
		bool GLGizmosManager::is_in_editing_mode(bool error_notification) const {
			...
			if (error_notification)
				wxGetApp().plater()->get_notification_manager()->push_notification(NotificationType::QuitSLAManualMode, NotificationManager::NotificationLevel::ErrorNotificationLevel, _u8L("You are currently editing SLA support points. Please, "
		...
		bool GLGizmosManager::is_hiding_instances() const {
			return (m_common_gizmos_data && m_common_gizmos_data->instances_hider() && m_common_gizmos_data->instances_hider()->is_valid());
		}

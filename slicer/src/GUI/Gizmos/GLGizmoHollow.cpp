...
namespace Slic3r {
	namespace GUI {
		...
		GLGizmoHollow::GLGizmoHollow(GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id) : GLGizmoSlaBase(parent, icon_filename, sprite_id, slaposAssembly) {
		}
		...
		void GLGizmoHollow::on_render() {
			...
			if (m_state == On && (sel_info->model_object() != selection.get_model()->objects[selection.get_object_idx()] || sel_info->get_active_instance() != selection.get_instance_idx())) {
			...
			if (info != nullptr) {
				...
				if (!drain_holes.empty()) {
					...
					for (size_t i = 0; i < drain_holes.size(); ++i) {
						...
						q.setFromTwoVectors(Vec3d::UnitZ(), instance_scaling_matrix_inverse * (-drain_hole.normal).cast<double>());
						const Eigen::AngleAxisd aa(q);
						const Transform3d matrix = transformation.get_matrix() * hole_matrix * Transform3d(aa.toRotationMatrix()) * Geometry::translation_transform(-drain_hole.height * Vec3d::UnitZ()) * Geometry::scale_transform(Vec3d(drain_hole.radius, drain_hole.radius, drain_hole.height + sla::HoleStickOutLength));
...
RENDER_AGAIN:
			...
			const float clipping_slider_left = std::max(ImGuiPureWrap::calc_text_size(m_desc.at("clipping_of_view")).x, ImGuiPureWrap::calc_text_size(m_desc.at("reset_direction")).x) + m_imgui->scaled(0.5f);

			const float settings_sliders_left = std::max(std::max({ImGuiPureWrap::calc_text_size(m_desc.at("offset")).x, ImGuiPureWrap::calc_text_size(m_desc.at("quality")).x, ImGuiPureWrap::calc_text_size(m_desc.at("closing_distance")).x, ImGuiPureWrap::calc_text_size(m_desc.at("hole_diameter")).x, ImGuiPureWrap::calc_text_size(m_desc.at("hole_depth")).x}) + m_imgui->scaled(0.5f), clipping_slider_left);

...
#include "GUI/Gizmos/GLGizmosCommon.hpp"
...
#include "libslic3r/src/TriangleMesh.hpp"
...
namespace Slic3r::GUI {
	...
	GLGizmoPainterBase::GLGizmoPainterBase(GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id) : GLGizmoBase(parent, icon_filename, sprite_id) {
	}
	...
	void GLGizmoPainterBase::data_changed(bool is_serializing) {
		...
		if (mo && selection.is_from_single_instance() && (m_schedule_update || mo->id() != m_old_mo_id || mo->volumes.size() != m_old_volumes_size)) {
	...
	GLGizmoPainterBase::ClippingPlaneDataWrapper GLGizmoPainterBase::get_clipping_plane_data() const {
		ClippingPlaneDataWrapper clp_data_out{{0.f, 0.f, 1.f, FLT_MAX},
		                                      {-FLT_MAX, FLT_MAX}};
	...
	void GLGizmoPainterBase::render_cursor_circle() {
		...
		if (!m_circle.is_initialized() || !m_old_center.isApprox(center) || std::abs(m_old_cursor_radius - radius) > EPSILON) {
			...
			for (unsigned int i = 0; i < steps_count; ++i) {
				...
				if (OpenGLManager::get_gl_info().is_core_profile()) {
					...
					init_data.add_vertex(Vec2f(2.0f * ((center.x() + ::cos(angle) * radius) * cnv_inv_width - 0.5f), -2.0f * ((center.y() + ::sin(angle) * radius) * cnv_inv_height - 0.5f)));
		...
		if (shader != nullptr) {
			...
			if (OpenGLManager::get_gl_info().is_core_profile()) {
#endif // !SLIC3R_OPENGL_ES
				const Transform3d view_model_matrix = Geometry::translation_transform(Vec3d(2.0f * (center.x() * cnv_inv_width - 0.5f), -2.0f * (center.y() * cnv_inv_height - 0.5f), 0.0)) * Geometry::scale_transform(Vec3d(2.0f * radius * cnv_inv_width, 2.0f * radius * cnv_inv_height, 1.0f));
	...
	void GLGizmoPainterBase::render_cursor_sphere(const Transform3d &trafo) const {
		...
		const Camera &camera = wxGetApp().plater()->get_camera();
		Transform3d view_model_matrix = camera.get_view_matrix() * trafo * Geometry::translation_transform(m_rr.hit.cast<double>()) * complete_scaling_matrix_inverse * Geometry::scale_transform(m_cursor_radius * Vec3d::Ones());
	...
	bool GLGizmoPainterBase::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down, bool control_down) {
		if (action == SLAGizmoEventType::MouseWheelUp || action == SLAGizmoEventType::MouseWheelDown) {
			if (control_down) {
				...
				pos = action == SLAGizmoEventType::MouseWheelDown ? std::max(0., pos - 0.01) : std::min(1., pos + 0.01);
				...
				if (m_tool_type == ToolType::BRUSH && (m_cursor_type == TriangleSelector::CursorType::SPHERE || m_cursor_type == TriangleSelector::CursorType::CIRCLE)) {
					m_cursor_radius = action == SLAGizmoEventType::MouseWheelDown ? std::max(m_cursor_radius - this->get_cursor_radius_step(), this->get_cursor_radius_min()) : std::min(m_cursor_radius + this->get_cursor_radius_step(), this->get_cursor_radius_max());
					m_parent.set_as_dirty();
					return true;
				} else if (m_tool_type == ToolType::SMART_FILL) {
					m_smart_fill_angle = action == SLAGizmoEventType::MouseWheelDown ? std::max(m_smart_fill_angle - SmartFillAngleStep, SmartFillAngleMin) : std::min(m_smart_fill_angle + SmartFillAngleStep, SmartFillAngleMax);
					m_parent.set_as_dirty();
					if (m_rr.mesh_id != -1) {
						const Selection &selection = m_parent.get_selection();
						const ModelObject *mo = m_c->selection_info()->model_object();
						...
						m_triangle_selectors[m_rr.mesh_id]->seed_fill_select_triangles(m_rr.hit, int(m_rr.facet), trafo_matrix_not_translate, this->get_clipping_plane_in_volume_coordinates(trafo_matrix), m_smart_fill_angle, m_paint_on_overhangs_only ? m_highlight_by_angle_threshold_deg : 0.f, true);
		...
		if (action == SLAGizmoEventType::LeftDown || action == SLAGizmoEventType::RightDown || (action == SLAGizmoEventType::Dragging && m_button_down != Button::None)) {
				...
				if (m_tool_type == ToolType::SMART_FILL || m_tool_type == ToolType::BUCKET_FILL || (m_tool_type == ToolType::BRUSH && m_cursor_type == TriangleSelector::CursorType::POINTER)) {
					for (const ProjectedMousePosition &projected_mouse_position: projected_mouse_positions) {
						...
						if (m_tool_type == ToolType::SMART_FILL)
							m_triangle_selectors[mesh_idx]->seed_fill_select_triangles(mesh_hit, facet_idx, trafo_matrix_not_translate, clp, m_smart_fill_angle, m_paint_on_overhangs_only ? m_highlight_by_angle_threshold_deg : 0.f, true);
					...
					if (projected_mouse_positions.size() == 1) {
						...
						std::unique_ptr<TriangleSelector::Cursor> cursor = TriangleSelector::SinglePointCursor::cursor_factory(first_position.mesh_hit, camera_pos, m_cursor_radius, m_cursor_type, trafo_matrix, clp);
						m_triangle_selectors[mesh_idx]->select_patch(int(first_position.facet_idx), std::move(cursor), new_state, trafo_matrix_not_translate, m_triangle_splitting_enabled, m_paint_on_overhangs_only ? m_highlight_by_angle_threshold_deg : 0.f);
					} else {
		...
		if (action == SLAGizmoEventType::Moving && (m_tool_type == ToolType::SMART_FILL || m_tool_type == ToolType::BUCKET_FILL || (m_tool_type == ToolType::BRUSH && m_cursor_type == TriangleSelector::CursorType::POINTER))) {
			...
			const TriangleSelector::ClippingPlane &clp = this->get_clipping_plane_in_volume_coordinates(trafo_matrix);
			if (m_tool_type == ToolType::SMART_FILL)
				m_triangle_selectors[m_rr.mesh_id]->seed_fill_select_triangles(m_rr.hit, int(m_rr.facet), trafo_matrix_not_translate, clp, m_smart_fill_angle, m_paint_on_overhangs_only ? m_highlight_by_angle_threshold_deg : 0.f);
			else if (m_tool_type == ToolType::BRUSH && m_cursor_type == TriangleSelector::CursorType::POINTER)
		...
		if ((action == SLAGizmoEventType::LeftUp || action == SLAGizmoEventType::RightUp) && m_button_down != Button::None) {
	...
	bool GLGizmoPainterBase::on_mouse(const wxMouseEvent &mouse_event) {
		...
		if (mouse_event.LeftDown()) {
			if ((!control_down || grabber_contains_mouse) && gizmo_event(SLAGizmoEventType::LeftDown, mouse_pos, mouse_event.ShiftDown(), mouse_event.AltDown(), false))
	...
	void GLGizmoPainterBase::update_raycast_cache(const Vec2d &mouse_position, const Camera &camera, const std::vector<Transform3d> &trafo_matrices) const {
		...
		for (int mesh_id = 0; mesh_id < int(trafo_matrices.size()); ++mesh_id) {
			...
			if (m_c->raycaster()->raycasters()[mesh_id]->unproject_on_mesh(mouse_position, trafo_matrices[mesh_id], camera, hit, normal, m_c->object_clipper()->get_clipping_plane(), &facet)) {
	...
	bool GLGizmoPainterBase::on_is_activable() const {
		...
		if (wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology() != ptFFF || !selection.is_single_full_instance() || wxGetApp().get_mode() == comSimple)
			return false;
	...
	bool GLGizmoPainterBase::on_is_selectable() const {
		return (wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology() == ptFFF && wxGetApp().get_mode() != comSimple);
	}

	CommonGizmosDataID GLGizmoPainterBase::on_get_requirements() const {
		return CommonGizmosDataID(int(CommonGizmosDataID::SelectionInfo) | int(CommonGizmosDataID::InstancesHider) | int(CommonGizmosDataID::Raycaster) | int(CommonGizmosDataID::ObjectClipper));
	}
	...
	void TriangleSelectorGUI::render(ImGuiWrapper *imgui, const Transform3d &matrix) {
		...
		for (auto &iva: m_iva_seed_fills) {
			...
			const ColorRGBA &color = TriangleSelectorGUI::get_seed_fill_color(color_idx == 1 ? enforcers_color : color_idx == 2 ? blockers_color : GLVolume::NEUTRAL_COLOR);
			iva.set_color(color);
			iva.render();
		}
	...
	void TriangleSelectorGUI::update_render_data() {
		...
		for (const Triangle &tr: m_triangles) {
			...
			GLModel::Geometry &iva = tr.is_selected_by_seed_fill() ? iva_seed_fills_data[tr_state] : tr.get_state() == TriangleStateType::ENFORCER ? iva_enforcers_data : iva_blockers_data;
			int &cnt = tr.is_selected_by_seed_fill() ? seed_fill_cnt[tr_state] : tr.get_state() == TriangleStateType::ENFORCER ? enf_cnt : blc_cnt;

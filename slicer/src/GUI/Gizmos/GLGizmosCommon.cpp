...
namespace Slic3r {
	namespace GUI {
		...
		CommonGizmosDataPool::CommonGizmosDataPool(GLCanvas3D *canvas) : m_canvas(canvas) {
		...
		ModelInstance *SelectionInfo::model_instance() const {
			...
			return inst_idx < int(m_model_object->instances.size()) ? m_model_object->instances[get_active_instance()] : nullptr;
		}
		...
		void InstancesHider::render_cut() const {
			...
			for (const ModelVolume *mv: mo->volumes) {
				...
				if (obj_clipper->is_valid() && obj_clipper->get_clipping_plane() && obj_clipper->get_position() != 0.) {

...
namespace DoubleSlider {
	...
	enum SelectedSlider {
		ssUndef, ssLower, ssHigher
	};

	class ImGuiControl {
		public:
			ImGuiControl(int lowerValue, int higherValue, int minValue, int maxValue, ImGuiSliderFlags flags = ImGuiSliderFlags_None, std::string name = "d_slider", bool use_lower_thumb = true);
			ImGuiControl() {}
			~ImGuiControl() {}
		...
		private:
			...
			bool draw_slider(int *higher_pos, int *lower_pos, std::string &higher_label, std::string &lower_label, const ImVec2 &pos, const ImVec2 &size, float scale = 1.0f);
	};
	...
	class Manager {
		public:
			...
			void Init(int lowerPos, int higherPos, int minPos, int maxPos, const std::string &name, bool is_horizontal) {
				m_ctrl = ImGuiControl(lowerPos, higherPos, minPos, maxPos, is_horizontal ? 0 : ImGuiSliderFlags_Vertical, name, !is_horizontal);

				m_ctrl.set_get_label_cb([this](int pos) { return get_label(pos); });
			};

			Manager() {}
			Manager(int lowerPos, int higherPos, int minPos, int maxPos, const std::string &name, bool is_horizontal) {

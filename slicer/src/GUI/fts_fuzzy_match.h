...
namespace fts {
	...
	namespace fuzzy_internal {
		static bool fuzzy_match_recursive(const char_type *pattern, const char_type *str, int &outScore, const char_type *const strBegin, pos_type const *srcMatches, pos_type *newMatches, int nextMatch, int recursionCount, const int recursionLimit);
	...
	static bool fuzzy_internal::fuzzy_match_recursive(
		...
		if (matched) {
			...
			outScore += matches[0] == int(group_start - strBegin) ? first_letter_bonus : std::max((matches[0] - int(group_start - strBegin)) * leading_letter_penalty, max_leading_letter_penalty);

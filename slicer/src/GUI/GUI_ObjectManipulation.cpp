...
namespace Slic3r {
	namespace GUI {
		...
		ObjectManipulation::ObjectManipulation(wxWindow *parent) : OG_Settings(parent, true) {
		...
		void ObjectManipulation::Enable(const bool enable) {
			...
			for (wxWindow *win: std::initializer_list<wxWindow *>{m_reset_scale_button, m_reset_rotation_button, m_drop_to_bed_button, m_check_inch, m_lock_bnt, m_reset_skew_button})
				win->Enable(enable);
		}
		...
		void ObjectManipulation::update_if_dirty() {
			...
			enum ManipulationEditorKey {
				mePosition = 0, meRotation, meScale, meSize
			};
		...
		void ObjectManipulation::update_reset_buttons_visibility() {
			...
			if (selection.is_single_full_instance() || selection.is_single_volume_or_modifier()) {
				const double min_z = selection.is_single_full_instance() ? selection.get_scaled_instance_bounding_box().min.z() : get_volume_min_z(*selection.get_first_volume());
		...
		wxString ObjectManipulation::coordinate_type_str(ECoordinatesType type) {
			switch (type) {
				case ECoordinatesType::World: {
					return _L("World coordinates");
				}
				case ECoordinatesType::Instance: {
					return _L("Object coordinates");
				}
				case ECoordinatesType::Local: {
					return _L("Part coordinates");
				}
				default: {
					assert(false);
					return _L("Unknown");
		...
		static const char axes[] = {'x', 'y', 'z'};
		ManipulationEditor::ManipulationEditor(ObjectManipulation *parent, const std::string &opt_key, int axis) : ::TextInput(parent->parent(), "", "", "", wxDefaultPosition, wxSize(int(5.8 * wxGetApp().em_unit()), wxDefaultCoord), wxTE_PROCESS_ENTER), m_opt_key(opt_key), m_axis(axis) {

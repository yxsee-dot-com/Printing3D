...
namespace Slic3r {
	namespace GUI {
		...
		class SceneRaycasterItem {
			...
			public:
				SceneRaycasterItem(int id, const MeshRaycaster &raycaster, const Transform3d &trafo, bool use_back_faces = false) : m_id(id), m_raycaster(&raycaster), m_trafo(trafo), m_use_back_faces(use_back_faces) {}
		...
		class SceneRaycaster {
			public:
				enum class EType {
					None, Bed, Volume, Gizmo, FallbackGizmo // Is used for gizmo grabbers which will be hit after all grabbers of Gizmo type
				};

				enum class EIdBase {
					Bed = 0, Volume = 1000, Gizmo = 1000000, FallbackGizmo = 2000000
				};
			...
			public:
				...
				std::shared_ptr<SceneRaycasterItem> add_raycaster(EType type, int picking_id, const MeshRaycaster &raycaster, const Transform3d &trafo, bool use_back_faces = false);

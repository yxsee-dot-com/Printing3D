...
namespace Slic3r {
	namespace GUI {
		...
		static wxString generate_html_row(const Config::Snapshot &snapshot, bool row_even, bool snapshot_active, bool dark_mode) {
			...
			text += snapshot_active ? dark_mode ? "#208a20" : "#B3FFCB" : (row_even ? get_color(wxGetApp().get_window_default_clr()/*wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW)*/) : dark_mode ? "#656565" : "#D5D5D5");
			text += "\">";
			text += "<td>";
			...
			text += wxString("<font size=\"5\"><b>") + (snapshot_active ? _(L("Active")) + ": " : "") + datetime + ": " + format_reason(snapshot.reason);

			if (!snapshot.comment.empty())
				text += " (" + wxString::FromUTF8(snapshot.comment.data()) + ")";
			text += "</b></font><br>";
			// End of row header.
			text += _(L("Printing3D version")) + ": " + snapshot.slic3r_version_captured.to_string() + "<br>";
			...
			for (const Config::Snapshot::VendorConfig &vc: snapshot.vendor_configs) {
				text += _(L("vendor")) + ": " + vc.name + ", " + _(L("version")) + ": " + vc.version.config_version.to_string() + ", " + _(L("min Printing3D version")) + ": " + vc.version.min_slic3r_version.to_string();
				if (vc.version.max_slic3r_version != Semver::inf())
					text += ", " + _(L("max Printing3D version")) + ": " + vc.version.max_slic3r_version.to_string();
				text += "<br>";
		...
		ConfigSnapshotDialog::ConfigSnapshotDialog(const Config::SnapshotDB &snapshot_db, const wxString &on_snapshot) : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _(L("Configuration Snapshots")), wxDefaultPosition, wxSize(45 * wxGetApp().em_unit(), 40 * wxGetApp().em_unit()), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxMAXIMIZE_BOX) {

...
#include "libslic3r/src/CSGMesh/SliceCSGMesh.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		bool MeshRaycaster::unproject_on_mesh(const Vec2d &mouse_pos, const Transform3d &trafo, const Camera &camera, Vec3f &position, Vec3f &normal, const ClippingPlane *clipping_plane, size_t *facet_idx) const {
			Vec3d point;
			Vec3d direction;
			CameraUtils::ray_from_screen_pos(camera, mouse_pos, point, direction);
			Transform3d inv = trafo.inverse();
			...
			for (i = 0; i < hits.size(); ++i) {
				...
				if (transformed_hit.z() >= SINKING_Z_THRESHOLD && (!clipping_plane || !clipping_plane->is_point_clipped(transformed_hit)))
					break;
			}
		...
		std::vector<unsigned> MeshRaycaster::get_unobscured_idxs(const Geometry::Transformation &trafo, const Camera &camera, const std::vector<Vec3f> &points, const ClippingPlane *clipping_plane) const {
		...
		bool MeshRaycaster::closest_hit(const Vec2d &mouse_pos, const Transform3d &trafo, const Camera &camera, Vec3f &position, Vec3f &normal, const ClippingPlane *clipping_plane, size_t *facet_idx) const {
			Vec3d point;
			Vec3d direction;

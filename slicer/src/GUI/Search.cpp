	...
	namespace Search {
		...
		void OptionsSearcher::append_options(DynamicPrintConfig *config, Preset::Type type) {
			auto emplace = [this, type](std::string key, const wxString &label, int id = -1) {
				...
				if (!label.IsEmpty())
					options.emplace_back(Option{boost::nowide::widen(key), type, (label + suffix).ToStdWstring(), (_(label) + suffix_local).ToStdWstring(), gc.group.ToStdWstring(), _(gc.group).ToStdWstring(), gc.category.ToStdWstring(), GUI::Tab::translate_category(gc.category, type).ToStdWstring()});
			};
		...
		void OptionsSearcher::append_preferences_option(const GUI::Line &opt_line) {
			...
			preferences_options.emplace_back(Search::Option{boost::nowide::widen(key), type, label.ToStdWstring(), _(label).ToStdWstring(), gc.group.ToStdWstring(), _(gc.group).ToStdWstring(), gc.category.ToStdWstring(), _(gc.category).ToStdWstring()});
		}
		...
		static Option create_option(const std::string &opt_key, const wxString &label, Preset::Type type, const GroupAndCategory &gc) {
			...
			return Option{boost::nowide::widen(get_key(opt_key, type)), type, (label + suffix).ToStdWstring(), (_(label) + suffix_local).ToStdWstring(), gc.group.ToStdWstring(), _(gc.group).ToStdWstring(), gc.category.ToStdWstring(), GUI::Tab::translate_category(category, type).ToStdWstring()};
		}
		...
		static const std::map<const char, int> icon_idxs = {{ImGui::PrintIconMarker,      0},
		...
		SearchDialog::SearchDialog(OptionsSearcher *searcher, wxWindow *parent) : GUI::DPIDialog(parent ? parent : GUI::wxGetApp().tab_panel(), wxID_ANY, _L("Search"), wxDefaultPosition, wxDefaultSize, wxSTAY_ON_TOP | wxRESIZE_BORDER), searcher(searcher) {

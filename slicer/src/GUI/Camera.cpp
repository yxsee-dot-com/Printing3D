	...
	namespace GUI {
		...
		void Camera::apply_projection(double left, double right, double bottom, double top, double near_z, double far_z) {
			...
			switch (m_type) {
				default:
				case EType::Ortho: {
					m_projection_matrix.matrix() << 2.0 * inv_dx, 0.0, 0.0, -(left + right) * inv_dx, 0.0, 2.0 * inv_dy, 0.0, -(bottom + top) * inv_dy, 0.0, 0.0, -2.0 * inv_dz, -(near_z + far_z) * inv_dz, 0.0, 0.0, 0.0, 1.0;
					break;
				}
				case EType::Perspective: {
					m_projection_matrix.matrix() << 2.0 * near_z * inv_dx, 0.0, (left + right) * inv_dx, 0.0, 0.0, 2.0 * near_z * inv_dy, (bottom + top) * inv_dy, 0.0, 0.0, 0.0, -(near_z + far_z) * inv_dz, -2.0 * near_z * far_z * inv_dz, 0.0, 0.0, -1.0, 0.0;
					break;
				}
			}
		}
		...
		double Camera::calc_zoom_to_bounding_box_factor(const BoundingBoxf3 &box, double margin_factor) const {
			...
			const std::vector<Vec3d> vertices = {box.min, {box.max(0), box.min(1), box.min(2)}, {box.max(0), box.max(1), box.min(2)}, {box.min(0), box.max(1), box.min(2)}, {box.min(0), box.min(1), box.max(2)}, {box.max(0), box.min(1), box.max(2)}, box.max, {box.min(0), box.max(1), box.max(2)}};
		...
		Vec3d Camera::validate_target(const Vec3d &target) const {
			...
			return {std::clamp(target(0), test_box.min(0), test_box.max(0)), std::clamp(target(1), test_box.min(1), test_box.max(1)), std::clamp(target(2), test_box.min(2), test_box.max(2))};
		}

...
void Chart::recalculate_line() {
	...
	if (points.size() > 1) {
		...
		for (int i = 1; i <= N - 1; ++i) {
			mu[i] = h[i] / (h[i] + h[i + 1]);
			lambda[i] = 1.f - mu[i];
			rhs[i] = 6 * (float(points[i + 1].y - points[i].y) / (h[i + 1] * (points[i + 1].x - points[i - 1].x)) - float(points[i].y - points[i - 1].y) / (h[i] * (points[i + 1].x - points[i - 1].x)));
		}
		...
		for (int x = m_rect.GetLeft(); x <= m_rect.GetRight(); ++x) {
			if (splines) {
				...
				if (points[0].x > x)
					...
					y = (rhs[i - 1] * pow(points[i].x - x, 3) + rhs[i] * pow(x - points[i - 1].x, 3)) / (6 * h[i]) + (points[i - 1].y - rhs[i - 1] * h[i] * h[i] / 6.f) * (points[i].x - x) / h[i] + (points[i].y - rhs[i] * h[i] * h[i] / 6.f) * (x - points[i - 1].x) / h[i];

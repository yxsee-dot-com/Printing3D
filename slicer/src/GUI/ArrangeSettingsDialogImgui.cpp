...
namespace Slic3r {
	namespace GUI {
		...
		ArrangeSettingsDialogImgui::ArrangeSettingsDialogImgui(ImGuiWrapper *imgui, AnyPtr<arr2::ArrangeSettingsDb> db) : m_imgui{imgui}, m_db{std::move(db)} {}
		...
		void ArrangeSettingsDialogImgui::render(float pos_x, float pos_y) {
			...
			ImGuiPureWrap::begin(_u8L("Arrange options"), ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse);

			Settings settings;
			read_settings(settings, m_db.get());

			ImGuiPureWrap::text(GUI::format(_L("Press %1%left mouse button to enter the exact value"), shortkey_ctrl_prefix()));
			...
			if (m_imgui->slider_float(_L("Spacing"), &settings.d_obj, dobj_min, dobj_max, "%5.2f")) {
				settings.d_obj = std::max(dobj_min, settings.d_obj);
				m_db->set_distance_from_objects(settings.d_obj);
			}

			if (m_imgui->slider_float(_L("Spacing from bed"), &settings.d_bed, dbed_min, dbed_max, "%5.2f")) {
				settings.d_bed = std::max(dbed_min, settings.d_bed);
				m_db->set_distance_from_bed(settings.d_bed);
			}
			...
			if (m_show_xl_combo_predicate() && settings.xl_align >= 0 && ImGuiPureWrap::combo(_u8L("Alignment"), {_u8L("Center"), _u8L("Rear left"), _u8L("Front left"), _u8L("Front right"), _u8L("Rear right"), _u8L("Random")}, settings.xl_align)) {
				if (settings.xl_align >= 0 && settings.xl_align < ArrangeSettingsView::xlpCount)
			...
			if (ImGuiPureWrap::combo(_u8L("Geometry handling"),
					// TRN ArrangeDialog: Type of "Geometry handling"
					                 {_u8L("Fast"),
					                  ...
					                  _u8L("Balanced"),
					                         // TRN ArrangeDialog: Type of "Geometry handling"
					                  _u8L("Accurate")}, settings.geom_handling)) {
				if (settings.geom_handling >= 0 && settings.geom_handling < ArrangeSettingsView::ghCount)
					m_db->set_geometry_handling(static_cast<ArrangeSettingsView::GeometryHandling>(

...
namespace DoubleSlider {
	...
	enum FocusedItem {
		fiNone, fiRevertIcon, fiOneLayerIcon, fiCogIcon, fiColorBand, fiActionIcon, fiSmartWipeTower, fiTick
	};

	enum DrawMode {
		dmRegular, dmSlaPrint, dmSequentialFffPrint,
	};

	enum LabelType {
		ltHeightWithLayer, ltHeight, ltEstimatedTime,
	};

	class DSForLayers : public Manager<double> {
		public:
			DSForLayers() : Manager<double>() {}
			DSForLayers(int lowerValue, int higherValue, int minValue, int maxValue, bool allow_editing);
			~DSForLayers() {}
			...
			void set_callback_on_check_gcode(std::function<void(Type)> cb) { m_ticks.set_callback_on_check_gcode(cb); }

			void set_callback_on_get_extruder_colors(std::function<std::vector<std::string>()> cb) { m_cb_get_extruder_colors = cb; }

			void set_callback_on_get_print(std::function<const Slic3r::Print &()> cb) { m_cb_get_print = cb; }

			void set_callback_on_change_app_config(std::function<void(const std::string &, const std::string &)> cb) { m_cb_change_app_config = cb; }

			void set_callback_on_empty_auto_color_change(std::function<void()> cb) { m_ticks.set_callback_on_empty_auto_color_change(cb); }

			void set_callback_on_get_custom_code(std::function<std::string(const std::string &, double)> cb) { m_ticks.set_callback_on_get_custom_code(cb); }

			void set_callback_on_get_pause_print_msg(std::function<std::string(const std::string &, double)> cb) { m_ticks.set_callback_on_get_pause_print_msg(cb); }

			void set_callback_on_get_new_color(std::function<std::string(const std::string &)> cb) { m_ticks.set_callback_on_get_new_color(cb); }

			void set_callback_on_show_info_msg(std::function<int(const std::string &, int)> cb) { m_ticks.set_callback_on_show_info_msg(cb); }

			void set_callback_on_show_warning_msg(std::function<int(const std::string &, int)> cb) { m_ticks.set_callback_on_show_warning_msg(cb); }

			void set_callback_on_get_extruders_cnt(std::function<int()> cb) { m_ticks.set_callback_on_get_extruders_cnt(cb); }

			void set_callback_on_get_extruders_sequence(std::function<bool(ExtrudersSequence &)> cb) { m_ticks.set_callback_on_get_extruders_sequence(cb); }

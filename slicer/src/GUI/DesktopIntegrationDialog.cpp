...
bool DesktopIntegrationDialog::is_integrated()
{
	...
	// confirmation that Printing3D.desktop exists
	struct stat buffer;
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
	...
	for (size_t i = 0; i < target_candidates.size(); ++i) {
		// Copy icon Printing3D.png from resources_dir()/icons to target_dir_icons/icons/
		if (contains_path_dir(target_candidates[i], "icons")) {
			target_dir_icons = target_candidates[i];
			std::string icon_path = GUI::format("%1%/icons/Printing3D.png",resources_dir());
			std::string dest_path = GUI::format("%1%/icons/%2%Printing3D%3%.png", target_dir_icons, icon_theme_path, version_suffix);
	...
	if(target_dir_icons.empty()) {
		BOOST_LOG_TRIVIAL(error) << "Copying Printing3D icon to icons directory failed.";
	} else
		// save path to icon
		app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/icons/%2%Printing3D%3%.png", target_dir_icons, icon_theme_path, version_suffix));
	...
	for (size_t i = 0; i < target_candidates.size(); ++i) {
		if (contains_path_dir(target_candidates[i], "applications")) {
			...
			std::string path = GUI::format("%1%/applications/Printing3D%2%.desktop", target_dir_desktop, version_suffix);
			if (create_desktop_file(path, desktop_file)) {
				candidate_found = true;
				BOOST_LOG_TRIVIAL(debug) << "Printing3D.desktop file installation success.";
				break;
			}
			else {
				// write failed - try another path
				BOOST_LOG_TRIVIAL(debug) << "Attempt to Printing3D.desktop file installation failed. failed path: " << target_candidates[i];
	...
	app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/Printing3D%2%.desktop", target_dir_desktop, version_suffix));
	...
	if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
		...
		if (!target_dir_icons.empty())
		{
			std::string icon_path = GUI::format("%1%/icons/Printing3D-gcodeviewer_192px.png",resources_dir());
			std::string dest_path = GUI::format("%1%/icons/%2%Printing3D-gcodeviewer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
			if (copy_icon(icon_path, dest_path))
				// save path to icon
		...
		std::string desktop_file_viewer = GUI::format(
			...
			"StartupNotify=false\n", name_suffix, version_suffix, excutable_path);
		std::string desktop_path = GUI::format("%1%/applications/Printing3DGcodeViewer%2%.desktop", target_dir_desktop, version_suffix);
		if (create_desktop_file(desktop_path, desktop_file_viewer))
			...
			show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. Printing3D desktop file was probably created successfully."));
		}
	}
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration()
{
	...
	std::string desktop_file_downloader = GUI::format(
		"[Desktop Entry]\n"
		"Name=Printing3D URL Protocol%1%\n"
	...
	std::string desktop_path = GUI::format("%1%/applications/Printing3DURLProtocol%2%.desktop", target_dir_desktop, version_suffix);
	if (create_desktop_file(desktop_path, desktop_file_downloader)) {
		...
		std::string command = GUI::format("xdg-mime default Printing3DURLProtocol%1%.desktop x-scheme-handler/prusaslicer", version_suffix);
	...
	for (size_t i = 0; i < target_candidates.size(); ++i) {
		if (contains_path_dir(target_candidates[i], "applications")) {
			target_dir_desktop = target_candidates[i];
			// Write slicer desktop file
			std::string path = GUI::format("%1%/applications/Printing3DURLProtocol%2%.desktop", target_dir_desktop, version_suffix);
			if (create_desktop_file(path, desktop_file_downloader)) {
				app_config->set("desktop_integration_URL_path", path);
				candidate_found = true;
				BOOST_LOG_TRIVIAL(debug) << "Printing3DURLProtocol.desktop file installation success.";
				break;
			}
			else {
				// write failed - try another path
				BOOST_LOG_TRIVIAL(debug) << "Attempt to Printing3DURLProtocol.desktop file installation failed. failed path: " << target_candidates[i];
				target_dir_desktop.clear();
			}
		}
	}
...
{
	// Try ro find any Printing3DURLProtocol.desktop files including alpha and beta and get rid of them
	...
	for (const std::string cand : target_candidates) {
		...
		for (const std::string& suffix : {"" , "-beta", "-alpha" , "_beta", "_alpha"}) {
			boost::filesystem::path file_path = apps_path / GUI::format("Printing3DURLProtocol%1%.desktop", suffix);
			boost::system::error_code ec;

		...
		class SLAImportDialog : public wxDialog, public SLAImportJobView {
			...
			public:
				SLAImportDialog(Plater *plater) : wxDialog{plater, wxID_ANY, "Import SLA archive"} {
					...
					m_filepicker = new wxFilePickerCtrl(this, wxID_ANY, from_u8(wxGetApp().app_config->get_last_dir()), _L("Choose SLA archive") + ":", get_readers_wildcard(), wxDefaultPosition, wxDefaultSize, wxFLP_DEFAULT_STYLE | wxFD_OPEN | wxFD_FILE_MUST_EXIST);
					...
					static const std::vector<wxString> inp_choices = {_(L("Import model and profile")), _(L("Import profile only")), _(L("Import model only"))};

					m_import_dropdown = new wxComboBox(this, wxID_ANY, inp_choices[0], wxDefaultPosition, wxDefaultSize, inp_choices.size(), inp_choices.data(), wxCB_READONLY | wxCB_DROPDOWN);
					...
					static const std::vector<wxString> qual_choices = {_L("Accurate"), _L("Balanced"), _L("Fast")};

					m_quality_dropdown = new wxComboBox(this, wxID_ANY, qual_choices[1], wxDefaultPosition, wxDefaultSize, qual_choices.size(), qual_choices.data(), wxCB_READONLY | wxCB_DROPDOWN);
				...
				SLAImportQuality get_quality() const override {
					switch (m_quality_dropdown->GetSelection()) {
						case 2:
							return SLAImportQuality::Fast;
						case 1:
							return SLAImportQuality::Balanced;
						case 0:
							return SLAImportQuality::Accurate;
						default:
							return SLAImportQuality::Balanced;
					}
				}

...
#include "libslic3r/src/SLA/Rotfinder.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		class RotoptimizeJob : public Job {
				...
				static inline const FindMethod Methods[] = {{L("Best surface quality"),    sla::find_best_misalignment_rotation, L("Optimize object rotation for best surface quality.")},
				                                            {L("Reduced overhang slopes"), sla::find_least_supports_rotation,    L("Optimize object rotation to have minimum amount of overhangs needing support "
						...
						// Just a min area bounding box that is done for all methods anyway.
						                                    {L("Lowest Z height"),         sla::find_min_z_height_rotation,      L("Rotate the model to have the lowest z height for faster print time.")}};

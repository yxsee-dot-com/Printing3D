...
#include "GUI/Jobs/ThreadSafeQueue.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		void BoostThreadWorker::run() {
			...
			while (!stop) {
				m_input_queue.consume_one(BlockingWait{0}, [this, &stop](JobEntry &e) {
					if (!e.job)
						stop = true;
					else {
		...
		BoostThreadWorker::BoostThreadWorker(std::shared_ptr<ProgressIndicator> pri, boost::thread::attributes &attribs, const char *name) : m_progress(std::move(pri)), m_input_queue{m_running}, m_output_queue{m_running}, m_name{name} {
			if (m_progress)
		...
		bool BoostThreadWorker::wait_for_idle(unsigned timeout_ms) {
			...
			while (!timeout_reached && !is_idle()) {
				timeout_reached = !m_output_queue.consume_one(BlockingWait{timeout_ms}, [this](WorkerMessage &msg) {
					msg.deliver(*this);
				});
			}

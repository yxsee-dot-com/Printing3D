	...
	namespace GUI {
		...
		void ArrangeJob::prepare_selected() {
			...
			if (auto wti = get_wipe_tower(*m_plater)) {
				...
				auto &cont = m_plater->get_selection().is_wipe_tower() ? m_selected : m_unselected;
				cont.emplace_back(std::move(ap));
			}
		...
		static void update_arrangepoly_slaprint(arrangement::ArrangePolygon &ret, const SLAPrintObject &po, const ModelInstance &inst) {
			...
			coord_t pad_infl = 0;
			{
				double infl = po.config().pad_enable.getBool() * (po.config().pad_brim_size.getFloat() + po.config().pad_around_object.getBool() * po.config().pad_object_gap.getFloat());
		...
		coord_t get_skirt_offset(const Plater *plater) {
			...
			if (plater->printer_technology() == ptFFF && plater->fff_print().has_skirt()) {
				...
				if (!print.objects().empty()) {
					skirt_inset = print.config().skirts.value * print.skirt_flow().width() + print.config().skirt_distance.value;
				}
			}
		...
		void ArrangeJob::process(Ctl &ctl) {
			...
			// finalize just here.
			ctl.update_status(int(count) * 100 / status_range(), ctl.was_canceled() ? _u8L("Arranging canceled.") : _u8L("Arranging done."));
		}

		ArrangeJob::ArrangeJob(Mode mode) : m_plater{wxGetApp().plater()}, m_selection_only{mode == Mode::SelectionOnly} {}
		...
		void ArrangeJob::finalize(bool canceled, std::exception_ptr &eptr) {
			...
			if (!m_unarranged.empty()) {
				...
				m_plater->get_notification_manager()->push_notification(GUI::format(_L("Arrangement ignored the following objects which can't fit into a single bed:\n%s"), concat_strings(names, "\n")));
			}
		}
		...
		arrangement::ArrangePolygon get_arrange_poly(ModelInstance *inst, const Plater *plater) {
			...
			if (plater->printer_technology() == ptSLA) {
				const SLAPrintObject *po = plater->sla_print().get_print_object_by_model_object_id(obj_id);

				if (po) {
					update_arrangepoly_slaprint(ap, *po, *inst);
				}
			} else {
				const PrintObject *po = plater->fff_print().get_print_object_by_model_object_id(obj_id);
		...
		arrangement::ArrangeParams get_arrange_params(Plater *p) {
			const arr2::ArrangeSettingsView *settings = p->canvas3D()->get_arrange_settings_view();
		...
		void assign_logical_beds(std::vector<arrangement::ArrangePolygon> &items, const arrangement::ArrangeBed &bed, double stride) {
			...
			for (auto &itm: items) {
				auto bedidx = std::max(arrangement::UNARRANGED, static_cast<int>(std::floor((get_extents(itm.transformed_poly()).min.x() - bedx) / stride)));

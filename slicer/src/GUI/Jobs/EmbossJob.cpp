...
#include <libslic3r/src/Format/OBJ.hpp> // load_obj for default mesh
#include <libslic3r/src/CutSurface.hpp> // use surface cuts
#include <libslic3r/src/BuildVolume.hpp> // create object
...
namespace {
	...
	void create_volume(TriangleMesh &&mesh, const ObjectID &object_id, const ModelVolumeType type, const std::optional<Transform3d> &trmat, const DataBase &data, GLGizmosManager::EType gizmo);
...
namespace {
	...
	bool start_create_volume_job(Worker &worker, const ModelObject &object, const std::optional<Transform3d> &volume_tr, DataBasePtr data, ModelVolumeType volume_type, GLGizmosManager::EType gizmo);
...
#include "libslic3r/src/SVG.hpp"
	...
	template<typename Fnc>
	TriangleMesh create_mesh_per_glyph(DataBase &input, Fnc was_canceled) {
	...
	void final_update_volume(TriangleMesh &&mesh, const DataUpdate &data, const Transform3d *tr) {
		...
		assert(plater->canvas3D()->get_gizmos_manager().get_current_type() == GLGizmosManager::Emboss || plater->canvas3D()->get_gizmos_manager().get_current_type() == GLGizmosManager::Svg);
	...
	bool is_valid(ModelVolumeType volume_type) {
		...
		if (volume_type == ModelVolumeType::MODEL_PART || volume_type == ModelVolumeType::NEGATIVE_VOLUME || volume_type == ModelVolumeType::PARAMETER_MODIFIER)
			return true;

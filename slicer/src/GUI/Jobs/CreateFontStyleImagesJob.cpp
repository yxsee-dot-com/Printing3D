...
CreateFontStyleImagesJob::CreateFontStyleImagesJob(StyleManager::StyleImagesData &&input) : m_input(std::move(input)), m_width(0), m_height(0) {
...
void CreateFontStyleImagesJob::process(Ctl &ctl) {
	...
	for (StyleManager::StyleImage &image: m_images) {
		...
		sla::RasterEncoder encoder = [&offset = image.offset, &pix = m_pixels, w = m_width, h = m_height](const void *ptr, size_t width, size_t height, size_t num_components) {

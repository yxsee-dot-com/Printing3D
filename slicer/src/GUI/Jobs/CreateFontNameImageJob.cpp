...
#include "libslic3r/src/SLA/AGGRaster.hpp"
...
void CreateFontImageJob::process(Ctl &ctl) {
	...
	sla::RasterEncoder encoder = [&pix = m_result, w = m_tex_size.x(), h = m_tex_size.y(), gray_level = m_input.gray_level](const void *ptr, size_t width, size_t height, size_t num_components) {
...
void CreateFontImageJob::finalize(bool canceled, std::exception_ptr &) {
	...
	*m_input.is_created = true;
	...
	BOOST_LOG_TRIVIAL(info)
		<< "Generate Preview font('" << m_input.font_name << "' id:" << m_input.index << ") " << "with text: '" << m_input.text << "' " << "texture_size " << m_input.size.x() << " x " << m_input.size.y();
}
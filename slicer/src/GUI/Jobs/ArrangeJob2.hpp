...
#include "libslic3r/src/Arrange/Tasks/ArrangeTask.hpp"
#include "libslic3r/src/Arrange/Tasks/FillBedTask.hpp"
#include "libslic3r/src/Arrange/Items/ArrangeItem.hpp"
#include "libslic3r/src/Arrange/SceneBuilder.hpp"
...
namespace Slic3r {
	...
	namespace GUI {
		...
		arr2::SceneBuilder build_scene(Plater &plater, ArrangeSelectionMode mode = ArrangeSelectionMode::Full);
		...
		struct ArrCtl : public arr2::ArrangeTaskBase::Ctl {
			...
			ArrCtl(Job::Ctl &ctl, int cnt, const std::string &m) : parent_ctl{ctl}, total{cnt}, msg{m} {}
		...
		class ArrangeJob_ : public Job {
			public:
				using ResultType = typename decltype(std::declval<ArrangeTaskT>().process_native(std::declval<arr2::ArrangeTaskCtl>()))::element_type;
			...
			public:
				...
				explicit ArrangeJob_(arr2::Scene &&scene, std::string task_msg, const Callbacks &cbs = {}) : m_scene{std::move(scene)}, m_cbs{cbs}, m_task_msg{std::move(task_msg)} {}
		};

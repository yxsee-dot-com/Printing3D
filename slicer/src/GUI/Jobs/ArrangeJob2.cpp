...
namespace Slic3r {
	namespace GUI {
		...
		class GUISelectionMask : public arr2::SelectionMask {
			...
			public:
				...
				std::vector<bool> selected_instances(int obj_id) const override {
					...
					auto icnt = obj_id < objcnt ? m_sel->get_model()->objects[obj_id]->instances.size() : 0;
		...
		class ArrangeableWT : public arr2::ArrangeableWipeTowerBase {
			...
			public:
				explicit ArrangeableWT(const ObjectID &oid, const GLCanvas3D::WipeTowerInfo &wti, std::function<bool()> sel_pred, const BoundingBox xl_bb = {}) : arr2::ArrangeableWipeTowerBase{oid, get_wtpoly(wti), std::move(sel_pred)}, m_orig_tr{wti.pos()}, m_orig_rot{wti.rotation()}, m_xl_bb{xl_bb} {}
		...
		struct WTH : public arr2::WipeTowerHandler {
			...
			WTH(const ObjectID &objid, const GLCanvas3D::WipeTowerInfo &w, std::function<bool()> sel_predicate = [] { return false; }) : wti(w), oid{objid}, sel_pred{std::move(sel_predicate)} {}

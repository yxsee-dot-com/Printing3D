...
namespace Slic3r {
	namespace GUI {
		...
		void FillBedJob::prepare() {
			...
			double unsel_area = std::accumulate(m_unselected.begin(), m_unselected.end(), 0., [](double s, const auto &ap) {
		...
		void FillBedJob::finalize(bool canceled, std::exception_ptr &eptr) {
			...
			if (added_cnt > 0) {
				...
				m_plater->sidebar().obj_list()->increase_object_instances(m_object_idx, size_t(added_cnt));
			}
		}

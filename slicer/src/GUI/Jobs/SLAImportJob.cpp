...
#include "libslic3r/src/Format/SL1.hpp"
#include "libslic3r/src/Format/SLAArchiveReader.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		SLAImportJob::SLAImportJob(const SLAImportJobView *view) : p{std::make_unique<priv>(wxGetApp().plater(), view)} {
			prepare();
		}
		...
		void SLAImportJob::process(Ctl &ctl) {
			...
			try {
				switch (p->sel) {
					...
					case Sel::modelOnly:
						p->config_substitutions = import_sla_archive(path, format_id, p->mesh, p->profile, p->quality, progr);
						break;
					case Sel::profileOnly:
						p->config_substitutions = import_sla_archive(path, format_id, p->profile);
						break;
				}
		...
		void SLAImportJob::finalize(bool canceled, std::exception_ptr &eptr) {
			...
			if (p->profile.empty()) {
				p->plater->get_notification_manager()->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::WarningNotificationLevel, _u8L("The imported SLA archive did not contain any presets. "
			...
			if (p->sel != Sel::modelOnly) {
				...
				for (auto object: objects)
					if (object->volumes.size() > 1) {
						Slic3r::GUI::show_info(nullptr, _(L("You cannot load SLA project with a multi-part object on the bed")) + "\n\n" + _(L("Please check your object list before preset changing.")), _(L("Attention!")));
						return;
					}
				...
				if (Preset::printer_technology(config) == ptSLA) {
					...
					p->plater->get_notification_manager()->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::WarningNotificationLevel, _u8L("The profile in the imported archive is corrupted and will not be loaded."));
				}
			}
			...
			if (!p->mesh.empty()) {
				...
				p->plater->get_notification_manager()->push_notification(NotificationType::CustomNotification, NotificationManager::NotificationLevel::WarningNotificationLevel, _u8L("No object could be retrieved from the archive. The slices might be corrupted or missing."));
			}

...
#include "libslic3r/src/MTUtils.hpp"
#include "libslic3r/src/SLA/Rotfinder.hpp"
#include "libslic3r/src/MinAreaBoundingBox.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		void RotoptimizeJob::process(Ctl &ctl) {
			...
			auto params = sla::RotOptimizeParams{}.accuracy(m_accuracy).print_config(&m_default_print_cfg).statucb([this, &prev_status, &ctl, &statustxt](int s) {
				if (s > 0 && s < 100)
					ctl.update_status(prev_status + s / m_selected_object_ids.size(), statustxt);

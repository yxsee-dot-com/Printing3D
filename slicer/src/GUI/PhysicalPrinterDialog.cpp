...
#define BORDER_W 10
		...
		PresetForPrinter::PresetForPrinter(PhysicalPrinterDialog *parent, const std::string &preset_name) : m_parent(parent) {
		...
		PhysicalPrinterDialog::PhysicalPrinterDialog(wxWindow *parent, wxString printer_name) : DPIDialog(parent, wxID_ANY, _L("Physical Printer"), wxDefaultPosition, wxSize(45 * wxGetApp().em_unit(), -1), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER), m_printer("", wxGetApp().preset_bundle->physical_printers.default_config()) {
		...
		void PhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup *m_optgroup) {
			...
			if (is_secret_store_ok()) {
				...
				line.full_width = 1;
				line.widget = [ca_file_hint](wxWindow *parent) {
					wxString info = GUI::format_wxstr(L"%1%:\n\t%2%\n", _L("Storing passwords"), GUI::format_wxstr(_L("On this system, %1% uses the system password store to safely store and read passwords and API keys."), SLIC3R_APP_NAME));
				...
				line.full_width = 1;
				line.widget = [ca_file_hint](wxWindow *parent) {
					wxString info = GUI::format_wxstr(L"%1%:\n\t%2%\n\t%3%\n", _L("Storing passwords"), GUI::format_wxstr(_L("On this system, %1% cannot access the system password store."), SLIC3R_APP_NAME), _L("Passwords and API keys are stored in plain text."));
					auto txt = new wxStaticText(parent, wxID_ANY, info);
					txt->SetFont(wxGetApp().normal_font());
					auto sizer = new wxBoxSizer(wxHORIZONTAL);
					sizer->Add(txt, 1, wxEXPAND);
			...
			m_opened_as_connect = false;// (m_last_host_type == htPrusaConnect);
		...
		void PhysicalPrinterDialog::update_host_type(bool printer_change) {
			...
			auto model_supports_prusalink = [](const std::string &model) {
				return model.size() >= 2 && ((boost::starts_with(model, "MK") && model[2] > '2' && model[2] <= '9') || boost::starts_with(model, "MINI") || boost::starts_with(model, "MK2.5") || boost::starts_with(model, "XL"));
			};
				...
				if (Preset *preset = wxGetApp().preset_bundle->printers.find_preset(preset_name)) {
					...
					if (preset->vendor) {
						...
						if (preset->vendor->name.find("Prusa Research") != std::string::npos) {
							...
							auto it = std::find_if(models.begin(), models.end(), [model_id](const VendorProfile::PrinterModel &model) { return model.id == model_id; });
		...
		void PhysicalPrinterDialog::update_full_printer_names() {
			...
			for (size_t i = 0; i < std::strlen(unusable_symbols); i++) {
				...
				if (pos != std::string::npos) {
					...
					InfoDialog(this, format_wxstr("%1%: \"%2%\" ", _L("Unexpected character"), str), _L("The following characters are not allowed in the name") + ": " + unusable_symbols).ShowModal();
		...
		void PhysicalPrinterDialog::OnOK(wxEvent &event) {
			...
			if (!repeat_presets.empty()) {
				...
				wxString msg_text = format_wxstr(_L_PLURAL("Following printer preset is duplicated:%1%"
				                                           "The above preset for printer \"%2%\" will be used just once.", "Following printer presets are duplicated:%1%"
			...
			if (m_printer.name != into_u8(m_default_name) && m_printer.name != into_u8(printer_name))

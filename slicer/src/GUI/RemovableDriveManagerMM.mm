		...
		if (err == 0) {
			...
			*/
			if (mediaEjectableKey != nullptr) {
				BOOL op = ejectable && ((deviceProtocolName != nullptr && (CFEqual(deviceProtocolName, CFSTR("USB")) || CFEqual(deviceProtocolName, CFSTR("Secure Digital")))) || (deviceModelKey != nullptr && CFEqual(deviceModelKey, CFSTR("SD Card Reader"))));

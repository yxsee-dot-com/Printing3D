...
namespace Slic3r {
	namespace GUI {
		...
		namespace {
			...
			enum TagCheckResult {
				TagCheckAffirmative, TagCheckNegative, TagCheckNotCompatible
			};
			...
			while (last_end < text.length() - 1) {
				...
				if (next_hard_end != std::string::npos && ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x < m_window_width - m_window_width_offset) {
					...
					if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width - m_window_width_offset) {
						...
						if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x > m_window_width - m_window_width_offset || ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x < (m_window_width - m_window_width_offset) / 5 * 3) {
		...
		void NotificationManager::HintNotification::render_close_button(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y) {
			...
			if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - win_size.x / 10.f, win_pos.y), ImVec2(win_pos.x, win_pos.y + win_size.y - 2 * m_line_height), true)) {
		...
		void NotificationManager::HintNotification::render_preferences_button(const float win_pos_x, const float win_pos_y) {
			...
			//hover
			if (ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_window_width / 15.f, win_pos_y + m_window_height - 1.75f * m_line_height), ImVec2(win_pos_x, win_pos_y + m_window_height), true)) {
		...
		void NotificationManager::HintNotification::render_documentation_button(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y) {
			...
			if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - m_line_height * 5.f, win_pos.y), ImVec2(win_pos.x - m_line_height * 2.5f, win_pos.y + win_size.y - 2 * m_line_height), true)) {
		...
		void NotificationManager::HintNotification::retrieve_data(bool new_hint/* = true*/) {
			...
			if (hint_data != nullptr) {
				NotificationData nd{NotificationType::DidYouKnowHint, NotificationLevel::HintNotificationLevel, 0, hint_data->text, hint_data->hypertext, nullptr, hint_data->follow_text};

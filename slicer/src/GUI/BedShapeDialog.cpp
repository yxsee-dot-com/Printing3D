...
namespace Slic3r {
	namespace GUI {
		...
		static std::string get_option_label(BedShape::Parameter param) {
			switch (param) {
				case BedShape::Parameter::RectSize  :
					return L("Size");
				case BedShape::Parameter::RectOrigin:
					return L("Origin");
				case BedShape::Parameter::Diameter  :
					return L("Diameter");
				default:
					assert(false);
					return {};
		...
		BedShape::PageType BedShape::get_page_type() {
			switch (m_build_volume.type()) {
				...
				case BuildVolume::Type::Invalid:
					return PageType::Rectangle;
				case BuildVolume::Type::Circle:
					return PageType::Circle;
		...
		BedShapeDialog::BedShapeDialog(wxWindow *parent) : DPIDialog(parent, wxID_ANY, _(L("Bed Shape")), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {}
		...
		void BedShapePanel::update_shape() {
			...
			switch (static_cast<BedShape::PageType>(page_idx)) {
				case BedShape::PageType::Rectangle: {
					...
					try { rect_size = boost::any_cast<Vec2d>(opt_group->get_value("rect_size")); } catch (const std::exception & /* e */) { return; }

					try { rect_origin = boost::any_cast<Vec2d>(opt_group->get_value("rect_origin")); } catch (const std::exception & /* e */) { return; }
				...
				case BedShape::PageType::Circle: {
					double diameter;
					try { diameter = boost::any_cast<double>(opt_group->get_value("diameter")); } catch (const std::exception & /* e */) { return; }
		...
		void BedShapePanel::load_texture() {
			wxFileDialog dialog(this, _L("Choose a file to import bed texture from (PNG/SVG):"), "", "", file_wildcards(FT_TEX), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
		...
		void BedShapePanel::load_model() {
			wxFileDialog dialog(this, _L("Choose an STL file to import bed model from:"), "", "", file_wildcards(FT_STL), wxFD_OPEN | wxFD_FILE_MUST_EXIST);

...
namespace Slic3r {
	namespace GUI {
		...
		Selection::Selection() : m_volumes(nullptr), m_model(nullptr), m_enabled(false), m_mode(Instance), m_type(Empty), m_valid(false), m_scale_factor(1.0f) {
		...
		void Selection::add(unsigned int volume_idx, bool as_single_selection, bool check_for_already_contained) {
			...
			if (wxGetApp().plater()->printer_technology() == ptSLA && volume->is_modifier && m_model->objects[volume->object_idx()]->volumes[volume->volume_idx()]->is_modifier())
				return;
		...
		void Selection::instances_changed(const std::vector<size_t> &instance_ids_selected) {
			...
			for (unsigned int volume_idx = 0; volume_idx < (unsigned int) m_volumes->size(); ++volume_idx) {
				const GLVolume *volume = (*m_volumes)[volume_idx];
				if (pt == ptSLA && volume->is_modifier && m_model->objects[volume->object_idx()]->volumes[volume->volume_idx()]->is_modifier())
					continue;
		...
		BoundingBoxf Selection::get_screen_space_bounding_box() {
			...
			if (!is_empty()) {
				...
				// vertices
				std::vector<Vec3d> vertices = {{box.min.x(), box.min.y(), box.min.z()},
		...
		void Selection::translate(const Vec3d &displacement, TransformationType transformation_type) {
			...
			assert(transformation_type.relative() || transformation_type.local());
		...
		void Selection::rotate(const Vec3d &rotation, TransformationType transformation_type) {
					...
					if (!is_single_volume_or_modifier()) {
						...
						if (transformation_type.instance()) {
							...
							if (transformation_type.local()) {
								...
								rotation_matrix = vol_matrix_no_offset.inverse() * inst_scale_matrix.inverse() * vol_rotation_matrix * rotation_matrix * vol_rotation_matrix.inverse() * inst_scale_matrix * vol_matrix_no_offset;
							}
						}
						transform_volume_relative(v, volume_data, transformation_type, rotation_matrix, m_cache.rotation_pivot);
					}
			...
			if (m_mode == Instance) {
				...
				const SyncRotationType type = (transformation_type.instance() && requires_general_synchronization) || (!transformation_type.instance() && rot_axis_max != 2) || rotation.isApprox(Vec3d::Zero()) ? SyncRotationType::GENERAL : SyncRotationType::NONE;
		...
		void Selection::scale_to_fit_print_volume(const BuildVolume &volume) {
			...
			switch (volume.type()) {
				case BuildVolume::Type::Rectangle: {
					undoredo_snapshot = fit_rectangle(volume, !undoredo_snapshot);
					break;
				}
				case BuildVolume::Type::Circle: {
					undoredo_snapshot = fit_circle(volume, !undoredo_snapshot);
					break;
				}
				default: {
					break;
			...
			if (wxGetApp().plater()->printer_technology() == ptFFF) {
				...
				if (layers_it != layers.rbegin() && layers_it != layers.rend()) {
					switch (volume.type()) {
						case BuildVolume::Type::Rectangle: {
							fit_rectangle(volume, !undoredo_snapshot, &(*layers_it));
							break;
						}
						case BuildVolume::Type::Circle: {
							fit_circle(volume, !undoredo_snapshot, &(*layers_it));
							break;
						}
						default: {
							break;
		...
		int Selection::bake_transform_if_needed() const {
			if ((is_single_full_instance() && wxGetApp().obj_manipul()->is_world_coordinates()) || (is_single_volume_or_modifier() && !wxGetApp().obj_manipul()->is_local_coordinates())) {
				...
				if (needs_baking) {
					MessageDialog dlg((wxWindow *) wxGetApp().mainframe, _L("The currently manipulated object is tilted or contains tilted parts (rotation angles are not multiples of 90°). "
					                                                        "Non-uniform scaling of tilted objects is only possible in non-local coordinate systems, "
					                                                        "once the rotation is embedded into the object coordinates.") + "\n" + _L("This operation is irreversible.") + "\n" + _L("Do you want to proceed?"), SLIC3R_APP_NAME, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION);
		...
		std::vector<unsigned int> Selection::get_volume_idxs_from_object(unsigned int object_idx) const {
			...
			for (unsigned int i = 0; i < (unsigned int) m_volumes->size(); ++i) {
				...
				if (v->object_idx() == (int) object_idx) {
					if (pt == ptSLA && v->is_modifier && m_model->objects[object_idx]->volumes[v->volume_idx()]->is_modifier())
						continue;
					idxs.push_back(i);
				}
			}
		...
		void Selection::ensure_on_bed() {
			...
			for (size_t i = 0; i < m_volumes->size(); ++i) {
				GLVolume *volume = (*m_volumes)[i];
				if (!volume->is_wipe_tower && !volume->is_modifier && std::find(m_cache.sinking_volumes.begin(), m_cache.sinking_volumes.end(), i) == m_cache.sinking_volumes.end()) {
		...
		void Selection::transform_instance_relative(GLVolume &volume, const VolumeCache &volume_data, TransformationType transformation_type, const Transform3d &transform, const Vec3d &world_pivot) {
		...
		void Selection::transform_volume_relative(GLVolume &volume, const VolumeCache &volume_data, TransformationType transformation_type, const Transform3d &transform, const Vec3d &world_pivot) {
			assert(transformation_type.relative());

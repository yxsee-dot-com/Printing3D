...
namespace Slic3r {
	namespace GUI {
		...
		UserAccountCommunication::UserAccountCommunication(wxEvtHandler *evt_handler, AppConfig *app_config) : wxEvtHandler(), m_evt_handler(evt_handler), m_app_config(app_config), m_polling_timer(new wxTimer(this)), m_token_timer(new wxTimer(this)) {
			...
			m_username = username;
			{
				...
				if (is_secret_store_ok()) {
					...
					if (m_remember_session) {
						tokens = m_session->get_access_token() + "|" + m_session->get_refresh_token() + "|" + std::to_string(m_session->get_next_token_timeout());
					}

...
#include "libslic3r/src/Config.hpp" //ConfigOptionMode
...
wxString TopBarMenus::get_workspace_name(int mode/* = -1*/) {
	...
	return mode == Slic3r::ConfigOptionMode::comSimple ? _L("Beginner mode") : mode == Slic3r::ConfigOptionMode::comAdvanced ? _L("Normal mode") : _L("Expert mode");
}
...
void TopBarMenus::ApplyWorkspacesMenu() {
	...
	for (const Slic3r::ConfigOptionMode &mode: {Slic3r::ConfigOptionMode::comSimple, Slic3r::ConfigOptionMode::comAdvanced, Slic3r::ConfigOptionMode::comExpert}) {
		...
		append_menu_item(&workspaces, wxID_ANY, label, label, [mode, this](wxCommandEvent &) {
			if (m_cb_get_mode && m_cb_save_mode && m_cb_get_mode() != mode)
				m_cb_save_mode(mode);

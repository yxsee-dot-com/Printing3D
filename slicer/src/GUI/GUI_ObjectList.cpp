...
#include "libslic3r/src/TextConfiguration.hpp"
#include "libslic3r/src/BuildVolume.hpp" // IWYU pragma: keep
...
namespace Slic3r {
	namespace GUI {
		...
		ObjectList::ObjectList(wxWindow *parent) : wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize,
#ifdef _WIN32
                                                                  wxBORDER_SIMPLE |
                                                                  #endif
                                                                  wxDV_MULTIPLE) {
			...
			Bind(wxEVT_DATAVIEW_SELECTION_CHANGED, [this](wxDataViewEvent &event) {
				...
				// see: https://github.com/prusa3d/Printing3D/issues/3802
#ifndef __WXOSX__
				...
				// see https://github.com/prusa3d/Printing3D/issues/3303
...
//    Bind(wxEVT_KEY_DOWN, &ObjectList::OnChar, this);
     ...
     */
			...
			AppendColumn(new wxDataViewColumn(_L("Name"), bmp_text_renderer, colName, 20 * em, wxALIGN_LEFT, wxDATAVIEW_COL_RESIZABLE));

			// column PrintableProperty (Icon) of the view control:
			AppendBitmapColumn(" ", colPrint, wxDATAVIEW_CELL_INERT, 3 * em, wxALIGN_CENTER_HORIZONTAL, wxDATAVIEW_COL_RESIZABLE);
			...
			AppendColumn(new wxDataViewColumn(_L("Extruder"), bmp_choice_renderer, colExtruder, 8 * em, wxALIGN_CENTER_HORIZONTAL, wxDATAVIEW_COL_RESIZABLE));

			// column ItemEditing of the view control:
			AppendBitmapColumn(_L("Editing"), colEditing, wxDATAVIEW_CELL_INERT, 3 * em, wxALIGN_CENTER_HORIZONTAL, wxDATAVIEW_COL_RESIZABLE);
		...
		void ObjectList::get_selected_item_indexes(int &obj_idx, int &vol_idx, const wxDataViewItem &input_item/* = wxDataViewItem(nullptr)*/) {
			...
			obj_idx = type & itObject ? m_objects_model->GetIdByItem(item) : type & itVolume ? m_objects_model->GetIdByItem(m_objects_model->GetTopParent(item)) : -1;
		...
		void ObjectList::get_selection_indexes(std::vector<int> &obj_idxs, std::vector<int> &vol_idxs) {
			...
			if (m_objects_model->GetItemType(sels[0]) & itVolume || (sels.Count() == 1 && m_objects_model->GetItemType(m_objects_model->GetParent(sels[0])) & itVolume)) {
				...
				for (wxDataViewItem item: sels) {
					const ItemType type = m_objects_model->GetItemType(item);
					obj_idxs.emplace_back(type & itObject ? m_objects_model->GetIdByItem(item) : m_objects_model->GetIdByItem(m_objects_model->GetTopParent(item)));
				}
			}
		...
		MeshErrorsInfo ObjectList::get_mesh_errors_info(const int obj_idx, const int vol_idx /*= -1*/, wxString *sidebar_info /*= nullptr*/) const {
			...
			const TriangleMeshStats &stats = vol_idx == -1 ? (*m_objects)[obj_idx]->get_object_stl_stats() : (*m_objects)[obj_idx]->volumes[vol_idx]->mesh().stats();
     ...
     */
		...
		ModelConfig &ObjectList::get_item_config(const wxDataViewItem &item) const {
			...
			return type & itVolume ? (*m_objects)[obj_idx]->volumes[vol_idx]->config : type & itLayer ? (*m_objects)[obj_idx]->layer_config_ranges[m_objects_model->GetLayerRangeByItem(item)] : (*m_objects)[obj_idx]->config;
		}
		...
		void ObjectList::update_extruder_values_for_items(const size_t max_extruder) {
			for (size_t i = 0; i < m_objects->size(); ++i) {
				...
				wxString extruder;
				if (!object->config.has("extruder") || size_t(object->config.extruder()) > max_extruder)
				...
				if (object->volumes.size() > 1) {
					for (size_t id = 0; id < object->volumes.size(); id++) {
						...
						if (!item) continue;
						if (!object->volumes[id]->config.has("extruder") || size_t(object->volumes[id]->config.extruder()) > max_extruder)
							extruder = _(L("default"));
						else
...
*/
     ...
     */
			...
			if (col != nullptr) {
				...
				if (title == " ")
					...
					if (is_windows10() && m_objects_model->HasWarningIcon(item) && mouse_pos.x > 2 * wxGetApp().em_unit() && mouse_pos.x < 4 * wxGetApp().em_unit())
						fix_through_winsdk();
					else if (evt_context_menu)
			...
			if (multiple_selection()) {
				...
				if (item) {
					...
					if (type & itVolume) {
						...
						menu = volume->is_text() ? plater->text_part_menu() : volume->is_svg() ? plater->svg_part_menu() : plater->part_menu();
					} else
						menu = type & itInstance ? plater->instance_menu() : type & itLayer ? plater->layer_menu() : printer_technology() == ptFFF ? plater->object_menu() : plater->sla_object_menu();
				} else if (evt_context_menu)
		...
		void ObjectList::key_event(wxKeyEvent &event) {
			if (event.GetKeyCode() == WXK_TAB)
				...
				if (std::find(numbers.begin(), numbers.end(), key_char) != numbers.end()) {
					...
					if (wxNumberFormatter::FromString(wxString(key_char), &extruder_number) && extruders_count() >= extruder_number)
    ...
    renderer->FinishEditing();
			...
			if ((mult_sel && !selected_instances_of_same_object()) || (!mult_sel && (GetSelection() != item))) {
				event.Veto();
				return;
			}
			...
			if (mult_sel) {
				...
				m_dragged_data.init(m_objects_model->GetObjectIdByItem(item), type & itVolume ? m_objects_model->GetVolumeIdByItem(item) : m_objects_model->GetInstanceIdByItem(item), type);
     ...
     */
		...
		bool ObjectList::can_drop(const wxDataViewItem &item) const {
			...
			if (!item.IsOk() || !(m_dragged_data.type() & (itVolume | itObject)) || m_objects_model->GetItemType(item) != m_dragged_data.type())
				return false;
			...
			if (m_dragged_data.type() & itVolume) {
				...
				if (dragged_item_v_type == ModelVolumeType::MODEL_PART) {
					if (only_one_solid_part)
						return false;
					return (m_objects_model->GetVolumeIdByItem(item) == 0 || (m_dragged_data.sub_obj_idx() == 0 && volumes[1]->type() == ModelVolumeType::MODEL_PART) || (m_dragged_data.sub_obj_idx() != 0 && volumes[0]->type() == ModelVolumeType::MODEL_PART));
				}
    ...
    **/
		...
		void ObjectList::add_category_to_settings_from_selection(const std::vector<std::pair<std::string, bool> > &category_options, wxDataViewItem item) {
			...
			const wxString snapshot_text = item_type & itLayer ? _L("Add Settings for Layers") : item_type & itVolume ? _L("Add Settings for Sub-object") : _L("Add Settings for Object");
			take_snapshot(snapshot_text);

			const DynamicPrintConfig &from_config = printer_technology() == ptFFF ? wxGetApp().preset_bundle->prints.get_edited_preset().config : wxGetApp().preset_bundle->sla_prints.get_edited_preset().config;
		...
		void ObjectList::add_category_to_settings_from_frequent(const std::vector<std::string> &options, wxDataViewItem item) {
			...
			const wxString snapshot_text = item_type & itLayer ? _L("Add Settings Bundle for Height range") : item_type & itVolume ? _L("Add Settings Bundle for Sub-object") : _L("Add Settings Bundle for Object");
			take_snapshot(snapshot_text);
      ...
      */
			...
			for (size_t i = 0; i < input_files.size(); ++i) {
				...
				dlg.Update(static_cast<int>(100.0f * static_cast<float>(i) / static_cast<float>(input_files.size())), _L("Loading file") + ": " + from_path(boost::filesystem::path(input_file).filename()));
				dlg.Fit();
      ...
      */
		...
		void ObjectList::load_shape_object_from_gallery(const wxArrayString &input_files) {
			...
			assert(!paths.empty());
			wxString snapshot_label = (paths.size() == 1 ? _L("Add Shape from Gallery") : _L("Add Shapes from Gallery")) + ": " + wxString::FromUTF8(paths.front().filename().string().c_str());
		...
		void ObjectList::load_mesh_object(const TriangleMesh &mesh, const std::string &name, bool center) {
			...
			new_object->instances[0]->set_offset(center ? to_3d(wxGetApp().plater()->build_volume().bounding_volume2d().center(), -new_object->origin_translation.z()) : bb.center());
		...
		void ObjectList::del_settings_from_config(const wxDataViewItem &parent_item) {
			...
			if ((opt_cnt == 1 && m_config->has("extruder")) || (is_layer_settings && opt_cnt == 2 && m_config->has("extruder") && m_config->has("layer_height")))
				return;
		...
		bool ObjectList::del_from_cut_object(bool is_cut_connector, bool is_model_part/* = false*/, bool is_negative_volume/* = false*/) {
			...
			const wxString title = is_cut_connector ? _L("Delete connector from object which is a part of cut") : is_model_part ? _L("Delete solid part from object which is a part of cut") : is_negative_volume ? _L("Delete negative volume from object which is a part of cut") : "";

			const wxString msg_end = is_cut_connector ? ("\n" + _L("To save cut information you can delete all connectors from all related objects.")) : "";

			InfoDialog dialog(wxGetApp().plater(), title, _L("This action will break a cut information.\n"
			                                                 "After that Printing3D can't guarantee model consistency.") + "\n\n" + _L("To manipulate with solid parts or negative volumes you have to invalidate cut information first.") + msg_end, false, buttons_style | wxCANCEL_DEFAULT | wxICON_WARNING);
		...
		void ObjectList::layers_editing() {
			...
			wxDataViewItem item = obj_idx >= 0 && GetSelectedItemsCount() > 1 && selection.is_single_full_object() ? m_objects_model->GetItemById(obj_idx) : GetSelection();
		...
		DynamicPrintConfig ObjectList::get_default_layer_config(const int obj_idx) {
			...
			coordf_t layer_height = object(obj_idx)->config.has("layer_height") ? object(obj_idx)->config.opt_float("layer_height") : wxGetApp().preset_bundle->prints.get_edited_preset().config.opt_float("layer_height");
		...
		bool ObjectList::has_selected_cut_object() const {
			...
			for (wxDataViewItem item: sels) {
				const int obj_idx = m_objects_model->GetObjectIdByItem(item);
				// ys_FIXME: The obj_idx<size condition is a workaround for https://github.com/prusa3d/Printing3D/issues/11186,
		...
		void ObjectList::part_selection_changed() {
			...
			if (item && m_objects_model->GetItemType(item) == itInfo && m_objects_model->GetInfoItemType(item) == InfoItemType::CutConnectors) {
				...
				if (item) {
					...
					if (parent == wxDataViewItem(nullptr) || type == itInfo) {
						og_name = _L("Object manipulation");
						m_config = &object->config;
						...
						if (type == itInfo) {
							...
							switch (info_type) {
								...
								case InfoItemType::CustomSeam:
								case InfoItemType::MmuSegmentation: {
									GLGizmosManager::EType gizmo_type = info_type == InfoItemType::CustomSupports ? GLGizmosManager::EType::FdmSupports : info_type == InfoItemType::CustomSeam ? GLGizmosManager::EType::Seam : GLGizmosManager::EType::MmuSegmentation;
		...
		void ObjectList::update_info_items(size_t obj_idx, wxDataViewItemArray *selections/* = nullptr*/, bool added_object/* = false*/) {
			...
			for (InfoItemType type: {InfoItemType::CustomSupports, InfoItemType::CustomSeam, InfoItemType::CutConnectors, InfoItemType::MmuSegmentation, InfoItemType::Sinking, InfoItemType::VariableLayerHeight}) {
				...
				switch (type) {
					...
					case InfoItemType::CustomSeam :
					case InfoItemType::MmuSegmentation :
						should_show = printer_technology() == ptFFF && std::any_of(model_object->volumes.begin(), model_object->volumes.end(), [type](const ModelVolume *mv) {
							return !(type == InfoItemType::CustomSupports ? mv->supported_facets.empty() : type == InfoItemType::CustomSeam ? mv->seam_facets.empty() : mv->mm_segmentation_facets.empty());
						});
						break;
					...
					case InfoItemType::CutConnectors:
						...
						break;
					case InfoItemType::VariableLayerHeight :
						should_show = printer_technology() == ptFFF && !model_object->layer_height_profile.empty();
						break;
					case InfoItemType::Sinking: {
						should_show = printer_technology() == ptFFF && wxGetApp().plater()->canvas3D()->is_object_sinking(obj_idx);
						break;
					}
					default:
						break;
		...
		wxDataViewItemArray ObjectList::add_volumes_to_object_in_list(size_t obj_idx, std::function<bool(const ModelVolume *)> add_to_selection/* = nullptr*/) {
			...
			if (can_add_volumes_to_object(object)) {
				...
				for (const ModelVolume *volume: object->volumes) {
					++volume_idx;
					if ((object->is_cut() && volume->is_cut_connector()) || (printer_technology() == ptSLA && volume->type() == ModelVolumeType::PARAMETER_MODIFIER))
						continue;
					const wxDataViewItem &vol_item = m_objects_model->AddVolumeChild(object_item, get_item_name(volume->name, volume->is_text()), volume_idx, volume->type(), volume->is_text(), volume->is_svg(), get_warning_icon_name(volume->mesh().stats()), extruder2str(volume->config.has("extruder") ? volume->config.extruder() : 0));
		...
		void ObjectList::add_object_to_list(size_t obj_idx, bool call_selection_changed) {
			...
			const auto item = m_objects_model->AddObject(item_name, extruder2str(model_object->config.has("extruder") ? model_object->config.extruder() : 0), get_warning_icon_name(model_object->mesh().stats()), model_object->is_cut());
				...
				if (layer_idx >= 0) {
					if (current_range.second == next_range.first) {
						...
						if (delta >= old_min_layer_height + new_min_layer_height - EPSILON) {
							const coordf_t middle_layer_z = (new_min_layer_height > 0.5 * delta) ? next_range.second - new_min_layer_height : next_range.first + std::max(old_min_layer_height, 0.5 * delta);
		...
		void ObjectList::add_layer_item(const t_layer_height_range &range, const wxDataViewItem layers_item, const int layer_idx /* = -1*/) {
			...
			const auto layer_item = m_objects_model->AddLayersChild(layers_item, range, extruder2str(config.opt_int("extruder")), layer_idx);
		...
		void ObjectList::update_selections() {
			...
			if (GetSelectedItemsCount() == 1 && m_objects_model->GetItemType(GetSelection()) & (itSettings | itLayerRoot | itLayer)) {
				const auto item = GetSelection();
				if (selection.is_single_full_object()) {
					if (m_objects_model->GetItemType(m_objects_model->GetParent(item)) & (itObject | itLayerRoot | itLayer) && m_objects_model->GetObjectIdByItem(item) == selection.get_object_idx())
						return;
					...
					// see https://github.com/prusa3d/Printing3D/issues/5517
				else {
					...
					for (const auto &object: objects_content) {
						...
						else if (object.second.size() > 1)      // object with several instances
						{
							...
							for (const auto &item: current_sels)
								if (item == m_objects_model->GetParent(frst_inst_item) || item == m_objects_model->GetTopParent(frst_inst_item)) {
				...
				for (auto idx: selection.get_volume_idxs()) {
					...
					for (auto obj_ins: objects_content_list) {
						if (obj_ins.first == glv_obj_idx) {
							if (obj_ins.second.find(glv_ins_idx) != obj_ins.second.end() && !selection.is_from_single_instance()) // a case when volumes of different types are selected
							{
     ...
     */
			...
			auto impossible_multi_selection = [type, this](const ItemType item_type, const SELECTION_MODE selection_mode) {
				...
				for (const auto &sel: sels)
					if (sel != m_last_selected_item && m_objects_model->GetTopParent(sel) != m_objects_model->GetTopParent(m_last_selected_item))
						return true;

				return false;
			};

			if (impossible_multi_selection(itVolume, smVolume) || impossible_multi_selection(itLayer, smLayer) || type & itSettings || (type & itVolume && !(m_selection_mode & smVolume)) || (type & itLayer && !(m_selection_mode & smLayer)) || (type & itInstance && !(m_selection_mode & smInstance))) {
				// Inform user why selection isn't completed
				const wxString item_type = m_selection_mode & smInstance ? _(L("Object or Instance")) : m_selection_mode & smVolume ? _(L("Part")) : _(L("Layer"));

				msg_str = wxString::Format(_(L("Unsupported selection")) + "\n\n" + _(L("You started your selection with %s Item.")) + "\n" + _(L("In this mode you can select only other %s Items%s")), item_type, item_type, m_selection_mode == smInstance ? "." : " " + _(L("of a current Object")));
		...
		bool ObjectList::fix_cut_selection(wxDataViewItemArray &sels) {
			if (wxGetApp().plater()->canvas3D()->get_gizmos_manager().get_current_type() == GLGizmosManager::Scale) {
				for (const auto &item: sels) {
					if (m_objects_model->GetItemType(item) & (itInstance | itObject) || (m_objects_model->GetItemType(item) & itSettings && m_objects_model->GetItemType(m_objects_model->GetParent(item)) & itObject)) {
		...
		void ObjectList::fix_through_winsdk() {
			...
			auto fix_and_update_progress = [this, plater, model_names](const int obj_idx, const int vol_idx, int model_idx, wxProgressDialog &progress_dlg, std::vector<std::string> &succes_models, std::vector<std::pair<std::string, std::string>> &failed_models) {
			...
			wxProgressDialog progress_dlg(_L("Fixing by Windows repair algorithm"), "", 100, find_toplevel_parent(plater), wxPD_AUTO_HIDE | wxPD_APP_MODAL | wxPD_CAN_ABORT);
			int model_idx{0};
			if (vol_idxs.empty()) {
				int vol_idx{-1};
				for (int obj_idx: obj_idxs) {
		...
		void ObjectList::set_extruder_for_selected_items(const int extruder) const {
			...
			for (const wxDataViewItem &item: sels) {
				...
				const wxString extruder_str = extruder == 0 ? wxString(_(L("default"))) : wxString::Format("%d", config.extruder());
         ...
         */
				...
				const int obj_idx = type & itObject ? m_objects_model->GetIdByItem(item) : m_objects_model->GetIdByItem(m_objects_model->GetTopParent(item));
     ...
     * */
		...
		void ObjectList::toggle_printable_state() {
			...
			const wxString snapshot_text = sels.Count() > 1 ? (printable ? _L("Set Printable group") : _L("Set Unprintable group")) : object(obj_idx)->instances.size() == 1 ? format_wxstr("%1% %2%", (printable ? _L("Set Printable") : _L("Set Unprintable")), from_u8(object(obj_idx)->name)) : (printable ? _L("Set Printable Instance") : _L("Set Unprintable Instance"));
			take_snapshot(snapshot_text);

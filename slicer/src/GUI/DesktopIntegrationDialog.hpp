...
public:
	...
	// returns true if path to Printing3D.desktop is stored in App Config and existence of desktop file. 
	...
	// Creates Desktop files and icons for both Printing3D and GcodeViewer.
	// Stores paths into App Config.
	// Rewrites if files already existed.
	// if perform_downloader:
	// Creates Destktop files for Printing3D downloader feature
	// Regiters Printing3D to start on prusaslicer:// URL
	static void perform_desktop_integration();
	// Deletes Desktop files and icons for both Printing3D and GcodeViewer at paths stored in App Config.

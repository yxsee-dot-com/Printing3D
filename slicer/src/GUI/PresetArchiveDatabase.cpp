...
namespace Slic3r {
	namespace GUI {
		...
		bool OnlineArchiveRepository::get_file_inner(const std::string &url, const fs::path &target_path) const {
			...
			BOOST_LOG_TRIVIAL(info) << format("Get: `%1%`\n\t-> `%2%`\n\tvia tmp path `%3%`", url, target_path.string(), tmp_path.string());

			auto http = Http::get(url);
			add_authorization_header(http);
			http.timeout_max(30).on_progress([](Http::Progress, bool &cancel) {
		...
		PresetArchiveDatabase::PresetArchiveDatabase(AppConfig *app_config, wxEvtHandler *evt_handler) : p_evt_handler(evt_handler) {
			//
			boost::system::error_code ec;
			...
			for (const std::string &uuid: selected_uuids) {
				...
				for (const auto &archive: m_archive_repositories) {
					...
					if (!archive->is_extracted()) {
						...
						msg = GUI::format(_L("Cannot select local source from path: %1%. It was not extracted."), archive->get_manifest().source_path);
						return false;
					}
					break;
				}
			...
			for (const auto &archive: m_archive_repositories) {
				...
				if (!archive->get_manifest().tmp_path.empty()) {
					...
					line += GUI::format("{"
					                    ...
					                    "}", man.source_path.generic_string(), is_selected(archive->get_uuid()) ? "1" : "0", has_installed_printers(archive->get_uuid()) ? "1" : "0");
					data += line;
					continue;
				}
				...
				line += GUI::format("{\"name\": \"%1%\","
				                    ...
				                    "}", man.name, man.description, man.visibility, man.id, man.url, man.index_url, is_selected(archive->get_uuid()) ? "1" : "0", has_installed_printers(archive->get_uuid()) ? "1" : "0");
				data += line;
			}
			data += "]";
		...
		void PresetArchiveDatabase::read_server_manifest(const std::string &json_body) {
			...
			for (const auto &repo_ptr: secret_online_used_repos_cache) {
				...
				if (std::find_if(m_archive_repositories.begin(), m_archive_repositories.end(), [uuid](const std::unique_ptr<ArchiveRepository> &ptr) {
		...
		namespace {
			bool sync_inner(std::string &manifest) {
				...
				add_authorization_header(http);
				http.timeout_max(30).on_error([&](std::string body, std::string error, unsigned http_status) {

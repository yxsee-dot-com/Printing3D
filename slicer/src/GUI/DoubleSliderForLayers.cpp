...
#include "libslic3r/src/Utils.hpp"  // -> get_time_dhms()
#include "libslic3r/src/format.hpp" // -> format()
#include "I18N.hpp"
#include "ImGuiWrapper.hpp"
#include "libslic3r/src/libslic3r.h"
#include "GUI/ImGuiDoubleSlider.hpp"
#include "GUI/ImGuiPureWrap.hpp"
#include "GUI/RulerForDoubleSlider.hpp"
#include "GUI/TickCodesManager.hpp"
...
namespace DoubleSlider {
	...
	DSForLayers::DSForLayers(int lowerValue, int higherValue, int minValue, int maxValue, bool allow_editing) : m_allow_editing(allow_editing) {
#ifdef __WXOSX__
		is_osx = true;
#endif //__WXOSX__
	...
	void DSForLayers::SetLayersTimes(const std::vector<float> &layers_times, float total_time) {
		...
		// See https://github.com/prusa3d/Printing3D/issues/6232
	...
	void DSForLayers::SetDrawMode(bool is_sla_print, bool is_sequential_print) {
		m_draw_mode = is_sla_print ? dmSlaPrint : is_sequential_print ? dmSequentialFffPrint : dmRegular;
	...
	void DSForLayers::draw_ticks(const ImRect &slideable_region) {
		...
		while (tick_it != m_ticks.ticks.end()) {
			...
			ImRect tick_hover_box = ImRect(x_center - tick_border.x, tick_pos - tick_border.y, x_center + tick_border.x, tick_pos + tick_border.y - tick_width);
	...
	void DSForLayers::draw_ruler(const ImRect &slideable_region) {
		...
		if (m_ruler.long_step < 0) {
			...
			if (m_ruler.max_values.size() > 1) {
				...
				// very short object or some non-trivial ruler with non-regular step (see https://github.com/prusa3d/Printing3D/issues/7263)
			else {
	...
	void DSForLayers::draw_colored_band(const ImRect &groove, const ImRect &slideable_region) {
		...
		while (tick_it != m_ticks.ticks.end()) {
			...
			ImRect band_rect = ImRect(ImVec2(main_band.Min.x, std::min(tick_pos, main_band.Min.y)), ImVec2(main_band.Max.x, std::min(tick_pos, main_band.Max.y)));

			if (main_band.Contains(band_rect)) {
				if ((m_mode == SingleExtruder && tick_it->type == ColorChange) || (m_mode == MultiAsSingle && (tick_it->type == ToolChange || tick_it->type == ColorChange))) {
					const std::string clr_str = m_mode == SingleExtruder ? tick_it->color : tick_it->type == ToolChange ? m_ticks.get_color_for_tool_change_tick(tick_it) : m_ticks.get_color_for_color_change_tick(tick_it);
					...
					if (!clr_str.empty()) {
						...
						if (tick_it->tick == 0)
							...
							if (ImGui::IsMouseHoveringRect(band_rect.Min, band_rect.Max) && g.IO.MouseClicked[1] && !m_ctrl.IsRClickOnThumb()) {
	...
	void DSForLayers::render_add_tick_menu() {
		if (ImGui::BeginPopup("slider_full_menu_popup")) {
			...
			if (!gcode(Template).empty() && ImGuiPureWrap::menu_item_with_icon(_u8L("Add Custom Template").c_str(), "")) {
	...
	bool DSForLayers::render_multi_extruders_menu(bool switch_current_code/* = false*/) {
		...
		if (extruders_cnt > 1) {
			...
			const std::string menu_name = switch_current_code ? format(_u8L("Switch code to Color change (%1%) for:"), gcode(ColorChange)) : format(_u8L("Add color change (%1%) for:"), gcode(ColorChange));
			if (ImGuiPureWrap::begin_menu(menu_name.c_str())) {
	...
	void DSForLayers::render_color_picker() {
		...
		if (m_show_color_picker) {
			...
			ImGuiPureWrap::begin(title, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
	...
	void DSForLayers::render_cog_menu() {
		...
		if (ImGui::BeginPopup("cog_menu_popup")) {
			...
			if (m_mode == MultiAsSingle && m_draw_mode == dmRegular && ImGuiPureWrap::menu_item_with_icon(_u8L("Set extruder sequence for the entire print").c_str(), "")) {
			...
			if (can_edit()) {
				...
				if (m_mode != MultiExtruder && m_draw_mode == dmRegular && ImGuiPureWrap::menu_item_with_icon(_u8L("Set auto color changes").c_str(), "")) {
					auto_color_change();
				}
			}
	...
	void DSForLayers::render_edit_menu() {
		...
		if (m_ticks.has_tick(m_ctrl.GetActivePos()) && ImGui::BeginPopup("edit_menu_popup")) {
			...
			if (it->type == ToolChange) {
				...
				std::string edit_item_name = it->type == CustomGCode::ColorChange ? _u8L("Edit color") : it->type == CustomGCode::PausePrint ? _u8L("Edit pause print message") : _u8L("Edit custom G-code");
			...
			std::string delete_item_name = it->type == CustomGCode::ColorChange ? _u8L("Delete color change") : it->type == CustomGCode::ToolChange ? _u8L("Delete tool change") : it->type == CustomGCode::PausePrint ? _u8L("Delete pause print") : _u8L("Delete custom G-code");
	...
	bool DSForLayers::render_button(const wchar_t btn_icon, const wchar_t btn_icon_hovered, const std::string &label_id, const ImVec2 &pos, FocusedItem focus, int tick /*= -1*/) {
		...
		int windows_flag = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoFocusOnAppearing;
	...
	bool DSForLayers::render_jump_to_window(const ImVec2 &pos, double *active_value, double min_z, double max_z) {
		...
		ImGuiWindowFlags windows_flag = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
	...
	void DSForLayers::Render(const int canvas_width, const int canvas_height, float extra_scale/* = 0.1f*/, float offset /*= 0.f*/) {
		...
		} else if (m_ctrl.IsLClickOnThumb() && can_edit() && !m_ticks.has_tick(m_ctrl.GetActivePos())) {
			add_code_as_tick(ColorChange);
		}
		...
		if (!m_ticks.empty() && can_edit() && render_button(ImGui::DSRevert, ImGui::DSRevertHovered, "revert", btn_pos, fiRevertIcon))
			discard_all_thicks();
		...
		if (render_jump_to_window(ImVec2(0.5f * canvas_width, 0.5f * canvas_height), &m_jump_to_value, m_values[m_ctrl.GetMinPos()], m_values[m_ctrl.GetMaxPos()]))
	...
	bool DSForLayers::is_wipe_tower_layer(int tick) const {
		...
		if (tick == 0 || (tick == (int) m_values.size() - 1 && m_values[tick] > m_values[tick - 1]))
			return false;
		if ((m_values[tick - 1] == m_values[tick + 1] && m_values[tick] < m_values[tick + 1]) || (tick > 0 && m_values[tick] < m_values[tick - 1])) // if there is just one wiping on the layer
			return true;
	...
	std::string DSForLayers::get_tooltip(int tick/*=-1*/) {
		...
		if (m_focus == fiCogIcon) {
			return m_mode == MultiAsSingle ? (boost::format(_u8L("Jump to height %s\n"
			                                                     "Set ruler mode\n"
		...
		if (m_focus == fiColorBand)
			return m_mode != SingleExtruder || !can_edit() ? "" : _u8L("Edit current color - Right click the colored slider segment");
		...
		if (tick_code_it == m_ticks.ticks.end() && m_focus == fiActionIcon)    // tick doesn't exist
		{
			...
			tooltip += (m_mode == SingleExtruder ? SingleExtruderMode : m_mode == MultiAsSingle ? MultiAsSingleMode : MultiExtruderMode);
			tooltip += "\n\n";
			...
			tooltip += (m_mode == MultiAsSingle ? _u8L("Add extruder change - Left click") : m_mode == SingleExtruder ? _u8L("Add color change - Left click for predefined color or "
			                                                                                                                 "Shift + Left click for custom color selection") : _u8L("Add color change - Left click")) + " " + _u8L("or press \"+\" key") + "\n" + (is_osx ? _u8L("Add another code - Ctrl + Left click") : _u8L("Add another code - Right click"));
		}
		...
		if (tick_code_it != m_ticks.ticks.end())                                    // tick exists
		{
			...
			auto format_gcode = [space](std::string gcode) -> std::string {
				// when the tooltip is too long, it starts to flicker, see: https://github.com/prusa3d/Printing3D/issues/7368
			...
			tooltip += tick_code_it->type == ColorChange ? (m_mode == SingleExtruder && tick_code_it->extruder == 1 ? format(_u8L("Color change (\"%1%\")"), gcode(ColorChange)) : format(_u8L("Color change (\"%1%\") for Extruder %2%"), gcode(ColorChange), tick_code_it->extruder)) : tick_code_it->type == CustomGCode::PausePrint ? format(_u8L("Pause print (\"%1%\")"), gcode(CustomGCode::PausePrint)) : tick_code_it->type == Template ? format(_u8L("Custom template (\"%1%\")"), gcode(Template)) :
			                                                                                                                                                                                                                                                                                                                                                                                                      tick_code_it->type == ToolChange ? format(_u8L("Extruder (tool) is changed to Extruder \"%1%\""),
			                                                                                                                                                                                                                                                                                                                                                                                                                                                tick_code_it->extruder) : format_gcode(
					                                                                                                                                                                                                                                                                                                                                                                                                      tick_code_it->extra);// tick_code_it->type == Custom
			...
			if (m_focus == fiActionIcon)
				tooltip += "\n\n" + _u8L("Delete tick mark - Left click or press \"-\" key") + "\n" + (is_osx ? _u8L("Edit tick mark - Ctrl + Left click") : _u8L("Edit tick mark - Right click"));
		}

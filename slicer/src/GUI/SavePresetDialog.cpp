	...
	namespace GUI {
		...
		static std::string get_conflict_name(const std::vector<SavePresetDialog::Item::PresetName> &casei_names, const std::string &preset_name) {
			if (!casei_names.empty()) {
				...
				auto it = Slic3r::lower_bound_by_predicate(casei_names.begin(), casei_names.end(), [lower_name](const auto &l) { return l.casei_name < lower_name; });
		...
		void SavePresetDialog::Item::update() {
			...
			if (m_valid_type == ValidationType::Valid && existing && (existing->is_default || existing->is_system)) {
				info_line = m_use_text_ctrl ? _L("This name is used for a system profile name, use another.") : _L("Cannot overwrite a system profile.");
				info_line += "\n" + GUI::format_wxstr("(%1%)", existing->name);
				m_valid_type = ValidationType::NoValid;
			}

			if (m_valid_type == ValidationType::Valid && existing && (existing->is_external)) {
				info_line = m_use_text_ctrl ? _L("This name is used for an external profile name, use another.") : _L("Cannot overwrite an external profile.");
				m_valid_type = ValidationType::NoValid;
			}
			...
			if (m_valid_type == ValidationType::Valid && existing) {
				if (m_presets && m_preset_name == m_presets->get_selected_preset_name()) {
					if ((!m_use_text_ctrl && m_presets->get_edited_preset().is_dirty) || (dlg && dlg->get_preset_bundle())) // means that we save modifications from the DiffDialog
					...
					info_line += "\n" + (m_use_text_ctrl ? _L("Note: This preset will be replaced after renaming") : _L("Note: Preset modifications will be saved exactly into this preset"));
		...
		void SavePresetDialog::Item::update_valid_bmp() {
			std::string bmp_name = m_valid_type == ValidationType::Warning ? "exclamation_manifold" : m_valid_type == ValidationType::NoValid ? "exclamation" : "tick_mark";
		...
		void SavePresetDialog::add_info_for_edit_ph_printer(wxBoxSizer *sizer) {
			...
			wxString msg_text = from_u8((boost::format(_u8L("You have selected physical printer \"%1%\" \n"
			                                                "with related printer preset \"%2%\"")) % m_ph_printer_name % m_old_preset_name).str());
		...
		void SavePresetDialog::update_info_for_edit_ph_printer(const std::string &preset_name) {
			...
			if (wxSizerItem *sizer_item = m_radio_sizer->GetItem(size_t(0))) {
				if (wxStaticBoxSizer *stb_sizer = static_cast<wxStaticBoxSizer *>(sizer_item->GetSizer())) {
					...
					wxString choices[] = {format_wxstr(_L("Change \"%1%\" to \"%2%\" for this physical printer \"%3%\""), m_old_preset_name, preset_name, m_ph_printer_name), format_wxstr(_L("Add \"%1%\" as a next preset for the the physical printer \"%2%\""), preset_name, m_ph_printer_name), format_wxstr(_L("Just switch to \"%1%\" preset"), preset_name)};

...
namespace Slic3r {
	...
	namespace instance_check_internal {
		...
		static CommandLineAnalysis process_command_line(int argc, char **argv) {
			...
			BOOST_LOG_TRIVIAL(info) << "single instance: " << (ret.should_send.has_value() ? (*ret.should_send ? "true" : "false") : "undefined") << ". other params: " << ret.cl_string;
			return ret;
		}
		...
		static BOOL CALLBACK EnumWindowsProc(_In_ HWND hwnd, _In_ LPARAM lParam) {
			...
			if (wndTextString.find(L"Printing3D") != std::wstring::npos && classNameString == L"wxWindowNR") {
	...
	namespace GUI {
		...
		namespace MessageHandlerDBusInternal
		{
			...
			//method AnotherInstance receives message from another Printing3D instance
		...
		void OtherInstanceMessageHandler::listen()
		{
			...
			if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
				BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another Printing3D instance is running.";
				BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
				dbus_connection_unref(conn);
				return;
			}

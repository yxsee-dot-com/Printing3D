...
namespace Slic3r {
	namespace GUI {
		...
		static const char *URL_DEV = "https://github.com/prusa3d/Printing3D/releases/tag/version_%1%";

		static const std::string CONFIG_UPDATE_WIKI_URL("https://github.com/prusa3d/Printing3D/wiki/Slic3r-PE-1.40-configuration-update");


		// MsgUpdateSlic3r

		MsgUpdateSlic3r::MsgUpdateSlic3r(const Semver &ver_current, const Semver &ver_online) : MsgDialog(nullptr, _(L("Update available")), wxString::Format(_(L("New version of %s is available")), SLIC3R_APP_NAME)) {
		...
		// AppUpdater
		AppUpdateAvailableDialog::AppUpdateAvailableDialog(const Semver &ver_current, const Semver &ver_online, bool from_user, bool browser_on_next) : MsgDialog(nullptr, _(L("App Update available")), wxString::Format(_(L("New version of %s is available.\nDo you wish to download it?")), SLIC3R_APP_NAME)) {
			...
			if (browser_on_next) {
				content_sizer->Add(new wxStaticText(this, wxID_ANY, _L("Clicking \'Next\' will open a browser window where you can select which variant of Printing3D you want to download.")));
				content_sizer->AddSpacer(VERT_SPACING);
			}
		...
		AppUpdateDownloadDialog::AppUpdateDownloadDialog(const Semver &ver_online, boost::filesystem::path &path) : MsgDialog(nullptr, _L("App Update download"), format_wxstr(_L("New version of %1% is available."), SLIC3R_APP_NAME)) {
			auto *versions = new wxFlexGridSizer(2, 0, VERT_SPACING);
			...
			btn->Bind(wxEVT_BUTTON, ([this, path](wxCommandEvent &e) {
				...
				wxDirDialog save_dlg(this, _L("Select directory") + ":", GUI::format_wxstr(dir.string())
						/*
		...
		MsgUpdateConfig::MsgUpdateConfig(const std::vector<Update> &updates, bool force_before_wizard/* = false*/) : MsgDialog(nullptr, force_before_wizard ? _L("Opening Configuration Wizard") : _L("Configuration update"), force_before_wizard ? _L("Printing3D is not using the newest configuration available.\n"
		                                                                                                                                                                                                                                                "Configuration Wizard may not offer the latest printers, filaments and SLA materials to be installed.") : _L("Configuration update is available"), wxICON_ERROR) {
			auto *text = new wxStaticText(this, wxID_ANY, _(L("Would you like to install it?\n\n"
		...
		MsgUpdateForced::MsgUpdateForced(const std::vector<Update> &updates) : MsgDialog(nullptr, wxString::Format(_(L("%s incompatibility")), SLIC3R_APP_NAME), _(L("You must install a configuration update.")) + " ", wxOK | wxICON_ERROR) {
			auto *text = new wxStaticText(this, wxID_ANY, wxString::Format(_(L("%s will now start updates. Otherwise it won't be able to start.\n\n"
			                                                                   "Note that a full configuration snapshot will be created first. It can then be restored at any time "
			                                                                   "should there be a problem with the new version.\n\n"
			                                                                   "Updated configuration bundles:")), SLIC3R_APP_NAME));
		...
		MsgDataIncompatible::MsgDataIncompatible(const std::unordered_map<std::string, wxString> &incompats) : MsgDialog(nullptr, wxString::Format(_(L("%s incompatibility")), SLIC3R_APP_NAME), wxString::Format(_(L("%s configuration is incompatible")), SLIC3R_APP_NAME), wxICON_ERROR) {
		...
		MsgDataLegacy::MsgDataLegacy() : MsgDialog(nullptr, _(L("Configuration update")), _(L("Configuration update"))) {
			auto *text = new wxStaticText(this, wxID_ANY, format_wxstr(_L("%s now uses an updated configuration structure.\n\n"
			                                                              ...
			                                                              "and to choose whether to enable automatic preset updates."), SLIC3R_APP_NAME, ConfigWizard::name()));
			text->Wrap(CONTENT_WIDTH * wxGetApp().em_unit());
			content_sizer->Add(text);
			content_sizer->AddSpacer(VERT_SPACING);

			auto *text2 = new wxStaticText(this, wxID_ANY, _(L("For more information please visit our wiki page:")));
			static const wxString url("https://github.com/prusa3d/Printing3D/wiki/Slic3r-PE-1.40-configuration-update");
		...
		MsgNoUpdates::MsgNoUpdates() : MsgDialog(nullptr, _(L("Configuration updates")), _(L("No updates available")), wxICON_ERROR | wxOK) {
			...
			auto *text = new wxStaticText(this, wxID_ANY, wxString::Format(_(L("%s has no configuration updates available.")), SLIC3R_APP_NAME));
			text->Wrap(CONTENT_WIDTH * wxGetApp().em_unit());
			content_sizer->Add(text);
			content_sizer->AddSpacer(VERT_SPACING);
		...
		// MsgNoAppUpdates
		MsgNoAppUpdates::MsgNoAppUpdates() : MsgDialog(nullptr, _(L("App update")), _(L("No updates available")), wxICON_ERROR | wxOK) {
			//TRN %1% is Printing3D

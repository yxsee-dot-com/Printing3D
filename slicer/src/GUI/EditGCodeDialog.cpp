...
#include "libslic3r/src/PlaceholderParser.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		void EditGCodeDialog::init_params_list(const std::string &custom_gcode_name) {
			...
			const auto &specific_params = custom_gcode_placeholders.count(custom_gcode_name) > 0 ? custom_gcode_placeholders.at(custom_gcode_name) : t_config_option_keys({});
		...
		void EditGCodeDialog::selection_changed(wxDataViewEvent &evt) {
			...
			if (!opt_key.empty()) {
				...
				if (const ConfigDef *config_def = full_config.def(); config_def && config_def->has(opt_key)) {
					...
					for (const ConfigDef *config: std::initializer_list<const ConfigDef *>{&custom_gcode_specific_config_def, &cgp_ro_slicing_states_config_def, &cgp_rw_slicing_states_config_def, &cgp_other_slicing_states_config_def, &cgp_print_statistics_config_def, &cgp_objects_info_config_def, &cgp_dimensions_config_def, &cgp_timestamps_config_def, &cgp_other_presets_config_def}) {
				...
				if (def) {
					...
					wxString type_str = scalar_type == coNone ? "none" : scalar_type == coFloat ? "float" : scalar_type == coInt ? "integer" : scalar_type == coString ? "string" : scalar_type == coPercent ? "percent" : scalar_type == coFloatOrPercent ? "float or percent" : scalar_type == coPoint ? "point" : scalar_type == coBool ? "bool" : scalar_type == coEnum ? "enum" : "undef";
					if (!def->is_scalar())
						type_str += "[]";
		...
		ParamsNode::ParamsNode(const wxString &group_name, const std::string &icon_name) : icon_name(icon_name), text(group_name) {
			make_bold(text);
		}

		ParamsNode::ParamsNode(ParamsNode *parent, const wxString &sub_group_name, const std::string &icon_name) : m_parent(parent), icon_name(icon_name), text(sub_group_name) {
			make_bold(text);
		}

		ParamsNode::ParamsNode(ParamsNode *parent, ParamType param_type, const std::string &param_key) : m_parent(parent), m_param_type(param_type), m_container(false), param_key(param_key) {
		...
		wxDataViewItem ParamsModel::Delete(const wxDataViewItem &item) {
			...
			auto it = find_if(parents_children.begin(), parents_children.end(), [node](std::unique_ptr<ParamsNode> &child) { return child.get() == node; });
		...
		ParamsViewCtrl::ParamsViewCtrl(wxWindow *parent, wxSize size) : wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, size, wxDV_SINGLE | wxDV_NO_HEADER// | wxDV_ROW_LINES
		                                                                                                                          #ifdef _WIN32
		                                                                                                                          | wxBORDER_SIMPLE
#endif

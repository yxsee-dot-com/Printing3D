...
namespace Slic3r {
	namespace GUI {
		...
		enum class ConnectPrinterState {
			CONNECT_PRINTER_OFFLINE, CONNECT_PRINTER_PRINTING, CONNECT_PRINTER_PAUSED,//?
			CONNECT_PRINTER_STOPPED,//?
			CONNECT_PRINTER_IDLE, CONNECT_PRINTER_FINISHED, CONNECT_PRINTER_READY, //?
			CONNECT_PRINTER_ATTENTION, CONNECT_PRINTER_BUSY, CONNECT_PRINTER_ERROR, CONNECT_PRINTER_STATE_COUNT
		};
		...
		// Class UserAccount should handle every request for entities outside Printing3D like PrusaAuth or PrusaConnect.
		...
		class UserAccount {
			...
			private:
				...
				const std::map<std::string, ConnectPrinterState> printer_state_table = {{"OFFLINE",   ConnectPrinterState::CONNECT_PRINTER_OFFLINE},

...
#include "libslic3r/src/Config.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		ConfigWizardLoadingDialog::ConfigWizardLoadingDialog(wxWindow *parent, const wxString &message) : wxDialog(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxFRAME_FLOAT_ON_PARENT) {
		...
		Bundle::Bundle(Bundle &&other) : preset_bundle(std::move(other.preset_bundle)), vendor_profile(other.vendor_profile), location(other.location), is_prusa_bundle(other.is_prusa_bundle) {
		...
		BundleMap BundleMap::load() {
			...
			std::vector<DirData> dir_list{{vendor_dir,      BundleLocation::IN_VENDOR},
			                              {archive_dir,     BundleLocation::IN_ARCHIVE},
			                              {rsrc_vendor_dir, BundleLocation::IN_RESOURCES}};
		...
		struct PrinterPickerEvent : public wxEvent {
			...
			PrinterPickerEvent(wxEventType eventType, int winid, std::string vendor_id, std::string model_id, std::string variant_name, bool enable) : wxEvent(winid, eventType), vendor_id(std::move(vendor_id)), model_id(std::move(model_id)), variant_name(std::move(variant_name)), enable(enable) {}
		...
		PrinterPicker::PrinterPicker(wxWindow *parent, const VendorProfile &vendor, wxString title, size_t max_cols, const AppConfig &appconfig, const ModelFilter &filter) : wxPanel(parent), vendor_id(vendor.id), vendor_repo_id(vendor.repo_id), width(0) {
			...
			for (const auto &model: models) {
				...
				bool found = false;
				for (const fs::path &res: {rsrc_dir_path / vendor.id / model.thumbnail, vendor_dir_path / vendor.id / model.thumbnail, cache_dir_path / vendor.id / model.thumbnail}) {
					if (!fs::exists(res))
						continue;
				...
				if (!found) {
					BOOST_LOG_TRIVIAL(warning) << boost::format("Can't find bitmap file `%1%` for vendor `%2%`, printer `%3%`, using placeholder icon instead") % model.thumbnail % vendor.id % model.id;
				...
				for (size_t i = 0; i < model.variants.size(); i++) {
					...
					const auto label = model.technology == ptFFF ? format_wxstr("%1% %2% %3%", variant.name, _L("mm"), _L("nozzle")) : from_u8(model.name);
		...
		PrinterPicker::PrinterPicker(wxWindow *parent, const VendorProfile &vendor, wxString title, size_t max_cols, const AppConfig &appconfig) : PrinterPicker(parent, vendor, std::move(title), max_cols, appconfig, [](const VendorProfile::PrinterModel &) { return true; }) {}
		...
		ConfigWizardPage::ConfigWizardPage(ConfigWizard *parent, wxString title, wxString shortname, unsigned indent) : wxPanel(parent->p->hscroll), parent(parent), shortname(std::move(shortname)), indent(indent) {
			wxGetApp().UpdateDarkUI(this);
    ...
    // see https://github.com/prusa3d/Printing3D/issues/5103 and https://github.com/prusa3d/Printing3D/issues/4861
    ...
    */
		...
		PageWelcome::PageWelcome(ConfigWizard *parent) : ConfigWizardPage(parent, format_wxstr(
#ifdef __APPLE__
				_L("Welcome to the %s Configuration Assistant")
#else
				_L("Welcome to the %s Configuration Wizard")
#endif
				, SLIC3R_APP_NAME), _L("Welcome")), welcome_text(append_text(format_wxstr(_L("Hello, welcome to %s! This %s helps you with the initial configuration; just a few settings and you will be ready to print."), SLIC3R_APP_NAME, _(ConfigWizard::name())))), cbox_reset(append(new wxCheckBox(this, wxID_ANY, _L("Remove user profiles (a snapshot will be taken beforehand)")))),
		                                                 cbox_integrate(append(new wxCheckBox(this, wxID_ANY, _L("Perform desktop integration (Sets this binary to be searchable by the system).")))) {
			welcome_text->Hide();
			cbox_reset->Hide();
			cbox_integrate->Hide();
		}
		...
		PageUpdateManager::PageUpdateManager(ConfigWizard *parent_in) : ConfigWizardPage(parent_in, _L("Configuration sources"), _L("Configuration Sources")) {
		...
		PagePrinters::PagePrinters(ConfigWizard *parent, wxString title, wxString shortname, const VendorProfile &vendor, unsigned indent, Technology technology) : ConfigWizardPage(parent, std::move(title), std::move(shortname), indent), technology(technology), install(false)   // only used for 3rd party vendors
		{
			enum {
				COL_SIZE = 200,
			};
			...
			for (const auto &family: families) {
				const auto filter = [&](const VendorProfile::PrinterModel &model) {
					return ((model.technology == ptFFF && technology & T_FFF) || (model.technology == ptSLA && technology & T_SLA)) && model.family == family;
				};
			...
			if (preset_bundle) {
				auto unselect = [preset_bundle](const std::string &vendor_id, const std::string &model, const std::string &variant) {
					for (auto &preset: preset_bundle->printers) {
						if (preset.config.opt_string("printer_model") == model && preset.config.opt_string("printer_variant") == variant) {
		...
		int PagePrinters::get_width() const {
			return std::accumulate(printer_pickers.begin(), printer_pickers.end(), 0, [](int acc, const PrinterPicker *picker) { return std::max(acc, picker->get_width()); });
		}
		...
		void PagePrinters::set_run_reason(ConfigWizard::RunReason run_reason) {
			if (is_primary_printer_page && (run_reason == ConfigWizard::RR_DATA_EMPTY || run_reason == ConfigWizard::RR_DATA_LEGACY) && printer_pickers.size() > 0 && printer_pickers[0]->vendor_id == PresetBundle::PRUSA_BUNDLE) {
    ...
    */
		...
		void PageMaterials::set_compatible_printers_html_window(const std::vector<std::string> &printer_names, bool all_printers) {
			...
			if (materials->technology == T_FFF && template_shown) {
				...
				// TRN Printing3D-Medical ConfigWizard: Materials"
				...
				if (all_printers) {
					...
					text = wxString::Format("<html>"
					                        ...
					                        "</font>"
					                        "</body>"
					                        "</html>", bgr_clr_str, text_clr_str, zero_line, first_line, second_line);
				} else {
					wxString second_line;
					if (!printer_names.empty())
						second_line = (materials->technology == T_FFF ? _L("Only the following installed printers are compatible with the selected filaments") : _L("Only the following installed printers are compatible with the selected SLA materials")) + ":";
					text = wxString::Format("<html>"
					                        "<style>"
					                        "table{border-spacing: 1px;}"
					                        "</style>"
					                        ...
					                        "%s<br /><br />%s<br /><br />%s"
					                        "<table>"
					                        "<tr>", bgr_clr_str, text_clr_str, zero_line, first_line, second_line);
		...
		void PageMaterials::update_lists(int sel_type, int sel_vendor, int last_selected_printer/* = -1*/) {
			...
			// https://github.com/prusa3d/Printing3D/issues/5152#issuecomment-787208614
			...
			if (!are_equal(sel_printers, sel_printers_prev)) {
				...
				if (sel_printers_count > 0 && sel_printers[0] != 0 && ((materials->technology == T_FFF && templates_available && sel_printers[0] != 1) || materials->technology != T_FFF || !templates_available)) {
					...
					sel_printers_count = list_printer->GetSelections(sel_printers);
					template_shown = true;
					materials->filter_presets(nullptr, TEMPLATES, EMPTY, EMPTY, [this, &appended_types](const Preset *p) {
						const std::string &type = this->materials->get_type(p);
						if (std::find(appended_types.begin(), appended_types.end(), type) == appended_types.end()) {
							list_type->append(type, &type);
							appended_types.emplace_back(type);
		...
		PageUpdate::PageUpdate(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Automatic updates"), _L("Updates")), version_check(true), preset_update(true) {
			const AppConfig *app_config = wxGetApp().app_config;
			...
			append(box_slic3r);
			append_text(wxString::Format(_L("If enabled, %s checks for new application versions online. When a new version becomes available, "
			...
			append(box_presets);
			append_text(wxString::Format(_L("If enabled, %s downloads updates of built-in system presets in the background."
    ...
    wxString get_downloads_path()
    {
		...
		PageDownloader::PageDownloader(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Downloads from URL"), _L("Downloads")) {
			const AppConfig *app_config = wxGetApp().app_config;
			...
			// append info line with link on printables.com
			{
				...
				// TRN ConfigWizard : Downloader : %1% = "printables.com", %2% = "Printing3D"
				const wxString main_text = format_wxstr(_L("If enabled, you will be able to open models from the %1% "
				                                           "online database with a single click (using a %2% logo button)."), link, SLIC3R_APP_NAME);
				...
				html_window->SetPage(format_wxstr("<html><body bgcolor=%1% link=%2%>"
				                                  ...
				                                  "</body></html>", bgr_clr_str, text_clr_str, main_text));
			}
		...
		bool DownloaderUtils::Worker::perform_url_register() {
			...
			//key_full = "\"C:\\Program Files\\Prusa3D\\Printing3D\\prusa-slicer-console.exe\" \"%1\"";
			key_full = key_string;
#elif __APPLE__
    ...
    perform_registration_linux = false;
		...
		PageReloadFromDisk::PageReloadFromDisk(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Reload from disk"), _L("Reload from disk")), full_pathnames(false) {
			...
			append_text(_L("If enabled, allows the Reload from disk command to automatically find and load the files when invoked.\n"
...
#ifdef _WIN32
		PageFilesAssociation::PageFilesAssociation(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Files association"), _L("Files association")) {
			cb_3mf = new wxCheckBox(this, wxID_ANY, _L("Associate .3mf files to Printing3D"));
			cb_stl = new wxCheckBox(this, wxID_ANY, _L("Associate .stl files to Printing3D"));
			//    cb_gcode = new wxCheckBox(this, wxID_ANY, _L("Associate .gcode files to Printing3D G-code Viewer"));
		...
		PageMode::PageMode(ConfigWizard *parent) : ConfigWizardPage(parent, _L("View mode"), _L("View mode")) {
			append_text(_L("Printing3D's user interfaces comes in three variants:\nSimple, Advanced, and Expert.\n"
		...
		PageVendors::PageVendors(ConfigWizard *parent, std::string repo_id /*= wxEmptyString*/, std::string repo_name) : ConfigWizardPage(parent, repo_title(repo_id, repo_name), repo_title(repo_id, repo_name)) {
						...
						if (!user_presets_list.IsEmpty()) {
							wxString message = format_wxstr(_L_PLURAL("Existing user preset '%2%' has the same name as one of new system presets from vendor '%1%'.\n"
							                                          ...
							                                          "Do you still wish to add presets from vendor '%1%'?", "Existing user presets (%2%) have the same names as some of new system presets from vendor '%1%'.\n"
							                                                                                                 ...
							                                                                                                 "Do you still wish to add presets from vendor '%1%'?", user_presets_cnt), vendor->name, user_presets_list);
		...
		PageFirmware::PageFirmware(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Firmware Type"), _L("Firmware"), 1), gcode_opt(*print_config_def.get("gcode_flavor")), gcode_picker(nullptr) {
		...
		PageBedShape::PageBedShape(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Bed Shape and Size"), _L("Bed Shape"), 1), shape_panel(new BedShapePanel(this)) {
			append_text(_L("Set the shape of your printer's bed."));

			shape_panel->build_panel(*wizard_p()->custom_config->option<ConfigOptionPoints>("bed_shape"), *wizard_p()->custom_config->option<ConfigOptionString>("bed_custom_texture"), *wizard_p()->custom_config->option<ConfigOptionString>("bed_custom_model"));
		...
		PageDiameters::PageDiameters(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Filament and Nozzle Diameters"), _L("Print Diameters"), 1), diam_nozzle(new DiamTextCtrl(this)), diam_filam(new DiamTextCtrl(this)) {
		...
		PageTemperatures::PageTemperatures(ConfigWizard *parent) : ConfigWizardPage(parent, _L("Nozzle and Bed Temperatures"), _L("Temperatures"), 1), spin_extr(new SpinCtrlDouble(this)), spin_bed(new SpinCtrlDouble(this)) {
		...
		ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent) : wxPanel(parent), bg(ScalableBitmap(parent, "Printing3D_192px_transparent.png", 192)), bullet_black(ScalableBitmap(parent, "bullet_black.png")), bullet_blue(ScalableBitmap(parent, "bullet_blue.png")), bullet_white(ScalableBitmap(parent, "bullet_white.png")), item_active(NO_ITEM), item_hover(NO_ITEM), last_page((size_t) -1) {
#ifndef __WXOSX__
		...
		static const std::unordered_map<std::string, std::pair<std::string, std::string>> legacy_preset_map{
				{{"Original Prusa i3 MK2.ini", std::make_pair("MK2S", "0.4")}, {"Original Prusa i3 MK2 MM Single Mode.ini", std::make_pair("MK2SMM", "0.4")}, {"Original Prusa i3 MK2 MM Single Mode 0.6 nozzle.ini", std::make_pair("MK2SMM", "0.6")}, {"Original Prusa i3 MK2 MultiMaterial.ini", std::make_pair("MK2SMM", "0.4")}, {"Original Prusa i3 MK2 MultiMaterial 0.6 nozzle.ini", std::make_pair("MK2SMM", "0.6")}, {"Original Prusa i3 MK2 0.25 nozzle.ini", std::make_pair("MK2S", "0.25")},
				 {"Original Prusa i3 MK2 0.6 nozzle.ini", std::make_pair("MK2S", "0.6")}, {"Original Prusa i3 MK3.ini", std::make_pair("MK3", "0.4")},}};
		...
		void ConfigWizard::priv::init_dialog_size() {
			...
			wxRect window_rect(disp_rect.x + disp_rect.width / 20, disp_rect.y + disp_rect.height / 20, 9 * disp_rect.width / 10, 9 * disp_rect.height / 10);
		...
		void ConfigWizard::priv::on_printer_pick(PagePrinters *page, const PrinterPickerEvent &evt) {
			...
			for (auto &pair: bundles) {
				...
				for (auto &preset: pair.second.preset_bundle->printers) {
					if (preset.config.opt_string("printer_model") == evt.model_id && preset.config.opt_string("printer_variant") == evt.variant_name) {
    ...
    */
			...
			if (!models_without_default.empty()) {
				...
				printer_names += "\n\n";
				std::string message = (technology & T_FFF ? GUI::format(_L("Following printer profiles has no default filament: %1%Please select one manually."), printer_names) : GUI::format(_L("Following printer profiles has no default material: %1%Please select one manually."), printer_names));
     ...
     */
		...
		bool ConfigWizard::priv::check_and_install_missing_materials(Technology technology, const std::string &only_for_model_id) {
				...
				for (const auto &pair: bundles) {
					...
					for (const auto &printer: pair.second.preset_bundle->printers) {
						if (printer.is_visible && printer.printer_technology() == technology) {
							...
							if ((only_for_model_id.empty() || only_for_model_id == printer_model->id) && printer_models_without_material.find(printer_model) == printer_models_without_material.end()) {
			...
			if (any_sla_selected && (technology & T_SLA)) {
				...
				if (!printer_models_without_material.empty()) {
					if (only_for_model_id.empty())
						ask_and_select_default_materials(_L("The following SLA printer models have no materials selected:") + "\n\n" + printer_model_list(printer_models_without_material) + "\n\n" + _L("Do you want to select default SLA materials for these printer models?"), printer_models_without_material, T_SLA);
					else
    ...
    if (DownloaderUtils::Worker::perform_registration_linux)
        DesktopIntegrationDialog::perform_downloader_desktop_integration();
			...
			if (!check_unsaved_preset_changes) {
				if ((check_unsaved_preset_changes = !first_added_filament.empty() || !first_added_sla_material.empty())) {
					header = !first_added_filament.empty() ? _L("A new filament was installed and it will be activated.") : _L("A new SLA material was installed and it will be activated.");
					if (!wxGetApp().check_and_keep_current_preset_changes(caption, header, act_btns, &apply_keeped_changes))
						return false;
				} else {
			...
			if (check_unsaved_preset_changes)
				preset_bundle->load_presets(*app_config, ForwardCompatibilitySubstitutionRule::EnableSilentDisableSystem, {preferred_model, preferred_variant, first_added_filament, first_added_sla_material});

			if (!only_sla_mode && page_custom->custom_wanted() && page_custom->is_valid_profile_name()) {
				// if unsaved changes was not cheched till this moment
				if (!check_unsaved_preset_changes && !wxGetApp().check_and_keep_current_preset_changes(caption, _L("Custom printer was installed and it will be activated."), act_btns, &apply_keeped_changes))
					return false;
		...
		void ConfigWizard::priv::load_pages_from_archive() {
			...
			if (!only_sla_mode && !page_filaments)
				add_page(page_filaments = new PageMaterials(q, &filaments, _L("Filament Profiles Selection"), _L("Filaments"), _L("Type:")));
			if (!page_sla_materials)
				add_page(page_sla_materials = new PageMaterials(q, &sla_materials, _L("SLA Material Profiles Selection") + " ", _L("SLA Materials"), _L("Type:")));
		...
		ConfigWizard::ConfigWizard(wxWindow *parent) : DPIDialog(parent, wxID_ANY, wxString(SLIC3R_APP_NAME) + " - " + _(name()), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER), p(new priv(this)) {
#ifdef __APPLE__
			...
			p->load_vendors();
			p->custom_config.reset(DynamicPrintConfig::new_from_defaults_keys({"gcode_flavor", "bed_shape", "bed_custom_texture", "bed_custom_model", "nozzle_diameter", "filament_diameter", "temperature", "bed_temperature",}));
    ...
    */
    ...
    static const wxString config_wizard_name_menu = L("Configuration &Assistant");
		...
		void ConfigWizard::on_dpi_changed(const wxRect &suggested_rect) {
			...
			msw_buttons_rescale(this, em, {wxID_APPLY, wxID_CANCEL, p->btn_sel_all->GetId(), p->btn_next->GetId(), p->btn_prev->GetId()});

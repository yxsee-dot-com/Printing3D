...
static const std::vector<int> _3DCONNEXION_VENDORS = {0x046d,  // LOGITECH = 1133 // Logitech (3Dconnexion is made by Logitech)
...
static const std::vector<int> _3DCONNEXION_DEVICES = {0xc603,    /* 50691 spacemouse plus XT */
                                                      0xc605,    /* 50693 cadman */
	...
	namespace GUI {
		...
		void Mouse3DController::save_config(AppConfig &appconfig) const {
			...
			for (const auto &key_value_pair: m_params_by_device) {
				...
				appconfig.set_mouse_device(device_name, params.translation.scale / Params::DefaultTranslationScale, params.translation.deadzone, params.rotation.scale / Params::DefaultRotationScale, params.rotation.deadzone, params.zoom.scale / Params::DefaultZoomScale, params.swap_yz);
			}
		}
		...
		bool Mouse3DController::connect_device() {
			...
			struct DeviceData {
				...
				DeviceData(const std::string &path, unsigned short usage_page, unsigned short usage) : path(path), usage_page(usage_page), usage(usage) {}
				...
				if (device.second.size() == 1) {
					...
					for (const DeviceData &data: device.second) {
						...
						if (data.has_valid_usage()) {
							...
							BOOST_LOG_TRIVIAL(error) << "3DConnexion device cannot be opened: " << data.path << " You may need to update /etc/udev/rules.d";
#if ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
		...
		bool Mouse3DController::handle_packet_translation(const DataPacketRaw &packet, const Params &params, State &state_in_out) {
			...
			Vec3d translation(-convert_input(packet[1], packet[2], deadzone), convert_input(packet[3], packet[4], deadzone), convert_input(packet[5], packet[6], deadzone));
		...
		bool Mouse3DController::handle_packet_rotation(const DataPacketRaw &packet, unsigned int first_byte, const Params &params, State &state_in_out) {
			...
			Vec3f rotation((float) convert_input(packet[first_byte + 0], packet[first_byte + 1], deadzone), (float) convert_input(packet[first_byte + 2], packet[first_byte + 3], deadzone), (float) convert_input(packet[first_byte + 4], packet[first_byte + 5], deadzone));

...
#include "libslic3r/src/BlacklistedLibraryCheck.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		class SendSystemInfoProgressDialog : public wxDialog {
			public:
				SendSystemInfoProgressDialog(wxWindow *parent, const wxString &message) : wxDialog(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxCAPTION) {
		...
		class ShowJsonDialog : public wxDialog {
			public:
				ShowJsonDialog(wxWindow *parent, const wxString &json, const wxSize &size) : wxDialog(parent, wxID_ANY, _L("Data to send"), wxDefaultPosition, size, wxCAPTION | wxRESIZE_BORDER) {
		...
		static bool should_dialog_be_shown() {
			...
			bool new_version = ((semver_current.maj() > semver_last_sent.maj()) || (semver_current.maj() == semver_last_sent.maj() && semver_current.min() > semver_last_sent.min()));
		...
		// Following function saves current Printing3D version into app config.
		...
		static std::string generate_system_info_json() {
			...
			pt::ptree data_node;
			data_node.put("Printing3DVersion", SLIC3R_VERSION);
		...
		SendSystemInfoDialog::SendSystemInfoDialog(wxWindow *parent) : m_system_info_json{generate_system_info_json()}, GUI::DPIDialog(parent, wxID_ANY, _L("Send system info"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE) {
			...
			std::string app_name;
			{
				...
				app_name = std::string(SLIC3R_APP_NAME) + " " + std::to_string(semver.maj()) + "." + std::to_string(semver.min()) + " " + (is_alpha ? "Alpha" : is_beta ? "Beta" : "");
			}
			...
			wxString text2 = GUI::format_wxstr(_L("We do not send any personal information nor anything that would allow us "
			                                      ...
			                                      "installation are sent. Printing3D is open source, if you want to "
			                                      "inspect the code actually performing the communication, see %1%."), std::string("<i>") + filename + "</i>");
			...
			wxString html = GUI::format_wxstr("<html><body bgcolor=%1%><font color=%2%>"
			                                  "<table><tr><td>"
			                                  "<img src = \"" + resources_dir() + "/icons/Printing3D_192px.png\" />"
			                                                                      "</td><td align=\"left\">" + text0 + "<br / ><br / >" + text1 + "<br /><br />"
			                                                                                                                                      "</td></tr></table>" + "<b>" + label2 + "</b><br />" + text2 + "</font></body></html>", bgr_clr_str, text_clr_str);
		...
		bool SendSystemInfoDialog::send_info(wxString &message) {
			...
			auto send = [&job_done, &result](const std::string &data) {
				...
				http.header("Content-Type", "application/json").timeout_max(6) // seconds
						.set_post_body(data).on_complete([&result](std::string body, unsigned status) {

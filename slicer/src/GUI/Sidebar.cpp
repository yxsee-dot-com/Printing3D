...
namespace Slic3r {
	namespace GUI {
		...
		ObjectInfo::ObjectInfo(wxWindow *parent) : wxStaticBoxSizer(new wxStaticBox(parent, wxID_ANY, _L("Info")), wxVERTICAL) {
		...
		enum SlicedInfoIdx {
			siFilament_g, siFilament_m, siFilament_mm3, siMaterial_unit, siCost, siEstimatedTime, siWTNumberOfToolchanges,
		...
		static void show_rich_tip(const wxString &tooltip, wxButton *btn) {
			...
			// (see https://github.com/prusa3d/Printing3D/issues/11988)
		...
		Sidebar::Sidebar(Plater *parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), m_plater(parent) {
			...
			auto init_combo = [this, margin_5](PlaterPresetComboBox **combo, wxString label, Preset::Type preset_type, bool filament) {
				...
				*combo = new PlaterPresetComboBox(m_presets_panel, preset_type);
				...
				if ((*combo)->edit_btn)
					combo_and_btn_sizer->Add((*combo)->edit_btn, 0, wxALIGN_CENTER_VERTICAL | wxLEFT | wxRIGHT, int(0.3 * wxGetApp().em_unit()));
			...
			is_msw ? scrolled_sizer->Add(m_presets_panel, 0, wxEXPAND | size_margin, margin_5) : scrolled_sizer->Add(m_presets_sizer, 0, wxEXPAND | size_margin, margin_5);
			...
			auto init_btn = [this](wxButton **btn, wxString label, const int button_height) {
				*btn = new wxButton(this, wxID_ANY, label, wxDefaultPosition, wxSize(-1, button_height), wxBU_EXACTFIT);
				wxGetApp().SetWindowVariantForButton((*btn));
		...
		void Sidebar::update_presets(Preset::Type preset_type) {
			...
			switch (preset_type) {
				case Preset::TYPE_FILAMENT: {
					const size_t extruder_cnt = print_tech != ptFFF ? 1 : dynamic_cast<ConfigOptionFloats *>(preset_bundle.printers.get_edited_preset().config.option("nozzle_diameter"))->values.size();
		...
		void Sidebar::on_select_preset(wxCommandEvent &evt) {
			...
			std::string preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias(preset_type, Preset::remove_suffix_modified(into_u8(combo->GetString(selection))), idx);
		...
		void Sidebar::show_info_sizer() {
			...
			if (m_mode < comExpert || objects.empty() || obj_idx < 0 || int(objects.size()) <= obj_idx || inst_idx < 0 || int(objects[obj_idx]->instances.size()) <= inst_idx || objects[obj_idx]->volumes.empty() ||                                            // hack to avoid crash when deleting the last object on the bed
			    (selection.is_single_full_object() && objects[obj_idx]->instances.size() > 1) || !(selection.is_single_full_instance() || selection.is_single_volume())) {
			...
			if (selection.is_single_volume()) {
				...
				if (vol_idxs.size() != 1)
					...
					// see https://github.com/prusa3d/Printing3D/issues/7408
					return;
			...
			m_object_info->info_facets->SetLabel(format_wxstr(_L_PLURAL("%1% (%2$d shell)", "%1% (%2$d shells)", stats.number_of_parts), static_cast<int>(model_object->facets_count()), stats.number_of_parts));
		...
		void Sidebar::update_sliced_info_sizer() {
			if (m_sliced_info->IsShown(size_t(0))) {
				if (m_plater->printer_technology() == ptSLA) {
					...
					wxString info_text = is_supports ? wxString::Format("%.2f \n%.2f \n%.2f", (ps.objects_used_material + ps.support_used_material) / 1000, ps.objects_used_material / 1000, ps.support_used_material / 1000) : wxString::Format("%.2f", (ps.objects_used_material + ps.support_used_material) / 1000);
					...
					if (cfg->option("bottle_cost")->getFloat() > 0.0 && cfg->option("bottle_volume")->getFloat() > 0.0) {
						double material_cost = cfg->option("bottle_cost")->getFloat() / cfg->option("bottle_volume")->getFloat();
					...
					wxString info_text = is_wipe_tower ? wxString::Format("%.2f \n%.2f \n%.2f", ps.total_used_filament / koef, (ps.total_used_filament - ps.total_wipe_tower_filament) / koef, ps.total_wipe_tower_filament / koef) : wxString::Format("%.2f", ps.total_used_filament / koef);
					...
					info_text = ps.total_cost == 0.0 ? "N/A" : is_wipe_tower ? wxString::Format("%.2f \n%.2f \n%.2f", ps.total_cost, (ps.total_cost - ps.total_wipe_tower_cost), ps.total_wipe_tower_cost) : wxString::Format("%.2f", ps.total_cost);
		...
		void Sidebar::set_btn_label(const ActionButtonType btn_type, const wxString &label) const {
			switch (btn_type) {
				case ActionButtonType::Reslice:
					m_btn_reslice->SetLabelText(label);
					break;
				case ActionButtonType::Export:
					m_btn_export_gcode->SetLabelText(label);
					break;

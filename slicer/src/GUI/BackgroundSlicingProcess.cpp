...
#include "libslic3r/src/GCode/PostProcessor.hpp"
#include "libslic3r/src/Format/SL1.hpp"
#include "libslic3r/src/Thread.hpp"
...
namespace Slic3r {
	...
	std::pair<std::string, bool> SlicingProcessCompletedEvent::format_error_message() const {
		...
		try {
			...
			error = GUI::format(_L("Printing3D has encountered a fatal error: \"%1%\""), ex.what()) + "\n\n" + _u8L("Please save your project and restart Printing3D. "
	...
	void BackgroundSlicingProcess::thread_proc() {
		...
		for (;;) {
			...
			if (m_print->cancel_status() != Print::CANCELED_INTERNAL) {
				...
				SlicingProcessCompletedEvent evt(m_event_finished_id, 0, (m_state == STATE_CANCELED) ? SlicingProcessCompletedEvent::Cancelled : exception ? SlicingProcessCompletedEvent::Error : SlicingProcessCompletedEvent::Finished, exception);
				wxQueueEvent(GUI::wxGetApp().mainframe->m_plater, evt.Clone());
	...
	static bool is_win32_seh_harware_exception(unsigned long ex) throw() {
		return ex == STATUS_ACCESS_VIOLATION || ex == STATUS_DATATYPE_MISALIGNMENT || ex == STATUS_FLOAT_DIVIDE_BY_ZERO || ex == STATUS_FLOAT_OVERFLOW || ex == STATUS_FLOAT_UNDERFLOW ||
		       #ifdef STATUS_FLOATING_RESEVERED_OPERAND
		       ex == STATUS_FLOATING_RESEVERED_OPERAND ||
		       #endif // STATUS_FLOATING_RESEVERED_OPERAND
		       ex == STATUS_ILLEGAL_INSTRUCTION || ex == STATUS_PRIVILEGED_INSTRUCTION || ex == STATUS_INTEGER_DIVIDE_BY_ZERO || ex == STATUS_INTEGER_OVERFLOW || ex == STATUS_STACK_OVERFLOW;
	}
	...
	Print::ApplyStatus BackgroundSlicingProcess::apply(const Model &model, const DynamicPrintConfig &config) {
		...
		if ((invalidated & PrintBase::APPLY_STATUS_INVALIDATED) != 0 && m_print->technology() == ptFFF && !m_fff_print->is_step_done(psGCodeExport)) {
	...
	void BackgroundSlicingProcess::prepare_upload() {
		...
		boost::filesystem::path source_path = boost::filesystem::temp_directory_path() / boost::filesystem::unique_path("." SLIC3R_APP_KEY ".upload.%%%%-%%%%-%%%%-%%%%");

...
namespace Slic3r {
	...
	class GLVolume {
		public:
			...
			enum EHoverState : unsigned char {
				HS_None, HS_Hover, HS_Select, HS_Deselect
			};
		...
		public:
			...
			struct CompositeID {
				...
				bool operator<(const CompositeID &rhs) const { return object_id < rhs.object_id || (object_id == rhs.object_id && (volume_id < rhs.volume_id || (volume_id == rhs.volume_id && instance_id < rhs.instance_id))); }
			};
			...
			size_t cpu_memory_used() const {
				return sizeof(*this) + this->model.cpu_memory_used() + this->print_zs.capacity() * sizeof(coordf_t) + this->offsets.capacity() * sizeof(size_t);
			}
	...
	class GLVolumeCollection {
		...
		public:
			...
			void load_object_auxiliary(const SLAPrintObject *print_object, int obj_idx,
			...
			void render(ERenderType type, bool disable_cullface, const Transform3d &view_matrix, const Transform3d &projection_matrix, std::function<bool(const GLVolume &)> filter_func = std::function < bool(const GLVolume &)
			>()) const;

		...
		class ArchiveRepository {
			public:
				struct RepositoryManifest {
					...
					RepositoryManifest(const std::string &id, const std::string &name, const std::string &url, const std::string &index_url = "", const std::string &description = "", const std::string &visibility = "", const boost::filesystem::path &tmp_path = "", const boost::filesystem::path &source_path = "") : id(id), name(name), url(url), index_url(index_url), description(description), visibility(visibility), tmp_path(tmp_path), source_path(source_path) {}
					RepositoryManifest(const RepositoryManifest &other) : id(other.id), name(other.name), url(other.url), index_url(other.index_url), description(other.description), visibility(other.visibility), tmp_path(other.tmp_path), source_path(other.source_path) {}
				};
		...
		class PresetArchiveDatabase {
			public:
				...
				const std::map<std::string, bool> &get_selected_repositories_uuid() const {
					assert(m_selected_repositories_uuid.size() == m_archive_repositories.size());
					return m_selected_repositories_uuid;
				}

...
#define IMG_PX_CNT  64
		...
		GalleryDialog::GalleryDialog(wxWindow *parent) : DPIDialog(parent, wxID_ANY, _L("Shape Gallery"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {
#ifndef _WIN32
			...
			m_list_ctrl = new wxListCtrl(this, wxID_ANY, wxDefaultPosition, wxSize(50 * wxGetApp().em_unit(), 35 * wxGetApp().em_unit()), wxLC_ICON | wxSIMPLE_BORDER);
			...
			auto add_btn = [this, buttons](size_t pos, int &ID, wxString title, wxString tooltip, void (GalleryDialog::* method)(wxEvent &), std::function<bool()> enable_fn = []() { return true; }) {
				ID = NewControlId();
		...
		static void generate_thumbnail_from_model(const std::string &filename) {
			if (!boost::algorithm::iends_with(filename, ".stl") && !boost::algorithm::iends_with(filename, ".obj")) {
		...
		void GalleryDialog::load_label_icon_list() {
			...
			auto add_files_from_gallery = [](std::vector<Item> &items, bool is_sys_dir, std::string &dir_path) {
				...
				for (auto &dir_entry: fs::directory_iterator(dir)) {
					TriangleMesh mesh;
					if ((is_gallery_file(dir_entry, ".stl") && mesh.ReadSTLFile(dir_entry.path().string().c_str())) || (is_gallery_file(dir_entry, ".obj") && load_obj(dir_entry.path().string().c_str(), &mesh)))
						sorted_names.push_back(dir_entry.path().filename().string());
				}

				// sort the filename case insensitive
				std::sort(sorted_names.begin(), sorted_names.end(), [](const std::string &a, const std::string &b) { return boost::algorithm::to_lower_copy(a) < boost::algorithm::to_lower_copy(b); });
			...
			for (const auto &item: list_items) {
				...
				wxImage image;
				if (!image.CanRead(from_u8(img_name)) || !image.LoadFile(from_u8(img_name), wxBITMAP_TYPE_PNG) || image.GetWidth() == 0 || image.GetHeight() == 0) {
					add_default_image(m_image_list, item.is_system, this);
					continue;
				}
		...
		void GalleryDialog::add_custom_shapes(wxEvent &event) {
			...
			wxFileDialog dialog(this, _L("Choose one or more files (STL, OBJ):"), from_u8(wxGetApp().app_config->get_last_dir()), "", file_wildcards(FT_GALLERY), wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
		...
		void GalleryDialog::change_thumbnail() {
			...
			wxFileDialog dialog(this, _L("Choose one PNG file:"), from_u8(wxGetApp().app_config->get_last_dir()), "", "PNG files (*.png)|*.png;*.PNG", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
			if (dialog.ShowModal() != wxID_OK)
				return;
					...
					if (!fs::exists(dest_dir / current.filename()))
						...
						for (auto &dir_entry: fs::directory_iterator(dest_dir))
							if (is_gallery_file(dir_entry, ".stl") || is_gallery_file(dir_entry, ".obj")) {
								...
								if (name.find(filename) != 0 || name[filename.size()] != ' ' || name[filename.size() + 1] != '(' || name[name.size() - 1] != ')')
									continue;

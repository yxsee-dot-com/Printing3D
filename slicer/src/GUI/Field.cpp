...
#include "libslic3r/src/enum_bitmask.hpp"
...
namespace Slic3r ::GUI {
	...
	void Field::get_value_by_opt_type(wxString &str, const bool check_value/* = true*/) {
		switch (m_opt.type) {
			...
			case coFloat: {
				...
				// see https://github.com/prusa3d/Printing3D/issues/6921
			...
			case coFloatOrPercent: {
				if (m_opt.type == coFloatOrPercent && m_opt.opt_key == "first_layer_height" && !str.IsEmpty() && str.Last() == '%') {
					...
					// see https://github.com/prusa3d/Printing3D/issues/7418
					wxString label = m_opt.full_label.empty() ? _(m_opt.label) : _(m_opt.full_label);
					show_error(m_parent, format_wxstr(_L("%s doesn't support percentage"), label));
				...
				if ((m_opt.type == coFloatOrPercent || m_opt.type == coFloatsOrPercents) && !str.IsEmpty() && str.Last() != '%') {
					...
					if (!str.ToDouble(&val)) {
						...
						show_error(m_parent, _(L("Invalid numeric input.")));
						set_value(double_to_string(val), true);
					} else if (((m_opt.sidetext.rfind("mm/s") != std::string::npos && val > m_opt.max) || (m_opt.sidetext.rfind("mm ") != std::string::npos && val > /*1*/m_opt.max_literal)) && (m_value.empty() || into_u8(str) != boost::any_cast<std::string>(m_value))) {
						if (!check_value) {
							m_value.clear();
							break;
						}
	...
	void TextCtrl::BUILD() {
		...
		switch (m_opt.type) {
			...
			case coFloats:
			case coFloat: {
				double val = m_opt.type == coFloats ? m_opt.get_default_value<ConfigOptionFloats>()->get_at(m_opt_idx) : m_opt.type == coFloat ? m_opt.default_value->getFloat() : m_opt.get_default_value<ConfigOptionPercents>()->get_at(m_opt_idx);
		...
		temp->SetFont(m_opt.is_code ? Slic3r::GUI::wxGetApp().code_font() : Slic3r::GUI::wxGetApp().normal_font());
	...
	void CheckBox::BUILD() {
		...
		bool check_value = m_opt.type == coBool ? m_opt.default_value->getBool() : m_opt.type == coBools ? m_opt.get_default_value<ConfigOptionBools>()->get_at(m_opt_idx) : false;
	...
	void SpinCtrl::BUILD() {
		...
		auto temp = new ::SpinInput(m_parent, text_value, "", wxDefaultPosition, size, wxTE_PROCESS_ENTER | wxSP_ARROW_KEYS, min_val, max_val, default_value);
	...
	void Choice::BUILD() {
		...
		choice_ctrl *temp;
		if (m_opt.gui_type != ConfigOptionDef::GUIType::undefined && m_opt.gui_type != ConfigOptionDef::GUIType::select_close) {
			m_is_editable = true;
	...
	void Choice::set_value(const boost::any &value, bool change_event) {
		...
		switch (m_opt.type) {
			...
			case coString:
			case coStrings: {
				wxString text_value = m_opt.type == coInt ? wxString::Format(_T("%i"), int(boost::any_cast<int>(value))) : boost::any_cast<wxString>(value);
				int sel_idx = -1;
				if (m_opt.enum_def) {
	...
	boost::any &Choice::get_value() {
		...
		if (m_opt.type == coEnum || m_opt.type == coEnums)
			...
			if (ret_enum < 0 || !m_opt.enum_def->has_values() || m_opt.type == coStrings || (into_u8(ret_str) != m_opt.enum_def->value(ret_enum) && ret_str != _(m_opt.enum_def->label(ret_enum))))
				// modifies ret_string!
				get_value_by_opt_type(ret_str);
	...
	boost::any &PointCtrl::get_value() {
		...
		} else if (m_opt.min > x || x > m_opt.max || m_opt.min > y || y > m_opt.max) {

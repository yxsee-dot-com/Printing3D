...
#include "libslic3r/src/I18N.hpp"
...
#include "libslic3r/src/Color.hpp"
#include "libslic3r/src/Format/SLAArchiveFormatRegistry.hpp"
...
#include "GUI/Preferences.hpp" // IWYU pragma: keep
#include "Tab.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		class SplashScreen : public wxSplashScreen {
			public:
				SplashScreen(const wxBitmap &bitmap, long splashStyle, int milliseconds, wxPoint pos = wxDefaultPosition) : wxSplashScreen(bitmap, splashStyle, milliseconds, static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, wxDefaultPosition, wxDefaultSize,
#ifdef __APPLE__
            ...
            wxYield();
				...
				void Decorate(wxBitmap &bmp) {
					...
					// this box will be 2/5 of the width of the bitmap, and be at the left.
					int width = lround(bmp.GetWidth() * 0.2);

					// load bitmap for logo
					BitmapCache bmp_cache;
					int logo_size = lround(width * 0.25);
					wxBitmap *logo_bmp_ptr = bmp_cache.load_png(wxGetApp().logo_name(), logo_size, logo_size);
			...
			private:
				...
				struct ConstantText {
					...
					void init(wxFont init_font) {
						...
						// credits infornation
						credits = "\n" + title + " " + _L("is based on Slic3r by Alessandro Ranellucci and Prusa Research and the RepRap community.") + "\n\n" + _L("Developed by https://www.yxsee.com") + "\n\n" + _L("Licensed under GNU AGPLv3.") + "\n\n\n\n\n\n\n";
    ...
    // (https://github.com/prusa3d/Printing3D/issues/2911)
    ...
    return res;
		...
		static const FileWildcards file_wildcards_by_type[FT_SIZE] = {
				...
				/* FT_3MF */
				                 {"3MF files"sv,        {".3mf"sv}},
				/* FT_GCODE */
				                 {"G-code files"sv,     {".gcode"sv, ".gco"sv,     ".bgcode"sv, ".bgc"sv, ".g"sv,       ".ngc"sv}},
				/* FT_MODEL */
				                 {"Known files"sv,      {".stl"sv,   ".obj"sv,     ".3mf"sv,    ".amf"sv, ".zip.amf"sv, ".xml"sv, ".step"sv, ".stp"sv, ".svg"sv}},
				/* FT_PROJECT */
				...
				/* FT_SL1 (deprecated, overriden by sla_wildcards) */
																																...
																																static void register_win32_dpi_event()
{
    ...
    wxWindow::MSWRegisterMessageHandler(WM_DPICHANGED_, [](wxWindow *win, WXUINT nMsg, WXWPARAM wParam, WXLPARAM lParam) {
		...
		static void generic_exception_handle() {
			...
			try {
				...
				wxString errmsg = _L("Printing3D has encountered a localization error. "
				                     "Please report to Printing3D team, what language was active and in which scenario "
        ...
        if (!m_print_config.empty())
            this->gui->mainframe->load_config(m_print_config);
				...
				if (!this->init_params->input_files.empty()) {
					...
					if (plater()->load_files(fns) && this->init_params->input_files.size() == 1) {
						...
						if (boost::algorithm::iends_with(filename, ".amf") || boost::algorithm::iends_with(filename, ".amf.xml") || boost::algorithm::iends_with(filename, ".3mf"))
			...
			// Set Printing3D version and save to Printing3D.ini or Printing3DGcodeViewer.ini.
		...
		GUI_App::GUI_App(EAppMode mode) : wxApp(), m_app_mode(mode), m_em_unit(10), m_imgui(new ImGuiWrapper()), m_removable_drive_manager(std::make_unique<RemovableDriveManager>()), m_other_instance_message_handler(std::make_unique<OtherInstanceMessageHandler>())
		//    , m_downloader(std::make_unique<Downloader>())
		{
			//app config initializes early becasuse it is used in instance checking in Printing3D.cpp
...
#else
			return m_opengl_mgr.init_glcontext(canvas, init_params != nullptr ? init_params->opengl_version : std::make_pair(0, 0), init_params != nullptr ? init_params->opengl_compatibiity_profile : false, init_params != nullptr ? init_params->opengl_debug : false);
		...
		void GUI_App::init_app_config() {
			...
			if (m_app_conf_exists) {
				...
				if (!error.empty()) {
					...
					if (is_editor()) {
						throw Slic3r::RuntimeError(format("Error parsing Printing3D config file, it is probably corrupted. "
		...
		void GUI_App::legacy_app_config_vendor_check() {
			...
			BOOST_LOG_TRIVIAL(warning) << "Printing3D has found legacy SLA printers. The printers will be "
		...
		std::string GUI_App::check_older_app_config(Semver current_version, bool backup) {
			...
			InfoDialog msg(nullptr, format_wxstr(_L("You are opening %1% version %2%."), SLIC3R_APP_NAME, SLIC3R_VERSION), backup ? format_wxstr(_L("The active configuration was created by <b>%1% %2%</b>,"
			                                                                                                                                        "\nwhile a newer configuration was found in <b>%3%</b>"
			                                                                                                                                        "\ncreated by <b>%1% %4%</b>."
			                                                                                                                                        "\n\nShall the newer configuration be imported?"
			                                                                                                                                        "\nIf so, your active configuration will be backed up before importing the new configuration."), SLIC3R_APP_NAME, current_version.to_string(), older_data_dir_path, last_semver.to_string()) : format_wxstr(_L("An existing configuration was found in <b>%3%</b>"
			                                                                                                                                                                                                                                                                                                                                                       "\ncreated by <b>%1% %2%</b>."
			                                                                                                                                                                                                                                                                                                                                                       "\n\nShall this configuration be imported?"), SLIC3R_APP_NAME, last_semver.to_string(), older_data_dir_path), true, wxYES_NO);
			...
			if (msg.ShowModal() == wxID_YES) {
				...
				if (backup) {
					...
					if (!GUI::Config::take_config_snapshot_cancel_on_error(*app_config, Config::Snapshot::SNAPSHOT_USER, "", _u8L("Continue and import newer configuration?"), &snapshot))
						return {};
					if (snapshot) {
    ...
    if (wxPlatformInfo::Get().GetArchName().substr(0, 2) == "64") {
        RichMessageDialog dlg(nullptr,
            _L("You are running a 32 bit build of Printing3D on 64-bit Windows."
                "\n32 bit build of Printing3D will likely not be able to utilize all the RAM available in the system."
                "\nPlease download and install a 64 bit build of Printing3D from https://www.prusa3d.cz/prusaslicer/."
                "\nDo you wish to continue?"),
            "Printing3D", wxICON_QUESTION | wxYES_NO);
        ...
        return false;
			...
			if (is_editor()) {
				...
				if (!msg.empty() && !ssl_accept) {
					RichMessageDialog dlg(nullptr, wxString::Format(_L("%s\nDo you want to continue?"), msg), "Printing3D", wxICON_QUESTION | wxYES_NO);
					dlg.ShowCheckBox(_L("Remember my choice"));
					if (dlg.ShowModal() != wxID_YES) return false;

					app_config->set("tls_cert_store_accepted", dlg.IsCheckBoxChecked() ? "yes" : "no");
					app_config->set("tls_accepted_cert_store_location", dlg.IsCheckBoxChecked() ? Http::tls_system_cert_store() : "");
				}
			}
			...
			if (app_config->get_bool("show_splash_screen")) {
				...
				wxBitmap bmp = SplashScreen::MakeBitmap(wxBitmap(from_u8(var(is_editor() ? "splashscreen.png" : "splashscreen-gcodepreview.png")), wxBITMAP_TYPE_PNG));
				...
				scrn = new SplashScreen(bmp.IsOk() ? bmp : get_bmp_bundle("Printing3D", 400)->GetPreferredBitmapSizeAtScale(1.0), wxSPLASH_CENTRE_ON_SCREEN | wxSPLASH_TIMEOUT, 4000, splashscreen_pos);
			...
			//                        , [](wxEvtHandler* evnthndlr) {wxGetApp().open_browser_with_warning_dialog("https://github.com/prusa3d/Printing3D/releases"); return true; }
			//                    );
			//                }
			//            }
			//            });
			...
			if (const std::string &crash_reason = app_config->get("restore_win_position");
				...
				InfoDialog dialog(nullptr, _L("Printing3D started after a crash"), format_wxstr(_L("Printing3D crashed last time when attempting to set window position.\n"
				                                                                                    "We are sorry for the inconvenience, it unfortunately happens with certain multiple-monitor setups.\n"
				                                                                                    "More precise reason for the crash: \"%1%\".\n"
				                                                                                    "For more information see our GitHub issue tracker: \"%2%\" and \"%3%\"\n\n"
				                                                                                    "To avoid this problem, consider disabling \"%4%\" in \"Preferences\". "
				                                                                                    "Otherwise, the application will most likely crash again next time."), "<b>" + from_u8(crash_reason) + "</b>", "<a href=http://github.com/prusa3d/Printing3D/issues/2939>#2939</a>", "<a href=http://github.com/prusa3d/Printing3D/issues/5573>#5573</a>", "<b>" + preferences_item + "</b>"), true, wxYES_NO);
																																...
																																static bool is_focused(HWND hWnd)
{
    ...
    // is wrong, which leads to https://github.com/prusa3d/Printing3D/issues/7603
    // and https://github.com/prusa3d/Printing3D/issues/3775. wxSYS_COLOUR_WINDOW
    ...
    if (wxPlatformInfo::Get().GetOSMajorVersion() == 10
		...
		std::vector<wxColour> GUI_App::get_mode_palette() {
			return {wxColor(m_mode_palette[0]), wxColor(m_mode_palette[1]), wxColor(m_mode_palette[2])};
		}
		...
		void GUI_App::check_printer_presets() {
			...
			for (const std::string &preset_name: preset_names)
				msg_text += "\n    \"" + from_u8(preset_name) + "\",";
			msg_text.RemoveLast();
			msg_text += "\n\n" + _L("But since this version of Printing3D we don't show this information in Printer Settings anymore.\n"
		...
		void GUI_App::load_project(wxWindow *parent, wxString &input_file) const {
			input_file.Clear();
			wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one file (3MF/AMF):"), app_config->get_last_dir(), "", file_wildcards(FT_PROJECT), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
		...
		void GUI_App::import_model(wxWindow *parent, wxArrayString &input_files) const {
			input_files.Clear();
			wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one or more files (STL/3MF/STEP/OBJ/AMF/SVG):"), from_u8(app_config->get_last_dir()), "", file_wildcards(FT_MODEL), wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
		...
		void GUI_App::import_zip(wxWindow *parent, wxString &input_file) const {
			wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose ZIP file") + ":", from_u8(app_config->get_last_dir()), "", file_wildcards(FT_ZIP), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
		...
		void GUI_App::load_gcode(wxWindow *parent, wxString &input_file) const {
			input_file.Clear();
			wxFileDialog dialog(parent ? parent : GetTopWindow(), _L("Choose one file (GCODE/GCO/G/BGCODE/BGC/NGC):"), app_config->get_last_dir(), "", file_wildcards(FT_GCODE), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
    ...
    return language;
		...
		bool GUI_App::load_language(wxString language, bool initial) {
			if (initial) {
				...
				// Get the active language from Printing3D.ini, or empty string if the key does not exist.
				language = app_config->get("translation_language");
				if (!language.empty())
					BOOST_LOG_TRIVIAL(trace) << boost::format("translation_language provided by Printing3D.ini: %1%") % language;
            ...
            // For example in our case it's trying to load "en_CZ" and as a result Printing3D catch warning message.
            ...
            temp_locale.Init(wxLANGUAGE_ENGLISH);
					...
					// Let the wxFileTranslationsLoader enumerate all translation dictionaries for Printing3D
			...
			if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
				BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by Printing3D: %1%") % language_info->CanonicalName.ToUTF8().data();
				language_info = nullptr;
			}

			if (language_info == nullptr) {
				// Printing3D does not support the Right to Left languages yet.
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
        ...
        BOOST_LOG_TRIVIAL(trace) << boost::format("Can't switch language to %1% (missing locales). Using %2% instead.")
			...
			if (!wxLocale::IsAvailable(language_info->Language)) {
				...
				wxString message = "Switching Printing3D to language " + language_info->CanonicalName + " failed.";
        ...
        message += "\nYou may need to reconfigure the missing locales, likely by running the \"locale-gen\" and \"dpkg-reconfigure locales\" commands.\n";
				...
				wxMessageBox(message, "Printing3D - Switching language failed", wxOK | wxICON_ERROR);
				if (initial)
		...
		ConfigOptionMode GUI_App::get_mode() {
			...
			return mode == "expert" ? comExpert : mode == "simple" ? comSimple : comAdvanced;
		}

		bool GUI_App::save_mode(const /*ConfigOptionMode*/int mode) {
			const std::string mode_str = mode == comExpert ? "expert" : mode == comSimple ? "simple" : "advanced";
			...
			if (mode == comSimple && !can_switch_to_simple(model())) {
				show_info(nullptr, _L("Simple mode supports manipulation with single-part object(s)\n"
				                      "or object(s) with support modifiers only.") + "\n\n" + _L("Please check your object list before mode changing."), _L("Change application mode"));
				return false;
			}
...
#else
			append_menu_item(local_menu, config_id_base + ConfigMenuPreferences, _L("&Preferences") + "\tCtrl+P", _L("Application preferences"), [](wxCommandEvent &) { wxGetApp().open_preferences(); }, "", nullptr, []() { return true; }, main_frame);
#endif
					...
					case ConfigMenuTakeSnapshot:
						...
						if (wxString action_name = _L("Taking a configuration snapshot");
							...
							if (dlg.ShowModal() == wxID_OK)
								if (const Config::Snapshot *snapshot = Config::take_config_snapshot_report_error(*app_config, Config::Snapshot::SNAPSHOT_USER, dlg.GetValue().ToUTF8().data());
										snapshot != nullptr)
					...
					case ConfigMenuSnapshots:
						if (check_and_save_current_preset_changes(_L("Loading a configuration snapshot"), "", false)) {
							...
							if (!dlg.snapshot_to_activate().empty()) {
								if (!Config::SnapshotDB::singleton().is_on_snapshot(*app_config) && !Config::take_config_snapshot_cancel_on_error(*app_config, Config::Snapshot::SNAPSHOT_BEFORE_ROLLBACK, "", GUI::format(_L("Continue to activate a configuration snapshot %1%?"), dlg.snapshot_to_activate())))
									break;
								try {
             ...
             */
						{
							...
							MessageDialog dialog(nullptr, _L("Switching the language will trigger application restart.\n"
							                                 "You will lose content of the plater.") + "\n\n" + _L("Do you want to proceed?"), title, wxICON_QUESTION | wxOK | wxCANCEL);
		...
		bool GUI_App::can_load_project() {
			...
			if (saved_project == wxID_CANCEL || (plater()->is_project_dirty() && saved_project == wxID_NO && !check_and_save_current_preset_changes(_L("Project is loading"), _L("Opening new project while some presets are unsaved."))))
				return false;
			return true;
		}
		...
		bool GUI_App::check_print_host_queue() {
			...
			MessageDialog dialog(mainframe, message, wxString(SLIC3R_APP_NAME) + " - " + _(L("Ongoing uploads")), wxICON_QUESTION | wxYES_NO | wxNO_DEFAULT);
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening Printing3D by drag & dropping a G-Code onto Printing3D icon in Finder,
...
void GUI_App::MacOpenURL(const wxString& url)
{
    ...
    if (boost::starts_with(narrow_url, "prusaslicer://open?file=")) {
		...
		wxString GUI_App::current_language_code_safe() const {
			...
			const std::map<wxString, wxString> mapping{{"cs", "cs_CZ",},
			                                           {"sk", "cs_CZ",},
			                                           {"de", "de_DE",},
			                                           {"es", "es_ES",},
			                                           {"fr", "fr_FR",},
    ...
    if (in_file.good()) {
		...
		namespace {
			bool open_dialog_hyperlink_checkbox(wxWindow *parent, AppConfig *app_config) {
				RichMessageDialog dialog(parent, _L("Open hyperlink in default browser?"), _L("Printing3D: Open hyperlink"), wxICON_QUESTION | wxYES_NO);
				...
				if (dialog.IsCheckBoxChecked()) {
					...
					MessageDialog msg_dlg(parent, msg, _L("Printing3D: Don't ask me again"), wxOK | wxCANCEL | wxICON_INFORMATION);
			...
			bool open_dialog_hyperlink(wxWindow *parent) {
				MessageDialog dialog(parent, _L("Open hyperlink in default browser?"), _L("Printing3D: Open hyperlink"), wxICON_QUESTION | wxYES_NO);
		...
		bool GUI_App::open_browser_with_warning_dialog(const wxString &url, wxWindow *parent/* = nullptr*/, bool force_remember_choice /*= true*/, int flags/* = 0*/) {
			enum class SupressHyperLinksOption {
				SHLO_UNCHECKED, SHLO_ALWAYS_SUPRESS, SHLO_ALWAYS_ALLOW
			};
			bool empty = app_config->get("suppress_hyperlinks").empty();
			bool checked = app_config->get_bool("suppress_hyperlinks");
			SupressHyperLinksOption opt_val = (empty ? SupressHyperLinksOption::SHLO_UNCHECKED : (checked ? SupressHyperLinksOption::SHLO_ALWAYS_SUPRESS : SupressHyperLinksOption::SHLO_ALWAYS_ALLOW));
			bool launch = true;
		...
		//        RichMessageDialog dialog(parent, _L("Open default browser with Prusa Account Log in page?\n(If you select 'Yes', you will not be asked again.)"), _L("Printing3D: Open Log in page"), wxICON_QUESTION | wxYES_NO);
		//         if (dialog.ShowModal() != wxID_YES)
		//             return false;
		...
		//{
		//    associate_file_type(L".3mf", L"Prusa.Slicer.1", L"Printing3D", true);
		//}
		//
		//void GUI_App::associate_stl_files()
		//{
		//    associate_file_type(L".stl", L"Prusa.Slicer.1", L"Printing3D", true);
		//}
		//
		//void GUI_App::associate_gcode_files()
		//{
		//    associate_file_type(L".gcode", L"Printing3D.GCodeViewer.1", L"Printing3DGCodeViewer", true);
		//}
		//
		//void GUI_App::associate_bgcode_files()
		//{
		//    associate_file_type(L".bgcode", L"Printing3D.GCodeViewer.1", L"Printing3DGCodeViewer", true);
		//}
		//#endif // __WXMSW__
					...
					for (const auto &opt: options) {
						...
						if (preset.config.option(opt.first)->is_scalar()) {
							...
							switch (preset.config.option(opt.first)->type()) {
								case coInts:
									opt_val = std::to_string(static_cast<const ConfigOptionInts *>(preset.config.option(opt.first))->values[0]);
									break;
								case coFloats:
									...
									break;
								case coStrings:
									opt_val = static_cast<const ConfigOptionStrings *>(preset.config.option(opt.first))->values[0];
									break;
								case coBools:
									opt_val = static_cast<const ConfigOptionBools *>(preset.config.option(opt.first))->values[0] ? "1" : "0";
									break;
								default:
									assert(true);
									continue;
		...
		void GUI_App::search_and_select_filaments(const std::string &material, bool avoid_abrasive, size_t extruder_index, std::string &out_message) {
			...
			// selected is ok
			if (!preset->is_default && preset->config.has("filament_type") && (!avoid_abrasive || preset->config.option<ConfigOptionBools>("filament_abrasive")->values[0] == false) && preset->config.option("filament_type")->serialize() == material) {
				return;
			}
			...
			for (const auto &filament: preset_bundle->extruders_filaments[extruder_index]) {
				if (filament.is_compatible && !filament.preset->is_default && filament.preset->is_visible && (!filament.preset->vendor || !filament.preset->vendor->templates_profile) && filament.preset->config.has("filament_type") && (!avoid_abrasive || filament.preset->config.option<ConfigOptionBools>("filament_abrasive")->values[0] == false) && filament.preset->config.option("filament_type")->serialize() == material && filament.preset->name.compare(0, 9, "Prusament") == 0 &&
			...
			for (const auto &filament: preset_bundle->extruders_filaments[extruder_index]) {
				if (filament.is_compatible && !filament.preset->is_default && filament.preset->is_visible && (!filament.preset->vendor || !filament.preset->vendor->templates_profile) && filament.preset->config.has("filament_type") && (!avoid_abrasive || filament.preset->config.option<ConfigOptionBools>("filament_abrasive")->values[0] == false) && filament.preset->config.option("filament_type")->serialize() == material && select_filament_preset(filament.preset, extruder_index)) {
					out_message += /*(extruder_count == 1)
                ? GUI::format(_L("Selected Filament:\n%1%"), filament_preset.preset->name)
                : */GUI::format(_L("Extruder %1%: Selected filament %2%"), extruder_index + 1, filament.preset->name) + "\n";
					return;
			...
			for (const auto &filament: preset_bundle->extruders_filaments[extruder_index]) {
				if (filament.is_compatible && !filament.preset->is_default && (!filament.preset->vendor || !filament.preset->vendor->templates_profile) && filament.preset->config.has("filament_type") && (!avoid_abrasive || filament.preset->config.option<ConfigOptionBools>("filament_abrasive")->values[0] == false) && filament.preset->config.option("filament_type")->serialize() == material && filament.preset->name.compare(0, 9, "Prusament") == 0 &&
				    select_filament_preset(filament.preset, extruder_index)) {
		...
		bool LogGui::ignorred_message(const wxString &msg) {
			for (const wxString &err: std::initializer_list<wxString>{wxString("cHRM chunk does not match sRGB"), wxString("known incorrect sRGB profile")}) {
				if (msg.Contains(err))
					return true;
			}
			return false;

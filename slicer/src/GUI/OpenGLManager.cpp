			...
			if (!m_gl_initialized) {
...
#if SLIC3R_OPENGL_ES
							_L("Printing3D requires OpenGL ES 3.0 capable graphics driver to run correctly, \n"
...
#else
							_L("Printing3D requires OpenGL 3.2 capable graphics driver to run correctly,\n"
							   "while OpenGL version %s, renderer %s, vendor %s was detected."), s_gl_info.get_version_string(), s_gl_info.get_renderer(), s_gl_info.get_vendor());
#endif // SLIC3R_OPENGL_ES
					message += "\n";
...
#ifdef _WIN32
					message += "\n";
					message += _L("As a workaround, you may run Printing3D with a software rendered 3D graphics by running prusa-slicer.exe with the --sw-renderer parameter.");
#endif
					wxMessageBox(message, wxString("Printing3D - ") + _L("Unsupported OpenGL version"), wxOK | wxICON_ERROR);
				}
				...
				if (valid_version) {
					...
					if (!result) {
						...
						wxMessageBox(message, wxString("Printing3D - ") + _L("Error loading shaders"), wxOK | wxICON_ERROR);
					}
#if !SLIC3R_OPENGL_ES
				...
				// texture of the bed (see: https://github.com/prusa3d/Printing3D/issues/8417).
				...
				if (boost::contains(gl_info.get_vendor(), "ATI Technologies Inc.") && (boost::contains(gl_info.get_renderer(), "Radeon") || boost::contains(gl_info.get_renderer(), "Custom")))
...
#else
		wxGLContext *OpenGLManager::init_glcontext(wxGLCanvas &canvas, const std::pair<int, int> &required_opengl_version, bool enable_compatibility_profile, bool enable_debug)
#endif // SLIC3R_OPENGL_ES
		{
			if (m_context == nullptr) {
#if SLIC3R_OPENGL_ES
				...
				const bool supports_core_profile = std::find(OpenGLVersions::core.begin(), OpenGLVersions::core.end(), std::make_pair(gl_major, gl_minor)) != OpenGLVersions::core.end();

...
namespace DoubleSlider {
	...
	enum ConflictType {
		ctNone, ctModeConflict, ctMeaninglessColorChange, ctMeaninglessToolChange, ctNotPossibleToolChange, ctRedundant
	};
	...
	struct ExtrudersSequence {
		...
		bool operator==(const ExtrudersSequence &other) const {
			return (other.is_mm_intervals == this->is_mm_intervals) && (other.interval_by_mm == this->interval_by_mm) && (other.interval_by_layers == this->interval_by_layers) && (other.random_sequence == this->random_sequence) && (other.color_repetition == this->color_repetition) && (other.extruders == this->extruders);
		}
		bool operator!=(const ExtrudersSequence &other) const {
			return (other.is_mm_intervals != this->is_mm_intervals) || (other.interval_by_mm != this->interval_by_mm) || (other.interval_by_layers != this->interval_by_layers) || (other.random_sequence != this->random_sequence) || (other.color_repetition != this->color_repetition) || (other.extruders != this->extruders);
		}
	...
	class TickCodeManager {
			...
			type)>                          m_cb_check_gcode_and_notify     { nullptr };
		...
		public:
			...
			void set_callback_on_empty_auto_color_change(std::function<void()> cb) { m_cb_notify_empty_color_change = cb; }

			void set_callback_on_check_gcode(std::function<void(Type)> cb) { m_cb_check_gcode_and_notify = cb; }

			void set_callback_on_get_custom_code(std::function<std::string(const std::string &, double)> cb) { m_cb_get_custom_code = cb; }

			void set_callback_on_get_pause_print_msg(std::function<std::string(const std::string &, double)> cb) { m_cb_get_pause_print_msg = cb; }

			void set_callback_on_get_new_color(std::function<std::string(const std::string &)> cb) { m_cb_get_new_color = cb; }

			void set_callback_on_show_info_msg(std::function<int(const std::string &, int)> cb) { m_cb_show_info_msg = cb; }

			void set_callback_on_show_warning_msg(std::function<int(const std::string &, int)> cb) { m_cb_show_warning_msg = cb; }

			void set_callback_on_get_extruders_cnt(std::function<int()> cb) { m_cb_get_extruders_cnt = cb; }

			void set_callback_on_get_extruders_sequence(std::function<bool(ExtrudersSequence &)> cb) { m_cb_get_extruders_sequence = cb; }
	};

...
namespace Slic3r {
	namespace GUI {
		...
		static GLenum get_primitive_mode(const GLModel::Geometry::Format &format) {
			switch (format.type) {
				case GLModel::Geometry::EPrimitiveType::Points: {
					return GL_POINTS;
				}
				default:
				...
				case GLModel::Geometry::EPrimitiveType::Lines: {
					return GL_LINES;
				}
				case GLModel::Geometry::EPrimitiveType::LineStrip: {
					return GL_LINE_STRIP;
				}
				case GLModel::Geometry::EPrimitiveType::LineLoop: {
					return GL_LINE_LOOP;
				}
			}
		}
		...
		bool contains(const BuildVolume &volume, const GLModel &model, bool ignore_bottom) {
			...
			switch (volume.type()) {
				...
				case BuildVolume::Type::Circle: {
					...
					return volume.max_print_height() == 0.0 ? all_vertices_inside(model.get_geometry(), [c, r2](const Vec3f &p) { return (to_2d(p) - c).squaredNorm() <= r2; }) :
				...
				case BuildVolume::Type::Custom:
					return volume.max_print_height() == 0.0 ? all_vertices_inside(model.get_geometry(), [&volume](const Vec3f &p) { return Geometry::inside_convex_polygon(volume.top_bottom_convex_hull_decomposition_bed(), to_2d(p).cast<double>()); }) : all_vertices_inside(model.get_geometry(), [&volume, z = volume.max_print_height() + epsilon](const Vec3f &p) { return Geometry::inside_convex_polygon(volume.top_bottom_convex_hull_decomposition_bed(), to_2d(p).cast<double>()) && p.z() <= z; });
				default:
					return true;
			}
		}

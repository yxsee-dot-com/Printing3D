...
#include "libslic3r/src/AABBTreeLines.hpp"
#include "libslic3r/src/ExPolygonsIndex.hpp"
#include "libslic3r/src/ClipperUtils.hpp"

#include "GUI/Selection.hpp"
#include "GUI/GLCanvas3D.hpp"
#include "GUI/GLModel.hpp"
...
namespace {
	...
	TextLines select_closest_contour(const std::vector<Polygons> &line_contours) {
		...
		for (const Polygons &polygons: line_contours) {
			...
			const Slic3r::Polygon &polygon = index.is_contour() ? expolygons[index.expolygons_index].contour : expolygons[index.expolygons_index].holes[index.hole_index()];
...
void TextLinesModel::init(const Transform3d &text_tr, const ModelVolumePtrs &volumes_to_slice,
	...
	m_lines = Slic3r::Emboss::create_text_lines(text_tr, volumes_to_slice, ff, fp, count_lines, &line_height_mm);
	if (m_lines.empty())
		return;
...
Slic3r::Emboss::TextLines Slic3r::Emboss::create_text_lines(const Transform3d &text_tr, const ModelVolumePtrs &volumes_to_slice, const FontFile &ff, const FontProp &fp, unsigned count_lines, double *line_height_mm_ptr) {

	...
	namespace GUI {
		...
		void RemovableDriveManager::eject_drive()
		{
...
#endif
			{
				...
				bool success = false;
				if (ec) {
					// The wait call can fail, as it did in https://github.com/prusa3d/Printing3D/issues/5507
		...
		void RemovableDriveManager::update() {
			...
			if (inside_update_lock.try_lock()) {
				...
				if (current_drives != m_current_drives) {
					...
					std::set_difference(current_drives.begin(), current_drives.end(), m_current_drives.begin(), m_current_drives.end(), std::inserter(new_drives, new_drives.begin()));
		...
		std::vector<DriveData>::const_iterator RemovableDriveManager::find_last_save_path_drive_data() const {
			return Slic3r::binary_find_by_predicate(m_current_drives.begin(), m_current_drives.end(), [this](const DriveData &data) { return data.path < m_last_save_path; }, [this](const DriveData &data) { return data.path == m_last_save_path; });
		}

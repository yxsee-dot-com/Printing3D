...
namespace Slic3r {
	namespace GUI {
		...
		FileGet::priv::priv(int ID, std::string &&url, const std::string &filename, wxEvtHandler *evt_handler, const boost::filesystem::path &dest_folder) : m_id(ID), m_url(std::move(url)), m_filename(filename), m_evt_handler(evt_handler), m_dest_folder(dest_folder) {
		}
		...
		void FileGet::priv::get_perform() {
			...
			Http::get(m_url).size_limit(DOWNLOAD_SIZE_LIMIT) //more?
					.set_range(range_string).on_progress([&](Http::Progress progress, bool &cancel) {

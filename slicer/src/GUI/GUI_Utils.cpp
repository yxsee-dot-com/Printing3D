	...
	namespace GUI {
...
#ifdef _WIN32
		template<class F>
		typename F::FN winapi_get_function(const wchar_t *dll, const char *fn_name) {
		...
		int get_dpi_for_window(const wxWindow *window) {
			...
			enum MONITOR_DPI_TYPE_ {
				...
				MDT_EFFECTIVE_DPI_ = 0, MDT_ANGULAR_DPI_ = 1, MDT_RAW_DPI_ = 2, MDT_DEFAULT_ = MDT_EFFECTIVE_DPI_,
			};
		...
		TaskTimer::~TaskTimer() {
			std::chrono::milliseconds stop_timer = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch());

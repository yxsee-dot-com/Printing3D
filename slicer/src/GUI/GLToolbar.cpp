...
///|/
#include "libslic3r/src/Point.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		GLToolbarItem::Data::Data() : name(""), icon_filename(""), tooltip(""), additional_tooltip(""), sprite_id(-1), visible(true), visibility_callback(Default_Visibility_Callback), enabling_callback(Default_Enabling_Callback) {
		}

		GLToolbarItem::GLToolbarItem(GLToolbarItem::EType type, const GLToolbarItem::Data &data) : m_type(type), m_state(Normal), m_data(data), m_last_action_type(Undefined), m_highlight_state(NotHighlighted) {
		}
		...
		void GLToolbarItem::render(const GLCanvas3D &parent, unsigned int tex_id, float left, float right, float bottom, float top, unsigned int tex_width, unsigned int tex_height, unsigned int icon_size) const {
			auto uvs = [this](unsigned int tex_width, unsigned int tex_height, unsigned int icon_size) -> GLTexture::Quad_UVs {
				...
				const char render_state = m_highlight_state == NotHighlighted ? (m_state == Pressed ? Normal :             // use regular icon, without black background
		...
		GLToolbar::Layout::Layout() : type(Horizontal), horizontal_orientation(HO_Center), vertical_orientation(VO_Center), top(0.0f), left(0.0f), border(5.0f), separator_size(5.0f), gap_size(6.0f), icons_size(Default_Icons_Size), scale(1.0f), width(0.0f), height(0.0f), dirty(true) {
		}

		GLToolbar::GLToolbar(GLToolbar::EType type, const std::string &name) : m_type(type), m_name(name), m_enabled(false), m_icons_texture_dirty(true), m_pressed_toggable_id(-1) {
		}
		...
		bool GLToolbar::on_mouse(wxMouseEvent &evt, GLCanvas3D &parent) {
			...
			if (item_id != -1) {
				...
				if (evt.LeftDown() || evt.LeftDClick()) {
					...
					processed = true;
					if (item_id != -2 && !m_items[item_id]->is_separator() && !m_items[item_id]->is_disabled() && (m_pressed_toggable_id == -1 || m_items[item_id]->get_last_action_type() == GLToolbarItem::Left)) {
					...
					m_mouse_capture.parent = &parent;
					processed = true;
					if (item_id != -2 && !m_items[item_id]->is_separator() && !m_items[item_id]->is_disabled() && (m_pressed_toggable_id == -1 || m_items[item_id]->get_last_action_type() == GLToolbarItem::Right)) {
						// mouse is inside an icon
		...
		void GLToolbar::update_hover_state_horizontal(const Vec2d &mouse_pos, GLCanvas3D &parent) {
			...
			for (GLToolbarItem *item: m_items) {
				...
				if (item->is_separator())
					...
					bool inside = (left <= (float) scaled_mouse_pos.x()) && ((float) scaled_mouse_pos.x() <= right) && (bottom <= (float) scaled_mouse_pos.y()) && ((float) scaled_mouse_pos.y() <= top);
		...
		void GLToolbar::update_hover_state_vertical(const Vec2d &mouse_pos, GLCanvas3D &parent) {
			...
			for (GLToolbarItem *item: m_items) {
				...
				if (item->is_separator())
					...
					GLToolbarItem::EState state = item->get_state();
					const bool inside = (left <= (float) scaled_mouse_pos.x()) && ((float) scaled_mouse_pos.x() <= right) && (bottom <= (float) scaled_mouse_pos.y()) && ((float) scaled_mouse_pos.y() <= top);
		...
		int GLToolbar::contains_mouse_horizontal(const Vec2d &mouse_pos, const GLCanvas3D &parent) const {
			...
			for (size_t id = 0; id < m_items.size(); ++id) {
				...
				if (item->is_separator()) {
					...
					if (left <= (float) scaled_mouse_pos.x() && (float) scaled_mouse_pos.x() <= right && bottom <= (float) scaled_mouse_pos.y() && (float) scaled_mouse_pos.y() <= top)
						return id;
		...
		void GLToolbar::render_background(float left, float top, float right, float bottom, float border_w, float border_h) const {
			...
			if (tex_id != 0 && tex_width > 0.0f && tex_height > 0.0f) {
				...
				if (m_layout.horizontal_orientation == Layout::HO_Left || m_layout.vertical_orientation == Layout::VO_Top)
					GLTexture::render_sub_texture(tex_id, left, internal_left, internal_top, top, {{internal_left_uv,  internal_bottom_uv},
					                                                                               {internal_right_uv, internal_bottom_uv},
					                                                                               {internal_right_uv, internal_top_uv},
					                                                                               {internal_left_uv,  internal_top_uv}});
				else
					GLTexture::render_sub_texture(tex_id, left, internal_left, internal_top, top, {{left_uv,          internal_top_uv},
					                                                                               {internal_left_uv, internal_top_uv},
				...
				if (m_layout.vertical_orientation == Layout::VO_Top)
					GLTexture::render_sub_texture(tex_id, internal_left, internal_right, internal_top, top, {{internal_left_uv,  internal_bottom_uv},
					                                                                                         {internal_right_uv, internal_bottom_uv},
					                                                                                         {internal_right_uv, internal_top_uv},
					                                                                                         {internal_left_uv,  internal_top_uv}});
				else
					GLTexture::render_sub_texture(tex_id, internal_left, internal_right, internal_top, top, {{internal_left_uv,  internal_top_uv},
					                                                                                         {internal_right_uv, internal_top_uv},
				...
				if (m_layout.horizontal_orientation == Layout::HO_Right || m_layout.vertical_orientation == Layout::VO_Top)
					GLTexture::render_sub_texture(tex_id, internal_right, right, internal_top, top, {{internal_left_uv,  internal_bottom_uv},
					                                                                                 {internal_right_uv, internal_bottom_uv},
					                                                                                 {internal_right_uv, internal_top_uv},
					                                                                                 {internal_left_uv,  internal_top_uv}});
				else
					GLTexture::render_sub_texture(tex_id, internal_right, right, internal_top, top, {{internal_right_uv, internal_top_uv},
				...
				if (m_layout.horizontal_orientation == Layout::HO_Left)
					GLTexture::render_sub_texture(tex_id, left, internal_left, internal_bottom, internal_top, {{internal_left_uv,  internal_bottom_uv},
					                                                                                           {internal_right_uv, internal_bottom_uv},
					                                                                                           {internal_right_uv, internal_top_uv},
					                                                                                           {internal_left_uv,  internal_top_uv}});
				else
					GLTexture::render_sub_texture(tex_id, left, internal_left, internal_bottom, internal_top, {{left_uv,          internal_bottom_uv},
					                                                                                           {internal_left_uv, internal_bottom_uv},
					                                                                                           {internal_left_uv, internal_top_uv},
					                                                                                           {left_uv,          internal_top_uv}});

				// center
				GLTexture::render_sub_texture(tex_id, internal_left, internal_right, internal_bottom, internal_top, {{internal_left_uv,  internal_bottom_uv},
				                                                                                                     {internal_right_uv, internal_bottom_uv},
				                                                                                                     {internal_right_uv, internal_top_uv},
				                                                                                                     {internal_left_uv,  internal_top_uv}});
				...
				if (m_layout.horizontal_orientation == Layout::HO_Right)
					GLTexture::render_sub_texture(tex_id, internal_right, right, internal_bottom, internal_top, {{internal_left_uv,  internal_bottom_uv},
					                                                                                             {internal_right_uv, internal_bottom_uv},
					                                                                                             {internal_right_uv, internal_top_uv},
					                                                                                             {internal_left_uv,  internal_top_uv}});
				else
				...
				if (m_layout.horizontal_orientation == Layout::HO_Left || m_layout.vertical_orientation == Layout::VO_Bottom)
					GLTexture::render_sub_texture(tex_id, left, internal_left, bottom, internal_bottom, {{internal_left_uv,  internal_bottom_uv},
					                                                                                     {internal_right_uv, internal_bottom_uv},
					                                                                                     {internal_right_uv, internal_top_uv},
					                                                                                     {internal_left_uv,  internal_top_uv}});
				else
					GLTexture::render_sub_texture(tex_id, left, internal_left, bottom, internal_bottom, {{left_uv,          bottom_uv},
				...
				if (m_layout.vertical_orientation == Layout::VO_Bottom)
					GLTexture::render_sub_texture(tex_id, internal_left, internal_right, bottom, internal_bottom, {{internal_left_uv,  internal_bottom_uv},
					                                                                                               {internal_right_uv, internal_bottom_uv},
					                                                                                               {internal_right_uv, internal_top_uv},
					                                                                                               {internal_left_uv,  internal_top_uv}});
				else
					GLTexture::render_sub_texture(tex_id, internal_left, internal_right, bottom, internal_bottom, {{internal_left_uv,  bottom_uv},
				...
				if (m_layout.horizontal_orientation == Layout::HO_Right || m_layout.vertical_orientation == Layout::VO_Bottom)
					GLTexture::render_sub_texture(tex_id, internal_right, right, bottom, internal_bottom, {{internal_left_uv,  internal_bottom_uv},
					                                                                                       {internal_right_uv, internal_bottom_uv},
					                                                                                       {internal_right_uv, internal_top_uv},
					                                                                                       {internal_left_uv,  internal_top_uv}});
				else
		...
		void GLToolbar::render_arrow(const GLCanvas3D &parent, GLToolbarItem *highlighted_item) {
			...
			if (tex_id != 0 && arr_tex_width > 0.0f && arr_tex_height > 0.0f) {
				...
				GLTexture::render_sub_texture(tex_id, left, right, bottom, top, {{left_uv,  top_uv},
				                                                                 {right_uv, top_uv},
				                                                                 {right_uv, bottom_uv},
				                                                                 {left_uv,  bottom_uv}});
			}
		...
		void GLToolbar::render_horizontal(const GLCanvas3D &parent) {
			...
			for (const GLToolbarItem *item: m_items) {
				...
				if (item->is_separator())
					...
					if (item->is_pressed())
						render_background(left - (id == m_items.size() ? border_w : margin_w), top + margin_h, left + icons_size_x + (id == 1 ? border_w : margin_w), top - icons_size_y - margin_h, border_w, border_h);

...
namespace DoubleSlider {
	...
	static bool behavior(ImGuiID id, const ImRect &region, const ImS32 v_min, const ImS32 v_max, ImS32 *out_value, ImRect *out_thumb, ImGuiSliderFlags flags = 0, bool change_on_mouse_move = false) {
	...
	static bool lclicked_on_thumb(ImGuiID id, const ImRect &region, const ImS32 v_min, const ImS32 v_max, const ImRect &thumb, ImGuiSliderFlags flags = 0) {
		ImGuiContext &context = *GImGui;

		if (context.ActiveId == id && context.ActiveIdSource == ImGuiInputSource_Mouse && context.IO.MouseReleased[0]) {
			const ImGuiAxis axis = (flags & ImGuiSliderFlags_Vertical) ? ImGuiAxis_Y : ImGuiAxis_X;
	...
	ImRect ImGuiControl::DrawOptions::groove(const ImVec2 &pos, const ImVec2 &size, bool is_horizontal) const {
		ImVec2 groove_start = is_horizontal ? ImVec2(pos.x + thumb_dummy_sz().x + text_dummy_sz().x, pos.y + size.y - groove_sz().y - dummy_sz().y) : ImVec2(pos.x + size.x - groove_sz().x - dummy_sz().x, pos.y + text_dummy_sz().y);
		ImVec2 groove_size = is_horizontal ? ImVec2(size.x - 2 * (thumb_dummy_sz().x + text_dummy_sz().x), groove_sz().y) : ImVec2(groove_sz().x, size.y - 2 * text_dummy_sz().y);
	...
	ImRect ImGuiControl::DrawOptions::draggable_region(const ImRect &groove, bool is_horizontal) const {
		ImRect draggable_region = is_horizontal ? ImRect(groove.Min.x, groove.GetCenter().y, groove.Max.x, groove.GetCenter().y) : ImRect(groove.GetCenter().x, groove.Min.y, groove.GetCenter().x, groove.Max.y);
		draggable_region.Expand(is_horizontal ? ImVec2(/*thumb_radius()*/0, draggable_region_sz().y) : ImVec2(draggable_region_sz().x, 0));
	...
	ImRect ImGuiControl::DrawOptions::slider_line(const ImRect &draggable_region, const ImVec2 &h_thumb_center, const ImVec2 &l_thumb_center, bool is_horizontal) const {
		...
		ImRect scroll_line = is_horizontal ? ImRect(ImVec2(l_thumb_center.x, mid.y - groove_sz().y / 2), ImVec2(h_thumb_center.x, mid.y + groove_sz().y / 2)) : ImRect(ImVec2(mid.x - groove_sz().x / 2, h_thumb_center.y), ImVec2(mid.x + groove_sz().x / 2, l_thumb_center.y));

		return scroll_line;
	}

	ImGuiControl::ImGuiControl(int lowerValue, int higherValue, int minValue, int maxValue, ImGuiSliderFlags flags, std::string name, bool use_lower_thumb) : m_selection(ssUndef), m_name(name), m_lower_pos(lowerValue), m_higher_pos(higherValue), m_min_pos(minValue), m_max_pos(maxValue), m_flags(flags), m_draw_lower_thumb(use_lower_thumb) {
	}

	int ImGuiControl::GetActivePos() const {
		return m_selection == ssLower ? m_lower_pos : m_selection == ssHigher ? m_higher_pos : -1;
	}
	...
	void ImGuiControl::draw_background(const ImRect &slideable_region) {
		...
		ImRect groove = is_horizontal() ? ImRect(slideable_region.Min.x, groove_center.y - groove_sz.y, slideable_region.Max.x, groove_center.y + groove_sz.y) : ImRect(groove_center.x - groove_sz.x, slideable_region.Min.y, groove_center.x + groove_sz.x, slideable_region.Max.y);
	...
	void ImGuiControl::draw_label(std::string label, const ImRect &thumb, bool is_mirrored /*= false*/, bool with_border /*= false*/) {
		...
		ImVec2 text_start = is_horizontal() ? ImVec2(thumb.Max.x + triangle_offset_x, thumb_center.y - text_size.y) : ImVec2(thumb.Min.x - text_size.x - triangle_offset_x, thumb_center.y - text_size.y);

		if (is_mirrored)
			text_start = is_horizontal() ? ImVec2(thumb.Min.x - text_size.x - triangle_offset_x, thumb_center.y - text_size.y) : ImVec2(thumb.Min.x - text_size.x - triangle_offset_x, thumb_center.y);
		...
		if (with_border) {
			...
			ImVec2 pos_1 = is_horizontal() ? ImVec2(text_rect_b.Min.x + rounding_b, text_rect_b.Max.y) : ImVec2(text_rect_b.Max.x - rounding_b, text_rect_b.Max.y);
			...
			if (is_mirrored) {
				pos_1 = is_horizontal() ? ImVec2(text_rect_b.Max.x - rounding_b - 1, text_rect_b.Max.y - 1) : ImVec2(text_rect_b.Max.x - rounding_b, text_rect_b.Min.y);
		...
		ImVec2 pos_1 = is_horizontal() ? ImVec2(text_rect.Min.x + rounding, text_rect.Max.y) : ImVec2(text_rect.Max.x - rounding, text_rect.Max.y);
		...
		if (is_mirrored) {
			pos_1 = is_horizontal() ? ImVec2(text_rect.Max.x - rounding - 1, text_rect.Max.y - 1) : ImVec2(text_rect.Max.x - rounding, text_rect.Min.y);
	...
	void ImGuiControl::apply_regions(int higher_pos, int lower_pos, const ImRect &draggable_region) {
		...
		m_regions.higher_slideable_region = is_horizontal() ? ImRect(draggable_region.Min + ImVec2(m_draw_lower_thumb ? thumb_radius : 0, 0), draggable_region.Max) : ImRect(draggable_region.Min, draggable_region.Max - ImVec2(0, m_combine_thumbs ? 0 : thumb_radius));
		m_regions.lower_slideable_region = is_horizontal() ? ImRect(draggable_region.Min, draggable_region.Max - ImVec2(thumb_radius, 0)) : ImRect(draggable_region.Min + ImVec2(0, thumb_radius), draggable_region.Max);

		// initialize the thumbs.
		float higher_thumb_pos = GetPositionInRect(higher_pos, m_regions.higher_slideable_region);
		m_regions.higher_thumb = is_horizontal() ? ImRect(higher_thumb_pos - thumb_radius, mid.y - thumb_radius, higher_thumb_pos + thumb_radius, mid.y + thumb_radius) : ImRect(mid.x - thumb_radius, higher_thumb_pos - thumb_radius, mid.x + thumb_radius, higher_thumb_pos + thumb_radius);

		float lower_thumb_pos = GetPositionInRect(lower_pos, m_regions.lower_slideable_region);
		m_regions.lower_thumb = is_horizontal() ? ImRect(lower_thumb_pos - thumb_radius, mid.y - thumb_radius, lower_thumb_pos + thumb_radius, mid.y + thumb_radius) : ImRect(mid.x - thumb_radius, lower_thumb_pos - thumb_radius, mid.x + thumb_radius, lower_thumb_pos + thumb_radius);
	}
	...
	bool ImGuiControl::draw_slider(int *higher_pos, int *lower_pos, std::string &higher_label, std::string &lower_label, const ImVec2 &pos, const ImVec2 &size, float scale) {
		ImGuiWindow *window = ImGui::GetCurrentWindow();
		if (window->SkipItems)
			return false;
		...
		if (m_draw_lower_thumb && !m_combine_thumbs && ImGui::ItemHoverable(m_regions.lower_thumb, id) && context.IO.MouseClicked[0])
			m_selection = ssLower;
		...
		if (!m_suppress_process_behavior) {
			if (m_selection == ssHigher) {
				pos_changed = behavior(id, m_regions.higher_slideable_region, m_min_pos, m_max_pos, higher_pos, &m_regions.higher_thumb, m_flags);
			} else if (m_draw_lower_thumb && !m_combine_thumbs) {
				pos_changed = behavior(id, m_regions.lower_slideable_region, m_min_pos, m_max_pos, lower_pos, &m_regions.lower_thumb, m_flags);
			}
		...
		if (!pos_changed && ImGui::ItemHoverable(move_size, id) && !ImGui::IsMouseDragging(0)) {
			...
			behavior(id, sl_region, m_min_pos, m_max_pos, &m_mouse_pos, &mouse_pos_rc, m_flags, true);
		...
		if ((!ImGui::ItemHoverable(active_thumb, id) && context.IO.MouseClicked[1]) || context.IO.MouseClicked[0])
		...
		if (m_draw_lower_thumb && !m_combine_thumbs) {
			...
			const bool mirror_label = is_horizontal() ? (higher_thumb_center.x - lower_thumb_center.x < text_size.x) : (lower_thumb_center.y - higher_thumb_center.y < text_size.y);

...
namespace Slic3r {
	namespace GUI {
		...
		SysInfoDialog::SysInfoDialog() : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, (wxGetApp().is_editor() ? wxString(SLIC3R_APP_NAME) : wxString(GCODEVIEWER_APP_NAME)) + " - " + _L("System Information"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {
			...
			m_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_NEVER);
			{
				...
				const auto text = wxString::Format("<html>"
				                                   ...
				                                   "</font>"
				                                   "</body>"
				                                   "</html>", bgr_clr_str, text_clr_str, text_clr_str, get_main_info(true));
				m_html->SetPage(text);
			...
			m_opengl_info_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_AUTO);
			{
				...
				if (!blacklisted_libraries.empty())
					blacklisted_libraries_message = wxString("<br><b>") + _L("Blacklisted libraries loaded into Printing3D process:") + "</b><br>" + blacklisted_libraries;
#endif // WIN32
				const auto text = GUI::format_wxstr("<html>"
				                                    ...
				                                    "</font>"
				                                    "</body>"
				                                    "</html>", bgr_clr_str, text_clr_str, text_clr_str, blacklisted_libraries_message, get_mem_info(true), wxGetApp().get_gl_info(false), "<b>" + _L("Eigen vectorization supported:") + "</b> " + Eigen::SimdInstructionSetsInUse());

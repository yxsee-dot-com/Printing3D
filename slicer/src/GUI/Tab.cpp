...
///|/
// #include "libslic3r/src/GCodeSender.hpp"
#include "GUI/BedShapeDialog.hpp"
...
#include "libslic3r/src/GCode/GCodeProcessor.hpp"
#include "libslic3r/src/GCode/GCodeWriter.hpp"
#include "libslic3r/src/GCode/Thumbnails.hpp"
...
namespace Slic3r {
	namespace GUI {
		...
		Tab::Tab(wxBookCtrlBase *parent, const wxString &title, Preset::Type type) : m_parent(parent), m_type(type), m_title(title) {
    ...
    main_sizer->Add(m_tmp_panel, 1, wxEXPAND | wxALL, 0);
			...
			// tree
			m_treectrl = new wxTreeCtrl(panel, wxID_ANY, wxDefaultPosition, wxSize(20 * m_em_unit, -1), wxTR_NO_BUTTONS | wxTR_HIDE_ROOT | wxTR_SINGLE | wxTR_NO_LINES | wxBORDER_SUNKEN | wxWANTS_CHARS);
        ...
        // see https://github.com/prusa3d/Printing3D/issues/5720
		...
		void Tab::add_scaled_button(wxWindow *parent, ScalableButton **btn, const std::string &icon_name, const wxString &label/* = wxEmptyString*/, long style /*= wxBU_EXACTFIT | wxNO_BORDER*/) {
    ...
    m_size_move *= -1;
			...
			if (m_presets_choice->IsShown())
				...
				// (see https://github.com/prusa3d/Printing3D/issues/10746)
		...
		void Tab::update_label_colours() {
			...
			while (cur_item) {
				...
				for (auto page: m_pages) {
					...
					const wxColor *clr = !page->m_is_nonsys_values ? &m_sys_label_clr : page->m_is_modified_values ? &m_modified_label_clr : &m_default_text_clr;
			...
			if (respect_vec_values) {
				switch (m_config->option(opt_key)->type()) {
					...
					case coFloatsOrPercents:
						add_correct_opts_to_options_list<ConfigOptionFloatsOrPercents>(opt_key, m_options_list, this, m_opt_status_value);
						break;
					case coEnums:
						add_correct_opts_to_options_list<ConfigOptionEnumsGeneric>(opt_key, m_options_list, this, m_opt_status_value);
						break;
					default:
		...
		void Tab::update_changed_tree_ui() {
			...
			while (cur_item) {
				...
				for (auto page: m_pages) {
					...
					const wxColor *clr = sys_page ? (m_is_default_preset ? &m_default_text_clr : &m_sys_label_clr) : modified_page ? &m_modified_label_clr : &m_default_text_clr;
		...
		static wxString support_combo_value_for_config(const DynamicPrintConfig &config, bool is_fff) {
			...
			return !config.opt_bool(support) ? _("None") : ((is_fff && !config.opt_bool("support_material_auto")) || (!is_fff && config.opt_bool("support_enforcers_only"))) ? _("For support enforcers only") : (config.opt_bool(buildplate_only) ? _("Support on build plate only") : _("Everywhere"));
		}
		...
		void Tab::on_value_change(const std::string &opt_key, const boost::any &value) {
			...
			if (is_fff ? (opt_key == "support_material" || opt_key == "support_material_auto" || opt_key == "support_material_buildplate_only") : (opt_key == "supports_enable" || opt_key == "support_tree_type" || opt_key == get_sla_suptree_prefix(*m_config) + "support_buildplate_only" || opt_key == "support_enforcers_only"))
			...
			if (m_postpone_update_ui) {
				...
				// see https://github.com/prusa3d/Printing3D/issues/7146
				return;
			}
		...
		void Tab::build_preset_description_line(ConfigOptionsGroup *optgroup) {
			...
			auto detach_preset_btn = [this](wxWindow *parent) {
				m_detach_preset_btn = new ScalableButton(parent, wxID_ANY, "lock_open_sys", _L("Detach from system preset"), wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
				...
				btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent &) {
					...
					wxString msg_text = system ? _(L("A copy of the current system preset will be created, which will be detached from the system preset.")) : _(L("The current custom preset will be detached from the parent system preset."));
					if (dirty) {
						msg_text += "\n\n";
		...
		void TabPrint::update_description_lines() {
			...
			if (m_active_page && m_active_page->title() == "Layers and perimeters" && m_recommended_thin_wall_thickness_description_line && m_top_bottom_shell_thickness_explanation) {
		...
		void TabPrint::update() {
			...
			// see https://github.com/prusa3d/Printing3D/issues/6814
		...
		std::vector<std::pair<std::string, std::vector<std::string>>> filament_overrides_option_keys{{"Travel lift",                      {"filament_retract_lift",              "filament_travel_ramping_lift", "filament_travel_max_lift", "filament_travel_slope",          "filament_travel_lift_before_obstacle", "filament_retract_lift_above",   "filament_retract_lift_below"}},
		                                                                                             {"Retraction",                       {"filament_retract_length",            "filament_retract_speed",       "filament_deretract_speed", "filament_retract_restart_extra", "filament_retract_before_travel",       "filament_retract_layer_change", "filament_wipe", "filament_retract_before_wipe",}},
		                                                                                             {"Retraction when tool is disabled", {"filament_retract_length_toolchange", "filament_retract_restart_extra_toolchange"}}};
		...
		std::optional<ConfigOptionsGroupShp> get_option_group(const Page *page, const std::string &title) {
			auto og_it = std::find_if(page->m_optgroups.begin(), page->m_optgroups.end(), [&](const ConfigOptionsGroupShp &og) {
		...
		void TabFilament::update_filament_overrides_page() {
			...
			const bool have_retract_length = (m_config->option("filament_retract_length")->is_nil() || m_config->opt_float("filament_retract_length", extruder_idx) > 0);

			const bool uses_ramping_lift = (m_config->option("filament_travel_ramping_lift")->is_nil() || m_config->opt_bool("filament_travel_ramping_lift", extruder_idx));

			const bool is_lifting = (m_config->option("filament_travel_max_lift")->is_nil() || m_config->opt_float("filament_travel_max_lift", extruder_idx) > 0 || m_config->option("filament_retract_lift")->is_nil() || m_config->opt_float("filament_retract_lift", extruder_idx) > 0);
			...
			for (const auto &[title, keys]: filament_overrides_option_keys) {
				...
				for (const std::string &opt_key: keys) {
					...
					if (title == "Travel lift" && uses_ramping_lift && opt_key == "filament_retract_lift" && !m_config->option("filament_travel_ramping_lift")->is_nil() && m_config->opt_bool("filament_travel_ramping_lift", extruder_idx)) {
						is_checked = false;
					}

					if (title == "Travel lift" && !is_lifting && (opt_key == "filament_retract_lift_above" || opt_key == "filament_retract_lift_below")) {
						is_checked = false;
					}

					if (title == "Travel lift" && !uses_ramping_lift && opt_key != "filament_travel_ramping_lift" && opt_key != "filament_retract_lift" && opt_key != "filament_retract_lift_above" && opt_key != "filament_retract_lift_below") {
						is_checked = false;
					}
		...
		void TabFilament::update_extruder_combobox() {
			const size_t extruder_cnt = m_preset_bundle->printers.get_selected_preset().printer_technology() == ptSLA ? m_extruders_cb->GetCount() : static_cast<const ConfigOptionFloats *>(m_preset_bundle->printers.get_edited_preset().config.option("nozzle_diameter"))->values.size();
		...
		void TabPrinter::build_print_host_upload_group(Page *page) {
			...
			wxString description_line_text = _L(""
			                                    ...
			                                    "into Printing3D/physical_printer directory.");

			Line line = {"", ""};
			line.full_width = 1;
			line.widget = [this, description_line_text](wxWindow *parent) {
				return description_line_widget(parent, m_presets->get_selected_preset().printer_technology() == ptFFF ? &m_fff_print_host_upload_description_line : &m_sla_print_host_upload_description_line, description_line_text);
			};
			optgroup->append_line(line);
		}
		...
		void TabPrinter::build_fff() {
			...
			m_sys_extruders_count = parent_preset == nullptr ? 0 : static_cast<const ConfigOptionFloats *>(parent_preset->config.option("nozzle_diameter"))->values.size();
		...
		void TabPrinter::extruders_count_changed(size_t extruders_count) {
			...
			} else if (m_extruders_count == 1 && m_preset_bundle->project_config.option<ConfigOptionFloats>("wiping_volumes_matrix")->values.size() > 1) {
     ...
     */
		...
		PageShp TabPrinter::build_kinematics_page() {
			...
			optgroup->on_change = [this](const t_config_option_key &opt_key, boost::any value) {
				if (opt_key == "machine_limits_usage" && static_cast<MachineLimitsUsage>(boost::any_cast<int>(value)) == MachineLimitsUsage::EmitToGCode && static_cast<GCodeFlavor>(m_config->option("gcode_flavor")->getInt()) == gcfKlipper) {
					DynamicPrintConfig new_conf = *m_config;
			...
			if (m_use_silent_mode) {
				std::vector<std::pair<std::string, std::string>> legend_columns = {{L("Normal"),  L("Values in this column are for Normal mode")},
		...
		const std::vector<std::string> extruder_options = {"min_layer_height", "max_layer_height", "extruder_offset", "retract_length", "retract_lift", "retract_lift_above", "retract_lift_below", "retract_speed", "deretract_speed", "retract_restart_extra", "retract_before_travel", "retract_layer_change", "wipe", "retract_before_wipe", "travel_ramping_lift", "travel_slope", "travel_max_lift", "travel_lift_before_obstacle", "nozzle_high_flow", "retract_length_toolchange",
		...
		void TabPrinter::build_extruder_pages(size_t n_before_extruders) {
			for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
				...
				auto reset_to_filament_color = [this, extruder_idx](wxWindow *parent) {
					ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "undo", _L("Reset to Filament Color"), wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
				...
				line.widget = [this, extruder_idx](wxWindow *parent) {
					ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "copy", _L("Apply below setting to other extruders"), wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
					auto sizer = new wxBoxSizer(wxHORIZONTAL);
					sizer->Add(btn);
...
#if 1
						dirty_options.erase(std::remove_if(dirty_options.begin(), dirty_options.end(), [](const std::string &opt) { return opt.find("extruder_colour") != std::string::npos || opt.find("nozzle_diameter") != std::string::npos; }), dirty_options.end());
 ...
 * */
     ...
     *  */
		...
		void TabPrinter::toggle_options() {
			...
			long val;
			if (m_active_page->title().StartsWith("Extruder ", &extruder_number) && extruder_number.ToLong(&val) && val > 0 && (size_t) val <= m_extruders_count) {
				size_t i = size_t(val - 1);
				bool have_retract_length = m_config->opt_float("retract_length", i) > 0;
				const bool ramping_lift = m_config->opt_bool("travel_ramping_lift", i);
				const bool lifts_z = (ramping_lift && m_config->opt_float("travel_max_lift", i) > 0) || (!ramping_lift && m_config->opt_float("retract_lift", i) > 0);
				...
				if (use_firmware_retraction && wipe) {
					...
					MessageDialog dialog(parent(), _(L("The Wipe option is not available when using the Firmware Retraction mode.\n"
					                                   "\nShall I disable it in order to enable Firmware Retraction?")), _(L("Firmware Retraction")), wxICON_WARNING | wxYES | wxNO);
			...
			if (m_active_page->title() == "Machine limits" && m_machine_limits_description_line) {
				assert(flavor == gcfMarlinLegacy || flavor == gcfMarlinFirmware || flavor == gcfRepRapFirmware || flavor == gcfKlipper);
...

			{
				...
				if (m_type == Slic3r::Preset::TYPE_PRINTER) {
					...
					if (printer_technology == ptFFF) {
						...
						static_cast<TabPrinter *>(this)->m_sys_extruders_count = parent_preset == nullptr ? 0 : static_cast<const ConfigOptionFloats *>(parent_preset->config.option("nozzle_diameter"))->values.size();
					}
				} else {
					on_presets_changed();
		...
		void Tab::update_btns_enabling() {
			...
			if (m_btn_edit_ph_printer)
				m_btn_edit_ph_printer->SetToolTip(m_preset_bundle->physical_printers.has_selection() ? _L("Edit physical printer") : _L("Add physical printer"));
		...
		bool Tab::select_preset(std::string preset_name, bool delete_current /*=false*/, const std::string &last_selected_ph_printer_name/* =""*/) {
			...
			if (current_dirty && !may_discard_current_dirty_preset(nullptr, preset_name)) {
				...
				bool new_preset_compatible = is_compatible_with_print(dependent.get_edited_preset_with_vendor_profile(), m_presets->get_preset_with_vendor_profile(*m_presets->find_preset(preset_name, true)), printer_profile);
				if (!canceled)
			...
			if (canceled) {
				if (m_type == Preset::TYPE_PRINTER) {
					if (!last_selected_ph_printer_name.empty() && m_presets->get_edited_preset().name == PhysicalPrinter::get_preset_name(last_selected_ph_printer_name)) {
				...
				auto update_compatible_type = [delete_current](bool technology_changed, bool on_page, bool show_incompatible_presets) {
					return (delete_current || technology_changed) ? PresetSelectCompatibleType::Always : on_page ? PresetSelectCompatibleType::Never : show_incompatible_presets ? PresetSelectCompatibleType::OnlyIfWasCompatible : PresetSelectCompatibleType::Always;
				};
				if (current_dirty || delete_current || print_tab || printer_tab)
					m_preset_bundle->update_compatible(update_compatible_type(technology_changed, print_tab, (print_tab ? this : wxGetApp().get_tab(Preset::TYPE_PRINT))->m_show_incompatible_presets), update_compatible_type(technology_changed, false, wxGetApp().get_tab(Preset::TYPE_FILAMENT)->m_show_incompatible_presets));
				// Initialize the UI from the current preset.
				if (printer_tab)
		...
		bool Tab::tree_sel_change_delayed() {
			// There is a bug related to Ubuntu overlay scrollbars, see https://github.com/prusa3d/Printing3D/issues/898 and https://github.com/prusa3d/Printing3D/issues/952.
     ...
     * but in full_config a filament_colors option aren't.*/
		...
		void Tab::rename_preset() {
			...
			if (m_type == Preset::TYPE_PRINTER && !m_preset_bundle->physical_printers.empty()) {
				...
				if (!ph_printers.empty()) {
					msg += _L_PLURAL("The physical printer below is based on the preset, you are going to rename.", "The physical printers below are based on the preset, you are going to rename.", ph_printers.size());
		...
		void Tab::delete_preset() {
			...
			if (m_presets_choice->is_selected_physical_printer()) {
				...
				if (m_type == Preset::TYPE_PRINTER && !physical_printers.empty()) {
					...
					if (!ph_printers.empty()) {
						msg += _L_PLURAL("The physical printer below is based on the preset, you are going to delete.", "The physical printers below are based on the preset, you are going to delete.", ph_printers.size());
						for (const std::string &printer: ph_printers)
							msg += "\n    \"" + from_u8(printer) + "\",";
						msg.RemoveLast();
					...
					if (!ph_printers_only.empty()) {
						msg += _L_PLURAL("The physical printer below is based only on the preset, you are going to delete.", "The physical printers below are based only on the preset, you are going to delete.", ph_printers_only.size());
		...
		void Tab::update_compatibility_ui() {
			m_btn_hide_incompatible_presets->SetBitmap(*get_bmp_bundle(m_show_incompatible_presets ? "flag_red" : "flag_green"));
			m_btn_hide_incompatible_presets->SetToolTip(m_show_incompatible_presets ? "Both compatible an incompatible presets are shown. Click to hide presets not compatible with the current printer." : "Only compatible presets are shown. Click to show both the presets compatible and not compatible with the current printer.");
		...
		wxSizer *Tab::compatible_widget_create(wxWindow *parent, PresetDependencies &deps) {
			...
			deps.btn = new ScalableButton(parent, wxID_ANY, "printer", format_wxstr(" %s %s", _L("Set"), dots), wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
			...
			deps.btn->Bind(wxEVT_BUTTON, ([this, parent, &deps](wxCommandEvent e) {
				...
				PresetCollection &depending_presets = (deps.type == Preset::TYPE_PRINTER) ? m_preset_bundle->printers : (printer_technology == ptFFF) ? m_preset_bundle->prints : m_preset_bundle->sla_prints;
		...
		void SubstitutionManager::validate_length() {
			if ((m_substitutions.size() % 4) != 0) {
				WarningDialog(m_parent, "Value of gcode_substitutions parameter will be cut to valid length", "Invalid length of gcode_substitutions parameter").ShowModal();
		...
		void SubstitutionManager::add_substitution(int substitution_id, const std::string &plain_pattern, const std::string &format, const std::string &params, const std::string &notes) {
		...
		wxSizer *TabPrint::create_manage_substitution_widget(wxWindow *parent) {
			...
			m_del_all_substitutions_btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent e) {
				if (MessageDialog(parent, _L("Are you sure you want to delete all substitutions?"), SLIC3R_APP_NAME, wxYES_NO | wxCANCEL | wxICON_QUESTION).ShowModal() != wxID_YES)
					return;
		...
		wxSizer *TabPrinter::create_bed_shape_widget(wxWindow *parent) {
			ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "printer", " " + _(L("Set")) + " " + dots, wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
			btn->SetFont(wxGetApp().normal_font());
			btn->SetSize(btn->GetBestSize());
			...
			btn->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) {
				...
				dlg.build_dialog(*m_config->option<ConfigOptionPoints>("bed_shape"), *m_config->option<ConfigOptionString>("bed_custom_texture"), *m_config->option<ConfigOptionString>("bed_custom_model"));
				if (dlg.ShowModal() == wxID_OK) {
    ...
    optgroup->append_single_option_line("area_fill");
		...
		std::vector<std::string> disable_tilt_options = {"tilt_down_initial_speed", "tilt_down_offset_steps", "tilt_down_offset_delay", "tilt_down_finish_speed", "tilt_down_cycles", "tilt_down_delay", "tilt_up_initial_speed", "tilt_up_offset_steps", "tilt_up_offset_delay", "tilt_up_finish_speed", "tilt_up_cycles", "tilt_up_delay"};
		...
		void TabSLAMaterial::update_sla_prusa_specific_visibility() {
			if (m_active_page && m_active_page->title() == "Material printing profile") {
				for (auto &title: {"", "Profile settings"}) {
					auto og_it = std::find_if(m_active_page->m_optgroups.begin(), m_active_page->m_optgroups.end(), [title](const ConfigOptionsGroupShp og) { return og->title == title; });
					if (og_it != m_active_page->m_optgroups.end())
						og_it->get()->Show(m_mode >= comAdvanced && is_prusa_printer());
				}

				auto og_it = std::find_if(m_active_page->m_optgroups.begin(), m_active_page->m_optgroups.end(), [](const ConfigOptionsGroupShp og) { return og->title == "Material printing profile"; });
				if (og_it != m_active_page->m_optgroups.end())
					og_it->get()->Show(m_mode >= comAdvanced && !is_prusa_printer());
		...
		static void add_options_into_line(ConfigOptionsGroupShp &optgroup, const std::vector<SamePair<std::string>> &prefixes, const std::string &optkey, const std::string &preprefix = std::string()) {
		...
		void TabSLAPrint::build_sla_support_params(const std::vector<SamePair<std::string>> &prefixes, const Slic3r::GUI::PageShp &page) {
		...
		void TabSLAMaterial::create_line_with_near_label_widget(ConfigOptionsGroupShp optgroup, const std::string &key) {
			if (optgroup->title == "Support head" || optgroup->title == "Support pillar")
				add_options_into_line(optgroup, {{"",          L("Default")},
				                                 {"branching", L("Branching")}}, key, "material_ow_");
			else {
		...
		std::vector<std::pair<std::string, std::vector<std::string>>> material_overrides_option_keys{{"Support head",         {"support_head_front_diameter", "support_head_penetration", "support_head_width"}},
		                                                                                             {"Support pillar",       {"support_pillar_diameter",}},
		                                                                                             {"Automatic generation", {"support_points_density_relative"}},
		                                                                                             {"Corrections",          {"absolute_correction",         "elefant_foot_compensation"}}};
		...
		void TabSLAPrint::build() {
			...
			build_sla_support_params({{"",          L("Default")},
			                          {"branching", L("Branching")}}, page);
			...
			if (m_active_page && m_active_page->title() == "Supports") {
				...
				if (m_support_object_elevation_description_line) {
					...
					if (is_visible) {
						...
						m_support_object_elevation_description_line->SetText(elev ? "" : format_wxstr(_L("\"%1%\" is disabled because \"%2%\" is on in \"%3%\" category.\n"
						                                                                                 "To enable \"%1%\", please switch off \"%2%\""), _L("Object elevation"), _L("Pad around object"), _L("Pad")));
					}
				}
			}
		}

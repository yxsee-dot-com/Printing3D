...
#endif
                              std::make_pair(clr_border_hovered, (int) StateColor::Hovered), std::make_pair(clr_border_normal, (int) StateColor::Normal));
#ifndef __WXMSW__
...
void StaticBox::doRender(wxDC &dc) {
	...
	if (background_color2.count() == 0) {
		...
		for (int y = 0; y < size.y; ++y) {
			...
			lr += dr;
			while (lr >= size.y) { ++r, lr -= size.y; }
			while (lr <= -size.y) { --r, lr += size.y; }
			lg += dg;
			while (lg >= size.y) { ++g, lg -= size.y; }
			while (lg <= -size.y) { --g, lg += size.y; }
			lb += db;
			while (lb >= size.y) { ++b, lb -= size.y; }
			while (lb <= -size.y) { --b, lb += size.y; }
		}
	}
}

...
BEGIN_EVENT_TABLE(DropDown, wxPopupTransientWindow)
				...
				EVT_LEFT_DOWN(DropDown::mouseDown) EVT_LEFT_UP(DropDown::mouseReleased) EVT_MOUSE_CAPTURE_LOST(DropDown::mouseCaptureLost) EVT_MOTION(DropDown::mouseMove) EVT_MOUSEWHEEL(DropDown::mouseWheelMoved)
...
DropDown::DropDown(std::vector<wxString> &texts, std::vector<wxBitmapBundle> &icons) : texts(texts), icons(icons), radius(Slic3r::GUI::wxGetApp().suppress_round_corners() ? 0 : 5), state_handler(this), text_color(0x363636), border_color(0xDBDBDB), selector_border_color(std::make_pair(0x00AE42, (int) StateColor::Hovered), std::make_pair(*wxWHITE, (int) StateColor::Normal)),
                                                                                       selector_background_color(std::make_pair(0xEDFAF2, (int) StateColor::Checked), std::make_pair(*wxWHITE, (int) StateColor::Normal)) {
}

DropDown::DropDown(wxWindow *parent, std::vector<wxString> &texts, std::vector<wxBitmapBundle> &icons, long style) : DropDown(texts, icons) {
	Create(parent, style);
}
...
void DropDown::render(wxDC &dc) {
	...
	if (has_bar) {
		...
		wxRect rect = {size.x - slider_width - 2, -offset.y * size.y / height + 2, slider_width, size.y * size.y / height - 3};
...
void DropDown::mouseDown(wxMouseEvent &event) {
	...
	const wxRect rect = {size.x - slider_width, -offset.y * size.y / height, slider_width - 2, size.y * size.y / height};

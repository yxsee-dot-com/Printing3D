	...
	namespace GUI {
		...
		// TRN Config wizard page headline.
			...
			if (logged) {
				...
				m_text = new wxStaticText(this, wxID_ANY, _L("Log in to control your printers remotely through the built-in Connect in Printing3D."));
		...
		bool ConfigWizardWebViewPage::login_changed() {
			...
			if (logged) {
				...
				// TRN Config wizard page with a log in web. first line of text.
				m_text->SetLabel(_L("Log in to control your printers remotely through the built-in Connect in Printing3D."));
			}
			return logged;
		}
...
case type: \
    ...
    break;
		...
		void ConfigWizardWebViewPage::on_navigation_request(wxWebViewEvent &evt) {
			...
			} else if (url.Find("accounts.google.com") != wxString::npos || url.Find("appleid.apple.com") != wxString::npos || url.Find("facebook.com") != wxString::npos) {

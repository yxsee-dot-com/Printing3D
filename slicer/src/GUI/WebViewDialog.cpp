...
#include "GUI/UserAccount.hpp"
#include "GUI/format.hpp"
#include "GUI/WebView.hpp"
#include "GUI/WebViewPlatformUtils.hpp"
#include "Utils/ServiceConfig.hpp"

#include "GUI/MsgDialog.hpp"
#include "GUI/Field.hpp"

#include <libslic3r/src/PresetBundle.hpp> // IWYU pragma: keep
...
namespace Slic3r {
	namespace GUI {
		...
		WebViewPanel::WebViewPanel(wxWindow *parent, const wxString &default_url, const std::vector<std::string> &message_handler_names, const std::string &loading_html/* = "loading"*/) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize), m_default_url(default_url), m_loading_html(loading_html), m_script_message_hadler_names(message_handler_names) {
...
void WebViewPanel::on_view_text_request(wxCommandEvent & WXUNUSED(evt)) {
	...
	wxTextCtrl *text = new wxTextCtrl(this, wxID_ANY, m_browser->GetPageText(), wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_RICH | wxTE_READONLY);
...
void WebViewPanel::on_run_script_custom(wxCommandEvent & WXUNUSED(evt)) {
	wxTextEntryDialog dialog(this, "Please enter JavaScript code to execute", wxGetTextFromUserPromptStr, m_javascript, wxOK | wxCANCEL | wxCENTRE | wxTE_MULTILINE);
...
void WebViewPanel::on_add_user_script(wxCommandEvent & WXUNUSED(evt)) {
	...
	wxTextEntryDialog dialog(this, "Enter the JavaScript code to run as the initialization script that runs before any script in the HTML document.", wxGetTextFromUserPromptStr, userScript, wxOK | wxCANCEL | wxCENTRE | wxTE_MULTILINE);
	if (dialog.ShowModal() != wxID_OK)
		return;
...
void WebViewPanel::on_set_custom_user_agent(wxCommandEvent & WXUNUSED(evt)) {
	...
	wxTextEntryDialog dialog(this, "Enter the custom user agent string you would like to use.", wxGetTextFromUserPromptStr, customUserAgent, wxOK | wxCANCEL | wxCENTRE);
	if (dialog.ShowModal() != wxID_OK)
		return;
...
SourceViewDialog::SourceViewDialog(wxWindow *parent, wxString source) : wxDialog(parent, wxID_ANY, "Source Code", wxDefaultPosition, wxSize(700, 500), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {
...
ConnectWebViewPanel::ConnectWebViewPanel(wxWindow *parent) : WebViewPanel(parent, GUI::from_u8(Utils::ServiceConfig::instance().connect_url()), {"_prusaSlicer"}, "connect_loading") {
...
PrinterWebViewPanel::PrinterWebViewPanel(wxWindow *parent, const wxString &default_url) : WebViewPanel(parent, default_url, {}) {
	if (!m_browser)
		return;
...
WebViewDialog::WebViewDialog(wxWindow *parent, const wxString &url, const wxString &dialog_name, const wxSize &size, const std::vector<std::string> &message_handler_names, const std::string &loading_html/* = "loading"*/) : DPIDialog(parent, wxID_ANY, dialog_name, wxDefaultPosition, size, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER), m_loading_html(loading_html), m_script_message_hadler_names(message_handler_names) {
	wxBoxSizer *topsizer = new wxBoxSizer(wxVERTICAL);
#ifdef DEBUG_URL_PANEL
	// Create the button
	bSizer_toolbar = new wxBoxSizer(wxHORIZONTAL);
...
PrinterPickWebViewDialog::PrinterPickWebViewDialog(wxWindow *parent, std::string &ret_val) : WebViewDialog(parent, GUI::from_u8(Utils::ServiceConfig::instance().connect_select_printer_url()), _L("Choose a printer"), wxSize(std::max(parent->GetClientSize().x / 2, 100 * wxGetApp().em_unit()), std::max(parent->GetClientSize().y / 2, 50 * wxGetApp().em_unit())), {"_prusaSlicer"}, "connect_loading"), m_ret_val(ret_val) {
	Centre();
}
...
void PrinterPickWebViewDialog::on_dpi_changed(const wxRect &suggested_rect) {
	...
	const wxSize &size = wxSize(std::max(parent->GetClientSize().x / 2, 100 * wxGetApp().em_unit()), std::max(parent->GetClientSize().y / 2, 50 * wxGetApp().em_unit()));
	SetMinSize(size);
	Fit();
	Refresh();
}

LoginWebViewDialog::LoginWebViewDialog(wxWindow *parent, std::string &ret_val, const wxString &url, wxEvtHandler *evt_handler) : WebViewDialog(parent, url, _L("Log in dialog"), wxSize(50 * wxGetApp().em_unit(), 80 * wxGetApp().em_unit()), {}), m_ret_val(ret_val), p_evt_handler(evt_handler) {
	Centre();
}
void LoginWebViewDialog::on_navigation_request(wxWebViewEvent &evt) {
	...
	if (url.starts_with(L"prusaslicer")) {
		...
		EndModal(wxID_OK);
	} else if (url.Find(L"accounts.google.com") != wxString::npos || url.Find(L"appleid.apple.com") != wxString::npos || url.Find(L"facebook.com") != wxString::npos) {

	...
	namespace GUI {
		...
		void Mouse3DController::init() {
			...
			if (load_driver_functions()) {
				...
				uint16_t error = has_new_driver ? SetConnexionHandlers(DeviceEvent, DeviceAdded, DeviceRemoved, false) : InstallConnexionHandlers(DeviceEvent, DeviceAdded, DeviceRemoved);
				...
				if (!error) {
					...
					clientID = RegisterConnexionClient(0, "\013PrusaSlicer", kConnexionClientModeTakeOver, kConnexionMaskAxis);

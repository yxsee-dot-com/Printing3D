///|/ Copyright (c) Prusa Research 2021 David Kocík @kocikdav
///|/
...
namespace Slic3r {
	namespace GUI {
		...
		class NotificationManager::HintNotification : public NotificationManager::PopNotification {
			public:
				HintNotification(const NotificationData &n, NotificationIDProvider &id_provider, wxEvtHandler *evt_handler, bool new_hint) : PopNotification(n, id_provider, evt_handler) {
			...
			protected:
				...
				virtual void render_minimize_button(const float win_pos_x, const float win_pos_y) override {}
				void render_preferences_button(const float win_pos_x, const float win_pos_y);
				void render_right_arrow_button(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
				void render_documentation_button(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
				void render_logo(const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);

...
#include "libslic3r/src/ModelArrange.hpp"
...
#include "libslic3r/src/ProfilesSharingUtils.hpp"
#include "libslic3r/src/Utils/DirectoriesUtils.hpp"

#include "Printing3D.hpp"
...
int CLI::run(int argc, char **argv) {
	...
	bool start_gui = m_actions.empty() &&
	                 ...
	                 std::find(m_transforms.begin(), m_transforms.end(), "cut") == m_transforms.end() && std::find(m_transforms.begin(), m_transforms.end(), "cut_x") == m_transforms.end() && std::find(m_transforms.begin(), m_transforms.end(), "cut_y") == m_transforms.end();
	...
	bool has_config_from_profiles = m_profiles_sharing.empty() && (!m_config.opt_string("print-profile").empty() || !m_config.option<ConfigOptionStrings>("material-profile")->values.empty() || !m_config.opt_string("printer-profile").empty());
	...
	if (it != m_actions.end()) {
		...
		if (semver.has_value() && (*semver) >= opengl_minimum) {
			...
			boost::nowide::cerr << "Required OpenGL version " << opengl_version_str << " is invalid. Must be greater than or equal to " << opengl_minimum.to_string() << "\n Option 'opengl-version' ignored." << std::endl;
		start_gui = true;
		m_actions.erase(it);
	}
	...
	for (const std::string& s : { "opengl-version", "opengl-compatibility", "opengl-debug", "opengl-aa", "gcodeviewer" }) {
		...
		if (it != m_actions.end()) {
			boost::nowide::cerr << "Parameter '" << s << "' is ignored, this Printing3D build is CLI only." << std::endl;
			m_actions.erase(it);
		}
	}
#endif // SLIC3R_GUI
	...
	for (auto const &opt_key: m_transforms) {
		if (opt_key == "merge") {
			...
			for (auto &model: m_models) {
				const bool all_objects_have_instances = std::none_of(model.objects.begin(), model.objects.end(), [](ModelObject *o) { return o->instances.empty(); });
			...
			for (auto &model: m_models) {
				...
				for (size_t i = 0; i < num_objects; ++i) {
					...
					Cut cut(model.objects.front(), 0, Geometry::translation_transform(m_config.opt_float("cut") * Vec3d::UnitZ()), ModelObjectCutAttribute::KeepLower | ModelObjectCutAttribute::KeepUpper | ModelObjectCutAttribute::PlaceOnCutUpper);
...
bool CLI::setup(int argc, char **argv) {
...
#ifdef WIN32
	// Notify user that a blacklisted DLL was injected into Printing3D process (for example Nahimic, see GH #5573).
	// We hope that if a DLL is being injected into a Printing3D process, it happens at the very start of the application,
	// thus we shall detect them now.
	if (BlacklistedLibraryCheck::get_instance().perform_check()) {
		std::wstring text = L"Following DLLs have been injected into the Printing3D process:\n\n";
		text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
		text += L"\n\n"
		        L"Printing3D is known to not run correctly with these DLLs injected. "
		        L"We suggest stopping or uninstalling these services if you experience "
		        L"crashes or unexpected behaviour while using Printing3D.\n"
		        L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes Printing3D "
		        L"to crash on a secondary monitor, see Printing3D github issue #5573";
	...
	for (const t_optiondef_map *options: {&cli_actions_config_def.options, &cli_transform_config_def.options, &cli_profiles_sharing_config_def.options, &cli_misc_config_def.options})
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const {
	boost::nowide::cout << SLIC3R_BUILD_ID << " " << "based on Slic3r"
	                    ...
	                    << " (without GUI support)"
	                    #endif /* SLIC3R_GUI */
	                    << std::endl << "https://github.com/prusa3d/Printing3D" << std::endl << std::endl << "Usage: prusa-slicer [ ACTIONS ] [ TRANSFORM ] [ OPTIONS ] [ file.stl ... ]" << std::endl << std::endl << "Actions:" << std::endl;
	...
	boost::nowide::cout << std::endl << "Print options are processed in the following order:" << std::endl << "\t1) Config keys from the command line, for example --fill-pattern=stars" << std::endl << "\t   (highest priority, overwrites everything below)" << std::endl << "\t2) Config files loaded with --load" << std::endl << "\t3) Config values loaded from amf or 3mf files" << std::endl;

	if (include_print_options) {
		boost::nowide::cout << std::endl;
		print_config_def.print_cli_help(boost::nowide::cout, true, [printer_technology](const ConfigOptionDef &def) { return printer_technology == ptAny || def.printer_technology == ptAny || printer_technology == def.printer_technology; });
	} else {
		boost::nowide::cout << std::endl << "Run --help-fff / --help-sla to see the full listing of print options." << std::endl;
	}
}
...
bool CLI::export_models(IO::ExportFormat format) {
	for (Model &model: m_models) {
		...
		switch (format) {
			case IO::AMF:
				success = Slic3r::store_amf(path.c_str(), &model, nullptr, false);
				break;
			case IO::OBJ:
				success = Slic3r::store_obj(path.c_str(), &model);
				break;
			case IO::STL:
				success = Slic3r::store_stl(path.c_str(), &model, true);
				break;
			case IO::TMF:
				success = Slic3r::store_3mf(path.c_str(), &model, nullptr, false);
				break;
			default:
				assert(false);
				break;
	...
	for (auto const &opt_key: m_profiles_sharing) {
		...
		if (opt_key == "query-printer-models") {
			...
			*/
			...
			if (ret.empty())
				boost::nowide::cerr << "Printer profile '" << m_config.opt_string("printer-profile") << "' wasn't found among installed printers." << std::endl << "Or the request can be wrong." << std::endl;
		} else {
			boost::nowide::cerr << "error: option not implemented yet: " << opt_key << std::endl;
			break;
		}
...
bool CLI::check_and_load_input_profiles(PrinterTechnology &printer_technology) {
	Slic3r::DynamicPrintConfig config = {};
	std::string ret = Slic3r::load_full_print_config(m_config.opt_string("print-profile"), m_config.option<ConfigOptionStrings>("material-profile")->values, m_config.opt_string("printer-profile"), config, printer_technology);
	if (!ret.empty()) {
...
extern "C" {
	// Enable printing stacktrace by default. It can be disabled by running Printing3D with "UBSAN_OPTIONS=print_stacktrace=0".

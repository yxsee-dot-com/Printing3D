...
#include "libslic3r/src/Time.hpp"
#include "libslic3r/src/Config.hpp"
#include "libslic3r/src/FileParserError.hpp"
	...
	namespace GUI {
		namespace Config {
			...
			void Snapshot::load_ini(const std::string &path) {
				...
				std::sort(this->vendor_configs.begin(), this->vendor_configs.begin(), [](const VendorConfig &cfg1, const VendorConfig &cfg2) { return cfg1.name < cfg2.name; });
			}
			...
			static void copy_config_dir_single_level(const boost::filesystem::path &path_src, const boost::filesystem::path &path_dst) {
				if (!boost::filesystem::is_directory(path_dst) && !boost::filesystem::create_directory(path_dst))
					throw Slic3r::RuntimeError(std::string("Printing3D was unable to create a directory at ") + path_dst.string());
			...
			SnapshotDB::const_iterator SnapshotDB::snapshot_with_vendor_preset(const std::string &vendor_name, const Semver &config_version) {
				...
				for (auto it = m_snapshots.begin(); it != m_snapshots.end(); ++it) {
					...
					auto it_vendor_config = std::lower_bound(snapshot.vendor_configs.begin(), snapshot.vendor_configs.end(), key, [](const Snapshot::VendorConfig &cfg1, const Snapshot::VendorConfig &cfg2) { return cfg1.name < cfg2.name; });
					if (it_vendor_config != snapshot.vendor_configs.end() && it_vendor_config->name == vendor_name && config_version == it_vendor_config->version.config_version) {
			...
			const Snapshot *take_config_snapshot_report_error(const AppConfig &app_config, Snapshot::Reason reason, const std::string &comment) {
				try {
					...
					show_error(static_cast<wxWindow *>(wxGetApp().mainframe), _L("Taking a configuration snapshot failed.") + "\n\n" + from_u8(err.what()));
					return nullptr;
				}
			}
			...
			bool take_config_snapshot_cancel_on_error(const AppConfig &app_config, Snapshot::Reason reason, const std::string &comment, const std::string &message, Snapshot const **psnapshot) {
				try {
					...
					RichMessageDialog dlg(static_cast<wxWindow *>(wxGetApp().mainframe), _L("Printing3D has encountered an error while taking a configuration snapshot.") + "\n\n" + from_u8(err.what()) + "\n\n" + from_u8(message), _L("Printing3D error"), wxYES_NO);

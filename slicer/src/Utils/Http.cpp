...
namespace Slic3r {
	...
	struct CurlGlobalInit {
		...
		CurlGlobalInit() {
			...
			if (!ssl_cafile || !fs::exists(fs::path(ssl_cafile))) {
				...
				if (!bundle)
					message = _u8L("Could not detect system SSL certificate store. "
								   "Printing3D will be unable to establish secure "
								   "network connections.");
				else
					message = Slic3r::GUI::format(
						_L("Printing3D detected system SSL certificate store in: %1%"),
						bundle);
			...
			if (CURLcode ec = ::curl_global_init(CURL_GLOBAL_DEFAULT)) {
				message += _u8L("CURL init has failed. Printing3D will be unable to establish "
	...
	struct Http::priv {
		enum {
			DEFAULT_TIMEOUT_CONNECT = 10, DEFAULT_TIMEOUT_MAX = 0, DEFAULT_SIZE_LIMIT = 5 * 1024 * 1024,
		};
	...
	Http::priv::priv(const std::string &url) : curl(::curl_easy_init()), form(nullptr), form_end(nullptr), headerlist(nullptr), error_buffer(CURL_ERROR_SIZE + 1, '\0'), limit(0), cancel(false) {
	...
	void Http::priv::form_add_file(const char *name, const fs::path &path, const char *filename) {
		...
		if (filename != nullptr) {
			::curl_formadd(&form, &form_end, CURLFORM_COPYNAME, name, CURLFORM_FILENAME, filename, CURLFORM_CONTENTTYPE, "application/octet-stream", CURLFORM_STREAM, static_cast<void *>(&stream), CURLFORM_CONTENTSLENGTH, static_cast<long>(size), CURLFORM_END);
		}
	}
	...
	std::string Http::priv::curl_error(CURLcode curlcode) {
		return (boost::format("%1%:\n%2%\n[Error %3%]") % ::curl_easy_strerror(curlcode) % error_buffer.c_str() % curlcode).str();
	}
	...
	bool is_transient_error(CURLcode res, long http_status) {
		...
		return res == CURLE_COULDNT_CONNECT || res == CURLE_COULDNT_RESOLVE_HOST || res == CURLE_OPERATION_TIMEDOUT;
	}
		...
		do {
			...
			if (retry) {
				...
				BOOST_LOG_TRIVIAL(error)
					<< "HTTP Transient error (code=" << res << ", http_status=" << http_status << "), retrying in " << delay.count() / 1000.0f << " s";
	...
	Http &Http::form_add(const std::string &name, const std::string &contents) {
		if (p) {
			::curl_formadd(&p->form, &p->form_end, CURLFORM_COPYNAME, name.c_str(), CURLFORM_COPYCONTENTS, contents.c_str(), CURLFORM_END);
		}
	...
	std::ostream &operator<<(std::ostream &os, const Http::Progress &progress) {
		os << "Http::Progress(" << "dltotal = " << progress.dltotal << ", dlnow = " << progress.dlnow << ", ultotal = " << progress.ultotal << ", ulnow = " << progress.ulnow << ")";
		return os;
	}

...
namespace Slic3r {
	...
	struct DnsResource {
		...
		DnsResource() : type(0), rclass(0), ttl(0) {}
	...
	std::ostream &operator<<(std::ostream &os, const DnsMessage &msg) {
		os << boost::format("DnsMessage(ID: %1%, Q: %2%, A: %3%, AAAA: %4%, services: [") % msg.header.id % (msg.question ? msg.question->name.c_str() : "none") % (msg.rr_a ? msg.rr_a->ip.to_string() : "none") % (msg.rr_aaaa ? msg.rr_aaaa->ip.to_string() : "none");
	...
	optional <BonjourRequest> BonjourRequest::make_PTR(const std::string &service, const std::string &protocol) {
		...
		// Add metadata
		static const unsigned char rq_meta[] = {0x00, 0x00, // Query ID (zero for mDNS)
		                                        0x00, 0x00, // Flags
		                                        0x00, 0x01, // One query
		...
		static const unsigned char ptr_tail[] = {0x05, // length of "label"
	...
	optional <BonjourRequest> BonjourRequest::make_A(const std::string &hostname) {
		...
		static const unsigned char ptr_tail[] = {0x05, // length of "local"
	...
	UdpSocket::UdpSocket(Bonjour::ReplyFn replyfn, const asio::ip::address &multicast_address, const asio::ip::address &interface_address, std::shared_ptr<boost::asio::io_service> io_service) : replyfn(replyfn), multicast_address(multicast_address), socket(*io_service), io_service(io_service) {
		try {
			// open socket
	...
	UdpSocket::UdpSocket(Bonjour::ReplyFn replyfn, const asio::ip::address &multicast_address, std::shared_ptr<boost::asio::io_service> io_service) : replyfn(replyfn), multicast_address(multicast_address), socket(*io_service), io_service(io_service) {
		try {
			// open socket
			boost::asio::ip::udp::endpoint listen_endpoint(multicast_address.is_v4() ? udp::v4() : udp::v6(), BonjourRequest::MCAST_PORT);
			socket.open(listen_endpoint.protocol());
	...
	void UdpSocket::async_receive() {
		try {
			...
			socket.async_receive_from(asio::buffer(session->buffer, session->buffer.size()), session->remote_endpoint, boost::bind(&UdpSocket::receive_handler, this, session, asio::placeholders::error, asio::placeholders::bytes_transferred));
		} catch (std::exception &e) {
			BOOST_LOG_TRIVIAL(error) << e.what();
		}
	}
	...
	void ResolveSession::handle_receive(const error_code &error, size_t bytes) {
		...
		if (dns_msg) {
			...
			if (!answer_name.empty()) {
				...
				std::transform(name_tolower.begin(), name_tolower.end(), name_tolower.begin(), [](unsigned char c) { return std::tolower(c); });
				std::string hostname_tolower = socket->get_hostname();
				std::transform(hostname_tolower.begin(), hostname_tolower.end(), hostname_tolower.begin(), [](unsigned char c) { return std::tolower(c); });
	...
	Bonjour::priv::priv(std::string &&service) : service(std::move(service)), protocol("tcp"), timeout(10), retries(1) {
		buffer.resize(DnsMessage::MAX_SIZE);
	}
	...
	BonjourReply::BonjourReply(boost::asio::ip::address ip, uint16_t port, std::string service_name, std::string hostname, BonjourReply::TxtData txt_data) : ip(std::move(ip)), port(port), service_name(std::move(service_name)), hostname(std::move(hostname)), txt_data(std::move(txt_data)) {
		std::string proto;
	...
	bool BonjourReply::operator==(const BonjourReply &other) const {
		return this->full_address == other.full_address && this->service_name == other.service_name;
	}
	...
	std::ostream &operator<<(std::ostream &os, const BonjourReply &reply) {
		os << boost::format("BonjourReply(%1%, %2%, %3%, %4%") % reply.ip.to_string() % reply.service_name % reply.hostname % reply.full_address;
	...
	Bonjour::Bonjour(std::string service) : p(new priv(std::move(service))) {}

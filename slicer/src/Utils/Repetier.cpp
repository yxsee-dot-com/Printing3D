...
namespace Slic3r {
	...
	Repetier::Repetier(DynamicPrintConfig *config) : host(config->opt_string("print_host")), apikey(config->opt_string("printhost_apikey")), cafile(config->opt_string("printhost_cafile")), port(config->opt_string("printhost_port")) {}
	...
	static bool validate_repetier(const boost::optional<std::string> &name, const boost::optional<std::string> &soft) {
		if (soft) {
			// See https://github.com/prusa3d/Printing3D/issues/7807:
	...
	wxString Repetier::get_test_failed_msg(wxString &msg) const {
		return GUI::format_wxstr("%s: %s\n\n%s", _L("Could not connect to Repetier"), msg, _L("Note: Repetier version at least 0.90.0 is required."));
	}
	...
	bool Repetier::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const {
		...
		auto url = upload_data.post_action == PrintHostPostUploadAction::StartPrint ? make_url((boost::format("printer/job/%1%") % port).str()) : make_url((boost::format("printer/model/%1%") % port).str());

		BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%, group: %7%") % name % upload_data.source_path % url % upload_filename.string() % upload_parent_path.string() % (upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false") % upload_data.group;
		...
		if (upload_data.post_action == PrintHostPostUploadAction::StartPrint) {
			...
			http.form_add("autostart", "true"); // See https://github.com/prusa3d/Printing3D/issues/7807#issuecomment-1235519371
		}

		http.form_add("a", "upload").form_add_file("filename", upload_data.source_path.string(), upload_filename.string()).on_complete([&](std::string body, unsigned status) {

...
namespace Slic3r {
	WifiScanner::WifiScanner() {
		...
		if (!wlanOpenHandleFunc || !wlanEnumInterfacesFunc || !wlanQueryInterfaceFunc || !wlanFreeMemoryFunc || !wlanGetProfileFunc || !wlanGetProfileListFunc || !wlanGetAvailableNetworkListFunc || !wlanCloseHandleFunc)
			return;
#endif // _WIN32
		m_init = true;
	}
	...
	void WifiScanner::fill_wifi_map(Slic3r::WifiSsidPskMap &wifi_map, std::string &connected_ssid) {
		...
		for (DWORD i = 0; i < interface_list->dwNumberOfItems; i++) {
			if (interface_list->InterfaceInfo[i].isState == wlan_interface_state_connected) {
				...
				if (wlanQueryInterfaceFunc(handle, &interface_list->InterfaceInfo[i].InterfaceGuid, wlan_intf_opcode_current_connection, NULL, &connectInfoSize, (PVOID *) &pConnectInfo, &opCode) == ERROR_SUCCESS && pConnectInfo && pConnectInfo->wlanAssociationAttributes.dot11Ssid.uSSIDLength) {
					connected_ssid = std::string((const char *) pConnectInfo->wlanAssociationAttributes.dot11Ssid.ucSSID, pConnectInfo->wlanAssociationAttributes.dot11Ssid.uSSIDLength);
			...
			int iRet = StringFromGUID2(interface_info_entry->InterfaceGuid, (LPOLESTR) &guid, sizeof(guid) / sizeof(*guid));

			if (wlanGetAvailableNetworkListFunc(handle, &interface_info_entry->InterfaceGuid, 0, NULL, &available_network_list) != ERROR_SUCCESS) {
				continue;
			}
			...
			for (unsigned int j = 0; j < available_network_list->dwNumberOfItems; j++) {
				...
				available_network_entry = (WLAN_AVAILABLE_NETWORK *) &available_network_list->Network[j];

				if (available_network_entry->dot11Ssid.uSSIDLength != 0)
					ssid = wxString(available_network_entry->dot11Ssid.ucSSID, available_network_entry->dot11Ssid.uSSIDLength);
				...
				if (wlanGetProfileListFunc(handle, &interface_list->InterfaceInfo[i].InterfaceGuid, NULL, &profile_list) != ERROR_SUCCESS) {
					continue;
				}
				...
				for (DWORD k = 0; k < profile_list->dwNumberOfItems; k++) {
					...
					if ((ret = wlanGetProfileFunc(handle, &interface_list->InterfaceInfo[i].InterfaceGuid, profile_list->ProfileInfo[k].strProfileName, NULL, &xmlstr, &flags, &access)) == ERROR_SUCCESS && xmlstr) {
						wxString xml(xmlstr);

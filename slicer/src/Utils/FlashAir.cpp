...
namespace Slic3r {
	...
	wxString FlashAir::get_test_failed_msg(wxString &msg) const {
		return GUI::format_wxstr("%s: %s\n%s", _u8L("Could not connect to FlashAir"), msg, _u8L("Note: FlashAir with firmware 2.00.02 or newer and activated upload function is required."));
	}
	...
	bool FlashAir::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const {
		...
		BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Uploading file %2% at %3% / %4%, filename: %5%") % name % upload_data.source_path % urlPrepare % urlUpload % upload_filename.string();
	...
	std::string FlashAir::timestamp_str() const {
		...
		unsigned long fattime = ((tm.tm_year - 80) << 25) | ((tm.tm_mon + 1) << 21) | (tm.tm_mday << 16) | (tm.tm_hour << 11) | (tm.tm_min << 5) | (tm.tm_sec >> 1);

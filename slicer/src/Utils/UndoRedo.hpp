...
namespace Slic3r {
	...
	namespace UndoRedo {
		...
		enum class SnapshotType : unsigned char {
			...
			Action, // Some action modifying project state, inside some EnteringGizmo / LeavingGizmo interval.
			GizmoAction, // Selection change at the Plater.
			Selection, // New project, Reset project, Load project ...
			ProjectSeparator, // Entering a Gizmo, which opens a secondary Undo / Redo stack.
			EnteringGizmo, // Leaving a Gizmo, which closes a secondary Undo / Redo stack.
			// No action modifying a project state was done between EnteringGizmo / LeavingGizmo.
			LeavingGizmoNoAction, // Leaving a Gizmo, which closes a secondary Undo / Redo stack.
		...
		struct SnapshotData {
			...
			enum Flags {
				VARIABLE_LAYER_EDITING_ACTIVE = 1, SELECTED_SETTINGS_ON_SIDEBAR = 2, SELECTED_LAYERROOT_ON_SIDEBAR = 4, SELECTED_LAYER_ON_SIDEBAR = 8, RECALCULATE_SLA_SUPPORTS = 16
			};
		};

		struct Snapshot {
			Snapshot(size_t timestamp) : timestamp(timestamp) {}
			Snapshot(const std::string &name, size_t timestamp, size_t model_id, const SnapshotData &snapshot_data) : name(name), timestamp(timestamp), model_id(model_id), snapshot_data(snapshot_data) {}
		...
		struct Selection : public Slic3r::ObjectBase {
			void clear() {
				mode = 0;
				volumes_and_instances.clear();
			}
			unsigned char mode = 0;
			std::vector<std::pair<size_t, size_t>> volumes_and_instances;
			template<class Archive>
			void serialize(Archive &ar) { ar(mode, volumes_and_instances); }
		};

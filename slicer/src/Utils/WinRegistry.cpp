...
namespace Slic3r {
	...
	static std::wstring get_current_user_string_sid() {
		...
		DWORD userSize = 0;
		if (!(!::GetTokenInformation(processToken.get(), TokenUser, nullptr, 0, &userSize) && GetLastError() == ERROR_INSUFFICIENT_BUFFER))
			return L"";
	...
	static std::wstring format_user_choice_string(const wchar_t *aExt, const wchar_t *aUserSid, const wchar_t *aProgId, SYSTEMTIME aTimestamp) {
		...
		// UserChoice hashes.
		const wchar_t *userExperience = L"User Choice set via Windows User Experience "
		                                L"{D18B6DD5-6124-4341-9318-804003BAFA0B}";

		const wchar_t *userChoiceFmt = L"%s%s%s"
		                               L"%08lx"
		                               L"%08lx"
		                               L"%s";

		int userChoiceLen = _scwprintf(userChoiceFmt, aExt, aUserSid, aProgId, fileTime.dwHighDateTime, fileTime.dwLowDateTime, userExperience);
		userChoiceLen += 1;  // _scwprintf does not include the terminator

		std::wstring userChoice(userChoiceLen, L'\0');
		_snwprintf_s(userChoice.data(), userChoiceLen, _TRUNCATE, userChoiceFmt, aExt, aUserSid, aProgId, fileTime.dwHighDateTime, fileTime.dwLowDateTime, userExperience);
		...
		if (NT_SUCCESS(::BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_MD5_ALGORITHM, nullptr, 0))) {
			...
			if (NT_SUCCESS(::BCryptCreateHash(hAlg, &hHash, nullptr, 0, nullptr, 0, 0))) {
				...
				if (NT_SUCCESS(::BCryptHashData(hHash, const_cast<unsigned char *>(bytes), bytesLen, 0))) {
					...
					if (!NT_SUCCESS(::BCryptFinishHash(hHash, reinterpret_cast<unsigned char *>(hash.data()), MD5_DWORDS * sizeof(DWORD), 0)))
						hash.clear();
				}
	...
	static std::wstring hash_string(const wchar_t *inputString) {
		...
		const DWORD C0s[DWORDS_PER_BLOCK][5] = {{md5[0] | 1, 0xCF98B111uL, 0x87085B9FuL, 0x12CEB96DuL, 0x257E1D83uL},
		                                        {md5[1] | 1, 0xA27416F5uL, 0xD38396FFuL, 0x7C932B89uL, 0xBFA49F69uL}};
		const DWORD C1s[DWORDS_PER_BLOCK][5] = {{md5[0] | 1, 0xEF0569FBuL, 0x689B6B9FuL, 0x79F8A395uL, 0xC3EFEA97uL},
	...
	static bool set_user_choice(const wchar_t *aExt, const wchar_t *aProgID) {
		...
		ls = ::RegCreateKeyExW(assocKey.get(), L"UserChoice", 0, nullptr, 0 /* options */, KEY_READ | KEY_WRITE, 0 /* security attributes */, &rawUserChoiceKey, nullptr);
		if (ls != ERROR_SUCCESS)
			return false;

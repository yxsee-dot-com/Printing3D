...
namespace Slic3r {
	...
	struct Update {
		...
		Update() {}
		Update(fs::path &&source, fs::path &&target, const Version &version, std::string vendor, std::string changelog_url, bool forced = false, std::vector<std::string> new_printers = {}) : source(std::move(source)), target(std::move(target)), version(version), vendor(std::move(vendor)), changelog_url(std::move(changelog_url)), forced_update(forced), new_printers(std::move(new_printers)) {}
	...
	struct Incompat {
		...
		Incompat(fs::path &&bundle, const Version &version, std::string vendor) : bundle(std::move(bundle)), version(version), vendor(std::move(vendor)) {}
	...
	PresetUpdater::priv::priv() : cache_path(fs::path(Slic3r::data_dir()) / "cache"), cache_vendor_path(cache_path / "vendor"), rsrc_path(fs::path(resources_dir()) / "profiles"), vendor_path(fs::path(Slic3r::data_dir()) / "vendor"), cancel(false) {
	...
	void PresetUpdater::priv::sync_config(const VendorMap &vendors, const GUI::ArchiveRepository *archive_repository) {
		...
		for (auto &index: index_db) {
			...
			auto archive_it = std::find_if(vendors_with_status.begin(), vendors_with_status.end(), [&index](const std::pair<std::string, VendorStatus> &element) { return element.first == index.vendor(); });
			...
			BOOST_LOG_TRIVIAL(debug) << format("Got index for vendor: %1%: current version: %2%, recommended version: %3%", vendor.name, vendor.config_version.to_string(), recommended.to_string());
	...
	// Version of slic3r that was running the last time and which was read out from Printing3D.ini is provided
	// as a parameter.
	Updates PresetUpdater::priv::get_config_updates(const Semver &old_slic3r_version) const {
		...
		for (const Index &idx: index_db) {
			...
			BOOST_LOG_TRIVIAL(debug) << format("Vendor: %1%, version installed: %2%%3%, version cached: %4%", vp.name, vp.config_version.to_string(), (ver_current_found ? "" : " (not found in index!)"), recommended->config_version.to_string());
			...
			if (recommended->config_version < vp.config_version) {
				BOOST_LOG_TRIVIAL(warning) << format("Recommended config version for the currently running Printing3D is older than the currently installed config for vendor %1%. This should not happen.", idx.vendor());
				continue;
			}
			...
			// Config bundle update situation. The recommended config bundle version for this Printing3D version from the index from the cache is newer
			...
			// 1) The last config index downloaded by any previously running Printing3D instance
			// 2) The last config index installed by any previously running Printing3D instance (older or newer) from its resources.
			// 3) The last config index installed by the currently running Printing3D instance from its resources.
				...
				if (rsrc_vp.valid()) {
					...
					if (recommended != rsrc_idx.end() && recommended->config_version == rsrc_vp.config_version && recommended->config_version > vp.config_version) {
						...
						BOOST_LOG_TRIVIAL(warning) << format("The recommended config version for vendor `%1%` in resources does not match the recommended\n"
						                                     " config version for this version of Printing3D. Corrupted installation?", idx.vendor());
					}
				}
			}
			...
			if (found) {
				...
				BOOST_LOG_TRIVIAL(warning) << format("Index for vendor %1% indicates update (%2%) but the new bundle was found neither in cache nor resources", idx.vendor(), recommended->config_version.to_string());
			}
		}
	...
	bool PresetUpdater::priv::perform_updates(Updates &&updates, const SharedArchiveRepositoryVector &repositories, bool snapshot) const {
		if (updates.incompats.size() > 0) {
			if (snapshot) {
				...
				if (!GUI::Config::take_config_snapshot_cancel_on_error(*GUI::wxGetApp().app_config, Snapshot::SNAPSHOT_DOWNGRADE, "", _u8L("Continue and install configuration updates?")))
					return false;
			}
			...
			if (snapshot) {
				BOOST_LOG_TRIVIAL(info) << "Taking a snapshot...";
				if (!GUI::Config::take_config_snapshot_cancel_on_error(*GUI::wxGetApp().app_config, Snapshot::SNAPSHOT_UPGRADE, "", _u8L("Continue and install configuration updates?")))
					return false;
			}
			...
			for (const auto &update: updates.updates) {
				...
				BOOST_LOG_TRIVIAL(info) << format("Deleting %1% obsolete presets", bundle.obsolete_presets.prints.size() + bundle.obsolete_presets.filaments.size() + bundle.obsolete_presets.printers.size());
	...
	void PresetUpdater::slic3r_update_notify() {
		...
		if (ver_online) {
			...
			if (*ver_online > Slic3r::SEMVER && (!ver_online_seen || *ver_online_seen < *ver_online)) {
				GUI::MsgUpdateSlic3r
				notification(Slic3r::SEMVER, *ver_online);
		...
		if (updates.incompats.size() > 0) {
			...
			for (const auto &incompat: updates.incompats) {
				...
				if (min_slic3r != Semver::zero() && max_slic3r != Semver::inf()) {
					restrictions = GUI::format_wxstr(_L("requires min. %s and max. %s"), min_slic3r.to_string(), max_slic3r.to_string());
			...
			if (incompatible_version) {
				...
				if (res == wxID_OK) {
					...
					if (!p->perform_updates(std::move(updates), repositories) || !reload_configs_update_gui())
						return R_INCOMPAT_EXIT;
	...
	void PresetUpdater::on_update_notification_confirm(const SharedArchiveRepositoryVector &repositories) {
		...
		if (res == wxID_OK) {
			BOOST_LOG_TRIVIAL(debug) << "User agreed to perform the update";
			if (p->perform_updates(std::move(p->waiting_updates), repositories) && reload_configs_update_gui()) {

...
namespace Slic3r {
	...
	class WifiScanner {
		...
		private:
			...
			using WlanOpenHandleFunc =
			DWORD(WINAPI
			*)(DWORD, PVOID, PDWORD, PHANDLE);
			using WlanEnumInterfacesFunc =
			DWORD(WINAPI
			*)(HANDLE, PVOID, PWLAN_INTERFACE_INFO_LIST*);
			using WlanQueryInterfaceFunc =
			DWORD(WINAPI
			*)(HANDLE, const GUID*, WLAN_INTF_OPCODE, PVOID, PDWORD, PVOID*, PWLAN_OPCODE_VALUE_TYPE);
			using WlanFreeMemoryFunc =
			VOID(WINAPI
			*)(PVOID);
			using WlanGetProfileFunc =
			DWORD(WINAPI
			*)(HANDLE, const GUID*, LPCWSTR, PVOID, LPWSTR*, DWORD*, DWORD*);
			using WlanGetProfileListFunc =
			DWORD(WINAPI
			*)(HANDLE, const GUID*, PVOID, PWLAN_PROFILE_INFO_LIST*);
			using WlanGetAvailableNetworkListFunc =
			DWORD(WINAPI
			*)(HANDLE, const GUID*, DWORD, PVOID, PWLAN_AVAILABLE_NETWORK_LIST*);
			using WlanCloseHandleFunc =
			DWORD(WINAPI
			*)(HANDLE, PVOID);

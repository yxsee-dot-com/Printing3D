...
namespace Slic3r {
	namespace Utils {
		...
		void TCPConsole::transmit_next_command() {
			...
			BOOST_LOG_TRIVIAL(debug) << boost::format("TCPConsole: transmitting '%3%' to %1%:%2%") % m_host_name % m_port_name % cmd;

			m_send_buffer = cmd + m_newline;

			set_deadline_in(m_write_timeout);
			boost::asio::async_write(m_socket, boost::asio::buffer(m_send_buffer), boost::bind(&TCPConsole::handle_write, this, boost::placeholders::_1, boost::placeholders::_2));
		}

		void TCPConsole::wait_next_line() {
			set_deadline_in(m_read_timeout);
			boost::asio::async_read_until(m_socket, m_recv_buffer, m_newline, boost::bind(&TCPConsole::handle_read, this, boost::placeholders::_1, boost::placeholders::_2));
		}
		...
		void TCPConsole::handle_write(const boost::system::error_code &ec, std::size_t) {
			m_error_code = ec;
			if (ec) {
		...
		bool TCPConsole::run_queue() {
			try {
				...
				BOOST_LOG_TRIVIAL(error) << boost::format("TCPConsole: Exception while talking with %1%:%2%: %3%") % m_host_name % m_port_name % e.what();

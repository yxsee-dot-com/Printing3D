...
namespace Slic3r {
	...
	namespace {
		...
		bool run_file(const boost::filesystem::path& path)
		{
			if (boost::filesystem::exists(path)) {
				...
				const char* argv2[] = { "open", "/Volumes/Printing3D", nullptr };
	...
	struct AppUpdater::priv {
		...
		bool http_get_file(const std::string &url, size_t size_limit, std::function<bool(Http::Progress)> progress_fn, std::function<bool(std::string /*body*/, std::string &error_message)> completefn, std::string &error_message) const;
	...
	AppUpdater::priv::priv() : m_cancel(false)
#ifdef __linux__
	...
	bool AppUpdater::priv::http_get_file(const std::string &url, size_t size_limit, std::function<bool(Http::Progress)> progress_fn, std::function<bool(std::string /*body*/, std::string &error_message)> complete_fn, std::string &error_message) const {
		bool res = false;
		Http::get(url).size_limit(size_limit).on_progress([&, progress_fn](Http::Progress progress, bool &cancel) {
			...
			error_message = GUI::format("Error getting: `%1%`: HTTP %2%, %3%", url, http_status, error);
	...
	boost::filesystem::path AppUpdater::priv::download_file(const DownloadAppData &data) const {
		...
		if (!res) {
			if (m_cancel) {
				...
				std::string message = (error_message.empty() ? std::string() : GUI::format(_u8L("Downloading new %1% has failed:\n%2%"), SLIC3R_APP_NAME, error_message));
				wxCommandEvent * evt = new wxCommandEvent(EVT_SLIC3R_APP_DOWNLOAD_FAILED);
				if (!message.empty()) {
					BOOST_LOG_TRIVIAL(error) << message;
					evt->SetString(message);
...
#if 0 //lm:is this meant to be ressurected? //dk: it is code that parses Printing3D.version2 in 2.4.0, It was deleted from PresetUpdater.cpp and I would keep it here for possible reference.
	...
	void AppUpdater::sync_version(const std::string &version_check_url, bool from_user) {
		...
		p->m_cancel = false;
		p->m_thread = std::thread([this, version_check_url]() {

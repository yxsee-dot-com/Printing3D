...
namespace Slic3r {
	...
	AstroBox::AstroBox(DynamicPrintConfig *config) : host(config->opt_string("print_host")), apikey(config->opt_string("printhost_apikey")), cafile(config->opt_string("printhost_cafile")) {}
	...
	wxString AstroBox::get_test_failed_msg(wxString &msg) const {
		return GUI::format_wxstr("%s: %s\n\n%s", _L("Could not connect to AstroBox"), msg, _L("Note: AstroBox version at least 1.1.0 is required."));
	}

...
#include "libslic3r/src/Channel.hpp"
...
namespace Slic3r {
	...
	void PrintHostJobQueue::priv::bg_thread_main() {
		...
		try {
			...
			while (!bg_exit) {
				...
				BOOST_LOG_TRIVIAL(debug) << boost::format("PrintHostJobQueue/bg_thread: Received job: [%1%]: `%2%` -> `%3%`, cancelled: %4%") % job_id % job.upload_data.upload_path % job.printhost->get_host() % job.cancelled;
	...
	void PrintHostJobQueue::priv::perform_job(PrintHostJob the_job) {
		...
		bool success = the_job.printhost->upload(std::move(the_job.upload_data), [this](Http::Progress progress, bool &cancel) { this->progress_fn(std::move(progress), cancel); }, [this](wxString error) { this->error_fn(std::move(error)); }, [this](wxString tag, wxString host) { this->info_fn(std::move(tag), std::move(host)); });

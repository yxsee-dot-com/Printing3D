...
namespace Slic3r {
	...
	enum class PrintHostPostUploadAction {
		None, StartPrint, StartSimulation, QueuePrint
	};
	...
	struct PrintHostJob {
		...
		PrintHostJob(PrintHostJob &&other) : upload_data(std::move(other.upload_data)), printhost(std::move(other.printhost)), cancelled(other.cancelled) {}

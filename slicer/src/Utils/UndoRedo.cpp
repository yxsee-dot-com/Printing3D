...
#include "GUI/3DScene.hpp" // IWYU pragma: keep
...
namespace Slic3r {
	namespace UndoRedo {
		...
		template<typename T>
		class ObjectHistory : public ObjectHistoryBase {
			public:
		...
		struct MutableHistoryInterval {
			private:
				struct Data {
					...
					bool matches_timestamp(uint64_t timestamp) {
						assert(timestamp > 0);
						assert(this->size > 8);
						return memcmp(this->data, &timestamp, 8) == 0;
					}
				};
			...
			public:
				...
				MutableHistoryInterval &operator=(MutableHistoryInterval &&rhs) {
...
namespace cereal {
	...
	// store just the ObjectID to this stream.
	template<class T>
	void save_by_value(BinaryOutputArchive &ar, const T &cfg) {
		ar(cereal::get_user_data<Slic3r::UndoRedo::StackImpl>(ar).save_mutable_object<T>(cfg));
	}
	// Load ObjectBase derived class from the Undo / Redo stack as a separate object
	// based on the ObjectID loaded from this stream.
	template<class T>
	void load_by_value(BinaryInputArchive &ar, T &cfg) {
		Slic3r::UndoRedo::StackImpl &stack = cereal::get_user_data<Slic3r::UndoRedo::StackImpl>(ar);
		size_t id;
		ar(id);
...
#include <libslic3r/src/Model.hpp> // IWYU pragma: keep
#include <libslic3r/src/TriangleMesh.hpp> // IWYU pragma: keep
#include <GUI/Selection.hpp> // IWYU pragma: keep
#include <GUI/Gizmos/GLGizmosManager.hpp> // IWYU pragma: keep

namespace Slic3r {
	namespace UndoRedo {

		template<typename T>
		std::shared_ptr<const T> &ImmutableObjectHistory<T>::shared_ptr(StackImpl &stack) {
		...
		template<typename T>
		ObjectID StackImpl::save_mutable_object(const T &object) {
		...
		template<typename T>
		ObjectID StackImpl::save_immutable_object(std::shared_ptr<const T> &object, bool optional) {
		...
		template<typename T>
		T *StackImpl::load_mutable_object(const Slic3r::ObjectID id) {
			T *target = new T();
		...
		template<typename T>
		std::shared_ptr<const T> StackImpl::load_immutable_object(const Slic3r::ObjectID id, bool optional) {
		...
		template<typename T>
		void StackImpl::load_mutable_object(const Slic3r::ObjectID id, T &target) {
			// First find a history stack for the ObjectID of this object instance.
			auto it_object_history = m_objects.find(id);
		...
		void Stack::take_snapshot(const std::string &snapshot_name, const Slic3r::Model &model, const Slic3r::GUI::Selection &selection, const Slic3r::GUI::GLGizmosManager &gizmos, const SnapshotData &snapshot_data) { pimpl->take_snapshot(snapshot_name, model, selection, gizmos, snapshot_data); }
		...
		bool Stack::undo(Slic3r::Model &model, const Slic3r::GUI::Selection &selection, Slic3r::GUI::GLGizmosManager &gizmos, const SnapshotData &snapshot_data, size_t time_to_load) { return pimpl->undo(model, selection, gizmos, snapshot_data, time_to_load); }

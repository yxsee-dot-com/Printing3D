...
// For starting another Printing3D instance on OSX.
...
namespace Slic3r {
	namespace GUI {
		...
		static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance, bool delete_after_load) {
			...
			// Don't call with wxEXEC_HIDE_CONSOLE, Printing3D in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
			{
				...
				// ((instance_type == NewSlicerInstanceType::Slicer) ? "Printing3D" : "PrusaGCodeViewer");
				// Just run Printing3D and give it a --gcodeviewer parameter.
				bin_path = bin_path.parent_path() / "Printing3D";
		...
		void start_new_gcodeviewer_open_file(wxWindow *parent) {
			wxFileDialog dialog(parent ? parent : wxGetApp().GetTopWindow(), _L("Open G-code file:"), from_u8(wxGetApp().app_config->get_last_dir()), wxString(), file_wildcards(FT_GCODE), wxFD_OPEN | wxFD_FILE_MUST_EXIST);

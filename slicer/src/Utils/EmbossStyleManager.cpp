...
#include <libslic3r/src/Utils.hpp> // ScopeGuard

#include "WxFontUtils.hpp"
#include "GUI/3DScene.hpp" // ::glsafe
#include "GUI/Jobs/CreateFontStyleImagesJob.hpp"
#include "GUI/ImGuiPureWrap.hpp" // check of font ranges
...
StyleManager::StyleManager(const ImWchar *language_glyph_range, const std::function<EmbossStyles()> &create_default_styles) : m_create_default_styles(create_default_styles), m_imgui_init_glyph_range(language_glyph_range) {}
...
namespace {
	...
	const HorizontalAlignToName horizontal_align_to_name = boost::assign::list_of<HorizontalAlignToName::relation>(FontProp::HorizontalAlign::left, "left")(FontProp::HorizontalAlign::center, "center")(FontProp::HorizontalAlign::right, "right");
	using VerticalAlignToName = boost::bimap<FontProp::VerticalAlign, std::string_view>;
	const VerticalAlignToName vertical_align_to_name = boost::assign::list_of<VerticalAlignToName::relation>(FontProp::VerticalAlign::top, "top")(FontProp::VerticalAlign::center, "middle")(FontProp::VerticalAlign::bottom, "bottom");
} // namespace
...
void StyleManager::swap(size_t i1, size_t i2) {
	if (i1 >= m_styles.size() || i2 >= m_styles.size()) return;
...
void StyleManager::init_style_images(const Vec2i &max_size, const std::string &text) {
...
ImFont *StyleManager::create_imgui_font(const std::string &text, double scale) {
	...
	ImFont *font = m_style_cache.atlas.AddFontFromMemoryTTF((void *) buffer.data(), buffer.size(), font_size, &font_config, m_style_cache.ranges.Data);

...
namespace Slic3r {
	...
	PrusaConnectNew::PrusaConnectNew(DynamicPrintConfig *config) : m_uuid(config->opt_string("print_host")), m_team_id(config->opt_string("printhost_apikey")) {}
	...
	bool PrusaConnectNew::init_upload(PrintHostUpload upload_data, std::string &out) const {
		...
		http.header("Authorization", "Bearer " + access_token).header("Content-Type", "application/json").set_post_body(request_body_json).on_complete([&](std::string body, unsigned status) {
	...
	bool PrusaConnectNew::upload(PrintHostUpload upload_data, ProgressFn progress_fn, ErrorFn error_fn, InfoFn info_fn) const {
		...
		const std::string url = GUI::format("%1%/app/teams/%2%/files/raw"
		                                    "?upload_id=%3%"
				//       "&force=true"
				//       "&printer_uuid=%4%"
				//       "&path=%5%"
		...
		BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%") % name % upload_data.source_path % url % upload_data.upload_path.filename().string() % upload_data.upload_path.parent_path().string() % (upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false");

		Http http = Http::put(std::move(url));
		http.set_put_body(upload_data.source_path).header("Content-Type", "text/x.gcode").header("Authorization", "Bearer " + access_token).on_complete([&](std::string body, unsigned status) {
	...
	bool PrusaConnectNew::get_storage(wxArrayString &storage_path, wxArrayString &storage_name) const {
		...
		auto http = Http::get(std::move(url));
		http.header("Authorization", "Bearer " + access_token).on_error([&](std::string body, std::string error, unsigned status) {

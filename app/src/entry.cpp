...
class OpenGLVersionCheck {
	public:
		...
		bool load_opengl_dll() {
			...
			wc.lpszClassName = L"Printing3D_opengl_version_check";
			wc.style = CS_OWNDC;
			if (RegisterClass(&wc)) {
				HWND hwnd = CreateWindowW(wc.lpszClassName, L"Printing3D_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID) this);
				if (hwnd) {
	...
	protected:
		...
		void check(HWND hWnd) {
			...
			PIXELFORMATDESCRIPTOR pfd = {sizeof(PIXELFORMATDESCRIPTOR), 1, PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_DOUBLEBUFFER, PFD_TYPE_RGBA,                // The kind of framebuffer. RGBA or palette.
...
//
	...
	if (load_mesa) {
		...
		if (!res) {
			MessageBox(nullptr, L"Printing3D was unable to automatically switch to MESA OpenGL library\nPlease, try to run the application using the '--sw-renderer' option.\n", L"Printing3D Warning", MB_OK);
			return -1;
		} else {
	...
	wcscat(path_to_slic3r, L"slicer.dll");
	//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
	HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
	if (hInstance_Slic3r == nullptr) {
		MessageBoxA(nullptr,"Slicer.dll was not loaded","title",0);
		return -1;
	}
	...
	if (slic3r_main == nullptr) {
		printf("could not locate the function slic3r_main in Printing3D.dll\n");
		return -1;
	}

...
namespace Slic3r {
	...
	enum class TriangleStateType : int8_t {
		...
		NONE = 0, ENFORCER = 1, BLOCKER = 2, // Maximum is 15. The value is serialized in TriangleSelector into 6 bits using a 2 bit prefix code.
		Extruder1 = ENFORCER, Extruder2 = BLOCKER, Extruder3, Extruder4, Extruder5, Extruder6, Extruder7, Extruder8, Extruder9, Extruder10, Extruder11, Extruder12, Extruder13, Extruder14, Extruder15, Count
	};
	...
	class TriangleSelector {
		...
		public:
			...
			class Sphere : public SinglePointCursor {
				public:
					Sphere() = delete;
					explicit Sphere(const Vec3f &center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_) : SinglePointCursor(center_, source_, radius_world, trafo_, clipping_plane_) {};
			...
			class Circle : public SinglePointCursor {
				public:
					Circle() = delete;
					explicit Circle(const Vec3f &center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_) : SinglePointCursor(center_, source_, radius_world, trafo_, clipping_plane_) {};
			...
			class Capsule3D : public DoublePointCursor {
				public:
					Capsule3D() = delete;
					explicit Capsule3D(const Vec3f &first_center_, const Vec3f &second_center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_) : TriangleSelector::DoublePointCursor(first_center_, second_center_, source_, radius_world, trafo_, clipping_plane_) {}
			...
			class Capsule2D : public DoublePointCursor {
				public:
					Capsule2D() = delete;
					explicit Capsule2D(const Vec3f &first_center_, const Vec3f &second_center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_) : TriangleSelector::DoublePointCursor(first_center_, second_center_, source_, radius_world, trafo_, clipping_plane_) {}
			...
			struct TriangleBitStreamMapping {
				...
				private:
					...
					void serialize(Archive &ar) { ar(triangle_idx, bitstream_start_idx); }
			};
			...
			struct TriangleSplittingData {
				...
				private:
					...
					template<class Archive>
					void serialize(Archive &ar) { ar(triangles_to_split, bitstream, used_states); }
			};
		...
		protected:
			...
			class Triangle {
				public:
					...
					Triangle(int a, int b, int c, int source_triangle, const TriangleStateType init_state) : verts_idxs{a, b, c}, source_triangle{source_triangle}, state{init_state} {
					...
					void set_state(TriangleStateType type) {
						assert(!is_split());
						state = type;
					}
					TriangleStateType get_state() const {
						assert(!is_split());
						return state;
					}
		...
		private:
			...
			void get_facets_strict_recursive(const Triangle &tr, const Vec3i &neighbors, TriangleStateType state, std::vector<stl_triangle_vertex_indices> &out_triangles) const;

...
namespace Slic3r {
	...
	namespace meshsplit_detail {
		...
		template<class Its, class Enable = void>
		struct ItsWithNeighborsIndex_ {
		...
		template<>
		struct ItsWithNeighborsIndex_<indexed_triangle_set> {
		...
		struct NeighborVisitor {
				NeighborVisitor(const indexed_triangle_set &its, const NeighborIndex &neighbor_index) : its(its), neighbor_index(neighbor_index) {
					m_visited.assign(its.indices.size(), false);
					m_facestack.reserve(its.indices.size());
				}
				NeighborVisitor(const indexed_triangle_set &its, NeighborIndex &&aneighbor_index) : its(its), neighbor_index(m_neighbor_index_data), m_neighbor_index_data(std::move(aneighbor_index)) {
					m_visited.assign(its.indices.size(), false);
					m_facestack.reserve(its.indices.size());
				}
			...
			private:
				...
				stack_el pop() {
					stack_el ret = m_facestack.back();
					m_facestack.pop_back();
					return ret;
				}
	...
	bool its_is_splittable(const Its &m) {
		...
		visitor.visit([&has_some](size_t idx) {
			has_some = true;
			return true;
		});
		if (has_some)
			// Just check whether there is any face of the 2nd patch.
			visitor.visit([&has_some2](size_t idx) {
				has_some2 = true;
				return false;
			});
	...
	std::vector<Vec3i> create_face_neighbors_index(ExPolicy &&ex, const indexed_triangle_set &its) {
		...
		execution::for_each(ex, size_t(0), indices.size(), [&neighbors, &indices, &vertex_triangles](size_t face_idx) {

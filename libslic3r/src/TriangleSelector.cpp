...
namespace Slic3r {
	...
	static bool test_line_inside_cylinder(const Vec3f &line_a, const Vec3f &line_b, const Vec3f &cylinder_P, const Vec3f &cylinder_Q, const float cylinder_radius) {
		...
		auto is_point_inside_finite_cylinder = [&cylinder_P, &cylinder_Q, &cylinder_radius, &cylinder_dir](const Vec3f &pt) {
			...
			return first_center_diff.dot(cylinder_dir) <= 0 && second_center_diff.dot(cylinder_dir) >= 0 && (first_center_diff.cross(cylinder_dir).norm() / cylinder_dir.norm()) <= cylinder_radius;
		};
	...
	void TriangleSelector::seed_fill_select_triangles(const Vec3f &hit, int facet_start, const Transform3d &trafo_no_translate, const ClippingPlane &clp, float seed_fill_angle, float highlight_by_angle_deg, bool force_reselection) {
		assert(facet_start < m_orig_size_indices);
		...
		if (tr->is_split()) {
			...
			if (num_of_children != 1) {
				for (int i = 0; i < num_of_children; ++i) {
					...
					this->precompute_all_neighbors_recursive(tr->children[i], child_neighbors, this->child_neighbors_propagated(*tr, neighbors_propagated, i, child_neighbors), neighbors_out, neighbors_propagated_out);
				}
			}
		}
	}
	...
	void TriangleSelector::append_touching_edges(int itriangle, int vertexi, int vertexj, std::vector<Vec2i> &touching_edges_out) const {
		...
		auto process_subtriangle = [this, &itriangle, &vertexi, &vertexj, &touching_edges_out](const int subtriangle_idx, Partition partition) -> void {
			...
			if (!m_triangles[subtriangle_idx].is_split()) {
				...
				append_touching_edges(subtriangle_idx, partition == Partition::First ? vertexi : midpoint, partition == Partition::First ? midpoint : vertexj, touching_edges_out);
			else
		...
		if (tr.number_of_split_sides() == 1) {
			...
			switch (edge) {
				case 0:
					...
					break;
				case 1:
					child_idx = partition == Partition::First ? 1 : 2;
					break;
				default:
					assert(edge == 2);
	...
	std::pair<int, int> TriangleSelector::triangle_subtriangles(const Triangle &tr, int vertexi, int vertexj) {
		...
		if (tr.number_of_split_sides() == 1) {
			return edge == next_idx_modulo(tr.special_side(), 3) ? std::make_pair(tr.children[0], tr.children[1]) : std::make_pair(tr.children[edge == tr.special_side() ? 0 : 1], -1);
		} else if (tr.number_of_split_sides() == 2) {
			return edge == next_idx_modulo(tr.special_side(), 3) ? std::make_pair(tr.children[2], -1) : edge == tr.special_side() ? std::make_pair(tr.children[0], tr.children[1]) : std::make_pair(tr.children[2], tr.children[0]);
		} else {
			assert(tr.number_of_split_sides() == 3);
			assert(tr.special_side() == 0);
			return edge == 0 ? std::make_pair(tr.children[0], tr.children[1]) : edge == 1 ? std::make_pair(tr.children[1], tr.children[2]) : std::make_pair(tr.children[2], tr.children[0]);
		}
	...
	int TriangleSelector::triangle_midpoint(const Triangle &tr, int vertexi, int vertexj) const {
		...
		if (tr.number_of_split_sides() == 1) {
			return edge == next_idx_modulo(tr.special_side(), 3) ? m_triangles[tr.children[0]].verts_idxs[2] : this->triangle_midpoint(m_triangles[tr.children[edge == tr.special_side() ? 0 : 1]], vertexi, vertexj);
		} else if (tr.number_of_split_sides() == 2) {
			return edge == next_idx_modulo(tr.special_side(), 3) ? this->triangle_midpoint(m_triangles[tr.children[2]], vertexi, vertexj) : edge == tr.special_side() ? m_triangles[tr.children[0]].verts_idxs[1] : m_triangles[tr.children[1]].verts_idxs[2];
		} else {
			assert(tr.number_of_split_sides() == 3);
			assert(tr.special_side() == 0);
			return (edge == 0) ? m_triangles[tr.children[0]].verts_idxs[1] : (edge == 1) ? m_triangles[tr.children[1]].verts_idxs[2] : m_triangles[tr.children[2]].verts_idxs[2];
		}
	}
	...
	bool TriangleSelector::select_triangle_recursive(int facet_idx, const Vec3i &neighbors, TriangleStateType type, bool triangle_splitting) {
		...
		if (num_of_inside_vertices == 0 && !m_cursor->is_pointer_in_triangle(*tr, m_vertices) && !m_cursor->is_edge_inside_cursor(*tr, m_vertices))
			return false;
	...
	void TriangleSelector::garbage_collect() {
		...
		m_triangles.erase(std::remove_if(m_triangles.begin() + m_orig_size_indices, m_triangles.end(), [](const Triangle &tr) { return !tr.valid(); }), m_triangles.end());
		m_vertices.erase(std::remove_if(m_vertices.begin() + m_orig_size_vertices, m_vertices.end(), [](const Vertex &vert) { return vert.ref_cnt == 0; }), m_vertices.end());
	...
	TriangleSelector::TriangleSelector(const TriangleMesh &mesh) : m_mesh{mesh}, m_neighbors(its_face_neighbors(mesh.its)), m_face_normals(its_face_normals(mesh.its)) {
		reset();
	}
	...
	void TriangleSelector::get_facets_strict_recursive(const Triangle &tr, const Vec3i &neighbors, TriangleStateType state, std::vector<stl_triangle_vertex_indices> &out_triangles) const {
		if (tr.is_split()) {
			for (int i = 0; i <= tr.number_of_split_sides(); ++i)
	...
	void TriangleSelector::get_facets_split_by_tjoints(const Vec3i &vertices, const Vec3i &neighbors, std::vector<stl_triangle_vertex_indices> &out_triangles) const {
		...
		Vec3i midpoints(this->triangle_midpoint(neighbors(0), vertices(1), vertices(0)), this->triangle_midpoint(neighbors(1), vertices(2), vertices(1)), this->triangle_midpoint(neighbors(2), vertices(0), vertices(2)));
		...
		switch (splits) {
			...
			case 1: {
				...
				int j = next_idx_modulo(i, 3);
				int k = next_idx_modulo(j, 3);
				this->get_facets_split_by_tjoints({vertices(i), vertices(j), midpoints(j)}, {neighbors(i), this->neighbor_child(neighbors(j), vertices(k), vertices(j), Partition::Second), -1}, out_triangles);
				this->get_facets_split_by_tjoints({midpoints(j), vertices(k), vertices(i)}, {this->neighbor_child(neighbors(j), vertices(k), vertices(j), Partition::First), neighbors(k), -1}, out_triangles);
				break;
			}
			case 2: {
				...
				int j = next_idx_modulo(i, 3);
				int k = next_idx_modulo(j, 3);
				this->get_facets_split_by_tjoints({vertices(i), midpoints(i), midpoints(k)}, {this->neighbor_child(neighbors(i), vertices(j), vertices(i), Partition::Second), -1, this->neighbor_child(neighbors(k), vertices(i), vertices(k), Partition::First)}, out_triangles);
				this->get_facets_split_by_tjoints({midpoints(i), vertices(j), midpoints(k)}, {this->neighbor_child(neighbors(i), vertices(j), vertices(i), Partition::First), -1, -1}, out_triangles);
				this->get_facets_split_by_tjoints({vertices(j), vertices(k), midpoints(k)}, {neighbors(j), this->neighbor_child(neighbors(k), vertices(i), vertices(k), Partition::Second), -1}, out_triangles);
				break;
			}
			default:
				assert(splits == 3);
				// Split to 4 triangles.
				this->get_facets_split_by_tjoints({vertices(0), midpoints(0), midpoints(2)}, {this->neighbor_child(neighbors(0), vertices(1), vertices(0), Partition::Second), -1, this->neighbor_child(neighbors(2), vertices(0), vertices(2), Partition::First)}, out_triangles);
				this->get_facets_split_by_tjoints({midpoints(0), vertices(1), midpoints(1)}, {this->neighbor_child(neighbors(0), vertices(1), vertices(0), Partition::First), this->neighbor_child(neighbors(1), vertices(2), vertices(1), Partition::Second), -1}, out_triangles);
				this->get_facets_split_by_tjoints({midpoints(1), vertices(2), midpoints(2)}, {this->neighbor_child(neighbors(1), vertices(2), vertices(1), Partition::First), this->neighbor_child(neighbors(2), vertices(0), vertices(2), Partition::Second), -1}, out_triangles);
		...
		if (tr->is_split()) {
			...
			if (num_of_children != 1) {
				for (int i = 0; i < num_of_children; ++i) {
					...
					// All children of this triangle were created by splitting a single source triangle of the original mesh.
					const Vec3i child_neighbors = this->child_neighbors(*tr, neighbors, i);
					this->get_seed_fill_contour_recursive(tr->children[i], child_neighbors, this->child_neighbors_propagated(*tr, neighbors_propagated, i, child_neighbors), edges_out);
				}
			}
		...
		struct Serializer {
			...
			void serialize(int facet_idx) {
				...
				if (split_sides) {
					...
					// Serialized in reverse order for compatibility with Printing3D 2.3.1.
					...
					if (n >= 3) {
						...
						// Simple case, compatible with Printing3D 2.3.1 and older for storing paint on supports and seams.
						// Store 2 bits of n.
	...
	TriangleSelector::Cursor::Cursor(const Vec3f &source_, float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_) : source{source_}, trafo{trafo_.cast<float>()}, clipping_plane{clipping_plane_} {
	...
	TriangleSelector::SinglePointCursor::SinglePointCursor(const Vec3f &center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_) : center{center_}, Cursor(source_, radius_world, trafo_, clipping_plane_) {
		// In case that the transformation is non-uniform, all checks whether
		// something is inside the cursor should be done in world coords.
	...
	bool TriangleSelector::DoublePointCursor::is_pointer_in_triangle(const Vec3f &p1_, const Vec3f &p2_, const Vec3f &p3_) const {
		return is_circle_pointer_inside_triangle(p1_, p2_, p3_, first_center, dir, uniform_scaling, trafo) || is_circle_pointer_inside_triangle(p1_, p2_, p3_, second_center, dir, uniform_scaling, trafo);
	}

		...
		void fill_config(PConf &pcfg, const ArrangeParams &params) {
			...
			switch (params.alignment) {
				...
				case Pivots::BottomRight:
					pcfg.alignment = PConf::Alignment::BOTTOM_RIGHT;
					break;
				case Pivots::TopLeft:
					pcfg.alignment = PConf::Alignment::TOP_LEFT;
					break;
				case Pivots::TopRight:
					pcfg.alignment = PConf::Alignment::TOP_RIGHT;
					break;
			}
...
#pragma warning(pop)
				...
				// reasonable results.
				std::tuple<double /*score*/, Box /*farthest point from bin center*/> objfunc(const Item &item, const Point &bincenter) {
					...
					switch (compute_case) {
						case BIG_ITEM: {
							...
							if (result.empty())
								...
								score = 0.50 * dist + (1.0 - R) * 0.20 * density + 0.30 * alignment_score;
			...
			public:
				AutoArranger(const TBin &bin, const ArrangeParams &params, std::function<void(unsigned)> progressind, std::function<bool(void)> stopcond) : m_pck(bin, params.min_obj_distance), m_bin(bin), m_bin_area(sl::area(bin)), m_norm(std::sqrt(m_bin_area)) {
				...
				template<class It>
				inline void operator()(It from, It to) {
					m_rtree.clear();
		...
		template<>
		std::function<double(const Item &)> AutoArranger<Box>::get_objfn() {
		...
		template<>
		std::function<double(const Item &)> AutoArranger<Circle>::get_objfn() {
			auto bincenter = m_bin.center();
		...
		void remove_large_items(std::vector<Item> &items, Bin &&bin) {
			auto it = items.begin();
			while (it != items.end())
				sl::isInside(it->transformedShape(), bin) ? ++it : it = items.erase(it);
		}

		template<class S>
		Radians min_area_boundingbox_rotation(const S &sh) {
			return minAreaBoundingBox < S, TCompute < S >, boost::rational<LargeInt>>
			(sh).angleToX();
		}

		template<class S>
		Radians fit_into_box_rotation(const S &sh, const _Box <TPoint<S>> &box) {
			return fitIntoBoxRotation < S, TCompute < S >, boost::rational<LargeInt>>
			(sh, box);
		}

		template<class BinT>
		// Arrange for arbitrary bin type
		...
		bool is_box(const Points &bed) {
			return !bed.empty() && ((1.0 - poly_area(bed) / area(BoundingBox(bed))) < 1e-3);
		}
		...
		void arrange(ArrangePolygons &items, const ArrangePolygons &excludes, const SegmentedRectangleBed &bed, const ArrangeParams &params) {
			...
			for (size_t bedidx = 0; bedidx < beds; ++bedidx) {
				...
				Point corr{0, 0};
				corr.x() = -std::min(0, pilebbx.min.x() - bedbb.min.x()) - std::max(0, pilebbx.max.x() - bedbb.max.x());
				corr.y() = -std::min(0, pilebbx.min.y() - bedbb.min.y()) - std::max(0, pilebbx.max.y() - bedbb.max.y());

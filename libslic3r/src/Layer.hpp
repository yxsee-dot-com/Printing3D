...
namespace Slic3r {
	...
	struct LayerSlice {
		...
		bool has_extrusions() const {
			for (const LayerIsland &island: islands) if (island.has_extrusions()) return true;
			return false;
		}
	};
	...
	class Layer {
		public:
			...
			virtual bool has_extrusions() const {
				for (auto layerm: m_regions) if (layerm->has_extrusions()) return true;
				return false;
			}
		...
		protected:
			...
			Layer(size_t id, PrintObject *object, coordf_t height, coordf_t print_z, coordf_t slice_z) : upper_layer(nullptr), lower_layer(nullptr),
	...
	class SupportLayer : public Layer {
		...
		protected:
			...
			SupportLayer(size_t id, size_t interface_id, PrintObject *object, coordf_t height, coordf_t print_z, coordf_t slice_z) : Layer(id, object, height, print_z, slice_z), m_interface_id(interface_id) {}

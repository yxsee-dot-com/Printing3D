...
namespace Slic3r {
	...
	enum class VisitorReturnMask : unsigned int {
		CONTINUE_LEFT = 1, CONTINUE_RIGHT = 2, STOP = 4,
	};
		...
		struct Visitor {
			...
			unsigned int operator()(size_t idx, size_t dimension) {
				if (this->filter(idx)) {
					...
					auto it = std::lower_bound(results.begin(), results.end(), res, [](auto &r1, auto &r2) {
	...
	std::vector<size_t> find_nearby_points(const KDTreeIndirectType &kdtree, const PointType &center, const typename KDTreeIndirectType::CoordType &max_distance, FilterFn filter) {
		...
		struct Visitor {
			...
			Visitor(const KDTreeIndirectType &kdtree, const PointType &center, const CoordType &max_distance, FilterFn filter) : kdtree(kdtree), center(center), max_distance_squared(max_distance * max_distance), filter(filter) {
			}
			unsigned int operator()(size_t idx, size_t dimension) {
				if (this->filter(idx)) {
	...
	template<typename KDTreeIndirectType, typename PointType>
	std::vector<size_t> find_nearby_points(const KDTreeIndirectType &kdtree, const PointType &center, const typename KDTreeIndirectType::CoordType &max_distance) {
	...
	std::vector<size_t> find_nearby_points(const KDTreeIndirectType &kdtree, const PointType &bb_min, const PointType &bb_max, FilterFn filter) {
		struct Visitor {
			...
			Visitor(const KDTreeIndirectType &kdtree, const PointType &bbmin, const PointType &bbmax, FilterFn filter) : kdtree(kdtree), bb_min{bbmin}, bb_max{bbmax}, filter(filter) {
			}
			unsigned int operator()(size_t idx, size_t dimension) {
				unsigned int ret = static_cast<unsigned int>(VisitorReturnMask::CONTINUE_LEFT) | static_cast<unsigned int>(VisitorReturnMask::CONTINUE_RIGHT);

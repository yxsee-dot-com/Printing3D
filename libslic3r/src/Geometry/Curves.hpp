...
namespace Slic3r {
	namespace Geometry {
		...
		template<int Dimension, typename NumberType>
		PolynomialCurve<Dimension, NumberType> fit_polynomial(const std::vector<Vec < Dimension, NumberType>>
		&observations,
		const std::vector<NumberType> &observation_points,
		const std::vector<NumberType> &weights, size_t
		order) {
	...
	++dim) {
	coefficients.
	row(dim) = QR.solve(data_points.row(dim).transpose());
}
...
PiecewiseFittedCurve<Dimension, NumberType, Kernel> fit_curve(const std::vector<Vec < Dimension, NumberType>>
&observations,
const std::vector<NumberType> &observation_points,
const std::vector<NumberType> &weights, size_t
segments_count,
...
result.
start = observation_points.front();
result.
segment_size = segment_size;
result.
endpoints_level_of_freedom = endpoints_level_of_freedom;
...
++index) {
data_points.
col(index) = observations[index] * sqrt_weights[index];
}
...
for (
int segment_index = start_segment_idx;
segment_index <
...
// Solve for linear least square fit
result.coefficients.
resize(Dimension, parameters_count
);
...
template<int Dimension, typename NumberType> PiecewiseFittedCurve<Dimension, NumberType, LinearKernel < NumberType>>
fit_linear_spline(const std::vector<Vec < Dimension, NumberType>>
&observations,
std::vector<NumberType> observation_points, std::vector<NumberType>
weights,
size_t segments_count, size_t
...
template<int Dimension, typename NumberType> PiecewiseFittedCurve<Dimension, NumberType, CubicBSplineKernel < NumberType>>
fit_cubic_bspline(const std::vector<Vec < Dimension, NumberType>>
&observations,
std::vector<NumberType> observation_points, std::vector<NumberType>
weights,
size_t segments_count, size_t
...
template<int Dimension, typename NumberType> PiecewiseFittedCurve<Dimension, NumberType, CubicCatmulRomKernel < NumberType>>
fit_catmul_rom_spline(const std::vector<Vec < Dimension, NumberType>>
&observations,
std::vector<NumberType> observation_points, std::vector<NumberType>
weights,
size_t segments_count, size_t

...
namespace Slic3r {
	...
	namespace Voronoi {
		...
		inline const Point &contour_point(const VD::cell_type &cell, const Line &line) { return ((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b); }
		inline Point &contour_point(const VD::cell_type &cell, Line &line) { return ((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b); }
		...
		enum class VertexCategory : unsigned char {
			...
			OnContour, // Vertex is inside the CCW input contour, holes are respected.
			Inside, // Vertex is outside the CCW input contour, holes are respected.
			Outside, // Not known yet.
			Unknown,
		};
		...
		enum class EdgeCategory : unsigned char {
			...
			PointsToContour, // This half-edge points inside, this VD::edge_type::vertex1().color() is Inside.
			PointsInside, // This half-edge points outside, this VD::edge_type::vertex1().color() is Outside.
			PointsOutside, // Not known yet.
			Unknown
		};
		...
		enum class CellCategory : unsigned char {
			...
			Boundary, // This Voronoi cell is completely inside.
			Inside, // This Voronoi cell is completely outside.
			Outside, // Not known yet.
			Unknown
		};
		...
		static inline bool edge_offset_has_intersection(const Vec2d &intersection_point) { return !edge_offset_no_intersection(intersection_point); }
		std::vector<Vec2d> edge_offset_contour_intersections(const VD &vd, const Lines &lines, const std::vector<double> &distances, double offset_distance);

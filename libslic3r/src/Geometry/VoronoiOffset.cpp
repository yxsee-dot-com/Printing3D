	...
	namespace Voronoi {
		...
		namespace detail {
			...
			inline bool vertex_equal_to_point(const VertexType &vertex, const Point &ipt) {
				...
				return ulp_cmp(vertex.x(), double(ipt.x()), ULPS) == ulp_cmp_type::EQUAL && ulp_cmp(vertex.y(), double(ipt.y()), ULPS) == ulp_cmp_type::EQUAL;
#else
			...
			double dist_to_site(const Lines &lines, const VD::cell_type &cell, const Vec2d &point) {
				...
				return cell.contains_point() ? (((cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT) ? line.a : line.b).cast<double>() - point).norm() : (Geometry::foot_pt<Vec2d>(line.a.cast<double>(), (line.b - line.a).cast<double>(), point) - point).norm();
			};
		...
		void annotate_inside_outside(VD &vd, const Lines &lines) {
			...
			for (const VD::edge_type &edge: vd.edges())
				if (edge.vertex1() == nullptr) {
					...
					// edge does not.
					assert(edge.is_secondary() ? (cell->contains_segment() != cell2->contains_segment()) : (cell->contains_point() == cell2->contains_point()));
					...
					if (line) {
						...
						if (on_contour && v1_category == VertexCategory::OnContour) {
							...
							assert(cell->contains_segment());
							annotate_cell(cell, on_contour ? CellCategory::Boundary : (vc == VertexCategory::Outside ? CellCategory::Outside : CellCategory::Inside));
							annotate_cell(cell2, (on_contour && cell2->contains_segment()) ? CellCategory::Boundary : (vc == VertexCategory::Outside ? CellCategory::Outside : CellCategory::Inside));
						}
					}
				}
		...
		std::vector<Vec2d> skeleton_edges_rough(const VD &vd, const Lines &lines,
			...
			for (const VD::edge_type &edge: vd.edges()) {
				...
				if (cell->contains_segment() && cell2->contains_segment()) {
					...
					if (cell->contains_point() != cell2->contains_point()) {
						...
						const Point &pt0 = cell->contains_point() ? contour_point(*cell, line0) : contour_point(*cell2, line1);
						const Line &line = cell->contains_segment() ? line0 : line1;
						std::tie(out[edge_idx], out[edge_idx2]) = detail::point_segment_dr_dl_thresholds(pt0, line, vertex_point(v0), vertex_point(v1), threshold_tan_alpha_half);
					} else {
						// Point - Point
						const Point &pt0 = contour_point(*cell, line0);
						const Point &pt1 = contour_point(*cell2, line1);
						std::tie(out[edge_idx], out[edge_idx2]) = detail::point_point_dr_dl_thresholds(pt0, pt1, vertex_point(v0), vertex_point(v1), threshold_tan_alpha_half);
					}
				}
			}

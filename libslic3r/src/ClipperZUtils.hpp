...
namespace Slic3r {
	...
	namespace ClipperZUtils {
		...
		inline ZPaths expolygons_to_zpaths(const ExPolygons &src, coord_t &base_idx) {
			ZPaths out;
			out.reserve(std::accumulate(src.begin(), src.end(), size_t(0), [](const size_t acc, const ExPolygon &expoly) { return acc + expoly.num_contours(); }));
		...
		class ClipperZIntersectionVisitor {
			public:
				...
				ClipperLib_Z::ZFillCallback clipper_callback() {
					return [this](const ZPoint &e1bot, const ZPoint &e1top, const ZPoint &e2bot, const ZPoint &e2top, ZPoint &pt) { return (*this)(e1bot, e1top, e2bot, e2top, pt); };
				}

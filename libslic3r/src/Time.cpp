...
namespace Slic3r {
	namespace Utils {
		...
		static const char *get_fmtstr(TimeFormat fmt) {
			switch (fmt) {
				case TimeFormat::gcode:
					return SLICER_UTC_TIME_FMT;
				case TimeFormat::iso8601Z:
					return ISO8601Z_TIME_FMT;
			}
		...
		std::string time2str(const time_t &t, TimeZone zone, TimeFormat fmt) {
			...
			switch (zone) {
				case TimeZone::local:
					ret = tm2str(_localtime_r(&t, &tms), fmtstr.c_str());
					break;
				case TimeZone::utc:
					ret = tm2str(_gmtime_r(&t, &tms), fmtstr.c_str());
					break;
			}

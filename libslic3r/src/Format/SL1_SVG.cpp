...
#include "Format/ZipperArchiveImport.hpp"
...
namespace Slic3r {
	...
	class SVGRaster : public sla::RasterBase {
		...
		public:
			SVGRaster(const BoundingBox &svgarea, sla::Resolution res, Trafo tr = {}) : m_bb{svgarea}, m_res{res}, m_trafo{tr}, m_sc{double(m_res.width_px) / m_bb.size().x(), double(m_res.height_px) / m_bb.size().y()} {
				...
				m_svg = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
				        ...
				        "<svg height=\"" + hf + "mm" + "\" width=\"" + wf + "mm" + "\" viewBox=\"0 0 " + w + " " + h + "\" style=\"fill: white; stroke: none; fill-rule: nonzero\" "
			...
			void draw(const ExPolygon &poly) override {
				...
				double tol = std::min(m_bb.size().x() / double(m_res.width_px), m_bb.size().y() / double(m_res.height_px));
	...
	void SL1_SVGArchive::export_print(const std::string fname, const SLAPrint &print, const ThumbnailsList &thumbnails, const std::string &projectname) {
	...
	ConfigSubstitutions SL1_SVGReader::read(std::vector<ExPolygons> &slices, DynamicPrintConfig &profile_out) {

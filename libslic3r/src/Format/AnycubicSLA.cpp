...
namespace Slic3r {
	...
	namespace {
		...
		template<class T>
		void crop_value(T &val, T val_min, T val_max) {
			if (val < val_min) {
				val = val_min;
			} else if (val > val_max) {
				val = val_max;
		...
		void fill_preview(anycubicsla_format_preview &p, anycubicsla_format_misc &/*m*/, const ThumbnailsList &thumbnails) {
		...
		void fill_header(anycubicsla_format_header &h, anycubicsla_format_misc &m, const SLAPrint &print, std::uint32_t layer_count) {
			...
			mat_cfg.load_from_ini_string(mnotes, ForwardCompatibilitySubstitutionRule::Enable);
			...
			if (mat_cfg.has(CFG_BOTTOM_LIFT_DISTANCE)) {
				m.bottom_lift_distance_mm = get_cfg_value_f(mat_cfg, CFG_BOTTOM_LIFT_DISTANCE, 8.0f);
				crop_value(h.lift_distance_mm, 0.0f, 100.0f);
			} else {
			...
			h.print_time_s = (h.bottom_layer_count * h.bottom_exposure_time_s) + ((layer_count - h.bottom_layer_count) * h.exposure_time_s) + (layer_count * h.lift_distance_mm / h.retract_speed_mms) + (layer_count * h.lift_distance_mm / h.lift_speed_mms) + (layer_count * h.delay_before_exposure_s);
	...
	std::unique_ptr<sla::RasterBase> AnycubicSLAArchive::create_raster() const {
		...
		sla::RasterBase::Orientation orientation = ro == sla::RasterBase::roPortrait ? sla::RasterBase::roPortrait : sla::RasterBase::roLandscape;
	...
	void AnycubicSLAArchive::export_print(const std::string fname, const SLAPrint &print, const ThumbnailsList &thumbnails, const std::string &/*projectname*/) {
		...
		intro.image_data_offset = intro.layer_data_offset + sizeof(layers_header) + (sizeof(anycubicsla_format_layer) * layer_count);
		...
		try {
			...
			layers_header.payload_size = intro.image_data_offset - intro.layer_data_offset - sizeof(layers_header.tag) - sizeof(layers_header.payload_size);

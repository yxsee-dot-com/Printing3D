...
namespace Slic3r {
	...
	bool load_step(const char *path, Model *model /*BBS:, ImportStepProgressFn proFn*/) {
		...
		assert(boost::algorithm::iends_with(occt_object.object_name, ".stp") || boost::algorithm::iends_with(occt_object.object_name, ".step"));
		...
		for (size_t i = 0; i < occt_object.volumes.size(); ++i) {
			...
			new_volume->name = occt_object.volumes[i].volume_name.empty() ? std::string("Part") + std::to_string(i + 1) : occt_object.volumes[i].volume_name;

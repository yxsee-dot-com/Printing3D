...
namespace ObjParser {
	...
	// https://github.com/prusa3d/Printing3D/issues/10380.
...
#define EATWS() while (*line == ' ' || *line == '\t') ++ line
		...
		try {
			...
			while ((len = ::fread(buf + lenPrev, 1, half_buf - 1, pFile)) != 0) {
				if (std::feof(pFile)) {
					...
					// https://github.com/prusa3d/Printing3D/issues/12157
	...
	bool objbinload(const char *path, ObjData &data) {
		...
		bool result = loadvector(pFile, data.coordinates) && loadvector(pFile, data.textureCoordinates) && loadvector(pFile, data.normals) && loadvector(pFile, data.parameters) && loadvector(pFile, data.mtllibs) && loadvectornameidx(pFile, data.usemtls) && loadvectornameidx(pFile, data.objects) && loadvectornameidx(pFile, data.groups) && loadvector(pFile, data.smoothingGroups) && loadvector(pFile, data.vertices);
	...
	extern bool objequal(const ObjData &data1, const ObjData &data2) {
		...
		return vectorequal(data1.coordinates, data2.coordinates) && vectorequal(data1.textureCoordinates, data2.textureCoordinates) && vectorequal(data1.normals, data2.normals) && vectorequal(data1.parameters, data2.parameters) && vectorequal(data1.mtllibs, data2.mtllibs) && vectorequal(data1.usemtls, data2.usemtls) && vectorequal(data1.objects, data2.objects) && vectorequal(data1.groups, data2.groups) && vectorequal(data1.vertices, data2.vertices);
	}

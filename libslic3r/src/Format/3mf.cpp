...
// 0 : 3MF files saved by older Printing3D or the painting gizmo wasn't used. No version definition in them.
...
namespace Slic3r {
    ...
    class _3MF_Importer : public _3MF_Base
    {
        ...
        // Semantic version of Printing3D, that generated this 3MF.
    ...
    bool _3MF_Importer::_load_model_from_file(const std::string& filename, Model& model, DynamicPrintConfig& config, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        if (m_version == 0) {
            // if the 3mf was not produced by Printing3D and there is more than one instance,
        ...
        const std::string& archive_filename)
    {
        if (stat.m_uncomp_size > 0) {
            ...
            // parsing 3MFs from before Printing3D 2.0.0 (which can have duplicated entries in the INI.
            // See https://github.com/prusa3d/Printing3D/issues/7155. We'll revert it for now.
        ...
        if (stat.m_uncomp_size > 0) {
            ...
            for (const auto& code : code_tree) {
                ...
                if (attr_tree.find("type") == attr_tree.not_found()) {
                    // It means that data was saved in old version (2.2.0 and older) of Printing3D
                    // read old data ... 
    ...
    bool _3MF_Importer::_handle_end_model()
    {
        ...
        if (m_version == 0) {
            // if the 3mf was not produced by Printing3D and there is only one object,
    ...
    bool _3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        // leading to https://github.com/prusa3d/Printing3D/issues/12502. Let's try to load both keys if the usual
        // one that Printing3D uses is not present.
    ...
    bool _3MF_Importer::_handle_end_metadata()
    {
        if (m_curr_metadata_name == SLIC3RPE_3MF_VERSION) {
            ...
            if (boost::starts_with(m_curr_characters, "Printing3D-"))
            ...
            check_painting_version(m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
                _u8L("The selected 3MF contains FDM supports painted object using a newer version of Printing3D and is not compatible."));
        } else if (m_curr_metadata_name == SLIC3RPE_SEAM_PAINTING_VERSION) {
            m_seam_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_seam_painting_version, SEAM_PAINTING_VERSION,
                _u8L("The selected 3MF contains seam painted object using a newer version of Printing3D and is not compatible."));
        } else if (m_curr_metadata_name == SLIC3RPE_MM_PAINTING_VERSION) {
            m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_mm_painting_version, MM_PAINTING_VERSION,
                _u8L("The selected 3MF contains multi-material painted object using a newer version of Printing3D and is not compatible."));
        }
    ...
    bool _3MF_Importer::_handle_start_config_object(const char** attributes, unsigned int num_attributes)
    {
        ...
        // Added because of github #3435, currently not used by Printing3D
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            if (m_prusaslicer_generator_version && 
                ...
                *m_prusaslicer_generator_version < *Semver::parse("2.4.0-alpha3"))
                // Printing3D 2.4.0-alpha2 contained a bug, where all vertices of a single object were saved for each volume the object contained.
            ...
            if (m_version == 0) {
                // if the 3mf was not produced by Printing3D and there is only one instance,
    ...
    bool _3MF_Exporter::_save_model_to_file(const std::string& filename, Model& model, const DynamicPrintConfig* config, const ThumbnailData* thumbnail_data)
    {
        ...
        // The content of this file is the same for each Printing3D 3mf.
    ...
    bool _3MF_Exporter::_add_model_config_file_to_archive(mz_zip_archive& archive, const Model& model, const IdToObjectDataMap &objects_data)
    {
        ...
        for (const IdToObjectDataMap::value_type& obj_metadata : objects_data) {
            ...
            // Output of instances count added because of github #3435, currently not used by Printing3D
...
bool _3MF_Exporter::_add_custom_gcode_per_print_z_file_to_archive( mz_zip_archive& archive, Model& model, const DynamicPrintConfig* config)
{
    ...
    if (!model.custom_gcode_per_print_z.gcodes.empty()) {
        ...
        for (const CustomGCode::Item& code : model.custom_gcode_per_print_z.gcodes) {
            ...
            // add gcode field data for the old version of the Printing3D
...
static void handle_legacy_project_loaded(unsigned int version_project_file, DynamicPrintConfig& config, const boost::optional<Semver>& prusaslicer_generator_version)
{
    if (! config.has("brim_separation")) {
        if (auto *opt_elephant_foot   = config.option<ConfigOptionFloat>("elefant_foot_compensation", false); opt_elephant_foot) {
            // Conversion from older Printing3D which applied brim separation equal to elephant foot compensation.
    ...
    // In Printing3D 2.5.0-alpha2 and 2.5.0-alpha3, we introduce several parameters for Arachne that depend

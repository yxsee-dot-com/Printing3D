...
namespace Slic3r {
	...
	struct ExtrusionAttributes : ExtrusionFlow {
		...
		ExtrusionAttributes(ExtrusionRole role, const ExtrusionFlow &flow, const bool maybe_self_crossing) : role{role}, ExtrusionFlow{flow}, maybe_self_crossing(maybe_self_crossing) {}
	...
	inline bool operator==(const ExtrusionAttributes &lhs, const ExtrusionAttributes &rhs) {
		return static_cast<const ExtrusionFlow &>(lhs) == static_cast<const ExtrusionFlow &>(rhs) && lhs.role == rhs.role && lhs.overhang_attributes == rhs.overhang_attributes;
	}
	...
	class ExtrusionMultiPath : public ExtrusionEntity {
		public:
			...
			void collect_polylines(Polylines &dst) const override {
				...
				if (!pl.empty()) dst.emplace_back(std::move(pl));
			}
			...
			double total_volume() const override {
				double volume = 0.;
				for (const auto &path: paths) volume += path.total_volume();
				return volume;
			}
	};
	...
	class ExtrusionLoop : public ExtrusionEntity {
		public:
			...
			ExtrusionLoop(ExtrusionPath &&path, ExtrusionLoopRole role = elrDefault) : m_loop_role(role) { this->paths.emplace_back(std::move(path)); }
	...
	inline void extrusion_entities_append_paths(ExtrusionEntitiesPtr &dst, Polylines &&polylines, const ExtrusionAttributes &attributes, bool can_reverse = true) {
		...
		for (Polyline &polyline: polylines)
			if (polyline.is_valid())
				dst.emplace_back(can_reverse ? new ExtrusionPath(std::move(polyline), attributes) : new ExtrusionPathOriented(std::move(polyline), attributes));
		polylines.clear();
	}
	...
	inline void extrusion_entities_append_loops_and_paths(ExtrusionEntitiesPtr &dst, Polylines &&polylines, const ExtrusionAttributes &attributes) {
		...
		for (Polyline &polyline: polylines)
			if (polyline.is_valid())
				dst.emplace_back(polyline.is_closed() ? static_cast<ExtrusionEntity *>(new ExtrusionLoop(ExtrusionPath{std::move(polyline), attributes})) : static_cast<ExtrusionEntity *>(new ExtrusionPath(std::move(polyline), attributes)));
		polylines.clear();
	}

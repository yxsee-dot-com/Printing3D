...
namespace Slic3r {
	...
	// The original infill polygon, not inflated.
			...
			lower_slices(_lower_slices), spacing(_spacing) {
		...
		for (size_t i_angle = 0; i_angle < candidates.size(); ++i_angle) {
			...
			Lines lines;
			{
				...
				for (coord_t y = bbox.min(1); y <= bbox.max(1); y += this->spacing)
					lines.push_back(Line(Point((coord_t) round(c * bbox.min(0) - s * y), (coord_t) round(c * y + s * bbox.min(0))), Point((coord_t) round(c * bbox.max(0) - s * y), (coord_t) round(c * y + s * bbox.max(0)))));
			}

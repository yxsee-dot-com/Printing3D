	...
	namespace __impl {
		...
		class Grid {
				...
				uint8_t get_tag_for_cell(const Coord &cell, TRasterValue<Rst> v) {
					...
					uint8_t t = ((is_within(sqr[0]) && isoval(*m_rst, sqr[0]) >= v)) + ((is_within(sqr[1]) && isoval(*m_rst, sqr[1]) >= v) << 1) + ((is_within(sqr[2]) && isoval(*m_rst, sqr[2]) >= v) << 2) + ((is_within(sqr[3]) && isoval(*m_rst, sqr[3]) >= v) << 3);
				...
				bool is_visited(size_t idx, Dir d = Dir::none) const {
					...
					return t == SquareTag::full || t == SquareTag::none || ((m_tags[idx] & 0xf0) >> 4) == ref;
				}
				...
				Edge _edge(const Coord &ringvertex) const {
					...
					switch (t) {
						...
						case SquareTag::ab:
						case SquareTag::abc:
							return {{tl(cell), Dir::down, m_rst},
							        {bl(cell)}};
						case SquareTag::b:
						case SquareTag::bc:
						case SquareTag::bcd:
							return {{bl(cell), Dir::right, m_rst},
							        {br(cell)}};
						case SquareTag::c:
							return {{br(cell), Dir::up, m_rst},
							        {tr(cell)}};
						case SquareTag::ac:
			...
			public:
				explicit Grid(const Rst &rst, const Coord &cellsz, const Coord &overlap) : m_rst{&rst}, m_cellsize{cellsz}, m_res_1{m_cellsize.r - 1, m_cellsize.c - 1}, m_window{overlap.r < cellsz.r ? cellsz.r - overlap.r : cellsz.r, overlap.c < cellsz.c ? cellsz.c - overlap.c : cellsz.c}, m_gridsize{2 + (long(rows(rst)) - overlap.r) / m_window.r, 2 + (long(cols(rst)) - overlap.c) / m_window.c}, m_tags(m_gridsize.r * m_gridsize.c, 0) {}
				...
				void tag_grid(ExecutionPolicy &&policy, TRasterValue<Rst> isoval) {
					// parallel for r
					for_each(std::forward<ExecutionPolicy>(policy), m_tags.begin(), m_tags.end(), [this, isoval](uint8_t &tag, size_t idx) {

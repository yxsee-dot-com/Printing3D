...
namespace Slic3r {
	namespace astar {
		...
		struct QNode // Queue node. Keeps track of scores g, and h
		{
			...
			QNode(TracerNodeT<Tracer> n = {}, size_t p = Unassigned, float gval = std::numeric_limits<float>::infinity(), float hval = 0.f) : node{std::move(n)}, parent{p}, queue_id{InvalidQueueID}, g{gval}, h{hval} {}
		};
		...
		bool search_route(const Tracer &tracer, const TracerNodeT<Tracer> &source, It out, NodeMap &&cached_nodes = {}) {
			...
			auto qopen = make_mutable_priority_queue<size_t, true>([&cached_nodes](size_t el, size_t qidx) {

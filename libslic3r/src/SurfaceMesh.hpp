...
namespace Slic3r {
	...
	class SurfaceMesh {
		public:
			explicit SurfaceMesh(const indexed_triangle_set &its) : m_its(its), m_face_neighbors(its_face_neighbors_par(its)) {}
			...
			Vertex_index source(Halfedge_index h) const {
				assert(!h.is_invalid());
				return Vertex_index(h.m_face, h.m_side);
			}
			Vertex_index target(Halfedge_index h) const {
				assert(!h.is_invalid());
				return Vertex_index(h.m_face, h.m_side == 2 ? 0 : h.m_side + 1);
			}
			...
			Halfedge_index prev(Halfedge_index h) const {
				assert(!h.is_invalid());
				h.m_side = (h.m_side == 0 ? 2 : h.m_side - 1);
				return h;
			}
			...
			Halfedge_index opposite(Halfedge_index h) const {
				...
				for (int i = 0; i < 3; ++i) {
					if (is_same_vertex(source(h_candidate), target(h))) {
						// Meshes in Printing3D should be fixed enough for the following not to happen.
			...
			Halfedge_index prev_around_target(Halfedge_index h) const {
				Halfedge_index op = opposite(h);
				return (op.is_invalid() ? Halfedge_index() : prev(op));
			}
			Halfedge_index next_around_source(Halfedge_index h) const {
				Halfedge_index op = opposite(h);
				return (op.is_invalid() ? Halfedge_index() : next(op));
			}
			...
			Vec3i get_face_neighbors(Face_index face_id) const {
				assert(int(face_id) < int(m_face_neighbors.size()));
				return m_face_neighbors[face_id];
			}

	...
	namespace FFFTreeSupport {
		...
		class TreeModelVolumes {
			public:
				...
				explicit TreeModelVolumes(const PrintObject &print_object, const BuildVolume &build_volume, coord_t max_move, coord_t max_move_slow, size_t current_mesh_idx,
				...
				coord_t ceilRadius(const coord_t radius, const bool min_xy_dist) const {
					assert(radius >= 0);
					return min_xy_dist ? this->ceilRadius(radius) :
				...
				coord_t getRadiusNextCeil(coord_t radius, bool min_xy_dist) const {
					assert(radius > 0);
					return min_xy_dist ? this->ceilRadius(radius) : this->ceilRadius(radius + m_current_min_xy_dist_delta) - m_current_min_xy_dist_delta;
				}
			...
			private:
				...
				class LayerPolygonCache {
					public:
						...
						Polygons &operator[](LayerIndex idx) {
							assert(idx >= m_idx_begin && idx < m_idx_end);
				...
				class RadiusLayerPolygonCache {
					...
					public:
						...
						RadiusLayerPolygonCache &operator=(RadiusLayerPolygonCache &&rhs) {
					...
					if (to_model) {
						switch (type) {
							case AvoidanceType::Fast:
								return m_avoidance_cache_to_model;
							case AvoidanceType::Slow:
								return m_avoidance_cache_to_model_slow;
							case AvoidanceType::Count:
								assert(false);
							case AvoidanceType::FastSafe:
								return m_avoidance_cache_holefree_to_model;
						}
					} else {
						switch (type) {
							case AvoidanceType::Fast:
							...
							case AvoidanceType::Slow:
								return m_avoidance_cache_slow;
							case AvoidanceType::Count:
								assert(false);
							case AvoidanceType::FastSafe:
								return m_avoidance_cache_holefree;
						}
					}
					assert(false);
					return m_avoidance_cache;

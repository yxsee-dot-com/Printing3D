...
namespace Slic3r {
	...
	class TriangleMeshDataAdapter {
		public:
			...
			TriangleMeshDataAdapter(const indexed_triangle_set &m, const Transform3d tr = Transform3d::Identity()) : its{m}, trafo{tr} {}
	};
	...
	VoxelGridPtr mesh_to_grid(const indexed_triangle_set &mesh, const MeshToGridParams &params) {
		...
		for (auto &m: meshparts) {
			auto subgrid = openvdb::tools::meshToVolume<openvdb::FloatGrid>(interrupter, TriangleMeshDataAdapter{m, trafo}, openvdb::math::Transform{}, params.exterior_bandwidth(), params.interior_bandwidth());
		...
		if (meshparts.empty()) {
			...
			grid = openvdb::tools::meshToVolume<openvdb::FloatGrid>(interrupter, TriangleMeshDataAdapter{mesh, trafo}, openvdb::math::Transform{}, params.exterior_bandwidth(), params.interior_bandwidth());
		}
	...
	VoxelGridPtr dilate_grid(const VoxelGrid &vgrid, float exteriorBandWidth, float interiorBandWidth) {
		...
		if (interiorBandWidth > 0.f)
			new_grid = openvdb::tools::dilateSdf(vgrid.grid, scale * interiorBandWidth, openvdb::tools::NN_FACE_EDGE, DilateIterations, openvdb::tools::FastSweepingDomain::SWEEP_LESS_THAN_ISOVALUE);

		auto &arg = new_grid ? *new_grid : vgrid.grid;

		if (exteriorBandWidth > 0.f)
			new_grid = openvdb::tools::dilateSdf(arg, scale * exteriorBandWidth, openvdb::tools::NN_FACE_EDGE, DilateIterations, openvdb::tools::FastSweepingDomain::SWEEP_GREATER_THAN_ISOVALUE);
	...
	double get_distance_raw(const Vec3f &p, const VoxelGrid &vgrid) {
		...
		auto grididx = vgrid.grid.transform().worldToIndexCellCentered({v.x(), v.y(), v.z()});

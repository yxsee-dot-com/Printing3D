...
#undef MAP_ERR_TO_COND
	...
	// https://github.com/prusa3d/Printing3D/issues/1298
	...
	bool is_gcode_file(const std::string &path) {
		return boost::iends_with(path, ".gcode") || boost::iends_with(path, ".gco") || boost::iends_with(path, ".g") || boost::iends_with(path, ".ngc") || boost::iends_with(path, ".bgcode") || boost::iends_with(path, ".bgc");
	}
...
namespace Slic3r {
	...
	std::string header_slic3r_generated() {
		return std::string("generated by Printing3D " SLIC3R_VERSION " on ") + Utils::utc_timestamp();
	}

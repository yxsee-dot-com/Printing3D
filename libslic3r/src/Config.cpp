...
namespace Slic3r {
	...
	std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const {
		...
		if (this->cli != ConfigOptionDef::nocli) {
			...
			// Neither '=' nor '!' is used in any of the cli parameters currently defined by Printing3D.
	...
	ConfigOption *ConfigOptionDef::create_empty_option() const {
		if (this->nullable) {
			switch (this->type) {
				case coFloat:
					return new ConfigOptionFloatNullable();
				case coInt:
					return new ConfigOptionIntNullable();
				case coFloats:
			...
			switch (this->type) {
				...
				case coEnum:
					return new ConfigOptionEnumGeneric(this->enum_def->m_enum_keys_map);
				case coEnums:
					return new ConfigOptionEnumsGeneric(this->enum_def->m_enum_keys_map);
				default:
	...
	ConfigOption *ConfigOptionDef::create_default_option() const {
		if (this->default_value)
			return (this->default_value->type() == coEnum) ?
			       ...
			       new ConfigOptionEnumGeneric(this->enum_def->m_enum_keys_map, this->default_value->getInt()) : (this->default_value->type() == coEnums) ?
			                                                                                                     // Special case: For a DynamicConfig, convert a templated enums to a generic enums.
			                                                                                                     new ConfigOptionEnumsGeneric(this->enum_def->m_enum_keys_map, this->default_value->getInts()) : this->default_value->clone();
		...
		for (const std::string &category: categories) {
			...
			for (const auto &opt: this->options) {
				...
				for (auto &arg: cli_args) {
					...
					if (def.type == coFloat || def.type == coInt || def.type == coFloatOrPercent || def.type == coFloats || def.type == coInts) {
						arg += " N";
	...
	void ConfigBase::set(const std::string &opt_key, int value, bool create) {
		...
		switch (opt->type()) {
			case coInt:
				static_cast<ConfigOptionInt *>(opt)->value = value;
				break;
			case coFloat:
				static_cast<ConfigOptionFloat *>(opt)->value = value;
				break;
			case coFloatOrPercent:
				...
				break;
			case coString:
				static_cast<ConfigOptionString *>(opt)->value = std::to_string(value);
				break;
			default:
	...
	void ConfigBase::set(const std::string &opt_key, double value, bool create) {
		...
		switch (opt->type()) {
			...
			case coFloatOrPercent:
				...
				break;
			case coString:
				static_cast<ConfigOptionString *>(opt)->value = float_to_string_decimal_point(value);
				break;
			default:
	...
	bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext &substitutions_ctxt, bool append) {
		...
		if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
			//FIXME Special handling of vectors of bools, quick and not so dirty solution before Printing3D 2.3.2 release.
			...
			auto result = nullable ? static_cast<ConfigOptionBoolsNullable *>(opt)->deserialize_with_substitutions(value, append, default_value) : static_cast<ConfigOptionBools *>(opt)->deserialize_with_substitutions(value, append, default_value);
		...
		if (substituted && (substitutions_ctxt.rule == ForwardCompatibilitySubstitutionRule::Enable || substitutions_ctxt.rule == ForwardCompatibilitySubstitutionRule::EnableSystemSilent)) {
	...
	double ConfigBase::get_abs_value(const t_config_option_key &opt_key) const {
		...
		if (raw_opt->type() == coFloatOrPercent) {
			...
			return opt_def->ratio_over.empty() ? 0. : static_cast<const ConfigOptionFloatOrPercent *>(raw_opt)->get_abs_value(this->get_abs_value(opt_def->ratio_over));
		}
	...
	ConfigSubstitutions ConfigBase::load(const std::string &filename, ForwardCompatibilitySubstitutionRule compatibility_rule) {
		...
		switch (file_type) {
			case EFileType::Ini: {
				return this->load_from_ini(filename, compatibility_rule);
			}
			case EFileType::AsciiGCode: {
				return this->load_from_gcode_file(filename, compatibility_rule);
			}
			case EFileType::BinaryGCode: {
				return this->load_from_binary_gcode_file(filename, compatibility_rule);
			}
			default: {
				throw Slic3r::RuntimeError(format("Invalid file %1%", filename));
			}
		}
	}
	...
	ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule) {
		...
		// Also the "; generated by Printing3D ..." comment line will be removed.
		size_t j = 0;
	...
	ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &filename, ForwardCompatibilitySubstitutionRule compatibility_rule) {
		...
		// Look for Slic3r or Printing3D header.
		...
		bool has_delimiters = false;
		{
			...
			static constexpr const char prusaslicer_gcode_header[] = "; generated by Printing3D ";
			std::string header;
			...
			if (!header_found)
				throw Slic3r::RuntimeError("Not a Printing3D / Slic3r PE generated g-code.");
		}
		...
		if (has_delimiters) {
			// Printing3D starting with 2.4.0-alpha0 delimits the config section stored into G-code with
			...
			// Slic3r or Printing3D older than 2.4.0-alpha0 do not emit any delimiter.
	...
	bool DynamicConfig::equals(const DynamicConfig &other) const {
		return !dynamic_config_iterate(*this, other, [](const t_config_option_key & /* key */, const ConfigOption *l, const ConfigOption *r) { return *l != *r; });
	}

...
#include <SLA/ZCorrection.hpp>
#include <ElephantFootCompensation.hpp>
#include <CSGMesh/ModelToCSGMesh.hpp>
#include <CSGMesh/SliceCSGMesh.hpp>
...
namespace Slic3r {
	...
	namespace {
		...
		const std::array<unsigned, slaposCount> OBJ_STEP_LEVELS = {13, // slaposAssembly
		                                                           13, // slaposHollowing,
		                                                           13, // slaposDrillHoles
		...
		std::string OBJ_STEP_LABELS(size_t idx) {
			switch (idx) {
				...
				case slaposAssembly:
					return _u8L("Assembling model from parts");
				case slaposHollowing:
					return _u8L("Hollowing model");
				case slaposDrillHoles:
					return _u8L("Drilling holes into model.");
		...
		const std::array<unsigned, slapsCount> PRINT_STEP_LEVELS = {10, // slapsMergeSlicesAndEval
		                                                            90, // slapsRasterize
		};
	...
	SLAPrint::Steps::Steps(SLAPrint *print) : m_print{print}, objcount{m_print->m_objects.size()}, ilhd{m_print->m_material_config.initial_layer_height.getFloat()}, ilh{float(ilhd)}, ilhs{scaled(ilhd)}, objectstep_scale{(max_objstatus - min_objstatus) / (objcount * 100.0)} {}
	...
	void SLAPrint::Steps::apply_printer_corrections(SLAPrintObject &po, SliceOrigin o) {
		...
		if (o == soModel) { // Z correction applies only to the model slices
			slices = sla::apply_zcorrection(slices, m_print->m_material_config.zcorrection_layers.getInt());
		}
	}

	indexed_triangle_set SLAPrint::Steps::generate_preview_vdb(SLAPrintObject &po, SLAPrintObjectStep step) {
		// Empirical upper limit to not get excessive performance hit
		constexpr double MaxPreviewVoxelScale = 12.;

		// update preview mesh
		double vscale = std::min(MaxPreviewVoxelScale, 1. / po.m_config.layer_height.getFloat());

		auto voxparams = csg::VoxelizeParams{}.voxel_scale(vscale).exterior_bandwidth(1.f).interior_bandwidth(1.f);
	...
	void SLAPrint::Steps::generate_preview(SLAPrintObject &po, SLAPrintObjectStep step) {
		...
		if (is_all_positive(r)) {
			...
			bool can_hollow = po.m_hollowing_data && po.m_hollowing_data->interior && !sla::get_mesh(*po.m_hollowing_data->interior).empty();

			bool hole_fail = false;
			if (step == slaposHollowing && is_pure_model) {
				if (can_hollow) {
					m = csgmesh_merge_positive_parts(r);
					sla::hollow_mesh(m, *po.m_hollowing_data->interior, sla::hfRemoveInsideTriangles);
				}
				...
				if (po.m_model_object->sla_drain_holes.empty()) {
					...
					auto ret = sla::hollow_mesh_and_drill(m, *po.m_hollowing_data->interior, drainholes, [/*&po, &drainholes, */&hole_fail](size_t i) {
					...
					if (ret & static_cast<int>(sla::HollowMeshResult::FaultyMesh)) {
						po.active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, _u8L("Mesh to be hollowed is not suitable for hollowing (does not "
						                                                                            "bound a volume)."));
					}

					if (ret & static_cast<int>(sla::HollowMeshResult::FaultyHoles)) {
						po.active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, _u8L("Unable to drill the current configuration of holes into the "
						                                                                            "model."));
					}
					...
					if (ret & static_cast<int>(sla::HollowMeshResult::DrillingFailed)) {
						po.active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, _u8L("Drilling holes into the mesh failed. "
					...
					if (hole_fail) {
						po.active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, _u8L("Failed to drill some holes into the model"));
		...
		if (!handled) { // Last resort to voxelization.
			po.active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, _u8L("Some parts of the print will be previewed with approximated meshes. "
	...
	static inline void clear_csg(std::multiset<CSGPartForStep> &s, SLAPrintObjectStep step) {
	...
	void SLAPrint::Steps::mesh_assembly(SLAPrintObject &po) {
		...
		csg::model_to_csgmesh(*po.model_object(), po.trafo(), csg_inserter{po.m_mesh_to_slice, slaposAssembly}, csg::mpartsPositive | csg::mpartsNegative | csg::mpartsDoSplits);
	...
	void SLAPrint::Steps::hollow_model(SLAPrintObject &po) {
		...
		if (!interior || sla::get_mesh(*interior).empty())
			...
			po.m_mesh_to_slice.emplace(slaposHollowing, csg::CSGPart{std::make_shared<indexed_triangle_set>(m), csg::CSGType::Difference});
	...
	void SLAPrint::Steps::drill_holes(SLAPrintObject &po) {
		...
		csg::model_to_csgmesh(*po.model_object(), po.trafo(), csg_inserter{po.m_mesh_to_slice, slaposDrillHoles}, csg::mpartsDrillHoles);
	...
	template<class Pred>
	static std::vector<ExPolygons> slice_volumes(const ModelVolumePtrs &volumes, const std::vector<float> &slice_grid, const Transform3d &trafo, const MeshSlicingParamsEx &slice_params, Pred &&predicate) {
	...
	template<class Cont>
	BoundingBoxf3 csgmesh_positive_bb(const Cont &csg) {
	...
	void SLAPrint::Steps::slice_model(SLAPrintObject &po) {
		...
		assert(po.m_mesh_to_slice.empty() || csg::get_operation(*po.m_mesh_to_slice.begin()) == csg::CSGType::Union);
		...
		switch (po.config().slicing_mode.value) {
			...
			case SlicingMode::EvenOdd:
				params.mode = MeshSlicingParams::SlicingMode::EvenOdd;
				break;
	...
	static void filter_support_points_by_modifiers(sla::SupportPoints &pts, const SuppPtMask &mask, const std::vector<float> &slice_grid) {
		assert((mask.blockers.empty() || mask.blockers.size() == slice_grid.size()) && (mask.enforcers.empty() || mask.enforcers.size() == slice_grid.size()));
		...
		for (size_t i = 0; i < pts.size(); ++i) {
			...
			if (it != slice_grid.end()) {
				...
				if (idx < mask.enforcers.size()) {
					for (size_t enf_idx = 0; !is_enforced && enf_idx < mask.enforcers[idx].size(); ++enf_idx) {
				...
				if (!is_enforced) {
					if (!mask.enforcers_only) {
						if (idx < mask.blockers.size()) {
							for (size_t blk_idx = 0; !is_blocked && blk_idx < mask.blockers[idx].size(); ++blk_idx) {
	...
	void SLAPrint::Steps::support_points(SLAPrintObject &po) {
		...
		if (!po.m_supportdata) {
			auto &meshp = po.get_mesh_to_print();
			assert(meshp);
			po.m_supportdata = std::make_unique<SLAPrintObject::SupportData>(*meshp);
		}

		po.m_supportdata->input.zoffset = csgmesh_positive_bb(po.m_mesh_to_slice).min.z();
		...
		if (mo.sla_points_status != sla::PointsStatus::UserModified) {
			...
			throw_if_canceled();
			sla::SupportPointGenerator auto_supports(po.m_supportdata->input.emesh, po.get_model_slices(), heights, config, [this]() { throw_if_canceled(); }, statuscb);
			...
			MeshSlicingParamsEx params;
			params.closing_radius = float(po.config().slice_closing_radius.value);
			std::vector<ExPolygons> blockers = slice_volumes(po.model_object()->volumes, po.m_model_height_levels, po.trafo(), params, [](const ModelVolume *vol) {
				return vol->is_support_blocker();
			});

			std::vector<ExPolygons> enforcers = slice_volumes(po.model_object()->volumes, po.m_model_height_levels, po.trafo(), params, [](const ModelVolume *vol) {
			...
			BOOST_LOG_TRIVIAL(debug)
				<< "Automatic support points: " << po.m_supportdata->input.pts.size();

			// Using RELOAD_SLA_SUPPORT_POINTS to tell the Plater to pass
			// the update status to GLGizmoSlaSupports
			report_status(-1, _u8L("Generating support points"), SlicingStatus::RELOAD_SLA_SUPPORT_POINTS);
		} else {
	...
	void SLAPrint::Steps::support_tree(SLAPrintObject &po) {
		...
		if (is_zero_elevation(po.config())) {
			remove_bottom_points(po.m_supportdata->input.pts, float(po.m_supportdata->input.zoffset + EPSILON));
		}
		...
		BOOST_LOG_TRIVIAL(debug) << "Processed support point count " << po.m_supportdata->input.pts.size();
	...
	void SLAPrint::Steps::generate_pad(SLAPrintObject &po) {
		...
		if (po.m_config.pad_enable.getBool()) {
			...
			if (!validate_pad(po.m_supportdata->pad_mesh.its, pcfg))
				throw Slic3r::SlicingError(_u8L("No pad can be generated for this model with the "
	...
	void SLAPrint::Steps::slice_supports(SLAPrintObject &po) {
		...
		if (sd) {
			...
			sd->support_slices = sla::slice(sd->tree_mesh.its, sd->pad_mesh.its, heights, float(po.config().slice_closing_radius.value), ctl);
		}
		...
		for (SLAPrintObject *o: m_print->m_objects) {
			...
			for (const SliceRecord &slicerecord: o->get_slice_index()) {
				if (!slicerecord.is_valid())
					throw Slic3r::SlicingError(_u8L("There are unprintable objects. Try to "
	...
	static int count_move_time(const std::string &axis_name, double length, int steprate) {
		...
		int l = int(length);
		return axis_name == "tower" ? Ms((int(l / (steprate * delay) + (steprate + l) / tower_comp_factor) + 1) * (delay * 1000)) : Ms((int(l / (steprate * delay) + tilt_comp_factor) + 1) * (delay * 1000));
	}
	...
	struct ExposureProfile {
		...
		std::map<TowerSpeeds, int> tower_speeds = {{tsLayer1,  800},
		                                           {tsLayer2,  1600},
		                                           {tsLayer3,  2400},
		                                           {tsLayer4,  3200},
		                                           {tsLayer5,  4000},
		...
		std::map<TiltSpeeds, int> tilt_speeds = {{tsMove120,   120},
		                                         {tsLayer200,  200},
		                                         {tsMove300,   300},
		                                         {tsLayer400,  400},
		                                         {tsLayer600,  600},
	...
	static int layer_peel_move_time(int layer_height_nm, ExposureProfile p) {
		...
		if (p.use_tilt) {
			...
			tilt += count_move_time("tilt", p.tilt_down_offset_steps, p.tilt_down_initial_speed);
			// initial down delay
			tilt += p.tilt_down_offset_delay_ms + sleep_delay;
			// profile change delay if down finish profile is different from down initial
			tilt += profile_change_delay;
			// cycle down movement
			tilt += p.tilt_down_cycles * count_move_time("tilt", int((tiltHeight - p.tilt_down_offset_steps) / p.tilt_down_cycles), p.tilt_down_finish_speed);
			// cycle down delay
			...
			tilt += profile_change_delay;
			// initial up movement
			tilt += count_move_time("tilt", tiltHeight - p.tilt_up_offset_steps, p.tilt_up_initial_speed);
			// initial up delay
			tilt += p.tilt_up_offset_delay_ms + sleep_delay;
			// profile change delay if up initial profile is different from down finish
			tilt += profile_change_delay;
			// finish up movement
			tilt += p.tilt_up_cycles * count_move_time("tilt", int(p.tilt_up_offset_steps / p.tilt_up_cycles), p.tilt_up_finish_speed);
			// cycle down delay
		...
		if (p.tower_hop_height_nm > 0) {
			tower += count_move_time("tower", nm_to_tower_microsteps(int(p.tower_hop_height_nm) + layer_height_nm), p.tower_speed);
			tower += count_move_time("tower", nm_to_tower_microsteps(int(p.tower_hop_height_nm)), p.tower_speed);
			tower += profile_change_delay;
		} else {
			tower += count_move_time("tower", nm_to_tower_microsteps(layer_height_nm), p.tower_speed);
			tower += profile_change_delay;
		}
	...
	void SLAPrint::Steps::merge_slices_and_eval_stats() {
		...
		auto printlayerfn = [this,
			...
			size_t c = std::accumulate(layer.slices().begin(), layer.slices().end(), size_t(0), [](size_t a, const SliceRecord &sr) {
				return a + sr.get_slice(soModel).size();
			});

			model_polygons.reserve(c);

			c = std::accumulate(layer.slices().begin(), layer.slices().end(), size_t(0), [](size_t a, const SliceRecord &sr) {
			...
			if (is_prusa_print) {
				...
				layer_times = layer_peel_move_time(l_height_nm, is_fast_layer ? below : above) + (is_fast_layer ? below : above).delay_before_exposure_ms + (is_fast_layer ? below : above).delay_after_exposure_ms + refresh_delay_ms * 5 +                  // ~ 5x frame display wait
				...
				const double tilt_time = material_config.material_print_speed == slamsSlow ? slow_tilt : material_config.material_print_speed == slamsHighViscosity ? hv_tilt : is_fast_layer ? fast_tilt : slow_tilt;
				...
				layer_times += (l_height * 5  // tower move
		...
		execution::for_each(ex_tbb, size_t(0), printer_input.size(), printlayerfn, execution::max_concurrency(ex_tbb));
	...
	void SLAPrint::Steps::rasterize() {
		...
		m_print->m_archiver->draw_layers(m_print->m_printer_input.size(), lvlfn, [this]() { return canceled(); }, ex_tbb);
	}

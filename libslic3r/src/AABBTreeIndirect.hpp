	...
	namespace AABBTreeIndirect {
		...
		class Tree {
			public:
				...
				enum : size_t {
					// Node is not used.
					npos = size_t(-1), // Inner node (not leaf).
					inner = size_t(-2)
				};
		...
		class BoundingBoxWrapper {
			public:
				...
				BoundingBoxWrapper(const size_t idx, const Slic3r::BoundingBox &bbox) : m_idx(idx),
		...
		namespace detail {
			...
			inline bool ray_box_intersect_invdir(const Eigen::MatrixBase<Derivedsource> &origin, const Eigen::MatrixBase<Deriveddir> &inv_dir, Eigen::AlignedBox<Scalar, 3> box, const Scalar &t0, const Scalar &t1) {
			...
			std::enable_if_t<std::is_same<typename V::Scalar, double>::value && std::is_same<typename W::Scalar, double>::value, bool> intersect_triangle(const V &orig, const V &dir, const W &vert0, const W &vert1, const W &vert2, double &t, double &u, double &v, double eps) {
			...
			template<typename V, typename W>
			std::enable_if_t<std::is_same<typename V::Scalar, double>::value && !std::is_same<typename W::Scalar, double>::value, bool> intersect_triangle(const V &origin, const V &dir, const W &v0, const W &v1, const W &v2, double &t, double &u, double &v, double eps) {
			...
			static inline bool intersect_ray_recursive_first_hit(RayIntersectorType &ray_intersector, size_t node_idx, Scalar min_t, igl::Hit &hit) {
				...
				if (node.is_leaf()) {
					...
					double t, u, v;
					if (intersect_triangle(ray_intersector.origin, ray_intersector.dir, ray_intersector.vertices[face(0)], ray_intersector.vertices[face(1)], ray_intersector.vertices[face(2)], t, u, v, ray_intersector.eps) && t > 0.) {
			...
			static inline void intersect_ray_recursive_all_hits(RayIntersectorType &ray_intersector, size_t node_idx) {
				...
				if (!ray_box_intersect_invdir(ray_intersector.origin, ray_intersector.invdir, node.bbox.template cast<Scalar>(), Scalar(0), std::numeric_limits<Scalar>::infinity()))
					return;
			...
			struct IndexedTriangleSetDistancer {
				...
				inline VectorType closest_point_to_origin(size_t primitive_index, ScalarType &squared_distance) const {
					...
					VectorType closest_point = closest_point_to_triangle<VectorType>(origin, this->vertices[triangle(0)].template cast<ScalarType>(), this->vertices[triangle(1)].template cast<ScalarType>(), this->vertices[triangle(2)].template cast<ScalarType>());
			...
			static inline Scalar squared_distance_to_indexed_primitives_recursive(IndexedPrimitivesDistancerType &distancer, size_t node_idx, Scalar low_sqr_d, Scalar up_sqr_d, size_t &i, Eigen::PlainObjectBase<typename IndexedPrimitivesDistancerType::VectorType> &c) {
			...
			template<typename IndexedPrimitivesDistancerType, typename Scalar>
			static inline void indexed_primitives_within_distance_squared_recurisve(const IndexedPrimitivesDistancerType &distancer, size_t node_idx, Scalar squared_distance_limit, std::vector<size_t> &found_primitives_indices) {
				const auto &node = distancer.tree.node(node_idx);
				assert(node.is_valid());
				if (node.is_leaf()) {
					Scalar sqr_dist;
					...
					if (node_right.bbox.squaredExteriorDistance(distancer.origin) < squared_distance_limit) {
						indexed_primitives_within_distance_squared_recurisve(distancer, right_node_idx, squared_distance_limit, found_primitives_indices);
					}
				}
			}
		...
		inline bool intersect_ray_first_hit(
			...
			auto ray_intersector = detail::RayIntersector<VertexType, IndexedFaceType, TreeType, VectorType>{vertices, faces, tree, origin, dir, VectorType(dir.cwiseInverse()), eps};
			return !tree.empty() && detail::intersect_ray_recursive_first_hit(ray_intersector, size_t(0), std::numeric_limits<Scalar>::infinity(), hit);
		}
		...
		inline bool intersect_ray_all_hits(
				...
				// Epsilon for the ray-triangle intersection, it should be proportional to an average triangle edge length.
				const double eps = 0.000001) {
			auto ray_intersector = detail::RayIntersectorHits<VertexType, IndexedFaceType, TreeType, VectorType>{{vertices, faces, {tree}, origin, dir, VectorType(dir.cwiseInverse()), eps}};
			if (tree.empty()) {
				hits.clear();
			} else {
		...
		inline typename VectorType::Scalar squared_distance_to_indexed_triangle_set(
			...
			return tree.empty() ? Scalar(-1) : detail::squared_distance_to_indexed_primitives_recursive(distancer, size_t(0), Scalar(0), std::numeric_limits<Scalar>::infinity(), hit_idx_out, hit_point_out);
		}
		...
		template<class G>
		auto intersecting(const G &g) { return Intersecting<G>{g}; }
		...
		template<class G>
		auto within(const G &g) { return Within<G>{g}; }
			...
			bool traverse_recurse(const Tree<Dims, T> &tree, size_t idx, Pred &&pred, Fn &&callback) {
				...
				if (tree.node(idx).is_leaf()) {
					...
					auto trv = [&](size_t idx) -> bool {
						return traverse_recurse(tree, idx, std::forward<Pred>(pred), std::forward<Fn>(callback));
					};

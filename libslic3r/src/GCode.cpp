...
NEXT:;
	...
	std::string OozePrevention::post_toolchange(GCodeGenerator &gcodegen) {
		return (gcodegen.config().standby_temperature_delta.value != 0) ? gcodegen.writer().set_temperature(this->_get_temp(gcodegen), true, gcodegen.writer().extruder()->id()) : std::string();
	}
	...
	int OozePrevention::_get_temp(const GCodeGenerator &gcodegen) const {
		...
		return (gcodegen.layer() == nullptr || gcodegen.layer()->id() == 0 || gcodegen.config().temperature.get_at(gcodegen.writer().extruder()->id()) == 0) ? gcodegen.config().first_layer_temperature.get_at(gcodegen.writer().extruder()->id()) : gcodegen.config().temperature.get_at(gcodegen.writer().extruder()->id());
	}
...
#define EXTRUDER_CONFIG(OPT) m_config.OPT.get_at(m_writer.extruder()->id())
	...
	void GCodeGenerator::PlaceholderParserIntegration::update_from_gcodewriter(const GCodeWriter &writer, const WipeTowerData &wipe_tower_data) {
		...
		if (this->num_extruders > 0) {
			...
			for (const Extruder &e: extruders) {
				...
				if (!wtuf.empty()) {
					auto it = std::lower_bound(wtuf.begin(), wtuf.end(), writer.get_position().z(), [](const auto &a, const float &val) { return a.first < val; });
					if (it == wtuf.end())
						it = wtuf.end() - 1;
    ...
    if (gap_over_supports != 0.) {
		...
		while (idx_object_layer < object.layers().size() || idx_support_layer < object.support_layers().size()) {
			...
			bool has_extrusions = (layer_to_print.object_layer && layer_to_print.object_layer->has_extrusions()) || (layer_to_print.support_layer && layer_to_print.support_layer->has_extrusions());
			...
			if ((layer_to_print.object_layer && layer_to_print.object_layer->has_extrusions())
				...
				double maximal_print_z = (last_extrusion_layer ? last_extrusion_layer->print_z() : 0.) + layer_to_print.layer()->height + std::max(0., extra_gap);
				// Negative support_contact_z is not taken into account, it can result in false positives in cases
				// where previous layer has object extrusions too (https://github.com/prusa3d/Printing3D/issues/2752)
		...
		if (!warning_ranges.empty()) {
			...
			for (i = 0; i < std::min(warning_ranges.size(), size_t(3)); ++i)
				warning += Slic3r::format(_u8L("Empty layer between %1% and %2%."), warning_ranges[i].first, warning_ranges[i].second) + "\n";
	...
	namespace DoExport {
		...
		static void update_print_estimated_stats(const GCodeProcessor &processor, const std::vector<Extruder> &extruders, PrintStatistics &print_statistics) {
			const GCodeProcessorResult &result = processor.get_result();
			print_statistics.estimated_normal_print_time = get_time_dhms(result.print_statistics.modes[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Normal)].time);
			print_statistics.estimated_silent_print_time = processor.is_stealth_time_estimator_enabled() ? get_time_dhms(result.print_statistics.modes[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Stealth)].time) : "N/A";
	...
	GCodeGenerator::GCodeGenerator(const Print *print) : m_origin(Vec2d::Zero()), m_enable_loop_clipping(true), m_enable_cooling_markers(false), m_enable_extrusion_role_markers(false), m_last_processor_extrusion_role(GCodeExtrusionRole::None), m_layer_count(0), m_layer_index(-1), m_layer(nullptr), m_object_layer_over_raft(false), m_volumetric_speed(0), m_last_extrusion_role(GCodeExtrusionRole::None), m_last_width(0.0f), m_brim_done(false), m_second_layer_things_done(false),
	                                                     m_silent_time_estimator_enabled(false), m_print(print) {}
	...
	void GCodeGenerator::do_export(Print *print, const char *path, GCodeProcessorResult *result, ThumbnailsGeneratorCallback thumbnail_cb) {
		...
		if (!validation_res.empty()) {
			...
			print->active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, _u8L("In the custom G-code were found reserved keywords:") + "\n" + reports + _u8L("This may cause problems in g-code visualization and printing time estimation."));
		}
	...
	namespace DoExport {
		static void init_gcode_processor(const PrintConfig &config, GCodeProcessor &processor, bool &silent_time_estimator_enabled) {
			silent_time_estimator_enabled = (config.gcode_flavor == gcfMarlinLegacy || config.gcode_flavor == gcfMarlinFirmware) && config.silent_mode;
			processor.reset();
			...
			for (auto object: print.objects()) {
				for (size_t region_id = 0; region_id < object->num_printing_regions(); ++region_id) {
					...
					for (auto layer: object->layers()) {
						...
						if (region.config().get_abs_value("perimeter_speed") == 0 || region.config().get_abs_value("small_perimeter_speed") == 0 || region.config().get_abs_value("external_perimeter_speed") == 0 || region.config().get_abs_value("bridge_speed") == 0)
							mm3_per_mm.push_back(layerm->perimeters().min_mm3_per_mm());
						if (region.config().get_abs_value("infill_speed") == 0 || region.config().get_abs_value("solid_infill_speed") == 0 || region.config().get_abs_value("top_solid_infill_speed") == 0 || region.config().get_abs_value("bridge_speed") == 0) {
							// Minimal volumetric flow should not be calculated over ironing extrusions.
							// Use following lambda instead of the built-it method.
							// https://github.com/prusa3d/Printing3D/issues/5082
				...
				if (object->config().get_abs_value("support_material_speed") == 0 || object->config().get_abs_value("support_material_interface_speed") == 0)
		...
		static std::string update_print_stats_and_format_filament_stats(const bool has_wipe_tower, const WipeTowerData &wipe_tower_data, const FullPrintConfig &config, const std::vector<Extruder> &extruders, unsigned int initial_extruder_id, int total_toolchanges, PrintStatistics &print_statistics, bool export_binary_data, bgcode::binarize::BinaryData &binary_data) {
...
static inline std::vector<const PrintInstance*> sort_object_instances_by_max_z(const Print &print)
{
	...
	static inline bool arc_welder_enabled(const PrintConfig &print_config) {
		return
			// Enabled
				...
				print_config.max_volumetric_extrusion_rate_slope_negative == 0. && print_config.max_volumetric_extrusion_rate_slope_positive == 0.;
	}

	static inline GCode::SmoothPathCache::InterpolationParameters interpolation_parameters(const PrintConfig &print_config) {
		return {scaled<double>(print_config.gcode_resolution.value), arc_welder_enabled(print_config) ? Geometry::ArcWelder::default_arc_length_percent_tolerance : 0};
	}
	...
	void GCodeGenerator::_do_export(Print &print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb) {
		...
		if (export_to_binary_gcode) {
			...
			if (!thumbnails.empty())
				GCodeThumbnails::generate_binary_thumbnails(thumbnail_cb, binary_data.thumbnails, thumbnails, [&print]() { print.throw_if_canceled(); });
			...
			const std::vector<std::string> keys_to_duplicate = {"printer_model", "filament_type", "filament_abrasive", "nozzle_diameter", "nozzle_high_flow", "bed_temperature", "brim_width", "fill_density", "layer_height", "temperature", "ironing", "support_material", "extruder_colour"};
			assert(std::is_sorted(slicer_metadata.begin(), slicer_metadata.end(), [](const auto &a, const auto &b) { return a.first < b.first; }));
			for (const std::string &key: keys_to_duplicate) {
				auto it = std::lower_bound(slicer_metadata.begin(), slicer_metadata.end(), std::make_pair(key, 0), [](const auto &a, const auto &b) { return a.first < b.first; });
		...
		if (print.config().max_volumetric_extrusion_rate_slope_positive.value > 0 || print.config().max_volumetric_extrusion_rate_slope_negative.value > 0)
		...
		if (!export_to_binary_gcode) {
			...
			if (!thumbnails.empty())
				GCodeThumbnails::export_thumbnails_to_file(thumbnail_cb, thumbnails, [&file](const char *sz) { file.write(sz); }, [&print]() { print.throw_if_canceled(); });
		}
		...
		if (print.config().complete_objects.value) {
			...
			for (; print_object_instance_sequential_active != print_object_instances_ordering.end(); ++print_object_instance_sequential_active) {
				...
				this->process_layers(print, tool_ordering, collect_layers_to_print(object), *print_object_instance_sequential_active - object.instances().data(), smooth_path_cache_global, file);
				++finished_objects;
			...
			if (has_wipe_tower && !layers_to_print.empty()) {
				...
				if (print.config().single_extruder_multi_material_priming) {
					...
					if (print.config().gcode_flavor == gcfMarlinLegacy || print.config().gcode_flavor == gcfMarlinFirmware) {
						...
						// (See https://github.com/prusa3d/Printing3D/issues/5441.)
						if (overlap) {
							print.active_step_add_warning(PrintStateBase::WarningLevel::CRITICAL, _u8L("Your print is very close to the priming regions. "
			...
			// Generate G-code, run the filters (vase mode, cooling buffer), run the G-code analyser
			// and export G-code into file.
			this->process_layers(print, tool_ordering, print_object_instances_ordering, layers_to_print, smooth_path_cache_global, file);
			file.write(m_label_objects.maybe_stop_instance());
			if (m_wipe_tower)
		...
		// Thus the Printing3D generated config will NOT be processed by the G-code post-processor, see GH issue #7952.
		...
		const std::string filament_stats_string_out = DoExport::update_print_stats_and_format_filament_stats(
				// Const inputs
				has_wipe_tower, print.wipe_tower_data(), this->config(), m_writer.extruders(), initial_extruder_id, tool_ordering.toolchanges_count(),
				// Modifies
		...
		if (export_to_binary_gcode) {
			...
			// The delimiters are structured as configuration key / value pairs to be parsable by older versions of Printing3D G-code viewer.
			{
	...
	void GCodeGenerator::smooth_path_interpolate(const ObjectLayerToPrint &object_layer_to_print, const GCode::SmoothPathCache::InterpolationParameters &params, GCode::SmoothPathCache &out) {
	...
	// Generate G-code, run the filters (vase mode, cooling buffer), run the G-code analyser
	// and export G-code into file.
	void GCodeGenerator::process_layers(const Print &print, const ToolOrdering &tool_ordering, const std::vector<const PrintInstance *> &print_object_instances_ordering, const std::vector<std::pair<coordf_t, ObjectsLayerToPrint>> &layers_to_print, const GCode::SmoothPathCache &smooth_path_cache_global, GCodeOutputStream &output_stream) {
		size_t layer_to_print_idx = 0;
		const GCode::SmoothPathCache::InterpolationParameters interpolation_params = interpolation_parameters(print.config());
		const auto smooth_path_interpolator = tbb::make_filter<void, std::pair<size_t, GCode::SmoothPathCache>>(slic3r_tbb_filtermode::serial_in_order, [this, &print, &layers_to_print, &layer_to_print_idx, &interpolation_params](tbb::flow_control &fc) -> std::pair<size_t, GCode::SmoothPathCache> {
		...
		const auto generator = tbb::make_filter<std::pair<size_t, GCode::SmoothPathCache>, LayerResult>(slic3r_tbb_filtermode::serial_in_order, [this, &print, &tool_ordering, &print_object_instances_ordering, &layers_to_print, &smooth_path_cache_global](std::pair<size_t, GCode::SmoothPathCache> in) -> LayerResult {
			...
			if (layer_to_print_idx == layers_to_print.size()) {
				...
				print.throw_if_canceled();
				return this->process_layer(print, layer.second, layer_tools, GCode::SmoothPathCaches{smooth_path_cache_global, in.second}, &layer == &layers_to_print.back(), &print_object_instances_ordering, size_t(-1));
			}
		});
		// The pipeline is variable: The vase mode filter is optional.
		const auto spiral_vase = tbb::make_filter<LayerResult, LayerResult>(slic3r_tbb_filtermode::serial_in_order, [spiral_vase = this->m_spiral_vase.get(), &layers_to_print](LayerResult in) -> LayerResult {
		...
		const auto pressure_equalizer = tbb::make_filter<LayerResult, LayerResult>(slic3r_tbb_filtermode::serial_in_order, [pressure_equalizer = this->m_pressure_equalizer.get()](LayerResult in) -> LayerResult {
			return pressure_equalizer->process_layer(std::move(in));
		});
		const auto cooling = tbb::make_filter<LayerResult, std::string>(slic3r_tbb_filtermode::serial_in_order, [cooling_buffer = this->m_cooling_buffer.get()](LayerResult in) -> std::string {
			if (in.nop_layer_result)
				return in.gcode;

			return cooling_buffer->process_layer(std::move(in.gcode), in.layer_id, in.cooling_buffer_flush);
		});
		const auto find_replace = tbb::make_filter<std::string, std::string>(slic3r_tbb_filtermode::serial_in_order, [find_replace = this->m_find_replace.get()](std::string s) -> std::string {
			return find_replace->process_layer(std::move(s));
		});
		const auto output = tbb::make_filter<std::string, void>(slic3r_tbb_filtermode::serial_in_order, [&output_stream](std::string s) { output_stream.write(s); });
	...
	// Generate G-code, run the filters (vase mode, cooling buffer), run the G-code analyser
	// and export G-code into file.
	void GCodeGenerator::process_layers(const Print &print, const ToolOrdering &tool_ordering, ObjectsLayerToPrint layers_to_print, const size_t single_object_idx, const GCode::SmoothPathCache &smooth_path_cache_global, GCodeOutputStream &output_stream) {
		size_t layer_to_print_idx = 0;
		const GCode::SmoothPathCache::InterpolationParameters interpolation_params = interpolation_parameters(print.config());
		const auto smooth_path_interpolator = tbb::make_filter<void, std::pair<size_t, GCode::SmoothPathCache>>(slic3r_tbb_filtermode::serial_in_order, [this, &print, &layers_to_print, &layer_to_print_idx, interpolation_params](tbb::flow_control &fc) -> std::pair<size_t, GCode::SmoothPathCache> {
			if (layer_to_print_idx >= layers_to_print.size()) {
		...
		const auto generator = tbb::make_filter<std::pair<size_t, GCode::SmoothPathCache>, LayerResult>(slic3r_tbb_filtermode::serial_in_order, [this, &print, &tool_ordering, &layers_to_print, &smooth_path_cache_global, single_object_idx](std::pair<size_t, GCode::SmoothPathCache> in) -> LayerResult {
			size_t layer_to_print_idx = in.first;
			if (layer_to_print_idx == layers_to_print.size()) {
				// Pressure equalizer need insert empty input. Because it returns one layer back.
				// Insert NOP (no operation) layer;
				...
				print.throw_if_canceled();
				return this->process_layer(print, {std::move(layer)}, tool_ordering.tools_for_layer(layer.print_z()), GCode::SmoothPathCaches{smooth_path_cache_global, in.second}, &layer == &layers_to_print.back(), nullptr, single_object_idx);
			}
		});
		// The pipeline is variable: The vase mode filter is optional.
		const auto spiral_vase = tbb::make_filter<LayerResult, LayerResult>(slic3r_tbb_filtermode::serial_in_order, [spiral_vase = this->m_spiral_vase.get(), &layers_to_print](LayerResult in) -> LayerResult {
	...
	std::string GCodeGenerator::placeholder_parser_process(const std::string &name, const std::string &templ, unsigned int current_extruder_id, const DynamicConfig *config_override) {
		...
		try {
			...
			for (const Extruder &e: m_writer.extruders()) {
				...
				if (eid < ppi.num_extruders) {
					...
					if (!is_approx(ppi.e_retracted[eid], ppi.opt_e_retracted->values[eid]) || !is_approx(ppi.e_restart_extra[eid], ppi.opt_e_restart_extra->values[eid]))
	...
	void GCodeGenerator::print_machine_envelope(GCodeOutputStream &file, const Print &print) {
		...
		if ((flavor == gcfMarlinLegacy || flavor == gcfMarlinFirmware || flavor == gcfRepRapFirmware) && print.config().machine_limits_usage.value == MachineLimitsUsage::EmitToGCode) {
			int factor = flavor == gcfRepRapFirmware ? 60 : 1; // RRF M203 and M566 are in mm/min
			file.write_format("M201 X%d Y%d Z%d E%d ; sets maximum accelerations, mm/sec^2\n", int(print.config().machine_max_acceleration_x.values.front() + 0.5), int(print.config().machine_max_acceleration_y.values.front() + 0.5), int(print.config().machine_max_acceleration_z.values.front() + 0.5), int(print.config().machine_max_acceleration_e.values.front() + 0.5));
			file.write_format("M203 X%d Y%d Z%d E%d ; sets maximum feedrates, %s\n", int(print.config().machine_max_feedrate_x.values.front() * factor + 0.5), int(print.config().machine_max_feedrate_y.values.front() * factor + 0.5), int(print.config().machine_max_feedrate_z.values.front() * factor + 0.5), int(print.config().machine_max_feedrate_e.values.front() * factor + 0.5), factor == 60 ? "mm / min" : "mm / sec");

			// Now M204 - acceleration. This one is quite hairy...
			if (flavor == gcfRepRapFirmware)
				// Uses M204 P[print] T[travel]
				file.write_format("M204 P%d T%d ; sets acceleration (P, T), mm/sec^2\n", int(print.config().machine_max_acceleration_extruding.values.front() + 0.5), int(print.config().machine_max_acceleration_travel.values.front() + 0.5));
			else if (flavor == gcfMarlinLegacy)
				// Legacy Marlin uses M204 S[print] T[retract]
				file.write_format("M204 S%d T%d ; sets acceleration (S) and retract acceleration (R), mm/sec^2\n", int(print.config().machine_max_acceleration_extruding.values.front() + 0.5), int(print.config().machine_max_acceleration_retracting.values.front() + 0.5));
			else if (flavor == gcfMarlinFirmware)
				// New Marlin uses M204 P[print] R[retract] T[travel]
				file.write_format("M204 P%d R%d T%d ; sets acceleration (P, T) and retract acceleration (R), mm/sec^2\n", int(print.config().machine_max_acceleration_extruding.values.front() + 0.5), int(print.config().machine_max_acceleration_retracting.values.front() + 0.5), int(print.config().machine_max_acceleration_travel.values.front() + 0.5));
			else
				assert(false);

			assert(is_decimal_separator_point());
			file.write_format(flavor == gcfRepRapFirmware ? "M566 X%.2lf Y%.2lf Z%.2lf E%.2lf ; sets the jerk limits, mm/min\n" : "M205 X%.2lf Y%.2lf Z%.2lf E%.2lf ; sets the jerk limits, mm/sec\n", print.config().machine_max_jerk_x.values.front() * factor, print.config().machine_max_jerk_y.values.front() * factor, print.config().machine_max_jerk_z.values.front() * factor, print.config().machine_max_jerk_e.values.front() * factor);
			if (flavor != gcfRepRapFirmware)
				file.write_format("M205 S%d T%d ; sets the minimum extruding and travel feed rate, mm/sec\n", int(print.config().machine_min_extruding_rate.values.front() + 0.5), int(print.config().machine_min_travel_rate.values.front() + 0.5));
			else {
	...
	std::vector<GCode::InstanceToPrint> GCodeGenerator::sort_print_object_instances(const std::vector<ObjectLayerToPrint> &object_layers,
	...
	namespace ProcessLayer {
		...
		static std::string emit_custom_color_change_gcode_per_print_z(GCodeGenerator &gcodegen, const CustomGCode::Item &custom_gcode, unsigned int current_extruder_id, unsigned int first_extruder_id, // ID of the first extruder printing this layer.
		...
		static std::string emit_custom_gcode_per_print_z(GCodeGenerator &gcodegen, const CustomGCode::Item &custom_gcode, unsigned int current_extruder_id,
	...
	namespace Skirt {
		...
		static std::map<unsigned int, std::pair<size_t, size_t>> make_skirt_loops_per_extruder_1st_layer(const Print &print, const LayerTools &layer_tools,
		...
		static std::map<unsigned int, std::pair<size_t, size_t>> make_skirt_loops_per_extruder_other_layers(const Print &print, const LayerTools &layer_tools,
				// Heights (print_z) at which the skirt has already been extruded.
				                                                                                            std::vector<coordf_t> &skirt_done) {
			// Extrude skirt at the print_z of the raft layers and normal object layers
			// not at the print_z of the interlaced support material layers.
			...
			if (print.has_skirt() && !print.skirt().entities.empty() && layer_tools.has_skirt &&
				...
				// of https://github.com/prusa3d/Printing3D/issues/5652, but it deserves a real fix.
				if (valid) {
#if 0
                ...
                skirt_loops_per_extruder_out[layer_tools.extruders.front()] = std::pair<size_t, size_t>(0, print.config().skirts.value);
					...
					// https://github.com/prusa3d/Printing3D/issues/469#issuecomment-322450619
					skirt_loops_per_extruder_all_printing(print, layer_tools, skirt_loops_per_extruder_out);
#endif
					assert(!skirt_done.empty());
					skirt_done.emplace_back(layer_tools.print_z);
	...
	bool GCodeGenerator::line_distancer_is_required(const std::vector<unsigned int> &extruder_ids) {
		for (const unsigned id: extruder_ids) {
			...
			if (this->m_config.travel_lift_before_obstacle.get_at(id) && this->m_config.travel_max_lift.get_at(id) > 0 && travel_slope > 0 && travel_slope < 90) {
				return true;
			}
		}
		return false;
	...
	GCode::Impl::Travels::ElevatedTravelParams get_ramping_layer_change_params(const Vec3d &from, const Vec3d &to, const Polyline &xy_path, const FullPrintConfig &config, const unsigned extruder_id, const GCode::TravelObstacleTracker &obstacle_tracker) {
		...
		const double lift_at_travel_end = std::min(elevation_params.lift_height, elevation_params.lift_height / elevation_params.slope_end * path_length);
	...
	std::string GCodeGenerator::get_ramping_layer_change_gcode(const Vec3d &from, const Vec3d &to, const unsigned extruder_id) {
		...
		const GCode::Impl::Travels::ElevatedTravelParams elevation_params{get_ramping_layer_change_params(from, to, xy_path, m_config, extruder_id, m_travel_obstacle_tracker)};
		return this->generate_ramping_layer_change_gcode(xy_path, from.z(), elevation_params);
	}

	std::string GCodeGenerator::generate_ramping_layer_change_gcode(const Polyline &xy_path, const double initial_elevation, const GCode::Impl::Travels::ElevatedTravelParams &elevation_params) const {
		using namespace GCode::Impl::Travels;

		const std::vector<double> ensure_points_at_distances = linspace(elevation_params.slope_end - elevation_params.blend_width / 2.0, elevation_params.slope_end + elevation_params.blend_width / 2.0, elevation_params.parabola_points_count);

		Points3 travel{generate_elevated_travel(xy_path.points, ensure_points_at_distances, initial_elevation, ElevatedTravelFormula{elevation_params})};
																															...
																															static inline bool validate_smooth_path(const GCode::SmoothPath &smooth_path, bool loop)
{
	...
	struct SmoothPathGenerator {
		...
		GCode::SmoothPath operator()(const Layer *layer, const ExtrusionEntityReference &extrusion_reference, const unsigned extruder_id, std::optional<InstancePoint> &previous_position) {
			...
			if (auto loop = dynamic_cast<const ExtrusionLoop *>(extrusion_entity)) {
				...
				GCode::SmoothPath smooth_path = smooth_path_cache.resolve_or_fit_split_with_seam(*loop, extrusion_reference.flipped(), scaled_resolution, seam_point, scaled<double>(0.0015));
				...
				if (enable_loop_clipping) {
					clip_end(smooth_path, scale_(nozzle_diameter) * LOOP_CLIPPING_LENGTH_OVER_NOZZLE_DIAMETER, scaled<double>(GCode::ExtrusionOrder::min_gcode_segment_length));
				}
	...
	std::vector<GCode::ExtrusionOrder::ExtruderExtrusions> GCodeGenerator::get_sorted_extrusions(const Print &print, const ObjectsLayerToPrint &layers, const LayerTools &layer_tools, const std::vector<InstanceToPrint> &instances_to_print, const GCode::SmoothPathCaches &smooth_path_caches, const bool first_layer) {
		// Map from extruder ID to <begin, end> index of skirt loops to be extruded with that extruder.
		// Extrude skirt at the print_z of the raft layers and normal object layers
		// not at the print_z of the interlaced support material layers.
		std::map<unsigned int, std::pair<size_t, size_t>> skirt_loops_per_extruder{first_layer ? Skirt::make_skirt_loops_per_extruder_1st_layer(print, layer_tools, m_skirt_done) : Skirt::make_skirt_loops_per_extruder_other_layers(print, layer_tools, m_skirt_done)};

		const SmoothPathGenerator smooth_path{m_seam_placer, smooth_path_caches, m_scaled_resolution, m_config, m_enable_loop_clipping};

		using GCode::ExtrusionOrder::ExtruderExtrusions;
		using GCode::ExtrusionOrder::get_extrusions;

		const std::optional<Point> previous_position{this->last_position ? std::optional{scaled(this->point_to_gcode(*this->last_position))} : std::nullopt};
		std::vector<ExtruderExtrusions> extrusions{get_extrusions(print, this->m_wipe_tower.get(), layers, first_layer, layer_tools, instances_to_print, skirt_loops_per_extruder, this->m_writer.extruder()->id(), smooth_path, !this->m_brim_done, previous_position)};
	...
	LayerResult GCodeGenerator::process_layer(const Print &print,
			// Set of object & print layers of the same PrintObject and with the same print_z.
			                                  const ObjectsLayerToPrint &layers, const LayerTools &layer_tools, const GCode::SmoothPathCaches &smooth_path_caches, const bool last_layer,
		...
		if (m_spiral_vase && layers.size() == 1 && support_layer == nullptr) {
			...
			if (enable) {
				for (const LayerRegion *layer_region: layer.regions())
					if (size_t(layer_region->region().config().bottom_solid_layers.value) > layer.id() || layer_region->perimeters().items_count() > 1u || layer_region->fills().items_count() > 0) {
						enable = false;
						break;
					}
			}
		...
		using GCode::ExtrusionOrder::ExtruderExtrusions;
		const std::vector<ExtruderExtrusions> extrusions{this->get_sorted_extrusions(print, layers, layer_tools, instances_to_print, smooth_path_caches, first_layer)};
		...
		gcode += std::string(";") + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Height) + float_to_string_decimal_point(height) + "\n";
		...
		if (!first_layer && !print.config().before_layer_gcode.value.empty()) {
			...
			config.set_key_value("max_layer_z", new ConfigOptionFloat(m_max_layer_z));
			gcode += this->placeholder_parser_process("before_layer_gcode", print.config().before_layer_gcode.value, m_writer.extruder()->id(), &config) + "\n";
		}
		...
		for (const ExtruderExtrusions &extruder_extrusions: extrusions) {
			gcode += (layer_tools.has_wipe_tower && m_wipe_tower) ? m_wipe_tower->tool_change(*this, extruder_extrusions.extruder_id, extruder_extrusions.extruder_id == layer_tools.extruders.back()) : this->set_extruder(extruder_extrusions.extruder_id, print_z);
			...
			if (!extruder_extrusions.overriden_extrusions.empty()) {
				...
				for (std::size_t i{0}; i < instances_to_print.size(); ++i) {
					...
					gcode += this->extrude_slices(instance, layers[instance.object_layer_to_print_id], overriden_extrusions.slices_extrusions);
				}
	...
	void GCodeGenerator::initialize_instance(const InstanceToPrint &print_instance, const ObjectLayerToPrint &layer_to_print) {
	...
	std::string GCodeGenerator::extrude_slices(const InstanceToPrint &print_instance, const ObjectLayerToPrint &layer_to_print, const std::vector<SliceExtrusions> &slices_extrusions) {
		const PrintObject &print_object = print_instance.print_object;

		m_layer = layer_to_print.layer();
		// To control print speed of the 1st object layer printed over raft interface.
		m_object_layer_over_raft = layer_to_print.object_layer && layer_to_print.object_layer->id() > 0 && print_object.slicing_parameters().raft_layers() == layer_to_print.object_layer->id();
	...
	std::string GCodeGenerator::change_layer(coordf_t previous_layer_z, coordf_t print_z, bool vase_mode, const Point &first_point, const bool first_layer) {
		std::string gcode;
		if (m_layer_count > 0)
		...
		const bool do_ramping_layer_change = (this->last_position && !vase_mode && print_z > previous_layer_z && this->m_config.travel_ramping_lift.get_at(extruder_id) && this->m_config.travel_slope.get_at(extruder_id) > 0 && this->m_config.travel_slope.get_at(extruder_id) < 90);
	...
	std::string GCodeGenerator::extrude_smooth_path(const GCode::SmoothPath &smooth_path, const bool is_loop, const std::string_view description, const double speed) {
		std::string gcode;
	...
	std::string GCodeGenerator::extrude_infill_ranges(const std::vector<InfillRange> &infill_ranges, const std::string &comment) {
		std::string gcode{};
	...
	std::string GCodeGenerator::extrude_perimeters(const PrintRegion &region, const std::vector<GCode::ExtrusionOrder::Perimeter> &perimeters, const InstanceToPrint &print_instance) {
		if (!perimeters.empty()) {
		...
		for (const GCode::ExtrusionOrder::Perimeter &perimeter: perimeters) {
			...
			this->m_travel_obstacle_tracker.mark_extruded(perimeter.extrusion_entity, print_instance.object_layer_to_print_id, print_instance.instance_id);
...
+ 1;
	...
	std::string GCodeGenerator::travel_to_first_position(const Vec3crd &point, const double from_z, const ExtrusionRole role, const std::function<std::string()> &insert_gcode) {
		...
		if (!EXTRUDER_CONFIG(travel_ramping_lift) && this->last_position) {
			...
			gcode = this->travel_to(*this->last_position, point.head<2>(), role, "travel to first layer point", insert_gcode);
		} else {
			double lift{EXTRUDER_CONFIG(travel_ramping_lift) ? EXTRUDER_CONFIG(travel_max_lift) : EXTRUDER_CONFIG(retract_lift)};
	...
	double cap_speed(double speed, const FullPrintConfig &config, int extruder_id, const ExtrusionAttributes &path_attr) {
		...
		if (path_attr.role == ExtrusionRole::InternalInfill) {
			const double infill_cap{path_attr.maybe_self_crossing ? config.filament_infill_max_crossing_speed.get_at(extruder_id) : config.filament_infill_max_speed.get_at(extruder_id)};
			if (infill_cap > 0) {
	...
	std::string GCodeGenerator::_extrude(const ExtrusionAttributes &path_attr, const Geometry::ArcWelder::Path &path, const std::string_view description, double speed, const EmitModifiers &emit_modifiers) {
		std::string gcode;
		...
		if (path_attr.overhang_attributes.has_value()) {
			...
			dynamic_print_and_fan_speeds = ExtrusionProcessor::calculate_overhang_speed(path_attr, this->m_config, m_writer.extruder()->id(), float(external_perimeter_reference_speed), float(speed), m_current_dynamic_fan_speed);
		}
		...
		if (last_was_wipe_tower || m_last_width != path_attr.width) {
			...
			gcode += std::string(";") + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Width) + float_to_string_decimal_point(m_last_width) + "\n";
		}

		if (last_was_wipe_tower || std::abs(m_last_height - path_attr.height) > EPSILON) {
			m_last_height = path_attr.height;

			gcode += std::string(";") + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Height) + float_to_string_decimal_point(m_last_height) + "\n";
		}
			...
			if (p != prev) {
				...
				if (it->radius != 0) {
					...
					radius = unscaled<double>(it->radius);
					{
						...
						ij = GCodeFormatter::quantize(Vec2d(Geometry::ArcWelder::arc_center(prev_exact.cast<double>(), p_exact.cast<double>(), double(radius), it->ccw()) - prev));
						if (ij == Vec2d::Zero())
	...
	std::string GCodeGenerator::generate_travel_gcode(const Points3 &travel, const std::string &comment, const std::function<std::string()> &insert_gcode) {
		std::string gcode;
	...
	bool GCodeGenerator::needs_retraction(const Polyline &travel, ExtrusionRole role) {
		...
		if (m_config.only_retract_when_crossing_perimeters && m_layer != nullptr && m_config.fill_density.value > 0 && m_retract_when_crossing_perimeters.travel_inside_internal_regions(*m_layer, travel))
	...
	Polyline GCodeGenerator::generate_travel_xy_path(const Point &start_point, const Point &end_point, const bool needs_retraction, bool &could_be_wipe_disabled) {

		const Point scaled_origin{scaled(this->origin())};
		const bool avoid_crossing_perimeters = (this->m_config.avoid_crossing_perimeters && !this->m_avoid_crossing_perimeters.disabled_once());
		...
		if (m_config.avoid_crossing_curled_overhangs) {
			if (avoid_crossing_perimeters) {
				...
				xy_path = this->m_avoid_crossing_curled_overhangs.find_path(start_point + scaled_origin, end_point + scaled_origin);
	...
	std::string GCodeGenerator::travel_to(const Point &start_point, const Point &end_point, ExtrusionRole role, const std::string &comment, const std::function<std::string()> &insert_gcode) {
		// check whether a straight travel move would need retraction

		bool could_be_wipe_disabled{false};
		bool needs_retraction = this->needs_retraction(Polyline{start_point, end_point}, role);

		Polyline xy_path{generate_travel_xy_path(start_point, end_point, needs_retraction, could_be_wipe_disabled)};
		...
		if (needs_retraction) {
			...
			if (*this->last_position != position_before_wipe) {
				xy_path = generate_travel_xy_path(*this->last_position, end_point, needs_retraction, could_be_wipe_disabled);
			}
		} else {
			m_wipe.reset_path();
		}
		...
		const Points3 travel = (can_be_flat ? GCode::Impl::Travels::generate_flat_travel(xy_path.points, initial_elevation) : GCode::Impl::Travels::generate_travel_to_extrusion(xy_path, m_config, extruder_id, initial_elevation, m_travel_obstacle_tracker, scaled(m_origin)));
	...
	std::string GCodeGenerator::set_extruder(unsigned int extruder_id, double print_z) {
		...
		if (m_config.single_extruder_multi_material && !m_config.wipe_tower) {
			int temp = (m_layer_index <= 0 ? m_config.first_layer_temperature.get_at(extruder_id) : m_config.temperature.get_at(extruder_id));

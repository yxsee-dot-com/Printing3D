...
namespace Slic3r {
	...
	enum class ExtrusionRoleModifier : uint16_t {
		...
		Perimeter, // Infill (top / bottom / solid inner / sparse inner / bridging inner ...)
		Infill, // Variable width extrusion
		Thin, // Support material extrusion
		Support, Skirt, Wipe, // 2) Extrusion modifiers
		External, Solid, Ironing, Bridge, // 3) Special types
		...
		Mixed, // Stopper, there should be maximum 16 modifiers defined for uint16_t bit mask.
		Count
	};
	...
	enum ExtrusionLoopRole {
		elrDefault, elrContourInternalPerimeter, elrSkirt,
	};
	...
	enum class GCodeExtrusionRole : uint8_t {
		None, Perimeter, ExternalPerimeter, OverhangPerimeter, InternalInfill, SolidInfill, TopSolidInfill, Ironing, BridgeInfill, GapFill, Skirt, SupportMaterial, SupportMaterialInterface, WipeTower, // Custom (user defined) G-code block, for example start / end G-code.
		Custom, // Stopper to count number of enums.
		Count
	};

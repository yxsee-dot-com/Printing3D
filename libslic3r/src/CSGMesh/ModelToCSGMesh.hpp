...
namespace Slic3r {
	namespace csg {
		...
		void model_to_csgmesh(const ModelObject &mo, const Transform3d &trafo, // Applies to all exported parts
					...
					if (do_splits && its_is_splittable(vol->mesh().its)) {
						...
						*out = std::move(part_begin);
						...
						its_split(vol->mesh().its, SplitOutputFn{[&out, &vol, &trafo](indexed_triangle_set &&its) {
							...
							CSGPart part{std::make_unique<indexed_triangle_set>(std::move(its)), CSGType::Union, (trafo * vol->get_matrix()).cast<float>()};
						...
						*out = std::move(part_end);
						...
						CSGPart part{&(vol->mesh().its), vol->is_model_part() ? CSGType::Union : CSGType::Difference, (trafo * vol->get_matrix()).cast<float>()};
			...
			if (do_drillholes) {
				...
				for (const sla::DrainHole &dhole: drainholes) {
					CSGPart part{std::make_unique<const indexed_triangle_set>(dhole.to_mesh()), CSGType::Difference};

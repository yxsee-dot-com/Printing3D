...
namespace Slic3r {
	namespace csg {
		...
		namespace detail {
			...
			inline void collect_nonempty_indices(csg::CSGType op, const std::vector<float> &slicegrid, const std::vector<ExPolygons> &slices, std::vector<size_t> &indices) {
				indices.clear();
		...
		std::vector<ExPolygons> slice_csgmesh_ex(const Range<ItCSG> &csgrange, const std::vector<float> &slicegrid, const MeshSlicingParamsEx &params, const std::function<void()> &throw_on_cancel = [] {}) {
			...
			for (const auto &csgpart: csgrange) {
				...
				if (its) {
					...
					execution::for_each(ex_tbb, nonempty_indices.begin(), nonempty_indices.end(), [op, &slices, &top](size_t i) {
				...
				if (get_stack_operation(csgpart) == CSGStackOp::Pop) {
					...
					execution::for_each(ex_tbb, nonempty_indices.begin(), nonempty_indices.end(), [&popslices, &prev_slices, popop](size_t i) {

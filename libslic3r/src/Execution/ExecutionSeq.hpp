...
namespace Slic3r {
	...
	template<>
	struct IsExecutionPolicy_<ExecutionSeq> : public std::true_type {
	};

	static constexpr ExecutionSeq ex_seq = {};

	template<class EP>
	struct IsSequentialEP_ {
		static constexpr bool value = false;
	};

	template<>
	struct IsSequentialEP_<ExecutionSeq> : public std::true_type {
	};
	...
	struct execution::Traits<EP, SequentialEPOnly<EP, void>> {
		...
		public:
			...
			static void for_each(const EP &, It from, It to, Fn &&fn, size_t /* ignore granularity */ = 1) {

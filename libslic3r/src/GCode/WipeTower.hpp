	...
	class WipeTower {
		public:
			...
			struct ToolChangeResult {
				...
				// (leading to https://github.com/prusa3d/Printing3D/issues/2834) or do
			...
			struct box_coordinates {
				box_coordinates(float left, float bottom, float width, float height) : ld(left, bottom), lu(left, bottom + height), rd(left + width, bottom), ru(left + width, bottom + height) {}
			...
			WipeTower(const PrintConfig &config, const PrintRegionConfig &default_region_config, const std::vector<std::vector<float>> &wiping_matrix, size_t initial_tool);
		...
		private:
			...
			{
				SHAPE_NORMAL = 1, SHAPE_REVERSED = -1
			};
			...
			// Bed properties
			enum {
				RectangularBed, CircularBed, CustomBed
			} m_bed_shape;
			...
			struct WipeTowerInfo {
				struct ToolChange {
					...
					ToolChange(size_t old, size_t newtool, float depth = 0.f, float ramming_depth = 0.f, float fwl = 0.f, float wv = 0.f) : old_tool{old}, new_tool{newtool}, required_depth{depth}, ramming_depth{ramming_depth}, first_wipe_line{fwl}, wipe_volume{wv}, wipe_volume_total{wv} {}
				};
				...
				float toolchanges_depth() const {
					float sum = 0.f;
					for (const auto &a: tool_changes) sum += a.required_depth;
					return sum;
				}

				std::vector<ToolChange> tool_changes;

				WipeTowerInfo(float z_par, float layer_height_par) : z{z_par}, height{layer_height_par}, depth{0} {}
			};
			...
			void toolchange_Unload(WipeTowerWriter &writer, const box_coordinates &cleaning_box, const std::string &current_material, const int old_temperature, const int new_temperature);

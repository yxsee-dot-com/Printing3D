...
namespace Slic3r::Seams::Shells::Impl {
	...
	Shells<> map_to_shells(Perimeters::LayerPerimeters &&layers, const Geometry::Mapping &mapping, const std::size_t shell_count) {
		...
		for (std::size_t layer_index{0}; layer_index < layers.size(); ++layer_index) {
			...
			for (std::size_t perimeter_index{0}; perimeter_index < perimeters.size(); perimeter_index++) {
				Perimeters::Perimeter &perimeter{perimeters[perimeter_index].perimeter};
				result[mapping[layer_index][perimeter_index]].push_back(Slice<>{std::move(perimeter), layer_index});
			}
		}
		return result;
	}
...
namespace Slic3r::Seams::Shells {
	Shells<> create_shells(Perimeters::LayerPerimeters &&perimeters, const double max_distance) {
		...
		const auto &[shell_mapping, shell_count]{Geometry::get_mapping(layer_sizes, [&](const std::size_t layer_index, const std::size_t item_index) -> Geometry::MappingOperatorResult {
			...
			const auto [perimeter_index, distance] = Geometry::pick_closest_bounding_box(layer[item_index].bounding_box, next_layer_bounding_boxes);

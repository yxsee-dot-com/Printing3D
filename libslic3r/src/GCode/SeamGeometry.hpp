...
namespace Slic3r::Seams::Geometry {
	...
	struct Extrusion {
		Extrusion(Polygon &&polygon, BoundingBox bounding_box, const double width, const ExPolygon &island_boundary);
	...
	std::size_t get_flat_size(const NestedVector &nested_vector) {
		return std::accumulate(nested_vector.begin(), nested_vector.end(), std::size_t{0}, [](const std::size_t sum, const auto &vector) { return sum + vector.size(); });
	}

	template<typename NestedVector>
	std::vector<std::pair<std::size_t, std::size_t>> get_flat_index2indices_table(const NestedVector &nested_vector) {
	...
	void iterate_nested(const NestedVector &nested_vector, const std::function<void(std::size_t, std::size_t)> &function) {
		...
		std::vector<std::pair<std::size_t, std::size_t>> index_table{get_flat_index2indices_table(nested_vector)};

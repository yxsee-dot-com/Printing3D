	...
	GCodeFindReplace::GCodeFindReplace(const std::vector<std::string> &gcode_substitutions) {
		...
		for (size_t i = 0; i < gcode_substitutions.size(); i += 4) {
			...
			try {
				...
				if (out.regexp) {
					out.regexp_pattern.assign(out.whole_word ? std::string("\\b") + out.plain_pattern + "\\b" : out.plain_pattern, (out.case_insensitive ? boost::regex::icase : 0) | boost::regex::optimize);
				} else {
	...
	std::string GCodeFindReplace::process_layer(const std::string &ain) {
		...
		for (const Substitution &substitution: m_substitutions) {
			if (substitution.regexp) {
				temp.clear();
				temp.reserve(in->size());
				boost::regex_replace(ToStringIterator(temp), in->begin(), in->end(), substitution.regexp_pattern, substitution.format, (substitution.single_line ? boost::match_single_line | boost::match_default : boost::match_not_dot_newline | boost::match_default) | boost::format_all);
				std::swap(out, temp);
			} else {
				if (in == &ain)
					out = ain;
				...
				if (substitution.case_insensitive) {
					if (substitution.whole_word)
						find_and_replace_whole_word(out, substitution.plain_pattern, substitution.format, [](const std::string &str, size_t start_pos, const std::string &match) {

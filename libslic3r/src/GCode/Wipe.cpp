...
namespace Slic3r::GCode {
	...
	std::string Wipe::wipe(GCodeGenerator &gcodegen, bool toolchange) {
		...
		if (double retract_length = extruder.retract_to_go(toolchange ? extruder.retract_length_toolchange() : extruder.retract_length());
			...
			auto wipe_arc = [&gcode, &gcodegen, &retract_length, xy_to_e, &wipe_linear](const Vec2d &prev_quantized, Vec2d &p, double radius_in, const bool ccw) {
				Vec2d p_quantized = GCodeFormatter::quantize(p);
				if (p_quantized == prev_quantized) {
					p = p_quantized;
					return false;
				...
				if (dE > retract_length - EPSILON) {
					if (dE > retract_length + EPSILON) {
						...
						p = GCodeFormatter::quantize(Vec2d(center + Eigen::Rotation2D((ccw ? angle : -angle) * (retract_length / dE)) * (prev_quantized - center)));
					} else
						p = p_quantized;
					dE = retract_length;
					done = true;

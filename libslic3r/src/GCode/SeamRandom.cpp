...
namespace Slic3r::Seams::Random {
	...
	namespace Impl {
		std::vector<PerimeterSegment> get_segments(const Perimeters::Perimeter &perimeter, const PointType point_type, const PointClassification point_classification) {
			...
			for (std::size_t index{0}; index < positions.size(); ++index) {
				...
				if (point_types[index] == point_type && point_classifications[index] == point_classification) {
					if (!current_begin) {
		...
		PerimeterSegment pick_random_segment(const std::vector<PerimeterSegment> &segments, std::mt19937 &random_engine) {
			double length{0.0};
		...
		SeamChoice pick_random_point(const PerimeterSegment &segment, const Perimeters::Perimeter &perimeter, std::mt19937 &random_engine) {
			const std::vector<Vec2d> &positions{perimeter.positions};
		...
		std::optional<SeamChoice> Random::operator()(const Perimeters::Perimeter &perimeter, const PointType point_type, const PointClassification point_classification) const {
			std::vector<PerimeterSegment> segments{get_segments(perimeter, point_type, point_classification)};
		...
		for (std::vector<Perimeters::BoundedPerimeter> &layer: perimeters) {
			...
			for (Perimeters::BoundedPerimeter &perimeter: layer) {
				if (perimeter.perimeter.is_degenerate) {
					...
					result.back().push_back(SeamPerimeterChoice{Seams::choose_seam_point(perimeter.perimeter, random), std::move(perimeter.perimeter)});
				}
			}
		}
		return result;

...
#include "GCode/SeamGeometry.hpp"
#include "GCode/ModelVisibility.hpp"
...
namespace Slic3r::Seams::Aligned {
	...
	namespace Impl {
		...
		const Perimeters::Perimeter::OptionalPointTree &pick_tree(const Perimeters::Perimeter::PointTrees &point_trees, const PointClassification &point_classification) {
		...
		SeamChoice pick_seam_option(const Perimeters::Perimeter &perimeter, const SeamOptions &options) {
			...
			unsigned closeset_point_value = point_value(types.at(options.closest), classifications[options.closest]);

			if (options.snapped) {
				unsigned snapped_point_value = point_value(types.at(*options.snapped), classifications[*options.snapped]);
			...
			unsigned adjacent_point_value = point_value(types.at(options.adjacent), classifications[options.adjacent]);
		...
		std::optional<std::size_t> snap_to_angle(const Vec2d &point, const std::size_t search_start, const Perimeters::Perimeter &perimeter, const double max_detour) {
		...
		SeamOptions get_seam_options(const Perimeters::Perimeter &perimeter, const Vec2d &prefered_position, const Perimeters::Perimeter::PointTree &points_tree, const double max_detour) {
			const std::vector<Vec2d> &positions{perimeter.positions};
			...
			const auto [previous_point, previous_distance] = Geometry::distance_to_segment_squared(previous_segment, prefered_position);
			...
			return {closest, adjacent, adjacent_forward, snapped, nearest_point,};
		}

		std::optional<SeamChoice> LeastVisible::operator()(const Perimeters::Perimeter &perimeter, const PointType point_type, const PointClassification point_classification) const {
		...
		std::optional<SeamChoice> Nearest::operator()(const Perimeters::Perimeter &perimeter, const PointType point_type, const PointClassification point_classification) const {
	...
	double VisibilityCalculator::operator()(const SeamChoice &choice, const Perimeters::Perimeter &perimeter) const {
		double visibility = points_visibility.calculate_point_visibility(to_3d(choice.position, perimeter.slice_z).cast<float>());
	...
	double VisibilityCalculator::get_angle_visibility_modifier(double angle, const double convex_visibility_modifier, const double concave_visibility_modifier) {
	...
	std::vector<SeamChoice> get_shell_seam(const Shells::Shell<> &shell, const std::function<SeamChoice(const Perimeters::Perimeter &, std::size_t)> &chooser) {
	...
	SeamCandidate get_seam_candidate(const Shells::Shell<> &shell, const Vec2d &starting_position, const SeamChoiceVisibility &visibility_calculator, const Params &params, const std::vector<std::vector<double>> &precalculated_visibility, const std::vector<LeastVisiblePoint> &least_visible_points) {
		using Perimeters::Perimeter, Perimeters::AngleType;

		std::vector<double> choice_visibilities(shell.size(), 1.0);
		std::vector<SeamChoice> choices{get_shell_seam(shell, [&, previous_position{starting_position}](const Perimeter &perimeter, std::size_t slice_index) mutable {
			SeamChoice candidate{Seams::choose_seam_point(perimeter, Impl::Nearest{previous_position, params.max_detour})};
			...
			const bool is_on_edge{candidate.previous_index == candidate.next_index && perimeter.angle_types[candidate.next_index] != AngleType::smooth};
			...
			const bool is_too_visible{choice_visibilities[slice_index] > least_visible.visibility + params.jump_visibility_threshold};
			const bool can_be_on_edge{!is_on_edge && perimeter.angle_types[least_visible.choice.next_index] != AngleType::smooth};
	...
	std::vector<ShellVertexVisibility> get_shells_vertex_visibility(const Shells::Shells<> &shells, const SeamChoiceVisibility &visibility_calculator) {
		std::vector<ShellVertexVisibility> result;

		result.reserve(shells.size());
		std::transform(shells.begin(), shells.end(), std::back_inserter(result), [](const Shells::Shell<> &shell) { return ShellVertexVisibility(shell.size()); });
		...
		Geometry::iterate_nested(shells, [&](const std::size_t shell_index, const std::size_t slice_index) {
			...
			for (std::size_t point_index{0}; point_index < positions.size(); ++point_index) {
				result[shell_index][slice_index].emplace_back(visibility_calculator(SeamChoice{point_index, point_index, positions[point_index]}, slice.boundary));
			}
		});
		return result;
	}
	...
	std::vector<ShellLeastVisiblePoints> get_shells_least_visible_points(const Shells::Shells<> &shells, const std::vector<ShellVertexVisibility> &precalculated_visibility) {
		std::vector<ShellLeastVisiblePoints> result;

		result.reserve(shells.size());
		std::transform(shells.begin(), shells.end(), std::back_inserter(result), [](const Shells::Shell<> &shell) { return ShellLeastVisiblePoints(shell.size()); });

		Geometry::iterate_nested(shells, [&](const std::size_t shell_index, const std::size_t slice_index) {
			const Shells::Shell<> &shell{shells[shell_index]};
			const Shells::Slice<> &slice{shell[slice_index]};
			const SeamChoice least_visibile{Seams::choose_seam_point(slice.boundary, Impl::LeastVisible{precalculated_visibility[shell_index][slice_index]})};
	...
	std::vector<ShellSeamCandidates> get_shells_seam_candidates(const Shells::Shells<> &shells, const std::vector<ShellStartingPositions> &starting_positions, const SeamChoiceVisibility &visibility_calculator, const std::vector<ShellVertexVisibility> &precalculated_visibility, const std::vector<ShellLeastVisiblePoints> &least_visible_points, const Params &params) {
		std::vector<ShellSeamCandidates> result;

		result.reserve(starting_positions.size());
		std::transform(starting_positions.begin(), starting_positions.end(), std::back_inserter(result), [](const ShellStartingPositions &positions) { return ShellSeamCandidates(positions.size()); });

		Geometry::iterate_nested(starting_positions, [&](const std::size_t shell_index, const std::size_t starting_position_index) {
			const Shells::Shell<> &shell{shells[shell_index]};
			using Perimeters::Perimeter, Perimeters::AngleType;

			result[shell_index][starting_position_index] = get_seam_candidate(shell, starting_positions[shell_index][starting_position_index], visibility_calculator, params, precalculated_visibility[shell_index], least_visible_points[shell_index]);
		});
		return result;
	}

	std::vector<SeamChoice> get_shell_seam(const Shells::Shell<> &shell, std::vector<SeamCandidate> seam_candidates, const Perimeters::Perimeter::OptionalPointTree &previous_points, const Params &params) {
		...
		for (std::size_t i{0}; i < seam_candidates.size(); ++i) {
			...
			double seam_candidate_visibility{is_near_previous ? -params.continuity_modifier * (params.max_detour - *previous_distance) / params.max_detour : 0.0};
	...
	std::vector<std::vector<SeamPerimeterChoice>> get_object_seams(Shells::Shells<> &&shells, const SeamChoiceVisibility &visibility_calculator, const Params &params) {
		const std::vector<ShellVertexVisibility> precalculated_visibility{get_shells_vertex_visibility(shells, visibility_calculator)};

		const std::vector<ShellLeastVisiblePoints> least_visible_points{get_shells_least_visible_points(shells, precalculated_visibility)};

		const std::vector<ShellStartingPositions> starting_positions{get_shells_starting_positions(shells)};

		const std::vector<ShellSeamCandidates> seam_candidates{get_shells_seam_candidates(shells, starting_positions, visibility_calculator, precalculated_visibility, least_visible_points, params)};
		...
		for (std::size_t shell_index{0}; shell_index < shells.size(); ++shell_index) {
			...
			tcb::span<const SeamPerimeterChoice> previous_seams{layer_index == 0 ? tcb::span<const SeamPerimeterChoice>{} : layer_seams[layer_index - 1]};
			std::vector<Vec2d> previous_seams_positions;
			std::transform(previous_seams.begin(), previous_seams.end(), std::back_inserter(previous_seams_positions), [](const SeamPerimeterChoice &seam) { return seam.choice.position; });
			...
			if (!previous_seams_positions.empty()) {
				previous_seams_positions_tree = Perimeters::Perimeter::PointTree{index_to_coord, index_to_coord.positions.size()};
			}

			std::vector<SeamChoice> seam{Aligned::get_shell_seam(shell, seam_candidates[shell_index], previous_seams_positions_tree, params)};

...
namespace Slic3r::Seams {
	std::optional<SeamChoice> maybe_choose_seam_point(const Perimeters::Perimeter &perimeter, const SeamPicker &seam_picker) {
		...
		std::vector<PointType> type_search_order{PointType::enforcer, PointType::common, PointType::blocker};
		std::vector<PointClassification> classification_search_order{PointClassification::embedded, PointClassification::common, PointClassification::overhang};
		for (const PointType point_type: type_search_order) {
			for (const PointClassification point_classification: classification_search_order) {
				if (std::optional<SeamChoice> seam_choice{seam_picker(perimeter, point_type, point_classification)}) {
					return seam_choice;
				}
			}

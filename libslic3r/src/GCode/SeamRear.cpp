...
namespace Slic3r::Seams::Rear {
	...
	namespace Impl {
		...
		struct RearestPointCalculator {
			...
			std::optional<SeamChoice> operator()(const Perimeters::Perimeter &perimeter, const PointType point_type, const PointClassification point_classification) {
	...
	std::vector<std::vector<SeamPerimeterChoice>> get_object_seams(std::vector<std::vector<Perimeters::BoundedPerimeter>> &&perimeters, const double rear_tolerance, const double rear_y_offset) {

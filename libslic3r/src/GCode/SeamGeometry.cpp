...
namespace Slic3r::Seams::Geometry {
	...
	namespace MappingImpl {
		 ...
		 */
		std::vector<std::size_t> assign_buckets(const std::vector<std::optional<Link>> &links, std::size_t &new_bucket_id) {
			std::vector<std::size_t> result;
			std::transform(links.begin(), links.end(), std::back_inserter(result), [&](const std::optional<Link> &link) {
				if (link) {
	...
	Vec2d get_polygon_normal(const std::vector<Vec2d> &points, const std::size_t index, const double min_arm_length) {
		...
		if (previous_index && next_index) {
			const Vec2d previous_normal{Geometry::get_normal(points.at(index) - points.at(*previous_index))};
	...
	std::pair<Mapping, std::size_t> get_mapping(const std::vector<std::size_t> &list_sizes, const MappingOperator &mapping_operator) {
		...
		for (std::size_t layer_index{0}; layer_index < list_sizes.size() - 1; ++layer_index) {
			...
			for (std::size_t item_index{0}; item_index < list_sizes[layer_index]; ++item_index) {
				const MappingOperatorResult next_item{mapping_operator(layer_index, item_index)};
				if (next_item) {
	...
	Extrusion::Extrusion(Polygon &&polygon, BoundingBox bounding_box, const double width, const ExPolygon &island_boundary) : polygon(polygon), bounding_box(std::move(bounding_box)), width(width), island_boundary(island_boundary) {
		this->island_boundary_bounding_boxes.push_back(island_boundary.contour.bounding_box());

		std::transform(this->island_boundary.holes.begin(), this->island_boundary.holes.end(), std::back_inserter(this->island_boundary_bounding_boxes), [](const Polygon &polygon) { return polygon.bounding_box(); });
	}
	...
	std::vector<Extrusions> get_extrusions(tcb::span<const Slic3r::Layer *const> object_layers) {
		...
		for (const Slic3r::Layer *object_layer: object_layers) {
			...
			for (const LayerSlice &slice: object_layer->lslices_ex) {
				std::vector<Extrusion> external_perimeters{get_external_perimeters(*object_layer, slice)};
	...
	BoundedPolygons project_to_geometry(const Geometry::Extrusions &external_perimeters, const double max_bb_distance) {
		...
		transform(external_perimeters.begin(), external_perimeters.end(), back_inserter(result), [&](const Geometry::Extrusion &external_perimeter) {
			const auto [choosen_index, _]{Geometry::pick_closest_bounding_box(external_perimeter.bounding_box, external_perimeter.island_boundary_bounding_boxes)};

			const double distance{Geometry::bounding_box_distance(external_perimeter.island_boundary_bounding_boxes[choosen_index], external_perimeter.bounding_box)};
			...
			if (distance > max_bb_distance) {
				...
				if (!expanded_extrusion.empty()) {
					return BoundedPolygon{expanded_extrusion.front(), expanded_extrusion.front().bounding_box(), external_perimeter.polygon.is_clockwise(), 0.0};
				}
			}

			const bool is_hole{choosen_index != 0};
			const Polygon &adjacent_boundary{!is_hole ? external_perimeter.island_boundary.contour : external_perimeter.island_boundary.holes[choosen_index - 1]};
	...
	std::vector<BoundedPolygons> convert_to_geometry(const std::vector<Geometry::Extrusions> &extrusions) {
		...
		for (const Geometry::Extrusions &layer: extrusions) {
			...
			using std::transform, std::back_inserter;
			transform(layer.begin(), layer.end(), back_inserter(result.back()), [&](const Geometry::Extrusion &extrusion) {
				return BoundedPolygon{extrusion.polygon, extrusion.bounding_box, extrusion.polygon.is_clockwise(), extrusion.width / 2.0};
			});
		}
	...
	void visit_near_forward(const std::size_t start_index, const std::size_t loop_size, const std::function<bool(std::size_t)> &visitor) {
	...
	void visit_near_backward(const std::size_t start_index, const std::size_t loop_size, const std::function<bool(std::size_t)> &visitor) {
		std::size_t last_index{loop_size - 1};
	...
	std::vector<double> get_embedding_distances(const std::vector<Vec2d> &points, const AABBTreeLines::LinesDistancer<Linef> &perimeter_distancer) {
	...
	std::vector<double> get_overhangs(const std::vector<Vec2d> &points, const AABBTreeLines::LinesDistancer<Linef> &previous_layer_perimeter_distancer, const double layer_height) {
		std::vector<double> result;
		result.reserve(points.size());
		using std::transform, std::back_inserter;
		transform(points.begin(), points.end(), back_inserter(result), [&](const Vec2d &point) {
	...
	std::pair<std::size_t, double> pick_closest_bounding_box(const BoundingBox &to, const BoundingBoxes &choose_from) {

...
#include "GCode/SeamShells.hpp"
#include "GCode/SeamAligned.hpp"
#include "GCode/SeamRear.hpp"
#include "GCode/SeamRandom.hpp"
...
namespace Slic3r::Seams {
	...
	ObjectLayerPerimeters get_perimeters(SpanOfConstPtrs<PrintObject> objects, const Params &params, const ObjectPainting &object_painting, const std::function<void(void)> &throw_if_canceled) {
		...
		for (const PrintObject *print_object: objects) {
			...
			const std::vector<Geometry::Extrusions> extrusions{Geometry::get_extrusions(print_object->layers())};
			const Perimeters::LayerInfos layer_infos{Perimeters::get_layer_infos(print_object->layers(), params.perimeter.elephant_foot_compensation)};
			const std::vector<Geometry::BoundedPolygons> projected{print_object->config().seam_position == spRandom ? Geometry::convert_to_geometry(extrusions) : Geometry::project_to_geometry(extrusions, params.max_distance)};
	...
	Perimeters::LayerPerimeters sort_to_layers(Shells::Shells<> &&shells) {
		...
		for (Shells::Shell<> &shell: shells) {
			for (Shells::Slice<> &slice: shell) {
				...
				result[slice.layer_index].push_back(Perimeters::BoundedPerimeter{std::move(slice.boundary), bounding_box});
			}
		}
		return result;
	}

	ObjectSeams precalculate_seams(const Params &params, ObjectLayerPerimeters &&seam_data, const std::function<void(void)> &throw_if_canceled) {
		ObjectSeams result;
		...
		for (auto &[print_object, layer_perimeters]: seam_data) {
			switch (print_object->config().seam_position.value) {
				case spAligned: {
					...
					Slic3r::ModelInfo::Visibility points_visibility{transformation, volumes, params.visibility, throw_if_canceled};
					throw_if_canceled();
					const Aligned::VisibilityCalculator visibility_calculator{points_visibility, params.convex_visibility_modifier, params.concave_visibility_modifier};

					Shells::Shells<> shells{Shells::create_shells(std::move(layer_perimeters), params.max_distance)};
					result[print_object] = Aligned::get_object_seams(std::move(shells), visibility_calculator, params.aligned);
					break;
				}
				case spRear: {
	...
	void Placer::init(SpanOfConstPtrs<PrintObject> objects, const Params &params, const std::function<void(void)> &throw_if_canceled) {
	...
	const SeamPerimeterChoice &choose_closest_seam(const std::vector<SeamPerimeterChoice> &seams, const Polygon &loop_polygon) {
		...
		const std::size_t choice_index{Geometry::pick_closest_bounding_box(loop_polygon.bounding_box(), choose_from).first};

		return seams[choice_index];
	}

	std::pair<std::size_t, Vec2d> project_to_extrusion_loop(const SeamChoice &seam_choice, const Perimeters::Perimeter &perimeter, const Linesf &loop_lines) {
		...
		const Vec2d edge{perimeter.positions[seam_choice.next_index] - perimeter.positions[seam_choice.previous_index]};
		const Vec2d normal{is_at_vertex ? Geometry::get_polygon_normal(perimeter.positions, seam_choice.previous_index, 0.1) : Geometry::get_normal(edge)};
	...
	std::optional<Vec2d> offset_along_loop_lines(const Vec2d &point, const std::size_t loop_line_index, const Linesf &loop_lines, const double offset) {
		double distance{0};
	...
	Point finalize_seam_position(const Polygon &loop_polygon, const SeamChoice &seam_choice, const Perimeters::Perimeter &perimeter, const double loop_width, const bool do_staggering) {
		const Linesf loop_lines{to_unscaled_linesf({ExPolygon{loop_polygon}})};
		const auto [loop_line_index, loop_point]{project_to_extrusion_loop(seam_choice, perimeter, loop_lines)};

		// ExtrusionRole::Perimeter is inner perimeter.
		if (do_staggering) {
			const double depth = (loop_point - seam_choice.position).norm() - loop_width / 2.0;
			...
			std::optional<Vec2d> staggered_point{offset_along_loop_lines(loop_point, loop_line_index, loop_lines, staggering_offset)};
	...
	struct NearestCorner {
		...
		std::optional<SeamChoice> operator()(const Perimeters::Perimeter &perimeter, const Perimeters::PointType point_type, const Perimeters::PointClassification point_classification) const {
			...
			for (std::size_t i{0}; i < perimeter.positions.size(); ++i) {
				if (perimeter.point_types[i] == point_type && perimeter.point_classifications[i] == point_classification && perimeter.angle_types[i] != Perimeters::AngleType::smooth) {
	...
	std::pair<SeamChoice, std::size_t> place_seam_near(const std::vector<Perimeters::BoundedPerimeter> &layer_perimeters, const ExtrusionLoop &loop, const Point &position, const double max_detour) {
		BoundingBoxes choose_from;
		...
		const std::optional<SeamChoice> corner_choice{Seams::maybe_choose_seam_point(layer_perimeters[choice_index].perimeter, nearest_corner)};
		...
		const SeamChoice nearest_choice{Seams::choose_seam_point(layer_perimeters[choice_index].perimeter, nearest)};
	...
	Point Placer::place_seam(const Layer *layer, const ExtrusionLoop &loop, const Point &last_pos) const {
		...
		if (po->config().seam_position.value == spNearest) {
			...
			if (has_2_or_3_perimeters) {
				if (seams_on_perimeters.size() == 2 && seams_on_perimeters[0].perimeter.is_hole != seams_on_perimeters[1].perimeter.is_hole) {
					const SeamPerimeterChoice &seam_perimeter_choice{seams_on_perimeters[0].perimeter.is_hole ? seams_on_perimeters[1] : seams_on_perimeters[0]};

...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    { EProducer::Printing3D, "generated by Printing3D" },
...
void GCodeProcessor::apply_config(const PrintConfig& config)
{
    ...
    m_producer = EProducer::Printing3D;
    ...
    std::function<void(void)> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by Printing3D,
        // extract the config from it
        if (m_producer == EProducer::Printing3D || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from Printing3D's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::Printing3D) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::Printing3D: { return process_prusaslicer_tags(comment); }
...
void GCodeProcessor::process_G92(const GCodeReader::GCodeLine& line)
{
    ...
    if (!any_found && !line.has_unknown_axis()) {
        // The G92 may be called for axes that Printing3D does not recognize, for example see GH issue #3510, 
...
void GCodeProcessor::process_M108(const GCodeReader::GCodeLine& line)
{
    ...
    // by the analyzer - see https://github.com/prusa3d/Printing3D/issues/2566
...
void GCodeProcessor::process_M132(const GCodeReader::GCodeLine& line)
{
    ...
    // Using this command to reset the axis origin to zero helps in fixing: https://github.com/prusa3d/Printing3D/issues/3082
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
        if (static_cast<PrintEstimatedStatistics::ETimeMode>(i) == PrintEstimatedStatistics::ETimeMode::Normal ||
            ...
            if (line.has_value('S', value)) {
                ...
                // It is also generated by Printing3D to control acceleration per extrusion type
...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 255) {
            ...
            // T-1 is a valid gcode line for RepRap Firmwares (used to deselects all tools) see https://github.com/prusa3d/Printing3D/issues/5677
            ...
            if (m_extruder_id != id) {
                if (((m_producer == EProducer::Printing3D || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) && id >= m_result.extruders_count) ||
                    ((m_producer != EProducer::Printing3D && m_producer != EProducer::Slic3rPE && m_producer != EProducer::Slic3r) && id >= m_result.extruder_colors.size()))

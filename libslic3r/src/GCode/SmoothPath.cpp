...
namespace Slic3r::GCode {
	...
	SmoothPath SmoothPathCache::resolve_or_fit_split_with_seam(const ExtrusionLoop &loop, const bool reverse, const double resolution, const Point &seam_point, const double seam_point_merge_distance_threshold) const {
		...
		if (!out.empty()) {
			...
			std::pair<Geometry::ArcWelder::Path, Geometry::ArcWelder::Path> split = Geometry::ArcWelder::split_at(path, proj, seam_point_merge_distance_threshold);

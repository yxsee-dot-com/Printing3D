...
namespace Slic3r {
	...
	class Zipper::Impl : public MZ_Archive {
		public:
			...
			std::string formatted_errorstr() const {
				return _u8L("Error with ZIP archive") + " " + m_zipname + ": " + get_errorstr();
			}
	...
	Zipper::Zipper(Zipper &&m) : m_impl(std::move(m.m_impl)), m_data(std::move(m.m_data)), m_entry(std::move(m.m_entry)), m_compression(m.m_compression) {}
	...
	void Zipper::add_entry(const std::string &name, const void *data, size_t l) {
		...
		switch (m_compression) {
			case NO_COMPRESSION:
				cmpr = MZ_NO_COMPRESSION;
				break;
			case FAST_COMPRESSION:
				cmpr = MZ_BEST_SPEED;
				break;
			case TIGHT_COMPRESSION:
				cmpr = MZ_BEST_COMPRESSION;
				break;
		}
	...
	void Zipper::finish_entry() {
		...
		if (!m_data.empty() && !m_entry.empty()) {
			...
			if (!mz_zip_writer_add_mem(&m_impl->arch, m_entry.c_str(), m_data.c_str(), m_data.size(), compression)) m_impl->blow_up();
		}

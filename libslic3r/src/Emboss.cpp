...
#include "../../deps/imgui/imgui/imstb_truetype.h" // stbtt_fontinfo
...
#include "SVG.hpp" // for visualize_heal
...
namespace {
	...
	void remove_bad(ExPolygons &expolygons) {
		expolygons.erase(std::remove_if(expolygons.begin(), expolygons.end(), [](const ExPolygon &p) { return p.contour.size() < 3; }), expolygons.end());
...
bool Emboss::divide_segments_for_close_point(ExPolygons &expolygons, double distance) {
	...
	while (it != divs.end()) {
		...
		if (count == 1) {
			...
			Fnc fnc = (dir[axis] < 0) ? Fnc([axis](const Point &p1, const Point &p2) { return p1[axis] > p2[axis]; }) : Fnc([axis](const Point &p1, const Point &p2) { return p1[axis] < p2[axis]; });
...
namespace {
	...
	Points get_points(const Duplicates &duplicate_indices) {
		...
		std::transform(duplicate_indices.begin(), duplicate_indices.end(), std::back_inserter(result), [](const Duplicate &d) { return d.point; });
		return result;
	}
	...
	ExPolygon create_bounding_rect(const ExPolygons &shape) {
		...
		Polygon rect({// CCW
				             bb.min, {bb.max.x(), bb.min.y()}, bb.max, {bb.min.x(), bb.max.y()}});

		Point offset = bb.size() * 0.1;
		Polygon hole({// CW
				             bb.min + offset, {bb.min.x() + offset.x(), bb.max.y() - offset.y()}, bb.max - offset, {bb.max.x() - offset.x(), bb.min.y() + offset.y()}});
...
namespace {
	ExPolygons letter2shapes(wchar_t letter, Point &cursor, const FontFile &font, Glyphs &cache, const FontProp &font_prop, fontinfo_opt &font_info_cache) {
		if (letter == '\n') {
			cursor.x() = 0;
...
bool Emboss::is_italic(const FontFile &font, unsigned int font_index) {
	...
	int name_id = 2;
	int length;
	const char *value = stbtt_GetFontNameString(info, &length, STBTT_PLATFORM_ID_MICROSOFT, STBTT_MS_EID_UNICODE_BMP, STBTT_MS_LANG_ENGLISH, name_id);
...
std::string Emboss::create_range_text(const std::string &text, const FontFile &font, unsigned int font_index, bool *exist_unknown) {
	...
	int prev_unicode = -1;
	ws.erase(std::remove_if(ws.begin(), ws.end(), [&prev_unicode, font_info, exist_unknown](wchar_t wc) -> bool {
		int unicode = static_cast<int>(wc);

		// skip white spaces
		if (unicode == '\n' || unicode == '\r' || unicode == '\t') return true;
...
namespace {
	...
	indexed_triangle_set polygons2model_duplicit(const ExPolygons &shape2d, const IProjection &projection, const Points &points, const Points &duplicits) {
		...
		for (uint32_t i = 0; i < changes.size(); ++i) {
			...
			if (max_index != std::numeric_limits<uint32_t>::max() && index <= max_index) continue; // duplicit point
...
indexed_triangle_set Emboss::polygons2model(const ExPolygons &shape2d, const IProjection &projection) {
	...
	return (duplicits.empty()) ? polygons2model_unique(shape2d, projection, points) : polygons2model_duplicit(shape2d, projection, points, duplicits);
}
...
namespace {
	float get_align_y_offset(FontProp::VerticalAlign align, unsigned count_lines, const FontFile &ff, const FontProp &fp) {
		...
		switch (align) {
			...
			case FontProp::VerticalAlign::top:
				return -ascent;
	...
	int32_t get_align_x_offset(FontProp::HorizontalAlign align, const BoundingBox &shape_bb, const BoundingBox &line_bb) {
		switch (align) {
			case FontProp::HorizontalAlign::right:
				return -shape_bb.max.x() + (shape_bb.size().x() - line_bb.size().x());
			case FontProp::HorizontalAlign::center:
				return -shape_bb.center().x() + (shape_bb.size().x() - line_bb.size().x()) / 2;

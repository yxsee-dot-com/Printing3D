...
namespace Slic3r {
	...
	namespace trianglulate_wall_detail {
		...
		class Triangulator {
				...
				void emplace_indices(std::vector<Vec3i> &indices) {
					Vec3i tr{int(onring->pos().first), int(onring->pos().second), int(offring->pos().first)};
	...
	void triangulate_wall(std::vector<Vec<3, Sc>> &pts, std::vector<Vec<3, I>> &ind, const Polygon &lower, const Polygon &upper, double lower_z_mm, double upper_z_mm) {

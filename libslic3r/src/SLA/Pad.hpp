...
namespace Slic3r {
	...
	namespace sla {
		...
		void pad_blueprint(const indexed_triangle_set &mesh,       // input mesh
		...
		void pad_blueprint(const indexed_triangle_set &mesh, ExPolygons &output, float samplingheight = 0.1f,  // The height range to sample
		...
		struct PadConfig {
			...
			inline PadConfig(double thickness, double height, double mergedist, double slope) : wall_thickness_mm(thickness), wall_height_mm(height), max_merge_dist_mm(mergedist), wall_slope(slope) {}
		...
		void create_pad(const ExPolygons &support_contours, const ExPolygons &model_contours, indexed_triangle_set &output_mesh, const PadConfig & = PadConfig(), ThrowOnCancel throw_on_cancel = [] {});

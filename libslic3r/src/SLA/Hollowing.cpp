...
namespace Slic3r {
	...
	namespace sla {
		...
		static double get_distance(const TriangleBubble &b, const Interior &interior) {
			...
			return D > interior.full_narrowb || ((Dst - R) < 0. && 2 * R > interior.thickness) ? std::nan("") : Dst - interior.iso_surface;
		}
		...
		void remove_inside_triangles(indexed_triangle_set &mesh, const Interior &interior, const std::vector<bool> &exclude_mask) {
			...
			struct MeshMods {
				...
				MeshMods(const indexed_triangle_set &mesh) : to_remove(mesh.indices.size(), false) {}
			...
			execution::for_each(exec_policy, size_t(0), faces.size(), [&](size_t face_idx) {
		...
		struct FaceHash {
			...
			static std::string facekey(const Vec3i &face, const std::vector<Vec3f> &vertices) {
				...
				std::array<Vec<3, int64_t>, 3> pts = {scaled<int64_t>(vertices[face(0)]), scaled<int64_t>(vertices[face(1)]), scaled<int64_t>(vertices[face(2)])};
		...
		int hollow_mesh_and_drill(indexed_triangle_set &hollowed_mesh, const Interior &interior, const DrainHoles &drainholes, std::function<void(size_t)> on_hole_fail) {
			auto tree = AABBTreeIndirect::build_aabb_tree_over_indexed_triangle_set(hollowed_mesh.vertices, hollowed_mesh.indices);
			...
			for (size_t i = 0; i < drainholes.size(); ++i) {
				...
				AABBTreeIndirect::traverse(tree, AABBTreeIndirect::intersecting(ebb), [&part_to_drill, &hollowed_mesh](const auto &node) {
			...
			if (!MeshBoolean::cgal::empty(*holes_mesh_cgal) && !MeshBoolean::cgal::does_bound_a_volume(*holes_mesh_cgal)) {
			...
			try {
				...
				hollowed_mesh = MeshBoolean::cgal::cgal_to_indexed_triangle_set(*hollowed_mesh_cgal);

				std::vector<bool> exclude_mask = create_exclude_mask(hollowed_mesh, interior, drainholes);

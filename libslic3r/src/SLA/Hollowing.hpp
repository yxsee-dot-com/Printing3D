...
namespace Slic3r {
	...
	namespace sla {
		...
		InteriorPtr generate_interior(const VoxelGrid &mesh, const HollowingConfig & = {}, const JobController &ctl = {});
		...
		double csgmesh_positive_maxvolume(const Cont &csg) {
			...
			for (const auto &m: csg) {
				...
				if (!skip && csg::get_mesh(m) && op == csg::CSGType::Union)
					mesh_vol = std::max(mesh_vol, double(its_volume(*(csg::get_mesh(m)))));
		...
		InteriorPtr generate_interior(const Range<It> &csgparts, const HollowingConfig &hc = {}, const JobController &ctl = {}) {
			...
			auto params = csg::VoxelizeParams{}.voxel_scale(voxsc).exterior_bandwidth(3.f).interior_bandwidth(3.f).statusfn([&ctl](int) {
			...
			ptr = redistance_grid(*ptr, IsoAtZero, params.exterior_bandwidth(), params.interior_bandwidth());

			return ptr ? generate_interior(*ptr, hc, ctl) : InteriorPtr{};
		}

		inline InteriorPtr generate_interior(const indexed_triangle_set &mesh, const HollowingConfig &hc = {}, const JobController &ctl = {}) {
		...
		enum class HollowMeshResult {
			Ok = 0, FaultyMesh = 1, FaultyHoles = 2, DrillingFailed = 4
		};

		// Return HollowMeshResult codes OR-ed.
		int hollow_mesh_and_drill(indexed_triangle_set &mesh, const Interior &interior, const DrainHoles &holes, std::function<void(size_t)> on_hole_fail = [](size_t) {});
		...
		sla::DrainHoles transformed_drainhole_points(const ModelObject &mo, const Transform3d &trafo);
		...
		// hole walls.
		std::vector<bool> create_exclude_mask(const indexed_triangle_set &its, const sla::Interior &interior, const std::vector<sla::DrainHole> &holes);

...
namespace Slic3r {
	namespace sla {
		...
		std::optional<DiffBridge> search_widening_path(Ex policy, const SupportableMesh &sm, const Vec3d &jp, const Vec3d &dir, double radius, double new_radius) {
			...
			auto oresult = solver.to_max().optimize([&policy, &sm, jp, radius, new_radius](const opt::Input<3> &input) {
				                                        ...
				                                        double ret = pinhead_mesh_hit(policy, sm.emesh, jp, d, radius, new_radius, t, sd).distance();
			                                        ...
			                                        bounds({{PI - sm.cfg.bridge_slope,            PI}, // Must not exceed the slope limit
			                                                {-PI,                                 PI}, // azimuth can be a full search
			                                                {radius + sm.cfg.head_back_radius_mm, fallback_ratio * sm.cfg.max_bridge_length_mm}}));
		...
		std::pair<bool, long> create_ground_pillar(Ex policy, SupportTreeBuilder &builder, const SupportableMesh &sm, const Vec3d &pinhead_junctionpt, const Vec3d &sourcedir, double radius, double end_radius, long head_id = SupportTreeNode::ID_UNSET) {
			...
			auto eval_limits = [&sm, &radius, &can_add_base, &gndlvl, &gap_dist, &jp_gnd](bool base_en = true) {
			...
			if (radius < sm.cfg.head_back_radius_mm && jp.z() - gndlvl > 20 * radius) {
				std::optional<DiffBridge> diffbr = search_widening_path(policy, sm, jp, dir, radius, sm.cfg.head_back_radius_mm);
			...
			if (sm.cfg.object_elevation_mm < EPSILON) {
				...
				Vec3d nexp = endp;
				double dlast = 0.;
				while (((dlast = std::sqrt(sm.emesh.squared_distance(to_floor(nexp)))) < gap_dist || !std::isinf(beam_mesh_hit(policy, sm.emesh, Beam{nexp, DOWN, radius, r2}, sd).distance())) && t < tmax) {
					t += radius;
					nexp = endp + t * d;
				}
				...
				if (dlast < gap_dist && can_add_base) {
					...
					while (((dlast = std::sqrt(sm.emesh.squared_distance(to_floor(nexp)))) < gap_dist || !std::isinf(beam_mesh_hit(policy, sm.emesh, Beam{nexp, DOWN, radius}, sd).distance())) && t < tmax) {
						t += radius;
						nexp = endp + t * d;
					}
				}
			...
			pillar_id = head_id >= 0 && !non_head ? builder.add_pillar(head_id, h) : builder.add_pillar(gp, h, radius, end_radius);
		...
		std::pair<bool, long> connect_to_ground(Ex policy, SupportTreeBuilder &builder, const SupportableMesh &sm, const Junction &j, const Vec3d &dir, double end_r) {
			...
			while (d < t && !std::isinf(tdown = beam_mesh_hit(policy, sm.emesh, Beam{hjp + d * dir, DOWN, r, r2}, sd).distance())) {
				d += r;
			}
		...
		std::pair<bool, long> search_ground_route(Ex policy, SupportTreeBuilder &builder, const SupportableMesh &sm, const Junction &j, double end_radius, const Vec3d &init_dir = DOWN) {
			...
			auto oresult = solver.to_max().optimize([&j, sd, &policy, &sm, &downdst, &end_radius](const opt::Input<2> &input) {
				                                        auto &[plr, azm] = input;
			                                        ...
			                                        bounds({{PI - sm.cfg.bridge_slope, PI},
			                                                {-PI,                      PI}}));

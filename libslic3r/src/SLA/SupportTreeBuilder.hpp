...
namespace Slic3r {
	namespace sla {
		...
		struct Head : public SupportTreeNode {
			...
			Head(double r_big_mm, double r_small_mm, double length_mm, double penetration, const Vec3d &direction = DOWN,  // direction (normal to the dull end)
		...
		struct Pillar : public SupportTreeNode {
			...
			Pillar(const Vec3d &endp, double h, double start_radius, double end_radius) : height{h}, r_start(start_radius), r_end(end_radius), endpt(endp), starts_from_head(false) {}

			Pillar(const Vec3d &endp, double h, double start_radius) : Pillar(endp, h, start_radius, start_radius) {}
		...
		struct DiffBridge : public Bridge {
			...
			DiffBridge(const Junction &j_s, const Junction &j_e) : Bridge{j_s.pos, j_e.pos, j_s.r}, end_r{j_e.r} {}
		};
		...
		class SupportTreeBuilder {
			...
			public:
				...
				template<class T>
				inline IntegerOnly<T, const Pillar &> pillar(T id) const {
					std::lock_guard<Mutex> lk(m_mutex);
				...
				template<class T>
				inline IntegerOnly<T, Pillar &> pillar(T id) {
					std::lock_guard<Mutex> lk(m_mutex);
					assert(id >= 0 && size_t(id) < m_pillars.size() && size_t(id) < std::numeric_limits<size_t>::max());

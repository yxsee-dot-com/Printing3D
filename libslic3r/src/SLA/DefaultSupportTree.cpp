...
#include <Optimize/NLoptOptimizer.hpp>
#include <SLA/Clustering.hpp>
...
#include "SLA/SupportTreeUtils.hpp"
#include "SLA/SupportTreeUtilsLegacy.hpp"
#include "libslic3r.h"
...
namespace Slic3r {
	namespace sla {
		...
		DefaultSupportTree::DefaultSupportTree(SupportTreeBuilder &builder, const SupportableMesh &sm) : m_sm(sm), m_support_nmls(sm.pts.size(), 3), m_builder(builder), m_points(sm.pts.size(), 3), m_thr(builder.ctl().cancelfn) {
		...
		bool DefaultSupportTree::execute(SupportTreeBuilder &builder, const SupportableMesh &sm) {
			...
			enum Steps {
				BEGIN, PINHEADS, CLASSIFY, ROUTING_GROUND, ROUTING_NONGROUND, CASCADE_PILLARS, MERGE_RESULT, DONE, ABORT, NUM_STEPS
				//...
			};
			...
			auto progress = [&builder, &pc]() {
				static const std::array<std::string, NUM_STEPS> stepstr{"Starting", "Generate pinheads", "Classification", "Routing to ground", "Routing supports to model surface", "Interconnecting pillars", "Merging support mesh", "Done", "Abort"};

				static const std::array<unsigned, NUM_STEPS> stepstate{0, 30, 50, 60, 70, 80, 99, 100, 0};
				...
				switch (pc) {
					...
					case CLASSIFY:
						...
						break;
					case ROUTING_GROUND:
						pc = ROUTING_NONGROUND;
						break;
					case ROUTING_NONGROUND:
						pc = CASCADE_PILLARS;
						break;
					case CASCADE_PILLARS:
						pc = MERGE_RESULT;
						break;
					case MERGE_RESULT:
						pc = DONE;
		...
		AABBMesh::hit_result DefaultSupportTree::pinhead_mesh_intersect(const Vec3d &s, const Vec3d &dir, double r_pin, double r_back, double width, double sd) {
			return sla::pinhead_mesh_hit(suptree_ex_policy, m_sm.emesh, s, dir, r_pin, r_back, width, sd);
		}

		AABBMesh::hit_result DefaultSupportTree::bridge_mesh_intersect(const Vec3d &src, const Vec3d &dir, double r, double sd) {
			return sla::beam_mesh_hit(suptree_ex_policy, m_sm.emesh, {src, dir, r}, sd);
		}

		bool DefaultSupportTree::interconnect(const Pillar &pillar, const Pillar &nextpillar) {
			...
			if (pillar_dist < 2 * m_sm.cfg.head_back_radius_mm || pillar_dist > m_sm.cfg.max_pillar_link_distance_mm) return false;
			...
			bool docrosses = pcm == PillarConnectionMode::cross || (pcm == PillarConnectionMode::dynamic && pillar_dist > 2 * m_sm.cfg.base_radius_mm);
			...
			while (ej.z() >= eupper.z() /*endz*/) {
				...
				if (docrosses) {
					...
					if (sjback.z() <= slower.z() && ejback.z() >= eupper.z() && bridge_mesh_distance(sjback, dirv(sjback, ejback), pillar.r_start) >= bridge_distance) {
		...
		bool DefaultSupportTree::connect_to_nearpillar(const Head &head, long nearpillar_id) {
		...
		bool DefaultSupportTree::create_ground_pillar(const Junction &hjp, const Vec3d &sourcedir, long head_id) {
			auto [ret, pillar_id] = sla::create_ground_pillar(suptree_ex_policy, m_builder, m_sm, hjp.pos, sourcedir, hjp.r, hjp.r, head_id);
		...
		void DefaultSupportTree::add_pinheads() {
			...
			auto nmls = normals(suptree_ex_policy, m_points, m_sm.emesh, m_sm.cfg.head_front_radius_mm, m_thr, filtered_indices);
			...
			for (const SupportPoint &sp: m_sm.pts) {
				m_thr();
				heads.emplace_back(NaNd, sp.head_front_radius, 0., m_sm.cfg.head_penetration_mm, Vec3d::Zero(),         // dir
			...
			filterfn = [this, &nmls, &heads, &filterfn](unsigned fidx, size_t i, double back_r) {
				...
				double w = lmin + 2 * back_r + 2 * m_sm.cfg.head_front_radius_mm - m_sm.cfg.head_penetration_mm;
				...
				if (t.distance() < w) {
					...
					auto oresult = solver.to_max().optimize([this, pin_r, back_r, hp](const opt::Input<3> &input) {
					                                        ...
					                                        bounds({{PI - m_sm.cfg.bridge_slope, PI},    // Must not exceed the slope limit
			...
			execution::for_each(suptree_ex_policy, size_t(0), filtered_indices.size(), [this, &filterfn, &filtered_indices](size_t i) {
		...
		void DefaultSupportTree::classify() {
			...
			auto predicate = [this](const PointIndexEl &e1, const PointIndexEl &e2) {
				...
				return d2d < 2 * m_sm.cfg.base_radius_mm && d3d < m_sm.cfg.max_bridge_length_mm;
			};

			m_pillar_clusters = cluster(ground_head_indices, pointfn, predicate, m_sm.cfg.max_bridges_on_pillar);
		}
		...
		void DefaultSupportTree::routing_to_ground() {
			...
			for (auto &cl: m_pillar_clusters) {
				...
				long lcid = cluster_centroid(cl, [&points](size_t idx) { return points.row(long(idx)); }, [thr](const Vec3d &p1, const Vec3d &p2) {
					thr();
		...
		bool DefaultSupportTree::connect_to_ground(Head &head) {
			auto [ret, pillar_id] = sla::search_ground_route(suptree_ex_policy, m_builder, m_sm, {head.junction_point(), head.r_back_mm}, head.r_back_mm, head.dir);
		...
		bool DefaultSupportTree::connect_to_model_body(Head &head) {
			...
			m_builder.add_anchor(head.r_back_mm, head.r_pin_mm, w, m_sm.cfg.head_penetration_mm, taildir, hitp);
			...
			while (nearest_id < 0 && !spindex.empty()) {
				...
				if (nearest_id >= 0) {
					if (size_t(nearest_id) < m_builder.pillarcount()) {
						if (!connect_to_nearpillar(source, nearest_id) || m_builder.pillar(nearest_id).r_start < source.r_back_mm) {
		...
		void DefaultSupportTree::routing_to_model() {
			...
			execution::for_each(suptree_ex_policy, m_iheads_onmodel.begin(), m_iheads_onmodel.end(), [this](const unsigned idx) {
				m_thr();
		...
		void DefaultSupportTree::interconnect_pillars() {
			...
			auto cascadefn = [this, d, &pairs, min_height_ratio, H1](const PointIndexEl &el) {
				...
				std::sort(qres.begin(), qres.end(), [qp](const PointIndexEl &e1, const PointIndexEl &e2) {
			...
			for (size_t pid = 0; pid < pillarcount; pid++) {
				...
				double min_dist = m_sm.cfg.pillar_base_safety_distance_mm + m_sm.cfg.base_radius_mm + EPSILON;
				...
				while (!found && alpha < 2 * PI) {
					for (unsigned n = 0; n < needpillars && (!n || canplace[n - 1]); n++) {
						...
						canplace[n] = std::isinf(hr.distance()) && std::sqrt(m_sm.emesh.squared_distance(gndsp)) > min_dist;
					}
				...
				if (found)
					for (unsigned n = 0; n < needpillars; n++) {
						...
						if (interconnect(pillar(), p)) {
							...
							double t = bridge_mesh_distance(pillarsp, dirv(pillarsp, s), pillar().r_start);

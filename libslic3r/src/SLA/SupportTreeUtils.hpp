...
namespace Slic3r {
	namespace sla {
		...
		struct Beam_ { // Defines a set of rays displaced along a cone's surface
			...
			Beam_(const Vec3d &s, const Vec3d &d, double R1, double R2) : src{s}, dir{d}, r1{R1}, r2{R2} {};

			Beam_(const Ball &src_ball, const Ball &dst_ball) : src{src_ball.p}, dir{dirv(src_ball.p, dst_ball.p)}, r1{src_ball.R} {
				r2 = src_ball.R;
			...
			Beam_(const Vec3d &s, const Vec3d &d, double R) : src{s}, dir{d}, r1{R}, r2{R} {}
		};
		...
		Hit beam_mesh_hit(Ex policy, const AABBMesh &mesh, const Beam_<RayCount> &beam, double sd) {
			...
			execution::for_each(policy, size_t(0), hits.size(), [&mesh, r_src, r_dst, src, dst, &ring, dir, sd, &hits](size_t i) {
				Hit &hit = hits[i];
		...
		Hit pinhead_mesh_hit(Ex ex, const AABBMesh &mesh, const Vec3d &s, const Vec3d &dir, double r_pin, double r_back, double width, double sd) {
			...
			execution::for_each(ex, size_t(0), hits.size(), [&m, &rings, sd, &hits](size_t i) {
				// Point on the circle on the pin sphere
		...
		Hit pinhead_mesh_hit(Ex ex, const AABBMesh &mesh, const Head &head, double safety_d) {
			return pinhead_mesh_hit(ex, mesh, head.pos, head.dir, head.r_pin_mm, head.r_back_mm, head.width_mm, safety_d);
		}
		...
		std::vector<size_t> non_duplicate_suppt_indices(const PtIndex &index, const SupportPoints &suppts, double eps) {
			...
			for (size_t i = 0; i < suppts.size(); ++i) {
				size_t closest_idx = find_closest_point(index, suppts[i].pos, [&i, &to_remove](size_t i_closest) {
					return i_closest != i && !to_remove[i_closest];
				});

				if (closest_idx < suppts.size() && (suppts[i].pos - suppts[closest_idx].pos).norm() < eps)
					to_remove[i] = true;
			}
		...
		bool optimize_pinhead_placement(Ex policy, const SupportableMesh &m, Head &head) {
			...
			double w = lmin + 2 * back_r + 2 * m.cfg.head_front_radius_mm - m.cfg.head_penetration_mm;
			...
			if (t.distance() < w) {
				...
				auto oresult = solver.to_max().optimize([&m, pin_r, back_r, hp, sd, policy](const opt::Input<3> &input) {
					                                        auto &[plr, azm, l] = input;

					                                        auto dir = spheric_to_dir(plr, azm).normalized();

					                                        return pinhead_mesh_hit(policy, m.emesh, hp, dir, pin_r, back_r, l, sd).distance();
		...
		std::optional<Head> calculate_pinhead_placement(Ex policy, const SupportableMesh &sm, size_t suppt_idx) {
			...
			Head head{sm.cfg.head_back_radius_mm, sp.head_front_radius, 0., sm.cfg.head_penetration_mm, Vec3d::Zero(),        // dir
		...
		inline long build_ground_connection(SupportTreeBuilder &builder, const SupportableMesh &sm, const GroundConnection &conn) {
		...
		template<class Fn> constexpr bool IsWideningFn = std::is_invocable_r_v</*retval*/ double, Fn, Ball /*source*/, Vec3d /*dir*/, double /*length*/>;
		...
		Vec3d check_ground_route(Ex policy, const SupportableMesh &sm, const Junction &source,      // source location
			...
			if (bridge_len > EPSILON && type == GroundRouteCheck::Full) {
				...
				Beam_<Samples> bridgebeam{Ball{source.pos, source.r}, Ball{bridge_end, bridge_r}};
			...
			if (brhit_dist < bridge_len) {
				...
				Beam_<Samples> gndbeam{{bridge_end, bridge_r},
				                       {gp,         end_radius}};
		...
		GroundConnection deepsearch_ground_connection(Ex policy, const SupportableMesh &sm, const Junction &source, WideningFn &&wideningfn, const Vec3d &init_dir = DOWN) {
			...
			auto bound_constraints = bounds({{PI - sm.cfg.bridge_slope, PI},   // bounds for polar angle
			...
			auto oresult = solver.to_min().optimize(z_fn, initvals({plr_init, azm_init, 0.}), bound_constraints);
			...
			while (zlvl > gndlvl && l <= l_max) {
				...
				zlvl = check_ground_route(policy, sm, source, n, l, wideningfn, GroundRouteCheck::PillarOnly).z();
		...
		struct DefaultWideningModel {
			...
			double operator()(const Ball &src, const Vec3d & /*dir*/, double len) {
				static_assert(IsWideningFn<DefaultWideningModel>, "DefaultWideningModel is not a widening function");
		...
		GroundConnection deepsearch_ground_connection(Ex policy, const SupportableMesh &sm, const Junction &source, const Vec3d &init_dir = DOWN) {
			return deepsearch_ground_connection(policy, sm, source, DefaultWideningModel{sm}, init_dir);
		}
		...
		bool optimize_anchor_placement(Ex policy, const SupportableMesh &sm, const Junction &from, Anchor &anchor) {
			...
			double lmin = 0;
			double lmax = std::min(sm.cfg.head_width_mm, distance(from.pos, anchor.pos) - 2 * from.r);

			double sd = sm.cfg.safety_distance(anchor.r_back_mm);

			Optimizer<AlgNLoptGenetic> solver(get_criteria(sm.cfg).stop_score(anchor.fullwidth()).max_iterations(100));

			solver.seed(0); // deterministic behavior

			auto oresult = solver.to_max().optimize([&sm, &anchor, sd, policy](const opt::Input<3> &input) {
				auto &[plr, azm, l] = input;
			...
			}, initvals({polar, azimuth, (lmin + lmax) / 2.}), bounds({{0.,   sm.cfg.bridge_slope}, // Must not exceed the slope limit
			                                                           {-PI,  PI}, // azimuth can be a full search
			                                                           {lmin, lmax}}));
		...
		inline bool is_outside_support_cone(const Vec3f &supp, const Vec3f &pt, float angle) {
			using namespace Slic3r;

			Vec3d D = (pt - supp).cast<double>();
			double dot_sq = -D.z() * std::abs(-D.z());

			return dot_sq < D.squaredNorm() * std::cos(angle) * std::abs(std::cos(angle));
		}
		...
		std::optional<Vec3f> find_merge_pt(const Vec3f &A, const Vec3f &B, float critical_angle) {
			...
			double t1 = (Db.y() * b.x() - b.y() * Db.x()) / (Da.x() * Db.y() - Da.y() * Db.x());

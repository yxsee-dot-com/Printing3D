...
namespace Slic3r {
	namespace sla {
		...
		namespace {
			...
			double get_misalginment_score(const TriangleMesh &mesh, const Transform3f &tr) {
				...
				auto accessfn = [&mesh, &tr](size_t fi) {
					...
					float score = fc.area * (std::abs(fc.normal.dot(Vec3f::UnitX())) + std::abs(fc.normal.dot(Vec3f::UnitY())) + std::abs(fc.normal.dot(Vec3f::UnitZ())));
			...
			std::vector<XYRotation> get_chull_rotations(const TriangleMesh &mesh, size_t max_count) {
				...
				std::sort(inputs.begin(), inputs.end(), [](const RotArea &ra, const RotArea &rb) {
			...
			std::array<double, N> find_min_score(Fn &&fn, It from, It to, StopCond &&stopfn) {
				...
				execution::for_each(ex_tbb, size_t(0), dist, [&stopfn, &scores, &fn, &from](size_t i) {
					if (stopfn()) return;
		...
		struct RotfinderBoilerplate {
			...
			RotfinderBoilerplate(const ModelObject &mo, const RotOptimizeParams &p) : mesh{get_mesh_to_rotate(mo)}, max_tries(p.accuracy() * MAX_TRIES), params{p} {}
		...
		Vec2d find_best_misalignment_rotation(const ModelObject &mo, const RotOptimizeParams &params) {
			...
			opt::Optimizer<opt::AlgBruteForce> solver(opt::StopCriteria{}.max_iterations(bp.max_tries).stop_condition([&bp] { return bp.stopcond(); }), gridsize);

			// We are searching rotations around only two axes x, y. Thus the
			// problem becomes a 2 dimensional optimization task.
			// We can specify the bounds for a dimension in the following way:
			auto bounds = opt::bounds({{-PI, PI},
			                           {-PI, PI}});

			auto result = solver.to_max().optimize([&bp](const XYRotation &rot) {
				bp.statusfn();
		...
		Vec2d find_least_supports_rotation(const ModelObject &mo, const RotOptimizeParams &params) {
			...
			if (is_on_floor(pocfg)) {
				...
				auto result = solver.to_min().optimize([&bp](const XYRotation &rot) {
					bp.statusfn();

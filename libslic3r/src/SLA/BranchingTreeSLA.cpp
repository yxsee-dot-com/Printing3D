...
#include "Pad.hpp"
#include "BranchingTree/BranchingTree.hpp"
...
namespace Slic3r {
	namespace sla {
		...
		class BranchingTreeBuilder : public branchingtree::Builder {
				...
				void build_subtree(size_t root) {
					traverse(m_cloud, root, [this](const branchingtree::Node &node) {
						if (node.left >= 0 && node.right >= 0) {
							...
							m_builder.add_diffbridge(from.pos.cast<double>(), tod, get_radius(from), toR);
			...
			public:
				BranchingTreeBuilder(SupportTreeBuilder &builder, const SupportableMesh &sm, const branchingtree::PointCloud &cloud) : m_builder{builder}, m_sm{sm}, m_cloud{cloud} {}
				...
				bool add_ground_bridge(const branchingtree::Node &from, const branchingtree::Node &/*to*/) override;
				...
				void report_unroutable(const branchingtree::Node &j) override {
					...
					BOOST_LOG_TRIVIAL(warning) << "Cannot route junction at " << j.pos.x() << " " << j.pos.y() << " " << j.pos.z();
		...
		bool BranchingTreeBuilder::add_bridge(const branchingtree::Node &from, const branchingtree::Node &to) {
			...
			auto hit = beam_mesh_hit(beam_ex_policy, m_sm.emesh, beam, m_sm.cfg.safety_distance_mm);
		...
		bool BranchingTreeBuilder::add_merger(const branchingtree::Node &node, const branchingtree::Node &closest, const branchingtree::Node &merge_node) {
			Vec3d from1d = node.pos.cast<double>(), from2d = closest.pos.cast<double>(), tod = merge_node.pos.cast<double>();
			...
			bool ret = hit1.distance() > (tod - from1d).norm() && hit2.distance() > (tod - from2d).norm();

			return ret;
		}

		bool BranchingTreeBuilder::add_ground_bridge(const branchingtree::Node &from, const branchingtree::Node &to) {
			bool ret = false;
			...
			if (it == m_gnd_connections.end()) {
				...
				auto conn = deepsearch_ground_connection(beam_ex_policy, m_sm, j, get_radius(to), init_dir);
		...
		bool BranchingTreeBuilder::add_mesh_bridge(const branchingtree::Node &from, const branchingtree::Node &to) {
			if (from.weight > m_sm.cfg.max_weight_on_model_support)
				return false;

			sla::Junction fromj = {from.pos.cast<double>(), get_radius(from)};

			auto anchor = m_sm.cfg.ground_facing_only ? std::optional<Anchor>{} : // If no mesh connections are allowed
			              calculate_anchor_placement(beam_ex_policy, m_sm, fromj, to.pos.cast<double>());

			if (anchor) {
				sla::Junction toj = {anchor->junction_point(), anchor->r_back_mm};

				auto hit = beam_mesh_hit(beam_ex_policy, m_sm.emesh, Beam{{fromj.pos, fromj.r},
				                                                          {toj.pos,   toj.r}}, 0.);
		...
		std::optional<Vec3f> BranchingTreeBuilder::suggest_avoidance(const branchingtree::Node &from, float max_bridge_len) const {
			std::optional<Vec3f> ret;
			...
			if (found_it != m_gnd_connections.end()) {
				...
				auto conn = deepsearch_ground_connection(beam_ex_policy, m_sm, j, get_radius(dst), sla::DOWN);
		...
		inline void build_pillars(SupportTreeBuilder &builder, BranchingTreeBuilder &vbuilder, const SupportableMesh &sm) {
		...
		void create_branching_tree(SupportTreeBuilder &builder, const SupportableMesh &sm) {
			...
			execution::for_each(ex_tbb, size_t(0), nondup_idx.size(), [&sm, &heads, &nondup_idx, &builder](size_t i) {
			...
			auto props = branchingtree::Properties{}.bed_shape(bedpolys).ground_level(sla::ground_level(sm)).max_slope(sm.cfg.bridge_slope).max_branch_length(sm.cfg.max_bridge_length_mm);

			auto meshpts = sm.cfg.ground_facing_only ? std::vector<branchingtree::Node>{} : branchingtree::sample_mesh(its, props.sampling_radius());

			auto bedpts = branchingtree::sample_bed(props.bed_shape(), float(props.ground_level()), props.sampling_radius());

			for (auto &bp: bedpts)
				bp.Rmin = sm.cfg.head_back_radius_mm;

			branchingtree::PointCloud nodes{std::move(meshpts), std::move(bedpts), std::move(leafs), props};

			BranchingTreeBuilder vbuilder{builder, sm, nodes};

			execution::for_each(ex_tbb, size_t(0), nodes.get_leafs().size(), [&nodes, &vbuilder](size_t leaf_idx) {
				vbuilder.suggest_avoidance(nodes.get_leafs()[leaf_idx], nodes.properties().max_branch_length());
			});

...
#include <SLA/DefaultSupportTree.hpp>
#include <SLA/BranchingTreeSLA.hpp>
#include <MTUtils.hpp>
	...
	namespace sla {
		...
		indexed_triangle_set create_support_tree(const SupportableMesh &sm, const JobController &ctl) {
			...
			if (sm.cfg.enabled) {
				...
				BOOST_LOG_TRIVIAL(info) << "Support tree creation took: " << duration<double>{stop - start}.count() << " seconds";
		...
		indexed_triangle_set create_pad(const SupportableMesh &sm, const indexed_triangle_set &support_mesh, const JobController &ctl) {
			...
			if (!sm.cfg.enabled || sm.pad_cfg.embed_object) {
				...
				// creation.
				sla::pad_blueprint(*sm.emesh.get_triangle_mesh(), model_contours, heights, ctl.cancelfn);
			}

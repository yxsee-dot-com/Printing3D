	...
	namespace sla {
		...
		class PillarIndex {
			...
			public:
				...
				template<class...Args>
				inline void guarded_insert(Args &&...args) {
				...
				template<class Fn>
				inline void foreach(Fn fn) { m_index.foreach(fn); }
				template<class Fn>
				inline void guarded_foreach(Fn fn) {
					std::lock_guard<Mutex> lck(m_mutex);
					m_index.foreach(fn);
				}
		...
		class DefaultSupportTree {
				...
				inline AABBMesh::hit_result ray_mesh_intersect(const Vec3d &s, const Vec3d &dir) {
				...
				AABBMesh::hit_result pinhead_mesh_intersect(const Vec3d &s, const Vec3d &dir, double r_pin, double r_back, double width) {
					return pinhead_mesh_intersect(s, dir, r_pin, r_back, width, r_back * m_sm.cfg.safety_distance_mm / m_sm.cfg.head_back_radius_mm);
				}
				...
				AABBMesh::hit_result bridge_mesh_intersect(const Vec3d &s, const Vec3d &dir, double r) {
					return bridge_mesh_intersect(s, dir, r, r * m_sm.cfg.safety_distance_mm / m_sm.cfg.head_back_radius_mm);
				}

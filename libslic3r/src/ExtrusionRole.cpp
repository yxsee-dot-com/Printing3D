...
namespace Slic3r {
	...
	GCodeExtrusionRole extrusion_role_to_gcode_extrusion_role(ExtrusionRole role) {
		...
		if (role.is_perimeter()) {
			return role.is_bridge() ? GCodeExtrusionRole::OverhangPerimeter : role.is_external() ? GCodeExtrusionRole::ExternalPerimeter : GCodeExtrusionRole::Perimeter;
		}
	...
	std::string gcode_extrusion_role_to_string(GCodeExtrusionRole role) {
		switch (role) {
			...
			case GCodeExtrusionRole::SupportMaterialInterface     :
				return L("Support material interface");

...
namespace Slic3r {
...
#endif
			struct Visitor {
				Visitor(const EdgeGrid::Grid &grid, const size_t idx_contour, const std::vector<ResampledPoint> &resampled_point_parameters, double dist_same_contour_reject) : grid(grid), idx_contour(idx_contour), resampled_point_parameters(resampled_point_parameters), dist_same_contour_reject(dist_same_contour_reject) {}
							...
							if ((!this->found || dist < this->distance) && this->dir_inside.dot(bisector) > 0) {
								...
								if (it_contour_and_segment->first == idx_contour) {
									...
									if (dist_along_contour < dist_same_contour_accept)
										...
										bool inside = (t == 0.) ? this->inside_corner(contour, ipt, this->point) : (t == 1.) ? this->inside_corner(contour, contour.segment_idx_next(ipt), this->point) : this->left_of_segment(contour, ipt, this->point);
			...
			for (int i = 0; i < int(compensation.size()); ++i) {
				...
				if (l2 < dist_min2) {
					...
					while (j != i) {
						...
						if (lnext > band) {
							...
							prev = use_min ? std::min(prev, lerp(compensation[jprev], compensation[j], (band - l) / lthis)) : lerp(compensation[jprev], compensation[j], (band - l) / lthis);
							break;
						}
				...
				if (l2 < dist_min2) {
					...
					while (j != i) {
						...
						if (lnext > band) {
							// Interpolate the compensation value.
							next = use_min ? std::min(next, lerp(compensation[jprev], compensation[j], (band - l) / lthis)) : lerp(compensation[jprev], compensation[j], (band - l) / lthis);
							break;
						}
	...
	ExPolygon elephant_foot_compensation(const ExPolygon &input_expoly, double min_contour_width, const double compensation) {
		...
		ExPolygon out;
		if (bbox_size.x() < min_contour_width_compensated + SCALED_EPSILON || bbox_size.y() < min_contour_width_compensated + SCALED_EPSILON || input_expoly.area() < min_contour_width_compensated * min_contour_width_compensated * 5.) {

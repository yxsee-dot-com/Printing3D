...
namespace Slic3r {
	...
	namespace SupportSpotsGenerator {
		...
		struct Params {
			Params(const std::vector<std::string> &filament_types, float max_acceleration, int raft_layers_count, BrimType brim_type, float brim_width) : max_acceleration(max_acceleration), raft_layers_count(raft_layers_count), brim_type(brim_type), brim_width(brim_width) {
		...
		struct SupportPoint {
			SupportPoint(SupportPointCause cause, const Vec3f &position, float spot_radius) : cause(cause), position(position), spot_radius(spot_radius) {}
		...
		struct PartialObject {
			PartialObject(Vec3f centroid, float volume, bool connected_to_bed) : centroid(centroid), volume(volume), connected_to_bed(connected_to_bed) {}
		...
		class ObjectPart {
			public:
				...
				ObjectPart(const std::vector<const ExtrusionEntityCollection *> &extrusion_collections, const bool connected_to_bed, const coordf_t print_head_z, const coordf_t layer_height, const std::optional<Polygons> &brim);

				void add(const ObjectPart &other);

				void add_support_point(const Vec3f &position, float sticking_area);

				float compute_elastic_section_modulus(const Vec2f &line_dir, const Vec3f &extreme_point, const Integrals &integrals) const;

				std::tuple<float, SupportPointCause> is_stable_while_extruding(const SliceConnection &connection, const ExtrusionLine &extruded_line, const Vec3f &extreme_point, float layer_z, const Params &params) const;
		};

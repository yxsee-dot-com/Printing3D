...
#define DEBUG_INTERSECTION_LINE 0
	...
	class IntersectionLine : public Line {
		public:
			...
			enum class FacetEdgeType {
				...
				General, // Two vertices are aligned with the cutting plane, the third vertex is below the cutting plane.
				Top, // Two vertices are aligned with the cutting plane, the third vertex is above the cutting plane.
				Bottom, // Two vertices are aligned with the cutting plane, the edge is shared by two triangles, where one
				...
				TopBottom, // All three vertices of a face are aligned with the cutting plane.
				Horizontal, // Edge
				Slab,
			};
			...
			enum {
				...
				EDGE0_NO_NEIGHBOR = 0x001, EDGE1_NO_NEIGHBOR = 0x002, EDGE2_NO_NEIGHBOR = 0x004, // Triangle edge added, because it makes a fold with another horizontal edge.
				EDGE0_FOLD = 0x010, EDGE1_FOLD = 0x020, EDGE2_FOLD = 0x040, // The edge cannot be a seed of a greedy loop extraction (folds are not safe to become seeds).
				NO_SEED = 0x100, SKIP = 0x200,
			};
			uint32_t flags{0};
	...
	inline FacetSliceType slice_facet(
			...
			const Eigen::Matrix<T, 3, 1, Eigen::DontAlign> *vertices, const stl_triangle_vertex_indices &indices, const Vec3i &edge_ids, const int idx_vertex_lowest, const bool horizontal, IntersectionLine &line_out) {
		...
		for (int j = 0; j < 3; ++j) {  // loop through facet edges
			...
			if (a->z() == slice_z) {
				...
				static_cast<Point &>(point) = t <= 0. ? v3f_scaled_to_contour_point(*a) : t >= 1. ? v3f_scaled_to_contour_point(*b) : v3f_scaled_to_contour_point(a->template head<2>().
						template cast<double>() * (1. - t) + b->template head<2>().
						template cast<double>() * t + Vec2d(0.5, 0.5));
				point.edge_id = edge_id;
				++num_points;
#endif
			}
	...
	void slice_facet_at_zs(
			...
			const std::vector<Vec3f> &mesh_vertices, const TransformVertex &transform_vertex_fn, const stl_triangle_vertex_indices &indices, const Vec3i &edge_ids,
			...
			const std::vector<float> &zs, std::vector<IntersectionLines> &lines, LinesMutexes &lines_mutex) {
	...
	static inline std::vector<IntersectionLines> slice_make_lines(const std::vector<stl_vertex> &vertices, const TransformVertex &transform_vertex_fn, const std::vector<stl_triangle_vertex_indices> &indices, const std::vector<Vec3i> &face_edge_ids, const std::vector<float> &zs, const ThrowOnCancel throw_on_cancel_fn) {
		std::vector<IntersectionLines> lines(zs.size(), IntersectionLines{});
		LinesMutexes lines_mutex;
		tbb::parallel_for(tbb::blocked_range<int>(0, int(indices.size())), [&vertices, &transform_vertex_fn, &indices, &face_edge_ids, &zs, &lines, &lines_mutex, throw_on_cancel_fn](const tbb::blocked_range<int> &range) {
	...
	static inline IntersectionLines slice_make_lines(const std::vector<stl_vertex> &mesh_vertices, const TransformVertex &transform_vertex_fn, const std::vector<stl_triangle_vertex_indices> &mesh_faces, const std::vector<Vec3i> &face_edge_ids, const float plane_z, FaceFilter face_filter) {
	...
	enum class FaceOrientation : char {
		...
		Up, // Z component of the normal is negative.
		Down, // Z component of the normal is zero.
		Vertical, // Triangle is degenerate, thus its normal is undefined. We may want to slice the degenerate triangles
	...
	void slice_facet_with_slabs(
			// Scaled or unscaled vertices. transform_vertex_fn may scale zs.
			const std::vector<Vec3f> &mesh_vertices, const std::vector<stl_triangle_vertex_indices> &mesh_triangles, const size_t facet_idx, const Vec3i &facet_neighbors, const Vec3i &facet_edge_ids,
			...
			const int num_edges, const std::vector<float> &zs, SlabLines &lines, LinesMutexes &lines_mutex) {
		...
		if (min_layer == max_layer || horizontal) {
			...
			assert(horizontal || zs.empty() || max_z < zs.front() || min_z > zs.back() || (min_layer == max_layer && min_layer != zs.end() && min_layer != zs.begin() && *(min_layer - 1) < min_z && *min_layer > max_z));
			if (horizontal && min_layer != zs.end() && *min_layer == min_z) {
				...
				if (size_t line_id = ProjectionFromTop ? slice_id : slice_id + 1; ProjectionFromTop || line_id < lines.at_slice.size())
					...
					for (int iedge = 0; iedge < 3; ++iedge)
						if (facet_neighbors(iedge) == -1) {
							...
							//						Point a=to_2d(vertices[i]).cast<coord_t>();
							//						Point b=to_2d(vertices[j]).cast<coord_t>();
							//                        IntersectionLine il {
							...
							IntersectionLine il;
							il.a = to_2d(vertices[i]).cast<coord_t>();
							il.b = to_2d(vertices[j]).cast<coord_t>();
							il.a_id = indices(i);
							il.b_id = indices(j);
							il.edge_a_id = -1;
							il.edge_b_id = -1;
							il.edge_type = ProjectionFromTop ? IntersectionLine::FacetEdgeType::Bottom : IntersectionLine::FacetEdgeType::Top;
				...
				for (int iedge = 0; iedge < 3; ++iedge)
					if (facet_neighbors(iedge) == -1) {
						...
						il.edge_a_id = -1;
						il.edge_b_id = -1;
						il.edge_type = IntersectionLine::FacetEdgeType::Slab;
						emit_slab_edge(il, slab_id, !ProjectionFromTop);
					}
			}
		} else {
					...
					for (int iedge = 0; iedge < 3; ++iedge)
						if (facet_neighbors(iedge) == -1) {
							...
							if (intersects_this && intersects_prev) {
								...
								IntersectionLine il2;
								il2.a = l.a;
								il2.b = l.b;
								il2.a_id = -1;
								il2.b_id = -1;
								il2.edge_a_id = edge_id;
								il2.edge_b_id = edge_id + num_edges;
								il2.edge_type = IntersectionLine::FacetEdgeType::Slab;
								emit_slab_edge(il2, slab_id, ProjectionFromTop != edge_up);
								...
								IntersectionLine il2;
								il2.a = to_2d(edge_up ? vertices[i] : vertices[j]).cast<coord_t>();
								il2.b = il.edge_a_id == edge_id ? il.a : il.b;
								il2.a_id = edge_up ? indices(i) : indices(j);
								il2.b_id = -1;
								il2.edge_a_id = -1;
								il2.edge_b_id = edge_id + num_edges;
								il2.edge_type = IntersectionLine::FacetEdgeType::Slab;
								...
								IntersectionLine il2;
								il2.a = il_prev.edge_a_id == edge_id ? il_prev.a : il_prev.b;
								il2.b = to_2d(edge_up ? vertices[j] : vertices[i]).cast<coord_t>();
								il2.a_id = -1;
								il2.b_id = edge_up ? indices(j) : indices(i);
								il2.edge_a_id = edge_id;
								il2.edge_b_id = -1;
								il2.edge_type = IntersectionLine::FacetEdgeType::Slab;
								...
								if (inside_slab) {
									...
									IntersectionLine il2;
									il2.a = to_2d(vertices[i]).cast<coord_t>();
									il2.b = to_2d(vertices[j]).cast<coord_t>();
									il2.a_id = indices(i);
									il2.b_id = indices(j);
									il2.edge_a_id = -1;
									il2.edge_b_id = -1;
									...
									emit_slab_edge(il2, slab_id, !ProjectionFromTop);
								}
							}
						}
				}
	...
	inline std::pair<SlabLines, SlabLines> slice_slabs_make_lines(const std::vector<stl_vertex> &vertices, const std::vector<stl_triangle_vertex_indices> &indices, const std::vector<Vec3i> &face_neighbors, const std::vector<Vec3i> &face_edge_ids,
			...
			// num_edges will be used to distinguish between intersections with the top and bottom plane.
			                                                      const int num_edges, const std::vector<FaceOrientation> &face_orientation, const std::vector<float> &zs, bool top, bool bottom, const ThrowOnCancel throw_on_cancel_fn) {
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, int(indices.size())), [&vertices, &indices, &face_neighbors, &face_edge_ids, num_edges, &face_orientation, &zs, top, bottom, &lines_top, &lines_bottom, &lines_mutex_top, &lines_mutex_bottom, throw_on_cancel_fn](const tbb::blocked_range<int> &range) {
			for (int face_idx = range.begin(); face_idx < range.end(); ++face_idx) {
				if ((face_idx & 0x0ffff) == 0)
					throw_on_cancel_fn();
	...
	struct OpenPolyline {
		...
		OpenPolyline(const IntersectionReference &start, const IntersectionReference &end, Points &&points) : start(start), end(end), points(std::move(points)), consumed(false) { this->length = Slic3r::length(this->points); }
		void reverse() {
			std::swap(start, end);
			...
			*/
			...
			for (;;) {
				...
				if (next_line == nullptr) {
					...
					if ((first_line->edge_a_id != -1 && first_line->edge_a_id == last_line->edge_b_id) || (first_line->a_id != -1 && first_line->a_id == last_line->b_id)) {
						...
						open_polylines.emplace_back(OpenPolyline(IntersectionReference(first_line->a_id, first_line->edge_a_id), IntersectionReference(last_line->b_id, last_line->edge_b_id), std::move(loop_pts)));
					}
					break;
				}
				/*
...
found:
				...
				// Check whether we closed this loop.
				if ((opl->start.edge_id != -1 && opl->start.edge_id == opl->end.edge_id) || (opl->start.point_id != -1 && opl->start.point_id == opl->end.point_id)) {
					// The current loop is complete. Add it to the output.
	...
	static std::vector<Polygons> make_loops(
			// Lines will have their flags modified.
			std::vector<IntersectionLines> &lines, const MeshSlicingParams &params, ThrowOnCancel throw_on_cancel) {
		std::vector<Polygons> layers;
		layers.resize(lines.size());
		tbb::parallel_for(tbb::blocked_range<size_t>(0, lines.size()), [&lines, &layers, &params, throw_on_cancel](const tbb::blocked_range<size_t> &range) {
	...
	static std::vector<Polygons> make_slab_loops(
			...
			int num_edges, ThrowOnCancel throw_on_cancel) {
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
		static int iRun = 0;
		++ iRun;
#endif // SLIC3R_DEBUG_SLICE_PROCESSING
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, int(lines.at_slice.size())), [&lines, num_edges, &layers, throw_on_cancel](const tbb::blocked_range<int> &range) {
	...
	static void make_expolygons(const Polygons &loops, const float closing_radius, const float extra_offset, ClipperLib::PolyFillType fill_type, ExPolygons *slices) {
		...
		*/
		...
		expolygons_append(*slices, offset_out > 0 && offset_in < 0 ? offset2_ex(union_ex(loops, fill_type), offset_out, offset_in) : offset_out > 0 ? offset_ex(union_ex(loops, fill_type), offset_out) : offset_in < 0 ? offset_ex(union_ex(loops, fill_type), offset_in) : union_ex(loops, fill_type));
	}
	...
	std::vector<Polygons> slice_mesh(const indexed_triangle_set &mesh,
			// Unscaled Zs
			                         const std::vector<float> &zs, const MeshSlicingParams &params, std::function<void()> throw_on_cancel) {
...

		{
			...
			if (zs.size() <= 1) {
				...
				if (is_identity(params.trafo)) {
					lines = slice_make_lines(mesh.vertices, [](const Vec3f &p) { return Vec3f(scaled<float>(p.x()), scaled<float>(p.y()), p.z()); }, mesh.indices, face_edge_ids, zs, throw_on_cancel);
				} else {
				...
				lines = slice_make_lines(transform_mesh_vertices_for_slicing(mesh, params.trafo), [](const Vec3f &p) { return p; }, mesh.indices, face_edge_ids, zs, throw_on_cancel);
			}
		}
	...
	Polygons slice_mesh(const indexed_triangle_set &mesh,
			// Unscaled Zs
			            const float plane_z, const MeshSlicingParams &params) {
		std::vector<IntersectionLines> lines;

		{
			...
			// It likely is not worthwile to copy the vertices. Apply the transformation in place.
			if (trafo_identity) {
				lines.emplace_back(slice_make_lines(mesh.vertices, [](const Vec3f &p) { return Vec3f(scaled<float>(p.x()), scaled<float>(p.y()), p.z()); }, mesh.indices, face_edge_ids, plane_z, [&face_mask](int face_idx) { return face_mask[face_idx]; }));
			} else {
				// Transform the vertices, scale up in XY, not in Z.
				lines.emplace_back(slice_make_lines(mesh.vertices, [tf](const Vec3f &p) { return tf * p; }, mesh.indices, face_edge_ids, plane_z, [&face_mask](int face_idx) { return face_mask[face_idx]; }));
			}
		}
	...
	std::vector<ExPolygons> slice_mesh_ex(const indexed_triangle_set &mesh, const std::vector<float> &zs, const MeshSlicingParamsEx &params, std::function<void()> throw_on_cancel) {
		...
		tbb::parallel_for(tbb::blocked_range<size_t>(0, layers_p.size()), [&layers_p, &params, &layers, throw_on_cancel](const tbb::blocked_range<size_t> &range) {
			...
			for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
				...
				Slic3r::make_expolygons(layers_p[layer_id], params.closing_radius, params.extra_offset, this_mode == MeshSlicingParams::SlicingMode::EvenOdd ? ClipperLib::pftEvenOdd : this_mode == MeshSlicingParams::SlicingMode::PositiveLargestContour ? ClipperLib::pftPositive : ClipperLib::pftNonZero, &expolygons);
	...
	void slice_mesh_slabs(const indexed_triangle_set &mesh,
			// Unscaled Zs
			              const std::vector<float> &zs, const Transform3d &trafo, std::vector<Polygons> *out_top, std::vector<Polygons> *out_bottom, std::function<void()> throw_on_cancel) {
		...
		std::pair<SlabLines, SlabLines> lines = slice_slabs_make_lines(vertices_transformed, mesh.indices, face_neighbors, face_edge_ids, num_edges, face_orientation, zs, out_top != nullptr, out_bottom != nullptr, throw_on_cancel);
	...
	static void triangulate_slice(indexed_triangle_set &its, IntersectionLines &lines, std::vector<int> &slice_vertices,
			...
			// Z height of the slice.
			                      float z, bool triangulate, bool normals_down) {
		...
		std::sort(map_vertex_to_index.begin(), map_vertex_to_index.end(), [](const std::pair<Vec2f, int> &l, const std::pair<Vec2f, int> &r) {
			return l.first.x() < r.first.x() || (l.first.x() == r.first.x() && (l.first.y() < r.first.y() || (l.first.y() == r.first.y() && l.second < r.second)));
		});
		...
		if (triangulate) {
			...
			for (size_t i = 0; i < triangles.size();) {
				...
				for (size_t j = 0; j < 3; ++j) {
					...
					auto it = lower_bound_by_predicate(map_vertex_to_index.begin(), map_vertex_to_index.end(), [&v](const std::pair<Vec2f, int> &l) { return l.first.x() < v.x() || (l.first.x() == v.x() && l.first.y() < v.y()); });
					int idx = -1;
	...
	Polygons project_mesh(const indexed_triangle_set &mesh, const Transform3d &trafo, std::function<void()> throw_on_cancel) {
		std::vector<Polygons> top, bottom;
		std::vector<float> zs{-1e10, 1e10};
	...
	void cut_mesh(const indexed_triangle_set &mesh, float z, indexed_triangle_set *upper, indexed_triangle_set *lower, bool triangulate_caps) {
		...
		for (int facet_idx = 0; facet_idx < int(mesh.indices.size()); ++facet_idx) {
			...
			if (min_z > z || (min_z == z && max_z > z)) {
				...
				int isolated_vertex = (vertices[0].z() > z) == (vertices[1].z() > z) ? 2 : (vertices[1].z() > z) == (vertices[2].z() > z) ? 0 : 1;

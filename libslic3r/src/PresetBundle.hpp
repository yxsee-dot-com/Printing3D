...
namespace Slic3r {
	...
	class PresetBundle {
		public:
			...
			void load_config_from_wizard(const std::string &name, DynamicPrintConfig config) { this->load_config_file_config(name, false, std::move(config)); }
			...
			void load_config_model(const std::string &name, DynamicPrintConfig config) { this->load_config_file_config(name, true, std::move(config)); }
			...
			enum LoadConfigBundleAttribute {
				...
				SaveImported, // Delete all old config profiles before loading.
				ResetUserProfile, // Load a system config bundle.
				LoadSystem, LoadVendorOnly,
			};

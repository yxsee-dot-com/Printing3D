...
namespace Slic3r::Arachne {
	...
	bool STHalfEdge::canGoUp(bool strict) const {
		...
		for (edge_t *outgoing = next; outgoing != twin; outgoing = outgoing->twin->next) {
			...
			if (!outgoing->twin) return false;
			assert(outgoing->twin->next);
			if (!outgoing->twin->next) return true; // This point is on the boundary?! Should never occur
		}
		return false;
	}
	...
	std::optional<coord_t> STHalfEdge::distToGoUp() const {
		...
		for (edge_t *outgoing = next; outgoing != twin; outgoing = outgoing->twin->next) {
			...
			assert(outgoing->twin);
			if (!outgoing->twin) return std::optional<coord_t>();
			assert(outgoing->twin->next);
			if (!outgoing->twin->next) return 0; // This point is on the boundary?! Should never occur
		}
		if (ret) {
	...
	void SkeletalTrapezoidationGraph::collapseSmallEdges(coord_t snap_dist) {
		...
		for (auto edge_it = edges.begin(); edge_it != edges.end();) {
			...
			//  | | > collapse sides
			//  o o
			if (should_collapse(quad_start->from, quad_end->to) && should_collapse(quad_start->to, quad_end->from)) { // Collapse start and end edges and remove whole cell

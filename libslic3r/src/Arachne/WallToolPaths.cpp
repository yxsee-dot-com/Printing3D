...
#include "Arachne/BeadingStrategy/BeadingStrategyFactory.hpp"
#include "Arachne/utils/ExtrusionJunction.hpp"
#include "Arachne/utils/ExtrusionLine.hpp"
#include "Arachne/utils/PolygonsPointIndex.hpp"
#include "Flow.hpp"
#include "Line.hpp"
#include "Polygon.hpp"
...
namespace Slic3r::Arachne {
	...
	WallToolPaths::WallToolPaths(const Polygons &outline, const coord_t bead_width_0, const coord_t bead_width_x, const size_t inset_count, const coord_t wall_0_inset, const coordf_t layer_height, const PrintObjectConfig &print_object_config, const PrintConfig &print_config) : outline(outline), bead_width_0(bead_width_0), bead_width_x(bead_width_x), inset_count(inset_count), wall_0_inset(wall_0_inset), layer_height(layer_height), print_thin_walls(Slic3r::Arachne::fill_outline_gaps),
	                                                                                                                                                                                                                                                                                  min_feature_size(scaled<coord_t>(print_object_config.min_feature_size.value)), min_bead_width(scaled<coord_t>(print_object_config.min_bead_width.value)), small_area_length(static_cast<double>(bead_width_0) / 2.),
	                                                                                                                                                                                                                                                                                  wall_transition_filter_deviation(scaled<coord_t>(print_object_config.wall_transition_filter_deviation.value)), wall_transition_length(scaled<coord_t>(print_object_config.wall_transition_length.value)),
	                                                                                                                                                                                                                                                                                  toolpaths_generated(false), print_object_config(print_object_config) {
	...
	void simplify(Polygon &thiss, const int64_t smallest_line_segment_squared, const int64_t allowed_error_distance_squared) {
		...
		for (size_t point_idx = 0; point_idx < thiss.points.size(); point_idx++) {
			...
			if (length2 < smallest_line_segment_squared && height_2 <= allowed_error_distance_squared) // removing the vertex doesn't introduce too much error.)
			{
				...
				if (next_length2 > 4 * smallest_line_segment_squared) {
					...
					if (!has_intersection || Line::distance_to_infinite_squared(intersection_point, previous, current) > double(allowed_error_distance_squared) || (intersection_point - previous).cast<int64_t>().squaredNorm() > smallest_line_segment_squared  // The intersection point is way too far from the 'previous'
	...
	const std::vector<VariableWidthLines> &WallToolPaths::generate() {
		...
		const auto beading_strat = BeadingStrategyFactory::makeStrategy(bead_width_0, bead_width_x, wall_transition_length, transitioning_angle, print_thin_walls, min_bead_width, min_feature_size, wall_split_middle_threshold, wall_add_middle_threshold, max_bead_count, wall_0_inset, wall_distribution_count);
		...
		SkeletalTrapezoidation wall_maker(prepared_outline, *beading_strat, beading_strat->getTransitioningAngle(), discretization_step_size, transition_filter_dist, allowed_filter_deviation, wall_transition_length);
		...
		assert(std::is_sorted(toolpaths.cbegin(), toolpaths.cend(), [](const VariableWidthLines &l, const VariableWidthLines &r) {
	...
	//                                svg.draw_text((last + here) / 2, std::to_string(line2.junctions[idx].region_id).c_str(), "black");
			...
			for (ExtrusionLine &wall_polygon: closed_polygons) {
				...
				if (wall_polygon.junctions.front().p != wall_polygon.junctions.back().p && (wall_polygon.junctions.back().p - wall_polygon.junctions.front().p).cast<double>().norm() < stitch_distance) {

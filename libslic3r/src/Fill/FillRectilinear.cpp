...
namespace Slic3r {
	...
	struct SegmentIntersection {
		...
		enum SegmentIntersectionType : char {
			UNKNOWN, OUTER_LOW, OUTER_HIGH, INNER_LOW, INNER_HIGH,
		};
		SegmentIntersectionType type{UNKNOWN};

		enum class LinkType : uint8_t {
			// Horizontal link (left or right).
			Horizontal, // Vertical link, up.
			Up, // Vertical link, down.
			Down, // Phony intersection point has no link.
			Phony,
		};

		enum class LinkQuality : uint8_t {
			Invalid, Valid, // Valid link, but too long to be followed.
			TooLong,
		};
	...
	struct ExPolygonWithOffset {
		public:
			ExPolygonWithOffset(const ExPolygon &expolygon, float angle, coord_t aoffset1,
			...
			const Polygon &contour(size_t idx) const { return is_contour_outer(idx) ? polygons_outer[idx] : polygons_inner[idx - n_contours_outer]; }
	...
	static inline bool intersection_on_prev_next_vertical_line_valid(const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iIntersection, SegmentIntersection::Side side) {
	...
	static inline bool intersection_on_prev_vertical_line_valid(const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iIntersection) {
		return intersection_on_prev_next_vertical_line_valid(segs, iVerticalLine, iIntersection, SegmentIntersection::Side::Left);
	}

	static inline bool intersection_on_next_vertical_line_valid(const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iIntersection) {
	...
	static inline coordf_t measure_perimeter_horizontal_segment_length(const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iIntersection, size_t iIntersection2) {
	...
	static inline void emit_perimeter_prev_next_segment(const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iInnerContour, size_t iIntersection, size_t iIntersection2, Polyline &out, bool dir_is_next) {
	...
	static inline coordf_t measure_perimeter_segment_on_vertical_line_length(const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iIntersection, size_t iIntersection2, bool forward) {
		const SegmentedIntersectionLine &il = segs[iVerticalLine];
		const SegmentIntersection &itsct = il.intersections[iIntersection];
		...
		return forward ? segment_length(poly, itsct.iSegment, p1, itsct2.iSegment, p2) : segment_length(poly, itsct2.iSegment, p2, itsct.iSegment, p1);
	}

	// Append the points of a perimeter segment when going from iIntersection to iIntersection2.
	// The first point (the point of iIntersection) will not be inserted,
	// the last point will be inserted.
	static inline void emit_perimeter_segment_on_vertical_line(const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, size_t iVerticalLine, size_t iInnerContour, size_t iIntersection, size_t iIntersection2, Polyline &out, bool forward) {
		const SegmentedIntersectionLine &il = segs[iVerticalLine];
		const SegmentIntersection &itsct = il.intersections[iIntersection];
		const SegmentIntersection &itsct2 = il.intersections[iIntersection2];
		const Polygon &poly = poly_with_offset.contour(iInnerContour);
	...
	static std::vector<SegmentedIntersectionLine> slice_region_by_vertical_lines(const ExPolygonWithOffset &poly_with_offset, size_t n_vlines, coord_t x0, coord_t line_spacing) {
		...
		for (size_t i_seg = 0; i_seg < segs.size(); ++i_seg) {
			...
			for (size_t i = 0; i < sil.intersections.size(); ++i) {
				...
				is.type = poly_with_offset.is_contour_outer(iContour) ? (low ? SegmentIntersection::OUTER_LOW : SegmentIntersection::OUTER_HIGH) : (low ? SegmentIntersection::INNER_LOW : SegmentIntersection::INNER_HIGH);
				bool take_next = true;
				if (j > 0) {
																															...
																															bool validate_segment_intersection_connectivity(const std::vector<SegmentedIntersectionLine> &segs)
{
	...
	static void connect_segment_intersections_by_contours(const ExPolygonWithOffset &poly_with_offset, std::vector<SegmentedIntersectionLine> &segs, const FillParams &params, const coord_t link_max_length) {
		for (size_t i_vline = 0; i_vline < segs.size(); ++i_vline) {
			...
			for (int i_intersection = 0; i_intersection < int(il.intersections.size()); ++i_intersection) {
				...
				itsct.prev_on_contour_type = same_prev ? (iprev < i_intersection ? SegmentIntersection::LinkType::Down : SegmentIntersection::LinkType::Up) : SegmentIntersection::LinkType::Horizontal;
				itsct.next_on_contour = inext;
				itsct.next_on_contour_type = same_next ? (inext < i_intersection ? SegmentIntersection::LinkType::Down : SegmentIntersection::LinkType::Up) : SegmentIntersection::LinkType::Horizontal;
				...
				if (params.dont_connect()) {
					...
					if (itsct.prev_on_contour_quality == SegmentIntersection::LinkQuality::Valid && (same_prev ? measure_perimeter_segment_on_vertical_line_length(poly_with_offset, segs, i_vline, iprev, i_intersection, forward) : measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline - 1, iprev, i_intersection)) > link_max_length)
						itsct.prev_on_contour_quality = SegmentIntersection::LinkQuality::TooLong;
					if (itsct.next_on_contour_quality == SegmentIntersection::LinkQuality::Valid && (same_next ? measure_perimeter_segment_on_vertical_line_length(poly_with_offset, segs, i_vline, i_intersection, inext, forward) : measure_perimeter_horizontal_segment_length(poly_with_offset, segs, i_vline, i_intersection, inext)) > link_max_length)
						itsct.next_on_contour_quality = SegmentIntersection::LinkQuality::TooLong;
				}
			}
	...
	static void traverse_graph_generate_polylines(const ExPolygonWithOffset &poly_with_offset, const FillParams &params, std::vector<SegmentedIntersectionLine> &segs, const bool consistent_pattern, Polylines &polylines_out) {
		...
		for (int i_vline = 0; i_vline < int(segs.size()); ++i_vline) {
			...
			for (int i_intersection = 0; i_intersection + 1 < int(vline.intersections.size()); ++i_intersection) {
				if (vline.intersections[i_intersection].type == SegmentIntersection::OUTER_LOW && vline.intersections[i_intersection + 1].type == SegmentIntersection::OUTER_HIGH) {
					bool consumed = false;
...
found:
			...
			if (try_connect) {
				...
				auto vertical_link_quality = (i_vertical == -1 || vline.intersections[i_vertical + (going_up ? 0 : -1)].consumed_vertical_up) ? SegmentIntersection::LinkQuality::Invalid : it->vertical_outside_quality();
#if 0
				...
				if (vertical_link_quality != SegmentIntersection::LinkQuality::Invalid) {
					...
					if (vertical_link_quality == SegmentIntersection::LinkQuality::Valid)
						...
						emit_perimeter_segment_on_vertical_line(poly_with_offset, segs, i_vline, it->iContour, i_intersection, i_vertical, *polyline_current, going_up ? it->has_left_vertical_up() : it->has_right_vertical_down());
					else {
			...
			if (polyline_current->points.size() <= 1 || (polyline_current->points.size() == 2 && std::abs(polyline_current->points.front()(0) - polyline_current->points.back()(0)) < SCALED_EPSILON && std::abs(polyline_current->points.front()(1) - polyline_current->points.back()(1)) < SCALED_EPSILON))
	...
	class AntPathMatrix {
		public:
			AntPathMatrix(const std::vector<MonotonicRegion> &regions, const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, const float initial_pheromone) : m_regions(regions), m_poly_with_offset(poly_with_offset), m_segs(segs),
			...
			AntPath &operator()(const MonotonicRegionLink &region_from, const MonotonicRegion &region_to, bool flipped_to) { return (*this)(*region_from.region, region_from.flipped, region_to, flipped_to); }
			AntPath &operator()(const MonotonicRegion &region_from, bool flipped_from, const MonotonicRegionLink &region_to) { return (*this)(region_from, flipped_from, *region_to.region, region_to.flipped); }
			AntPath &operator()(const MonotonicRegionLink &region_from, const MonotonicRegionLink &region_to) { return (*this)(*region_from.region, region_from.flipped, *region_to.region, region_to.flipped); }
    ...
    auto test_overlap = [&consumed](int segment, int low, int high) {
		...
		for (int i_vline_seed = 0; i_vline_seed < int(segs.size()); ++i_vline_seed) {
			...
			for (int i_intersection_seed = 1; i_intersection_seed + 1 < int(vline_seed.intersections.size());) {
				while (i_intersection_seed < int(vline_seed.intersections.size()) && vline_seed.intersections[i_intersection_seed].type != SegmentIntersection::INNER_LOW)
					++i_intersection_seed;
    ...
    const std::string                               &path)
{
	...
	static float montonous_region_path_length(const MonotonicRegion &region, bool dir, const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs) {
		...
		for (;;) {
			...
			const SegmentIntersection *right = going_up ? &vertical_run_top(vline_right, vline_right.intersections[iright]) : &vertical_run_bottom(vline_right, vline_right.intersections[iright]);
	...
	static std::vector<MonotonicRegionLink> chain_monotonic_regions(std::vector<MonotonicRegion> &regions, const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, std::mt19937_64 &rng) {
		...
		for (int round = 0; round < num_rounds && num_rounds_no_change < num_rounds_no_change_exit; ++round) {
			...
			for (int ant = 0; ant < num_ants; ++ant) {
				...
				*(queue.begin() + first_idx) = std::move(queue.back());
				...
				assert(validate_unprocessed());
				print_ant("\tRegion (%1%:%2%,%3%) (%4%:%5%,%6%)", path.back().region->left.vline, path.back().flipped ? path.back().region->left.high : path.back().region->left.low, path.back().flipped ? path.back().region->left.low : path.back().region->left.high, path.back().region->right.vline, path.back().flipped == path.back().region->flips ? path.back().region->right.high : path.back().region->right.low,
				...
				// Measure path length.
				assert(!path.empty());
				float path_length = std::accumulate(path.begin(), path.end() - 1, path.back().region->length(path.back().flipped), [&path_matrix](const float l, const MonotonicRegionLink &r) {
	...
	static void polylines_from_paths(const std::vector<MonotonicRegionLink> &path, const ExPolygonWithOffset &poly_with_offset, const std::vector<SegmentedIntersectionLine> &segs, Polylines &polylines_out) {
		...
		auto finish_polyline = [&polyline, &polylines_out]() {
			...
			// Handle nearly zero length edges.
			if (polyline->points.size() <= 1 || (polyline->points.size() == 2 && std::abs(polyline->points.front().x() - polyline->points.back().x()) < SCALED_EPSILON && std::abs(polyline->points.front().y() - polyline->points.back().y()) < SCALED_EPSILON))
				polylines_out.pop_back();
				...
				if (std::abs(polyline->points.front().x() - pl_prev.points.back().x()) < SCALED_EPSILON && std::abs(polyline->points.front().y() - pl_prev.points.back().y()) < SCALED_EPSILON) {
	...
	bool FillRectilinear::fill_surface_by_lines(const Surface *surface, const FillParams &params, float angleBase, float pattern_shift, Polylines &polylines_out) {
		...
		ExPolygonWithOffset poly_with_offset(surface->expolygon, -rotate_vector.first, float(scale_(this->overlap - (0.5 - INFILL_OVERLAP_OVER_SPACING) * this->spacing)), float(scale_(this->overlap - 0.5f * this->spacing)));
	...
	Polylines FillGrid::fill_surface(const Surface *surface, const FillParams &params) {
		Polylines polylines_out;
		if (!this->fill_surface_by_multilines(surface, params, {{0.f,              0.f},
		                                                        {float(M_PI / 2.), 0.f}}, polylines_out))
	...
	Polylines FillTriangles::fill_surface(const Surface *surface, const FillParams &params) {
		...
		if (!this->fill_surface_by_multilines(surface, params, {{0.f,                   0.f},
		                                                        {float(M_PI / 3.),      0.f},
		                                                        {float(2. * M_PI / 3.), 0.}}, polylines_out))
	...
	Polylines FillStars::fill_surface(const Surface *surface, const FillParams &params) {
		...
		if (!this->fill_surface_by_multilines(surface, params, {{0.f,                   0.f},
		                                                        {float(M_PI / 3.),      0.f},
		                                                        {float(2. * M_PI / 3.), float((3. / 2.) * this->spacing / params.density)}}, polylines_out))
	...
	Polylines FillCubic::fill_surface(const Surface *surface, const FillParams &params) {
		...
		if (!this->fill_surface_by_multilines(surface, params, {{0.f,                   float(dx)},
		                                                        {float(M_PI / 3.),      -float(dx)},
		                                                        {float(M_PI * 2. / 3.), float(dx)}}, polylines_out))
	...
	Points sample_grid_pattern(const ExPolygon &expolygon, coord_t spacing, const BoundingBox &global_bounding_box) {
		...
		std::vector<SegmentedIntersectionLine> segs = slice_region_by_vertical_lines(poly_with_offset, (global_bounding_box.max.x() - global_bounding_box.min.x() + spacing - 1) / spacing, global_bounding_box.min.x(), spacing);

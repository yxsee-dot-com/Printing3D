...
namespace Slic3r {
	...
	void FillLine::_fill_surface_single(const FillParams &params, unsigned int thickness_layers, const std::pair<float, Point> &direction, ExPolygon expolygon, Polylines &polylines_out) {
		...
		if (params.density > 0.9999f && !params.dont_adjust) {
			...
			bounding_box.merge(align_to_grid(bounding_box.min, Point(this->_line_spacing, this->_line_spacing), direction.second.rotated(-direction.first)));
		}
		...
		if (!polylines.empty()) { // prevent calling leftmost_point() on empty collections
			...
			for (Polyline &polyline: chain_polylines(std::move(polylines))) {
				if (!first) {
					...
					if (this->_can_connect(std::abs(distance(0)), std::abs(distance(1))) && expolygon_off.contains(Line(last_point, first_point))) {
						// Append the polyline.

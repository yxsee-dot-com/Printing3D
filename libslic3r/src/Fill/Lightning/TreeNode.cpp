...
namespace Slic3r::FillLightning {
	...
	void Node::propagateToNextLayer(std::vector<NodeSPtr> &next_trees, const Polygons &next_outlines, const EdgeGrid::Grid &outline_locator, const coord_t prune_distance, const coord_t smooth_magnitude, const coord_t max_remove_colinear_dist) const {
	...
	Node::Node(const Point &p, const std::optional<Point> &last_grounding_location /*= std::nullopt*/) : m_is_root(true), m_p(p), m_last_grounding_location(last_grounding_location) {}
	...
	Node::RectilinearJunction Node::straighten(const coord_t magnitude, const Point &junction_above, const coord_t accumulated_dist, const int64_t max_remove_colinear_dist2) {
		...
		if (m_children.size() == 1) {
			...
			{ // remove nodes on linear segments
				...
				if (parent_node && (child_p->m_p - parent_node->m_p).cast<int64_t>().squaredNorm() < max_remove_colinear_dist2 && Line::distance_to_squared(m_p, parent_node->m_p, child_p->m_p) < close_enough * close_enough) {

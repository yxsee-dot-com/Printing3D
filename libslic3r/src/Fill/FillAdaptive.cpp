...
namespace Slic3r {
	namespace FillAdaptive {
		...
		static const std::array<Vec3d, 8> child_centers{Vec3d(-1, -1, -1), Vec3d(1, -1, -1), Vec3d(-1, 1, -1), Vec3d(1, 1, -1), Vec3d(-1, -1, 1), Vec3d(1, -1, 1), Vec3d(-1, 1, 1), Vec3d(1, 1, 1)};
		...
		static constexpr std::array<std::array<int, 8>, 3> child_traversal_order{std::array<int, 8>{2, 3, 0, 1, 6, 7, 4, 5}, std::array<int, 8>{4, 0, 6, 2, 5, 1, 7, 3}, std::array<int, 8>{1, 5, 0, 4, 3, 7, 2, 6},};
		...
		struct Octree {
			...
			Octree(const Vec3d &origin, const std::vector<CubeProperties> &cubes_properties) : root_cube(pool.construct(origin)), origin(origin), cubes_properties(cubes_properties) {}
		...
		std::pair<double, double> adaptive_fill_line_spacing(const PrintObject &print_object) {
			...
			for (size_t region_id = 0; region_id < print_object.num_printing_regions(); ++region_id) {
				...
				region_fill_data.push_back(RegionFillData({has_adaptive_infill ? Tristate::Maybe : Tristate::No, has_support_infill ? Tristate::Maybe : Tristate::No, config.fill_density, infill_extrusion_width != 0. ? infill_extrusion_width : default_infill_extrusion_width}));
		...
		struct FillContext {
			...
			FillContext(const Octree &octree, double z_position, int direction_idx) : cubes_properties(octree.cubes_properties), z_position(z_position), traversal_order(child_traversal_order[direction_idx]), cos_a(cos(direction_angles[direction_idx])), sin_a(sin(direction_angles[direction_idx])) {
		...
		static void generate_infill_lines_recursive(FillContext &context, const Cube *cube,
				// Address of this wall in the octree,  used to address context.temp_lines.
				                                    int address, int depth) {
			assert(cube != nullptr);
			...
			if (z_diff_abs < cubes_properties[depth].line_z_distance) {
				...
				Vec2d from(0.5 * cubes_properties[depth].diagonal_length * (zdist - z_diff_abs) / zdist, cubes_properties[depth].line_xy_distance - (zdist + z_diff) / sqrt(2.));
		...
		struct Intersection {
			...
			Intersection(const Line &closest_line, const Line &intersect_line, Polyline *intersect_pl, const Point &intersect_point, bool front) : closest_line(&closest_line), intersect_line(&intersect_line), intersect_pl(intersect_pl), intersect_point(intersect_point), front(front) {
		...
		static inline Intersection *get_nearest_intersection(std::vector<std::pair<Intersection *, double>> &intersect_line, const size_t first_idx) {
			...
			if (first_idx == 0)
				...
				take_next = iprev.first->fresh() && inext.first->fresh() ? inext.second - ithis.second < ithis.second - iprev.second : inext.first->fresh();
			}
		...
		static void add_hook(const Intersection &intersection, const double scaled_offset, const coordf_t hook_length, double scaled_trim_distance, const rtree_t &rtree, const Lines &lines_src) {
			...
			[[maybe_unused]] bool intersection_found = intersection.intersect_line->intersection(create_offset_line(*intersection.closest_line, intersection, scaled_offset), &hook_start);
			...
			// dir is normalized.
			auto max_hook_length = [hook_length, scaled_trim_distance, &lines_src](const Vec2d &pt, const Vec2d &dir, const std::vector<std::pair<rtree_segment_t, size_t>> &hook_intersections, bool self_intersection, const std::optional<Line> &self_intersection_line, const Point &self_intersection_point) {
		...
		static Polylines connect_lines_using_hooks(Polylines &&lines, const ExPolygon &boundary, const double spacing, const coordf_t hook_length, const coordf_t hook_length_max) {
			...
			auto update_merged_polyline = [&lines, update_merged_polyline_idx](Intersection &intersection) {
				...
				if (intersection.fresh()) {
					assert(intersection.intersect_pl->points.front() == intersection.intersect_point || intersection.intersect_pl->points.back() == intersection.intersect_point);
			...
			for (size_t min_idx = 0; min_idx < intersections.size();) {
				...
				// All the nearest points (T-joints) ending at the same line are projected onto this line. Because of it, it can easily find the nearest point.
				{
					...
					for (; max_idx < intersections.size() && intersections[min_idx].closest_line == intersections[max_idx].closest_line && intersections[min_idx].left == intersections[max_idx].left; ++max_idx)
				...
				for (size_t first_idx = 0; first_idx < intersect_line.size(); ++first_idx) {
					...
					if (nearest_i.fresh()) {
						could_connect = first_i.intersect_line->intersection(offset_line, &first_i_point) && nearest_i.intersect_line->intersection(offset_line, &nearest_i_point);
					...
					if (could_connect) {
						...
						closest.clear();
						rtree.query(bgi::intersects(mk_rtree_seg(first_i_point, nearest_i_point)) && bgi::satisfies([&first_i, &nearest_i, &lines_src](const auto &item) {
							return item.second != (long unsigned int) (first_i.intersect_line - lines_src.data()) && item.second != (long unsigned int) (nearest_i.intersect_line - lines_src.data());
		...
		void Filler::_fill_surface_single(const FillParams &params, unsigned int thickness_layers, const std::pair<float, Point> &direction, ExPolygon expolygon, Polylines &polylines_out) {
			...
			Polylines all_polylines;
			{
				...
				std::array<FillContext, 3> contexts{FillContext{*adapt_fill_octree, this->z, 0}, FillContext{*adapt_fill_octree, this->z, 1}, FillContext{*adapt_fill_octree, this->z, 2}};
		...
		OctreePtr build_octree(
				...
				const std::vector<Vec3d> &overhang_triangles, coordf_t line_spacing, bool support_overhangs_only) {
			...
			if (cubes_properties.size() > 1) {
				...
				auto process_triangle = [octree_ptr, max_depth, diag_half](const Vec3d &a, const Vec3d &b, const Vec3d &c) {
					octree_ptr->insert_triangle(a, b, c, octree_ptr->root_cube, BoundingBoxf3(octree_ptr->root_cube->center - diag_half, octree_ptr->root_cube->center + diag_half), max_depth);
				};

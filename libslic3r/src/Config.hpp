...
namespace std {
	template<>
	struct hash<Slic3r::FloatOrPercent> {
...
namespace Slic3r {
	...
	enum ConfigOptionType {
		coVectorType = 0x4000, coNone = 0, // single float
		coFloat = 1, // vector of floats
		coFloats = coFloat + coVectorType, // single int
		coInt = 2, // vector of ints
		coInts = coInt + coVectorType, // single string
		coString = 3, // vector of strings
		coStrings = coString + coVectorType, // percent value. Currently only used for infill.
		coPercent = 4, // percents value. Currently used for retract before wipe only.
		coPercents = coPercent + coVectorType, // a fraction or an absolute value
		coFloatOrPercent = 5, // vector of the above
		coFloatsOrPercents = coFloatOrPercent + coVectorType, // single 2d point (Point2f). Currently not used.
		coPoint = 6, // vector of 2d points (Point2f). Currently used for the definition of the print bed and for the extruder offsets.
		coPoints = coPoint + coVectorType, coPoint3 = 7, //    coPoint3s       = coPoint3 + coVectorType,
		// single boolean value
		coBool = 8, // vector of boolean values
		coBools = coBool + coVectorType, // a generic enum
		coEnum = 9, // vector of enum values
		coEnums = coEnum + coVectorType,
	};

	enum ConfigOptionMode {
		comSimple = 0, comAdvanced, comExpert, comUndef
	};

	enum PrinterTechnology : unsigned char {
		// Fused Filament Fabrication
		ptFFF, // Stereolitography
		ptSLA, // Unknown, useful for command line processing
		ptUnknown, // Any technology, useful for parameters compatible with both ptFFF and ptSLA
		ptAny
	};

	enum ForwardCompatibilitySubstitutionRule {
		// Disable susbtitution, throw exception if an option value is not recognized.
		Disable, // Enable substitution of an unknown option value with default. Log the substitution.
		Enable, // Enable substitution of an unknown option value with default. Don't log the substitution.
		EnableSilent, // Enable substitution of an unknown option value with default. Log substitutions in user profiles, don't log substitutions in system profiles.
		EnableSystemSilent, // Enable silent substitution of an unknown option value with default when loading user profiles. Throw on an unknown option value in a system profile.
	...
	// When parsing a configuration value, if the old_value is not understood by this Printing3D version,
	// it is being substituted with some default value that this Printing3D could work with.
	...
	struct NilValueTempl {
		using NilType = T;
		static_assert(always_false<T>::value, "Type has no well defined nil value");
	};

	template<class T>
	struct NilValueTempl<T, std::enable_if_t<std::is_integral_v<T>, void>> {
		using NilType = T;
		static constexpr auto value = std::numeric_limits<T>::max();
	};

	template<>
	struct NilValueTempl<bool> : public NilValueTempl<int> {
	};
	...
	template<class T>
	struct NilValueTempl<T, std::enable_if_t<std::is_floating_point_v<T>, void>> {
		using NilType = T;
	...
	template<>
	struct NilValueTempl<std::string> {
	...
	template<class T>
	static constexpr NilType<T> NilValue() noexcept { return NilValueTempl<remove_cvref_t<T>>::value; }
	...
	class ConfigOptionSingle : public ConfigOption {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(this->value); }
	};

	template<class T> using ConfigOptionSingleNullable = ConfigOptionSingle<T, true>;
	...
	class ConfigOptionVector : public ConfigOptionVectorBase {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(this->values); }
	};
	...
	class ConfigOptionFloatTempl : public ConfigOptionSingle<double, NULLABLE> {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionSingle<double, NULLABLE>>(this)); }
	};
	...
	class ConfigOptionFloatsTempl : public ConfigOptionVector<double> {
		public:
			...
			// A scalar is nil, or all values of a vector are nil.
			bool is_nil() const override {
				for (auto v: this->values) if (!std::isnan(v)) return false;
				return true;
			}
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionVector<double>>(this)); }
	};
	...
	class ConfigOptionIntTempl : public ConfigOptionSingle<int, NULLABLE> {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionSingle<int, NULLABLE>>(this)); }
	};
	...
	class ConfigOptionIntsTempl : public ConfigOptionVector<int> {
		public:
			...
			// A scalar is nil, or all values of a vector are nil.
			bool is_nil() const override {
				for (auto v: this->values) if (v != nil_value()) return false;
				return true;
			}
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionVector<int>>(this)); }
	};
	...
	class ConfigOptionString : public ConfigOptionSingle<std::string> {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionSingle<std::string>>(this)); }
	};
	...
	class ConfigOptionStrings : public ConfigOptionVector<std::string> {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionVector<std::string>>(this)); }
	};
	...
	class ConfigOptionPercent : public ConfigOptionFloat {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionFloat>(this)); }
	};
	...
	class ConfigOptionPercentsTempl : public ConfigOptionFloatsTempl<NULLABLE> {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionFloatsTempl<NULLABLE>>(this)); }
	};
	...
	class ConfigOptionFloatOrPercent : public ConfigOptionPercent {
		public:
			...
			bool operator==(const ConfigOptionFloatOrPercent &rhs) const throw() { return this->value == rhs.value && this->percent == rhs.percent; }
			size_t hash() const throw() override {
				size_t seed = std::hash<double>{}(this->value);
				return this->percent ? seed ^ 0x9e3779b9 : seed;
			}
			bool operator<(const ConfigOptionFloatOrPercent &rhs) const throw() { return this->value < rhs.value || (this->value == rhs.value && int(this->percent) < int(rhs.percent)); }
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionPercent>(this), percent); }
	};
	...
	class ConfigOptionFloatsOrPercentsTempl : public ConfigOptionVector<FloatOrPercent> {
		public:
			...
			// A scalar is nil, or all values of a vector are nil.
			bool is_nil() const override {
				for (auto v: this->values) if (!std::isnan(v.value)) return false;
				return true;
			}
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionVector<FloatOrPercent>>(this)); }
	};
	...
	class ConfigOptionPoint : public ConfigOptionSingle<Vec2d> {
		public:
			...
			bool deserialize(const std::string &str, bool append = false) override {
				UNUSED(append);
				char dummy;
				return sscanf(str.data(), " %lf , %lf %c", &this->value(0), &this->value(1), &dummy) == 2 || sscanf(str.data(), " %lf x %lf %c", &this->value(0), &this->value(1), &dummy) == 2;
			}
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionSingle<Vec2d>>(this)); }
	};
	...
	class ConfigOptionPoints : public ConfigOptionVector<Vec2d> {
		public:
			...
			bool operator<(const ConfigOptionPoints &rhs) const throw() { return std::lexicographical_compare(this->values.begin(), this->values.end(), rhs.values.begin(), rhs.values.end(), [](const auto &l, const auto &r) { return l < r; }); }
			bool is_nil(size_t) const override { return false; }
	...
	class ConfigOptionPoint3 : public ConfigOptionSingle<Vec3d> {
		public:
			...
			bool operator<(const ConfigOptionPoint3 &rhs) const throw() { return this->value.x() < rhs.value.x() || (this->value.x() == rhs.value.x() && (this->value.y() < rhs.value.y() || (this->value.y() == rhs.value.y() && this->value.z() < rhs.value.z()))); }
			...
			bool deserialize(const std::string &str, bool append = false) override {
				UNUSED(append);
				char dummy;
				return sscanf(str.data(), " %lf , %lf , %lf %c", &this->value(0), &this->value(1), &this->value(2), &dummy) == 3 || sscanf(str.data(), " %lf x %lf x %lf %c", &this->value(0), &this->value(1), &this->value(2), &dummy) == 3;
			}
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionSingle<Vec3d>>(this)); }
	};
	...
	class ConfigOptionBool : public ConfigOptionSingle<bool> {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionSingle<bool>>(this)); }
	};
	...
	class ConfigOptionBoolsTempl : public ConfigOptionVector<unsigned char> {
		public:
			...
			explicit ConfigOptionBoolsTempl(std::initializer_list<bool> il) {
				...
				for (bool b: il) values.emplace_back((unsigned char) b);
			}
			explicit ConfigOptionBoolsTempl(std::initializer_list<unsigned char> il) {
				values.reserve(il.size());
				for (unsigned char b: il) values.emplace_back(b);
			}
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionVector<unsigned char>>(this)); }
	};
	...
	class ConfigOptionEnums : public ConfigOptionVector<T> {
		public:
			...
			ConfigOptionEnums<T> &operator=(const ConfigOption *opt) {
				this->set(opt);
				return *this;
			}
	...
	class ConfigOptionEnumGeneric : public ConfigOptionInt {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ConfigOptionInt>(this)); }
	};
	...
	class ConfigOptionEnumDef {
		public:
			...
			std::optional<std::reference_wrapper<const std::string>> enum_to_value(int enum_val) const {
				...
				return opt.has_value() ? std::optional<std::reference_wrapper<const std::string>>{this->value(*opt)} : std::optional<std::reference_wrapper<const std::string>>{};
			}

			std::optional<std::reference_wrapper<const std::string>> enum_to_label(int enum_val) const {
				assert(this->is_valid_closed_enum());
				auto opt = this->enum_to_index(enum_val);
				return opt.has_value() ? std::optional<std::reference_wrapper<const std::string>>{this->label(*opt)} : std::optional<std::reference_wrapper<const std::string>>{};
			}
	...
	class ConfigOptionDef {
		public:
			enum class GUIType {
				undefined, // Open enums, integer value could be one of the enumerated values or something else.
				i_enum_open, // Open enums, float value could be one of the enumerated values or something else.
				f_enum_open, // Open enums, string value could be one of the enumerated values or something else.
				select_open, // Color picker, string value.
				color, // Currently unused.
				slider, // Static text
				legend, // Vector value, but edited as a single string.
				one_string, // Close parameter, string value could be one of the list values.
				select_close, // Password, string vaule is hidden by asterisk.
				password,
			};
			static bool is_gui_type_enum_open(const GUIType gui_type) { return gui_type == ConfigOptionDef::GUIType::i_enum_open || gui_type == ConfigOptionDef::GUIType::f_enum_open || gui_type == ConfigOptionDef::GUIType::select_open; }
			...
			ConfigOption *load_option_from_archive(Archive &archive) const {
				if (this->nullable) {
					switch (this->type) {
						case coFloat: {
							auto opt = new ConfigOptionFloatNullable();
							archive(*opt);
							return opt;
						}
						case coInt: {
							auto opt = new ConfigOptionIntNullable();
							archive(*opt);
							return opt;
						}
						case coFloats: {
						...
						default:
							throw ConfigurationError(std::string("ConfigOptionDef::load_option_from_archive(): Unknown nullable option type for option ") + this->opt_key);
					}
				} else {
					switch (this->type) {
						case coFloat: {
						...
						case coEnum: {
							auto opt = new ConfigOptionEnumGeneric(this->enum_def->m_enum_keys_map);
							archive(*opt);
							return opt;
						}
						case coEnums: {
							auto opt = new ConfigOptionEnumsGeneric(this->enum_def->m_enum_keys_map);
							archive(*opt);
							return opt;
						}
						default:
							throw ConfigurationError(std::string("ConfigOptionDef::load_option_from_archive(): Unknown option type for option ") + this->opt_key);
					}
				}
			}
			...
			ConfigOption *save_option_to_archive(Archive &archive, const ConfigOption *opt) const {
				if (this->nullable) {
					switch (this->type) {
						case coFloat:
							archive(*static_cast<const ConfigOptionFloatNullable *>(opt));
							break;
						case coInt:
							archive(*static_cast<const ConfigOptionIntNullable *>(opt));
							break;
						case coFloats:
							archive(*static_cast<const ConfigOptionFloatsNullable *>(opt));
							break;
						case coInts:
							archive(*static_cast<const ConfigOptionIntsNullable *>(opt));
							break;
						case coPercents:
							archive(*static_cast<const ConfigOptionPercentsNullable *>(opt));
							break;
						case coBools:
							archive(*static_cast<const ConfigOptionBoolsNullable *>(opt));
							break;
						default:
							throw ConfigurationError(std::string("ConfigOptionDef::save_option_to_archive(): Unknown nullable option type for option ") + this->opt_key);
					}
				} else {
					switch (this->type) {
						case coFloat:
							archive(*static_cast<const ConfigOptionFloat *>(opt));
							break;
						case coFloats:
							archive(*static_cast<const ConfigOptionFloats *>(opt));
							break;
						case coInt:
							archive(*static_cast<const ConfigOptionInt *>(opt));
							break;
						case coInts:
							archive(*static_cast<const ConfigOptionInts *>(opt));
							break;
						case coString:
							archive(*static_cast<const ConfigOptionString *>(opt));
							break;
						case coStrings:
							archive(*static_cast<const ConfigOptionStrings *>(opt));
							break;
						case coPercent:
							archive(*static_cast<const ConfigOptionPercent *>(opt));
							break;
						case coPercents:
							archive(*static_cast<const ConfigOptionPercents *>(opt));
							break;
						case coFloatOrPercent:
							archive(*static_cast<const ConfigOptionFloatOrPercent *>(opt));
							break;
						case coFloatsOrPercents:
							archive(*static_cast<const ConfigOptionFloatsOrPercents *>(opt));
							break;
						case coPoint:
							archive(*static_cast<const ConfigOptionPoint *>(opt));
							break;
						case coPoints:
							archive(*static_cast<const ConfigOptionPoints *>(opt));
							break;
						case coPoint3:
							archive(*static_cast<const ConfigOptionPoint3 *>(opt));
							break;
						case coBool:
							archive(*static_cast<const ConfigOptionBool *>(opt));
							break;
						case coBools:
							archive(*static_cast<const ConfigOptionBools *>(opt));
							break;
						case coEnum:
							archive(*static_cast<const ConfigOptionEnumGeneric *>(opt));
							break;
						case coEnums:
							archive(*static_cast<const ConfigOptionEnumsGeneric *>(opt));
							break;
						default:
							throw ConfigurationError(std::string("ConfigOptionDef::save_option_to_archive(): Unknown option type for option ") + this->opt_key);
					}
				}
	...
	class ConfigBase : public ConfigOptionResolver {
		...
		public:
			...
			template<class T>
			T *opt(const t_config_option_key &opt_key, bool create = false) { return dynamic_cast<T *>(this->optptr(opt_key, create)); }
			template<class T>
			const T *opt(const t_config_option_key &opt_key) const { return dynamic_cast<const T *>(this->optptr(opt_key)); }

			// Get definition for a particular option.
			// Returns null if such an option definition does not exist.
			const ConfigOptionDef *option_def(const t_config_option_key &opt_key) const { return this->def()->get(opt_key); }
			...
			void set(const std::string &opt_key, bool value, bool create = false) { this->option_throw<ConfigOptionBool>(opt_key, create)->value = value; }
			...
			void set(const std::string &opt_key, const char *value, bool create = false) { this->option_throw<ConfigOptionString>(opt_key, create)->value = value; }
			void set(const std::string &opt_key, const std::string &value, bool create = false) { this->option_throw<ConfigOptionString>(opt_key, create)->value = value; }
	...
	class DynamicConfig : public virtual ConfigBase {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(options); }
	};

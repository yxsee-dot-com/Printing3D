...
namespace Slic3r {
	...
	bool TriangleMesh::ReadSTLFile(const char *input_file, bool repair) {
		...
		m_stats.repaired_errors = {stl.stats.edges_fixed, stl.stats.degenerate_facets, stl.stats.facets_removed, stl.stats.facets_reversed, stl.stats.backwards_edges};
	...
	indexed_triangle_set its_make_cube(double xd, double yd, double zd) {
		...
		return {{{0, 1, 2}, {0, 2, 3}, {4, 5, 6}, {4, 6, 7}, {0, 4, 7}, {0, 7, 1}, {1, 7, 6}, {1, 6, 2}, {2, 6, 5}, {2, 5, 3}, {4, 0, 3}, {4, 3, 5}},
	...
	indexed_triangle_set its_make_prism(float width, float length, float height) {
		...
		return {{{0,  1,  2}, // side 1
		        ...
		        {{-x, -y, 0.f}, {x, -y, 0.f}, {0.f, -y, height}, {-x, y, 0.f}, {x, y, 0.f}, {0.f, y, height},}};
	}
	...
	indexed_triangle_set its_make_sphere(double radius, double fa) {
		...
		for (int iter = 0; iter < iterations; ++iter) {
			...
			for (int i = 0; i < orig_indices_size; ++i) { // iterate over all old triangles
				...
				std::array<std::pair<int, int>, 3> edge_children = {std::make_pair(i, last_triangle_idx + 2), std::make_pair(last_triangle_idx + 2, last_triangle_idx + 3), std::make_pair(last_triangle_idx + 3, i)};
				...
				for (int n = 0; n < 3; ++n) { // for all three edges
					...
					if (divided_triangles[edge_neighbor][0].neighbor == -1) {
						...
						int idx = -1;
						while (indices[new_neighbors_per_edge[n].first][++idx] != middle_vertices_idxs[n]);
	...
	indexed_triangle_set its_make_snap(double r, double h, float space_proportion, float bulge_proportion) {
		...
		auto add_sub_mesh = [add_side_vertices, add_side_facets, get_m_len, b_height, t_height, b_angle, t_angle, b_angle_step, t_angle_step](indexed_triangle_set &mesh, float center_x, float angle_rotation, int frst_vertex_id) {
			auto &vertices = mesh.vertices;
			auto &facets = mesh.indices;

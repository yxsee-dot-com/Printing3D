...
namespace Slic3r {
	namespace arr2 {
		...
		struct RectangleBed {
			...
			explicit RectangleBed(coord_t w, coord_t h, Point c = {0, 0}) : bb{{c.x() - w / 2, c.y() - h / 2},
			                                                                   {c.x() + w / 2, c.y() + h / 2}} {}
		...
		class CircleBed {
			...
			public:
				...
				explicit CircleBed(const Point &c, double r) : m_center(c), m_radius(r) {}
		...
		using ArrangeBed = boost::variant<InfiniteBed, RectangleBed, CircleBed, IrregularBed>;
		...
		inline double area(const IrregularBed &bed) {
			return std::accumulate(bed.poly.begin(), bed.poly.end(), 0., [](double s, auto &p) { return s + p.area(); });
		}
		...
		template<class Bed, class En = void>
		struct IsRectangular_ : public std::false_type {
		};
		template<>
		struct IsRectangular_<RectangleBed> : public std::true_type {
		};
		template<>
		struct IsRectangular_<BoundingBox> : public std::true_type {
		};

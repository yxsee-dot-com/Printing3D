	...
	namespace arr2 {
		namespace firstfit {
			...
			struct ItemArrangedVisitor {
				...
				static void on_arranged(ArrItem &itm, const Bed &bed, const Range<PIt> &packed_items, const Range<RIt> &remaining_items) {}
			};
			...
			struct DefaultOnArrangedFn {
				...
				void operator()(ArrItem &itm, const Bed &bed, const Range<PIt> &packed, const Range<RIt> &remaining) {
					ItemArrangedVisitor<StripCVRef<ArrItem>>::on_arranged(itm, bed, packed, remaining);
				}
			};
			...
			template<class CompareFn  = DefaultItemCompareFn, class OnArrangedFn = DefaultOnArrangedFn, class StopCondition = DefaultStopCondition>
			struct SelectionStrategy {
				CompareFn cmpfn;
				OnArrangedFn on_arranged_fn;
				StopCondition cancel_fn;

				SelectionStrategy(CompareFn cmp = {}, OnArrangedFn on_arranged = {}, StopCondition stopcond = {}) : cmpfn{cmp}, on_arranged_fn{std::move(on_arranged)}, cancel_fn{std::move(stopcond)} {}
			};

		} // namespace firstfit

		template<class... Args>
		struct SelStrategyTag_<firstfit::SelectionStrategy<Args...>> {
			using Tag = firstfit::SelectionTag;
		};

		template<class It, class ConstIt, class TBed, class SelStrategy, class PackStrategy>
		void arrange(SelStrategy &&sel, PackStrategy &&ps, const Range<It> &items, const Range<ConstIt> &fixed, const TBed &bed, const firstfit::SelectionTag &) {
			...
			auto get_or_init_context = [&ps, &bed, &bed_contexts](int bedidx) -> Context & {
				...
				if (ctx_it == bed_contexts.end()) {
					auto res = bed_contexts.emplace(bedidx, create_context<ArrItem>(ps, bed, bedidx));
			...
			while (it != sorted_items.end() && !is_cancelled()) {
				...
				while (!was_packed && !is_cancelled()) {
					for (; !was_packed && !is_cancelled(); bedidx++) {
						...
						auto remaining = Range{std::next(static_cast<SConstIt>(it)), sorted_items.cend()};
						...
						if (was_packed) {
							...
							auto packed_range = Range{sorted_items.cbegin(), static_cast<SConstIt>(it)};

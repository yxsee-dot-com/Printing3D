	...
	namespace arr2 {
		...
		void EdgeCache::sample_contour(double accuracy, std::vector<ContourLocation> &samples) {
			...
			for (size_t i = 0; i < N; i += S) {
				samples.emplace_back(ContourLocation{0, m_contour.distances[i] / m_contour.distances.back()});
			}
			...
			for (size_t hidx = 1; hidx <= m_holes.size(); ++hidx) {
				...
				for (size_t i = 0; i < NH; i += SH) {
					samples.emplace_back(ContourLocation{hidx, hc.distances[i] / hc.distances.back()});
				}
			}
		}
		...
		Vec2crd coords(const Polygon &poly, const std::vector<double> &distances, double distance) {
			...
			auto edge = idx == long(pts.size() - 1) ? Line(pts.back(), pts.front()) : Line(pts[idx], pts[idx + 1]);

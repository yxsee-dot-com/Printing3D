...
namespace Slic3r {
	...
	ExPolygons ifp_convex(const arr2::RectangleBed &obed, const Polygon &convexpoly) {
		...
		Polygon innerNfp{Point{bedMinx + leftOffset, bedMaxy + topOffset}, Point{bedMaxx + rightOffset, bedMaxy + topOffset}, Point{bedMaxx + rightOffset, bedMiny + bottomOffset}, Point{bedMinx + leftOffset, bedMiny + bottomOffset}, Point{bedMinx + leftOffset, bedMaxy + topOffset}};
	...
	ExPolygons ifp_convex(const arr2::IrregularBed &bed, const Polygon &convexpoly) {
		...
		std::copy_if(ifp.begin(), ifp.end(), std::back_inserter(ret), [](const Polygon &p) { return p.is_clockwise(); });
	...
	Vec2crd reference_vertex(const Polygon &poly) {
		Vec2crd ret{std::numeric_limits<coord_t>::min(), std::numeric_limits<coord_t>::min()};
	...
	Vec2crd min_vertex(const Polygon &poly) {
		Vec2crd ret{std::numeric_limits<coord_t>::max(), std::numeric_limits<coord_t>::max()};
	...
	template<class It>
	It find_min_anglex_edge(It from) {
		bool found = false;
		auto it = from;
		while (!found) {
	...
	static void buildPolygon(const std::vector<Line> &edgelist, Polygon &rpoly, Point &top_nfp) {

...
namespace Slic3r {
	namespace arr2 {
		...
		template<class Kernel, class En = void>
		struct KernelTraits_ {
			...
			static double placement_fitness(const Kernel &k, const ArrItem &itm, const Vec2crd &transl) {
			...
			static bool on_start_packing(Kernel &k, ArrItem &itm, const Bed &bed, const Ctx &packing_context, const Range<RemIt> &remaining_items) {

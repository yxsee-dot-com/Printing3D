...
namespace Slic3r {
	namespace arr2 {
		...
		class TrafoOnlyArrangeItem {
			...
			public:
				...
				explicit TrafoOnlyArrangeItem(const ArrItm &other)
						: m_bed_idx{arr2::get_bed_index(other)}, m_priority{arr2::get_priority(other)}, m_translation(arr2::get_translation(other)), m_rotation{arr2::get_rotation(other)} {}
		...
		template<>
		struct DataStoreTraits_<TrafoOnlyArrangeItem> {
		...
		template<>
		struct IsMutableItem_<TrafoOnlyArrangeItem> : public std::true_type {
		};

		template<>
		struct WritableDataStoreTraits_<TrafoOnlyArrangeItem> {
			static constexpr bool Implemented = true;

			template<class T>
			static void set(TrafoOnlyArrangeItem &itm, const std::string &key, T &&data) {

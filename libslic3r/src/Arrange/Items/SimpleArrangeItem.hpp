...
namespace Slic3r {
	namespace arr2 {
		...
		template<>
		struct NFPArrangeItemTraits_<SimpleArrangeItem> {
			template<class Context, class Bed, class StopCond>
			static ExPolygons calculate_nfp(const SimpleArrangeItem &item, const Context &packing_context, const Bed &bed, StopCond &&stop_cond) {
				...
				for (const SimpleArrangeItem &fixed_part: fixed_items) {
					Polygon subnfp = nfp_convex_convex_legacy(fixed_part.outline(), item.outline());
		...
		template<>
		struct IsMutableItem_<SimpleArrangeItem> : public std::true_type {
		};
		...
		template<>
		struct ImbueableItemTraits_<SimpleArrangeItem> {

...
namespace Slic3r {
	namespace arr2 {
		...
		template<>
		struct DataStoreTraits_<ArbitraryDataStore> {
		...
		template<>
		struct WritableDataStoreTraits_<ArbitraryDataStore> {
			static constexpr bool Implemented = true;

			template<class T>
			static void set(ArbitraryDataStore &store, const std::string &key, T &&data) {

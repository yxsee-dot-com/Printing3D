...
#include "Arrange/Core/NFP/NFP.hpp"
#include "Arrange/Items/MutableItemTraits.hpp"
#include "Arrange/Arrange.hpp"
#include "Arrange/Tasks/ArrangeTask.hpp"
#include "Arrange/Tasks/FillBedTask.hpp"
#include "Arrange/Tasks/MultiplySelectionTask.hpp"
#include "Arrange/Items/ArbitraryDataStore.hpp"
#include "Arrange/Core/ArrangeBase.hpp"
#include "Arrange/Core/ArrangeItemTraits.hpp"
#include "Arrange/Core/DataStoreTraits.hpp"
#include "Point.hpp"
#include "Polygon.hpp"
#include "libslic3r.h"
	...
	namespace arr2 {
		...
		class DecomposedShape {
			...
			public:
				...
				explicit DecomposedShape(std::initializer_list<Point> pts) : DecomposedShape(Polygon{pts}) {}
		...
		class ArrangeItem {
			...
			public:
				...
				explicit ArrangeItem(DecomposedShape shape) : m_shape(std::move(shape)), m_envelope{&m_shape} {}

				explicit ArrangeItem(DecomposedShape shape, DecomposedShape envelope) : m_shape(std::move(shape)), m_envelope{std::make_unique<DecomposedShape>(std::move(envelope))} {}

				explicit ArrangeItem(const ExPolygons &shape);
				explicit ArrangeItem(Polygon shape);
				explicit ArrangeItem(std::initializer_list<Point> pts) : ArrangeItem(Polygon{pts}) {}
		...
		template<>
		struct ArrangeItemTraits_<ArrangeItem> {
		...
		template<>
		struct DataStoreTraits_<ArrangeItem> {
		...
		template<>
		struct WritableDataStoreTraits_<ArrangeItem> {
			static constexpr bool Implemented = true;

			template<class T>
			static void set(ArrangeItem &itm, const std::string &key, T &&data) {
				itm.datastore().add(key, std::forward<T>(data));
			}
		};

		template<class FixedIt, class StopCond = DefaultStopCondition>
		static Polygons calculate_nfp_unnormalized(const ArrangeItem &item, const Range<FixedIt> &fixed_items, StopCond &&stop_cond = {}) {
			size_t cap = 0;
		...
		template<>
		struct NFPArrangeItemTraits_<ArrangeItem> {
			template<class Context, class Bed, class StopCond>
			static ExPolygons calculate_nfp(const ArrangeItem &item, const Context &packing_context, const Bed &bed, StopCond &&stopcond) {
			...
			static double envelope_area(const ArrangeItem &itm) {
				return itm.envelope().area_unscaled() * scaled<double>(1.) * scaled<double>(1.);
			}

			static double fixed_area(const ArrangeItem &itm) {
				return itm.shape().area_unscaled() * scaled<double>(1.) * scaled<double>(1.);
			}
		...
		template<>
		struct IsMutableItem_<ArrangeItem> : public std::true_type {
		};
		...
		extern template
		struct ImbueableItemTraits_<ArrangeItem>;

		extern template
		class ArrangeableToItemConverter<ArrangeItem>;

		extern template
		struct ArrangeTask<ArrangeItem>;
		extern template
		struct FillBedTask<ArrangeItem>;
		extern template
		struct MultiplySelectionTask<ArrangeItem>;

		extern template
		class Arranger<ArrangeItem>;

...
#include "Arrange/Core/NFP/NFPConcave_Tesselate.hpp"
	...
	namespace arr2 {
		...
		const Polygons &DecomposedShape::transformed_outline() const {
			...
			if (!m_transformed_outline_valid) {
				...
				m_area = std::accumulate(m_transformed_outline.begin(), m_transformed_outline.end(), 0., [sc](double s, const auto &p) {
					return s + p.area() / sc;
				});
		...
		ArrangeItem::ArrangeItem(const ExPolygons &shape) : m_shape{decompose(shape)}, m_envelope{&m_shape} {}

		ArrangeItem::ArrangeItem(Polygon shape) : m_shape{decompose(shape)}, m_envelope{&m_shape} {}

...
namespace Slic3r {
	namespace arr2 {
		...
		template<class Itm>
		struct IsMutableItem_ : public std::false_type {
		};
		...
		template<class Itm, class En = void>
		struct MutableItemTraits_ {
			...
			static void set_allowed_rotations(Itm &itm, const std::vector<double> &rotations) {
				itm.set_allowed_rotations(rotations);
			}
		};

		template<class T> using MutableItemTraits = MutableItemTraits_<StripCVRef<T>>;

		template<class T> constexpr bool IsMutableItem = IsMutableItem_<T>::value;
		template<class T, class TT = T> using MutableItemOnly = std::enable_if_t<IsMutableItem<T>, TT>;

		template<class Itm>
		void set_priority(Itm &itm, int p) {
			MutableItemTraits<Itm>::set_priority(itm, p);
		}

		template<class Itm>
		void set_convex_shape(Itm &itm, const Polygon &shape) {
			MutableItemTraits<Itm>::set_convex_shape(itm, shape);
		}

		template<class Itm>
		void set_shape(Itm &itm, const ExPolygons &shape) {
		...
		template<class Itm>
		void set_envelope(Itm &itm, const ExPolygons &envelope) {
		...
		template<class ArrItem>
		int reduce_priority(ArrItem &itm) {
			int ret = get_priority(itm) - 1;
			set_priority(itm, ret);
		...
		template<class It>
		int lowest_priority(const Range<It> &item_range) {
			auto minp_it = std::min_element(item_range.begin(), item_range.end(), [](auto &itm1, auto &itm2) {
				return get_priority(itm1) < get_priority(itm2);
			});

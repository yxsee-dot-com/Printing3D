...
#include "Arrange/Items/TrafoOnlyArrangeItem.hpp"
...
namespace Slic3r {
	namespace arr2 {
		...
		struct MultiplySelectionTaskResult : public ArrangeResult {
			...
			template<class ArrItem>
			void add_new_item(const ArrItem &itm) {
				to_add.emplace_back(itm);
			}

			template<class It>
			void add_new_items(const Range<It> &items_range) {
		...
		struct MultiplySelectionTask : public ArrangeTaskBase {
			...
			static std::unique_ptr<MultiplySelectionTask> create(const Scene &sc, size_t multiply_count, const ArrangeableToItemConverter<ArrItem> &converter);

			static std::unique_ptr<MultiplySelectionTask> create(const Scene &sc, size_t multiply_count) {

...
namespace Slic3r {
	namespace arr2 {
		...
		template<class ArrItem>
		struct ArrangeTask : public ArrangeTaskBase {
			struct ArrangeSet {
			...
			static std::unique_ptr<ArrangeTask> create(const Scene &sc, const ArrangeableToItemConverter<ArrItem> &converter);
			...
			int item_count_to_process() const override {
				return static_cast<int>(printable.selected.size() + unprintable.selected.size());
			}
		};

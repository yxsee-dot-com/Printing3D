...
namespace Slic3r {
	namespace arr2 {
		...
		std::unique_ptr<MultiplySelectionTask<ArrItem>> MultiplySelectionTask<ArrItem>::create(const Scene &scene, size_t count, const ArrangeableToItemConverter<ArrItem> &itm_conv) {
		...
		template<class ArrItem>
		std::unique_ptr<MultiplySelectionTaskResult> MultiplySelectionTask<ArrItem>::process_native(Ctl &ctl) {
			...
			class MultiplySelectionCtl : public ArrangerCtl<ArrItem> {
				...
				public:
					MultiplySelectionCtl(ArrangeTaskCtl &p, MultiplySelectionTask<ArrItem> &slf) : parent{p}, self{slf} {}

...
#include "Arrange/SegmentedRectangleBed.hpp"
...
namespace Slic3r {
	namespace arr2 {
		...
		using XLBed = SegmentedRectangleBed<std::integral_constant<size_t, 4>, std::integral_constant<size_t, 4>>;
		...
		template<class... Args>
		struct ExtendedBed_ {
			using Type = boost::variant<XLBed, /* insert other types if needed*/ Args...>;
		};

		template<class... Args>
		struct ExtendedBed_<boost::variant<Args...>> {
		...
		void visit_bed(BedFn &&fn, const ExtendedBed &bed) {
			boost::apply_visitor(fn, bed);
		}

		template<class BedFn>
		void visit_bed(BedFn &&fn, ExtendedBed &bed) {
			boost::apply_visitor(fn, bed);
		}
		...
		void SceneBuilderBase<Subclass>::build_scene(Scene &sc) &&{
			...
			coord_t inset = std::max(scaled(m_settings->get_distance_from_bed()), m_skirt_offs + m_brims_offs);
		...
		class ArrangeTaskBase {
			public:
				...
				[[nodiscard]] static std::unique_ptr<ArrangeTaskBase> create(Tasks task_type, const Scene &sc);

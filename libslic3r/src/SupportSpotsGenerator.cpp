...
namespace Slic3r::SupportSpotsGenerator {
	...
	ExtrusionLine::ExtrusionLine(const Vec2f &a, const Vec2f &b, float len, const ExtrusionEntity *origin_entity) : a(a), b(b), len(len), origin_entity(origin_entity) {}

	ExtrusionLine::ExtrusionLine(const Vec2f &a, const Vec2f &b) : a(a), b(b), len((a - b).norm()), origin_entity(nullptr) {}
	...
	Integrals::Integrals(const Polygon &polygon) {
		...
		for (size_t i = 2; i < polygon.points.size(); i++) {
			...
			auto [area, first_moment_of_area, second_moment_area, second_moment_of_area_covariance] = compute_moments_of_area_of_triangle(p0, p1, p2);
	...
	Integrals::Integrals(float area, Vec2f x_i, Vec2f x_i_squared, float xy) : area(area), x_i(std::move(x_i)), x_i_squared(std::move(x_i_squared)), xy(xy) {}
	...
	SliceConnection estimate_slice_connection(size_t slice_idx, const Layer *layer) {
		...
		Polygons overlap = intersection(ClipperUtils::clip_clipper_polygons_with_subject_bbox(slice_polys, below_bb), ClipperUtils::clip_clipper_polygons_with_subject_bbox(below_polys, slice_bb));
	...
	float estimate_curled_up_height(float distance, float curvature, float layer_height, float flow_width, float prev_line_curled_height, Params params) {
		...
		if (distance > params.malformation_distance_factors.first * flow_width && distance < params.malformation_distance_factors.second * flow_width) {
	...
	std::vector<ExtrusionLine> check_extrusion_entity_stability(const ExtrusionEntity *entity, const LayerRegion *layer_region, const LD &prev_layer_lines, const AABBTreeLines::LinesDistancer<Linef> &prev_layer_boundary, const Params &params) {
		...
		if (entity->role().is_bridge() && !entity->role().is_perimeter()) {
			...
			std::vector<ExtrusionProcessor::ExtendedPoint> annotated_points = ExtrusionProcessor::estimate_points_properties<true>(entity->as_polyline().points, prev_layer_boundary, config);
			...
			for (size_t i = 0; i < annotated_points.size(); ++i) {
				...
				SupportPointCause potential_cause = std::abs(curr_point.curvature) > 0.1 ? SupportPointCause::FloatingBridgeAnchor : SupportPointCause::LongBridge;
				...
				float max_bridge_len = std::max(params.support_points_interface_radius * 2.0f, params.bridge_distance / ((1.0f + std::abs(curr_point.curvature)) * (1.0f + std::abs(curr_point.curvature)) * (1.0f + std::abs(curr_point.curvature))));

				if (!bridging_dir.has_value() && curr_point.distance > flow_width && line_len > params.bridge_distance * 0.6) {
					bridging_dir = line_dir;
				}

				if (curr_point.distance > flow_width && potential_cause == SupportPointCause::LongBridge && bridging_dir.has_value() && bridging_dir->dot(line_dir) < 0.8) { // skip backward direction of bridge - supported by forward points enough
			...
			config.max_line_length = params.bridge_distance;
			config.flow_width = flow_width;
			std::vector<ExtrusionProcessor::ExtendedPoint> annotated_points = ExtrusionProcessor::estimate_points_properties<false>(entity->as_polyline().points, prev_layer_lines, config);

			std::vector<ExtrusionLine> lines_out;
			lines_out.reserve(annotated_points.size());
			float bridged_distance = annotated_points.front().position != annotated_points.back().position ? (params.bridge_distance + 1.0f) : 0.0f;
			for (size_t i = 0; i < annotated_points.size(); ++i) {
				ExtrusionProcessor::ExtendedPoint &curr_point = annotated_points[i];
				const ExtrusionProcessor::ExtendedPoint &prev_point = i > 0 ? annotated_points[i - 1] : annotated_points[i];
				float line_len = (prev_point.position - curr_point.position).norm();
				...
				line_out.curled_up_height = estimate_curled_up_height(middle_distance, 0.5 * (prev_point.curvature + curr_point.curvature), layer_region->layer()->height, flow_width, bottom_line.curled_up_height, params);
	 ...
	 */
	float compute_second_moment(const Integrals &integrals, const Vec2f &axis_direction) {
		...
		moment_tensor << I_xx, I_xy, I_xy, I_yy;
	...
	ObjectPart::ObjectPart(const std::vector<const ExtrusionEntityCollection *> &extrusion_collections, const bool connected_to_bed, const coordf_t print_head_z, const coordf_t layer_height, const std::optional<Polygons> &brim) {
		if (connected_to_bed) {
		...
		for (const ExtrusionEntityCollection *collection: extrusion_collections) {
			...
			for (const ExtrusionEntity *entity: collection->flatten()) {
				...
				if (const auto *path = dynamic_cast<const ExtrusionPath *>(entity);
					...
					throw std::runtime_error("Failed to construct object part from extrusions!"
	...
	float ObjectPart::compute_elastic_section_modulus(const Vec2f &line_dir, const Vec3f &extreme_point, const Integrals &integrals) const {
		...
		float extreme_fiber_dist = line_alg::distance_to(Linef(centroid.head<2>().cast<double>(), (centroid.head<2>() + Vec2f(line_dir.y(), -line_dir.x())).cast<double>()), extreme_point.head<2>().cast<double>());
	...
	std::tuple<float, SupportPointCause> ObjectPart::is_stable_while_extruding(const SliceConnection &connection, const ExtrusionLine &extruded_line, const Vec3f &extreme_point, float layer_z, const Params &params) const {
		...
		float extruder_conflict_force = params.standard_extruder_conflict_force + std::min(extruded_line.curled_up_height, 1.0f) * params.malformations_additive_conflict_extruder_force;
		...
		// section for bed calculations
		{
			...
			if (bed_total_torque > 0) {
				return {bed_total_torque / bed_conflict_torque_arm, (this->connected_to_bed ? SupportPointCause::SeparationFromBed : SupportPointCause::UnstableFloatingPart)};
			}
		}
	...
	bool has_brim(const Layer *layer, const Params &params) {
		return int(layer->id()) == params.raft_layers_count && params.raft_layers_count == 0 && params.brim_type != BrimType::btNoBrim && params.brim_width > 0.0;
	}
	...
	void reckon_new_support_point(ObjectPart &part, SliceConnection &weakest_conn, SupportPoints &supp_points, SupportGridFilter &supports_presence_grid, const SupportPoint &support_point, bool is_global = false) {
		...
		if (weakest_conn.area > EPSILON) { // Do not add it to the weakest connection if it is not valid - does not exist
			...
			weakest_conn.second_moment_of_area_accumulator += area * support_point.position.head<2>().cwiseProduct(support_point.position.head<2>());
	...
	LocalSupports compute_local_supports(const std::vector<EnitityToCheck> &entities_to_check, const std::optional<Linesf> &previous_layer_boundary, const LD &prev_layer_ext_perim_lines, size_t slices_count, const Params &params) {
		...
		AABBTreeLines::LinesDistancer<Linef> prev_layer_boundary_distancer = (previous_layer_boundary ? AABBTreeLines::LinesDistancer<Linef>{*previous_layer_boundary} : AABBTreeLines::LinesDistancer<Linef>{});

		if constexpr (debug_files) {
			for (const auto &e_to_check: entities_to_check) {
				for (const auto &line: check_extrusion_entity_stability(e_to_check.e, e_to_check.region, prev_layer_ext_perim_lines, prev_layer_boundary_distancer, params)) {
			...
			tbb::parallel_for(tbb::blocked_range<size_t>(0, entities_to_check.size()), [&entities_to_check, &prev_layer_ext_perim_lines, &prev_layer_boundary_distancer, &unstable_lines_per_slice, &ext_perim_lines_per_slice, &params](tbb::blocked_range<size_t> r) {
	...
	std::optional<PartialObject> to_partial_object(const ObjectPart &part) {
		if (part.volume > EPSILON) {
			return PartialObject{part.volume_centroid_accumulator / part.volume, part.volume, part.connected_to_bed};
		}
		return {};
	}

	SliceMappings update_active_object_parts(const Layer *layer, const Params &params, const std::vector<SliceConnection> &precomputed_slice_connections, const SliceMappings &previous_slice_mappings, ActiveObjectParts &active_object_parts, PartialObjects &partial_objects) {
		...
		for (size_t slice_idx = 0; slice_idx < layer->lslices_ex.size(); ++slice_idx) {
			...
			const std::optional<Polygons> brim{has_brim(layer, params) ? std::optional{get_brim(layer->lslices[slice_idx], params.brim_type, params.brim_width)} : std::nullopt};
			ObjectPart new_part{extrusion_collections, connected_to_bed, layer->print_z, layer->height, brim};
			...
			if (connection_to_below.area < EPSILON) { // new object part emerging
				...
				auto estimate_conn_strength = [bottom_z](const SliceConnection &conn) {
					...
					Vec2f variance = (conn.second_moment_of_area_accumulator / conn.area - centroid.head<2>().cwiseProduct(centroid.head<2>()));
	...
	void reckon_global_supports(const tbb::concurrent_vector<ExtrusionLine> &external_perimeter_lines, const coordf_t layer_bottom_z, const Params &params, ObjectPart &part, SliceConnection &weakest_connection, SupportPoints &supp_points, SupportGridFilter &supports_presence_grid) {
		...
		for (const ExtrusionLine &line: external_perimeter_lines) {
			if ((unchecked_dist + line.len < params.min_distance_between_support_points && line.curled_up_height < params.curling_tolerance_limit) || line.len < EPSILON) {
	...
	std::tuple<SupportPoints, PartialObjects> check_stability(const PrintObject *po, const PrecomputedSliceConnections &precomputed_slices_connections, const PrintTryCancel &cancel_func, const Params &params) {
		...
		for (size_t layer_idx = 0; layer_idx < po->layer_count(); ++layer_idx) {
			...
			std::optional<Linesf> prev_layer_boundary = layer->lower_layer != nullptr ? std::optional{to_unscaled_linesf(layer->lower_layer->lslices)} : std::nullopt;

			LocalSupports local_supports{compute_local_supports(gather_entities_to_check(layer), prev_layer_boundary, prev_layer_ext_perim_lines, layer->lslices_ex.size(), params)};
			...
			for (size_t slice_idx = 0; slice_idx < layer->lslices_ex.size(); ++slice_idx) {
				...
				if (layer_idx > 1) {
					for (const auto &l: local_supports.unstable_lines_per_slice[slice_idx]) {
						...
						SupportPoint support_point{*l.support_point_generated, to_3d(l.b, bottom_z), params.support_points_interface_radius};
	...
	void estimate_supports_malformations(SupportLayerPtrs &layers, float flow_width, const Params &params) {
		...
		for (SupportLayer *l: layers) {
			...
			for (const ExtrusionEntity *extrusion: l->support_fills.flatten().entities) {
				...
				ExtrusionProcessor::PropertiesEstimationConfig config{};
				config.flow_width = flow_width;
				auto annotated_points = ExtrusionProcessor::estimate_points_properties<false>(pol.points, prev_layer_lines, config);
				...
				for (size_t i = 0; i < annotated_points.size(); ++i) {
					...
					ExtrusionLine line_out{a.position.cast<float>(), b.position.cast<float>(), float((a.position - b.position).norm()), extrusion};
					...
					line_out.curled_up_height = estimate_curled_up_height(middle_distance * sign, 0.5 * (a.curvature + b.curvature), l->height, flow_width, bottom_line.curled_up_height, params);
		...
		for (Layer *l: layers) {
			...
			for (const LayerRegion *layer_region: l->regions()) {
				for (const ExtrusionEntity *extrusion: layer_region->perimeters().flatten().entities) {
					...
					config.add_corners = true;
					config.flow_width = flow_width;
					auto annotated_points = ExtrusionProcessor::estimate_points_properties<false>(extrusion_pts, prev_layer_lines, config);
	...
	std::vector<std::pair<SupportPointCause, bool>> gather_issues(const SupportPoints &support_points, PartialObjects &partial_objects) {
		...
		std::sort(partial_objects.begin(), partial_objects.end(), [](const PartialObject &left, const PartialObject &right) { return left.volume > right.volume; });

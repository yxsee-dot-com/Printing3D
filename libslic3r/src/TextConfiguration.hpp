...
namespace Slic3r {
	...
	struct FontProp {
		...
		bool operator==(const FontProp &other) const {
			return char_gap == other.char_gap && line_gap == other.line_gap && per_glyph == other.per_glyph && align == other.align && is_approx(size_in_mm, other.size_in_mm) && is_approx(boldness, other.boldness) && is_approx(skew, other.skew);
		}
	...
	struct EmbossStyle {
		...
		// undo / redo stack recovery
		template<class Archive>
		void serialize(Archive &ar) { ar(name, path, type, prop); }
	};
	...
	struct TextConfiguration {
		...
		// undo / redo stack recovery
		template<class Archive>
		void serialize(Archive &ar) { ar(style, text); }
	};

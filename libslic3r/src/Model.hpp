	...
	namespace Internal {
		...
		class StaticSerializationWrapper {
			...
			private:
				...
				template<class Archive>
				void load(Archive &ar) { cereal::load_by_value(ar, wrapped); }
				template<class Archive>
				void save(Archive &ar) const { cereal::save_by_value(ar, wrapped); }
				T &wrapped;
		};
	}
...
#define OBJECTBASE_DERIVED_COPY_MOVE_CLONE(TYPE) \
	...
	class ModelMaterial final : public ObjectBase {
		...
		private:
			...
			ModelMaterial() : ObjectBase(-1), config(-1) {
	...
	enum class CutConnectorShape : int {
		Triangle, Square, Hexagon, Circle, Undef
		//,D-shape
	};
	...
	struct CutConnectorAttributes {
		...
		bool operator<(const CutConnectorAttributes &other) const {
			return this->type < other.type || (this->type == other.type && this->style < other.style) || (this->type == other.type && this->style == other.style && this->shape < other.shape);
		}
	...
	struct CutConnector {
		...
		CutConnector() : pos(Vec3d::Zero()), rotation_m(Transform3d::Identity()), radius(5.f), height(10.f), radius_tolerance(0.f), height_tolerance(0.1f), z_angle(0.f) {}

		CutConnector(Vec3d p, Transform3d rot, float r, float h, float rt, float ht, float za, CutConnectorAttributes attributes) : pos(p), rotation_m(rot), radius(r), height(h), radius_tolerance(rt), height_tolerance(ht), z_angle(za), attribs(attributes) {}

		CutConnector(const CutConnector &rhs) : CutConnector(rhs.pos, rhs.rotation_m, rhs.radius, rhs.height, rhs.radius_tolerance, rhs.height_tolerance, rhs.z_angle, rhs.attribs) {}
	...
	enum class ModelVolumeType : int {
		INVALID = -1, MODEL_PART = 0, NEGATIVE_VOLUME, PARAMETER_MODIFIER, SUPPORT_BLOCKER, SUPPORT_ENFORCER,
	};
	...
	enum class ConversionType : int {
		CONV_TO_INCH, CONV_FROM_INCH, CONV_TO_METER, CONV_FROM_METER,
	};
	...
	class FacetsAnnotation final : public ObjectWithTimestamp {
		...
		private:
			...
			template<class Archive>
			void serialize(Archive &ar) { ar(cereal::base_class<ObjectWithTimestamp>(this), m_data); }
	...
	class ModelVolume final : public ObjectBase {
		public:
			...
			struct CutInfo {
				...
				CutInfo() = default;
				CutInfo(CutConnectorType type, float rad_tolerance, float h_tolerance, bool processed = false) : is_connector(true), is_processed(processed), connector_type(type), radius_tolerance(rad_tolerance), height_tolerance(h_tolerance) {}
	...
	enum ModelInstanceEPrintVolumeState : unsigned char {
		ModelInstancePVS_Inside, ModelInstancePVS_Partly_Outside, ModelInstancePVS_Fully_Outside, ModelInstanceNum_BedStates
	};
	...
	class ModelInstance final : public ObjectBase {
		...
		private:
			...
			// Constructor, which assigns a new unique ID.
			explicit ModelInstance(ModelObject *object, const ModelInstance &other) : m_transformation(other.m_transformation), print_volume_state(ModelInstancePVS_Inside), printable(other.printable), object(object) { assert(this->id().valid() && this->id() != other.id()); }
	...
	class ModelWipeTower final : public ObjectBase {
		...
		private:
			...
			void serialize(Archive &ar) { ar(position, rotation); }
	};
	...
	class Model final : public ObjectBase {
		public:
			...
			explicit Model(Model &&rhs) : ObjectBase(-1) {
				assert(this->id().invalid());
				this->assign_copy(std::move(rhs));
				assert(this->id().valid());
				assert(this->id() == rhs.id());
			...
			enum class LoadAttribute : int {
				AddDefaultInstances, CheckVersion
			};

...
namespace Slic3r {
	namespace opt {
		...
		class Optimizer {
			public:
				...
				Optimizer(const StopCriteria &) {
					static_assert(always_false<Method>::value, "Optimizer unimplemented for given method!");
				}
				...
				Result<N> optimize(Func && /*func*/, const Input<N> &/*initvals*/, const Bounds<N> & /*bounds*/, const std::tuple<EqConstraints...> &eq_constraints = {}, const std::tuple<IneqConstraints...> &ineq_constraint = {}) { return {}; }
		...
		template<size_t N>
		Bounds<N> bounds(const Bound (&b)[N]) { return detail::to_arr(b); }
		template<size_t N>
		Input<N> initvals(const double (&a)[N]) { return detail::to_arr(a); }
		template<size_t N>

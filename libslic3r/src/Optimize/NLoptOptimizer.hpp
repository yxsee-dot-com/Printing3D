...
namespace Slic3r {
	namespace opt {
		...
		namespace detail {
			...
			class NLoptOpt {
					...
					struct OptData {
						...
						OptData(const Fn &f, NLoptOpt *s, nlopt_opt nlopt_raw) : fn{f}, self{s}, opt_raw{nlopt_raw} {}
					};

					template<class Fn, size_t N>
					static double optfunc(unsigned n, const double *params, double *gradient, void *data) {
						assert(n == N);
					...
					template<class Fn, size_t N>
					static double constrain_func(unsigned n, const double *params, double *gradient, void *data) {
						assert(n == N);
					...
					template<size_t N>
					static void set_up(NLoptRAII &nl, const Bounds<N> &bounds, const StopCriteria &stopcr) {
					...
					Result<N> optimize(NLoptRAII &nl, Fn &&fn, const Input<N> &initvals, const std::tuple<EqFns...> &equalities, const std::tuple<IneqFns...> &inequalities) {
						Result<N> r;
						...
						switch (m_dir) {
							case OptDir::MIN:
								nlopt_set_min_objective(nl.ptr, optfunc<Fn, N>, &data);
								break;
							case OptDir::MAX:
								nlopt_set_max_objective(nl.ptr, optfunc<Fn, N>, &data);
								break;
						}
				...
				public:
					...
					template<class Fn, size_t N, class...EqFns, class...IneqFns>
					Result<N> optimize(Fn &&f, const Input<N> &initvals, const Bounds<N> &bounds, const std::tuple<EqFns...> &equalities, const std::tuple<IneqFns...> &inequalities) {
						if constexpr (IsAUGLAG<M>) {
							...
							if constexpr (GetNLoptAlg_Local<M> < NLOPT_NUM_ALGORITHMS) {
								...
								return optimize(nl_wrap, std::forward<Fn>(f), initvals, equalities, inequalities);
							} else {
								return optimize(nl_wrap, std::forward<Fn>(f), initvals, equalities, inequalities);
							}
						} else {
							...
							if constexpr (GetNLoptAlg_Local<M> < NLOPT_NUM_ALGORITHMS) {
								...
								return optimize(nl_glob, std::forward<Fn>(f), initvals, equalities, inequalities);
							} else {
								return optimize(nl_glob, std::forward<Fn>(f), initvals, equalities, inequalities);
							}
						}
					...
					explicit NLoptOpt(const StopCriteria &stopcr_glob = {}) : m_stopcr(stopcr_glob) {}
		...
		template<class Alg> constexpr bool SupportsEqualities = detail::AlgFeatures_<remove_cvref_t<Alg>>::SupportsEqualities;

		template<class Alg> constexpr bool SupportsInequalities = detail::AlgFeatures_<remove_cvref_t<Alg>>::SupportsInequalities;
		...
		class Optimizer<M, detail::NLoptOnly<M>> {
			...
			public:
				...
				Result<N> optimize(Func &&func, const Input<N> &initvals, const Bounds<N> &bounds, const std::tuple<EqFns...> &eq_constraints = {}, const std::tuple<IneqFns...> &ineq_constraint = {}) {
					static_assert(std::tuple_size_v<std::tuple<EqFns...>> == 0 || SupportsEqualities<M>, "Equality constraints are not supported.");

					static_assert(std::tuple_size_v<std::tuple<IneqFns...>> == 0 || SupportsInequalities<M>, "Inequality constraints are not supported.");

					return m_opt.optimize(std::forward<Func>(func), initvals, bounds, eq_constraints, ineq_constraint);
				}
